...
description: Create comprehensive integration tests for syncable entities in BeeAX. Use when writing integration tests for metadata entities, covering validator exceptions, input transpilation errors, and CRUD operations. Tests are MANDATORY for all syncable entities.
---
...
npx jest test/integration/metadata/suites/my-entity --config=packages/beeax-server/jest.config.mjs

# Run specific test file
npx jest test/integration/metadata/suites/my-entity/failing-my-entity-creation.integration-spec.ts --config=packages/beeax-server/jest.config.mjs

# Update snapshots
npx jest test/integration/metadata/suites/my-entity --updateSnapshot --config=packages/beeax-server/jest.config.mjs
```
...
**Congratulations!** You've successfully created a new syncable entity in BeeAX's workspace migration system.
