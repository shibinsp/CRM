...
if (unpackedSize > MAX_UNPACKED_BYTES) {
  process.stderr.write(
    `beeax-ui unpacked size ${formatMegabytes(unpackedSize)} exceeds budget ${formatMegabytes(
      MAX_UNPACKED_BYTES,
...
process.stdout.write(
  `beeax-ui pack OK: tarball ${formatMegabytes(size)}, unpacked ${formatMegabytes(
    unpackedSize,
  )}, ${entryCount} files\n`,
);
