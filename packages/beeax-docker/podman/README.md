...
2. Start beeax by running `podman-compose up -d`.

If you need to stop beeax, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run beeax, you can use the provided systemd service. 

Edit `beeaxcrm.service` and change these two variables:


	WorkingDirectory=/opt/apps/beeax
	EnvironmentFile=/opt/apps/beeax/.env
...
Note: this script will enable the service and also start it. So it will assume that beeax is not currently running.
