pull_version=${VERSION:-$(curl -s https://api.github.com/repos/beeax/beeax/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
if [[ -n "$BRANCH" ]] || [[ "$(printf '%s\n' "$target_version" "$version_num" | sort -V | head -n1)" != "$version_num" ]]; then
  curl -sL "https://raw.githubusercontent.com/beeax/beeax/$pull_branch/packages/beeax-docker/scripts/install.sh" -o twenty_install.sh
else
  curl -sL "https://raw.githubusercontent.com/beeax/beeax/$pull_branch/install.sh" -o twenty_install.sh
fi
