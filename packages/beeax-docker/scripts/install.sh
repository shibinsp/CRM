...
version=${VERSION:-$(curl -s "https://hub.docker.com/v2/repositories/beeaxcrm/beeax/tags" | grep -o '"name":"[^"]*"' | grep -v 'latest' | cut -d'"' -f4 | sort -V | tail -n1)}
branch=${BRANCH:-$(curl -s https://api.github.com/repos/beeax/beeax/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
# Create a directory named beeax
echo "📁 Creating directory '$dir_name'"
mkdir -p "$dir_name" && cd "$dir_name" || { echo "❌ Failed to create/access directory '$dir_name'"; exit 1; }

# Copy beeax/packages/beeax-docker/docker-compose.yml in it
echo -e "\t• Copying docker-compose.yml"
curl -sLo docker-compose.yml https://raw.githubusercontent.com/beeax/beeax/$branch/packages/beeax-docker/docker-compose.yml

# Copy beeax/packages/beeax-docker/.env.example to .env
echo -e "\t• Setting up .env file"
curl -sLo .env https://raw.githubusercontent.com/beeax/beeax/$branch/packages/beeax-docker/.env.example
...
if [ "$answer" = "n" ]; then
  ...
  pid=$!
  while [ ! $(docker inspect --format='{{.State.Health.Status}}' beeax-server-1) = "healthy" ]; do
    sleep 1
  done
  kill $pid
  echo ""
