...
Deploy BeeAX CRM on Kubernetes with server, worker, PostgreSQL, and Redis components.
...
helm install my-beeax ./packages/beeax-docker/helm/beeax \
  --namespace beeaxcrm --create-namespace
```
...
- Database `beeax` and schema `core` are created automatically by server init container
...
```bash
helm lint ./packages/beeax-docker/helm/beeax
helm template my-beeax ./packages/beeax-docker/helm/beeax
helm plugin install https://github.com/quintush/helm-unittest
helm unittest ./packages/beeax-docker/helm/beeax
```
...
helm install my-beeax ./packages/beeax-docker/helm/beeax -f values-secrets.yaml
```
