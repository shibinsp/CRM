...
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_roles WHERE rolname='beeax'" | grep -q 1 \
  || su-exec postgres psql -h localhost -c "CREATE ROLE beeax WITH LOGIN PASSWORD 'beeax' SUPERUSER"

# Create database if it doesn't exist
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_database WHERE datname='default'" | grep -q 1 \
  || su-exec postgres createdb -h localhost -O beeax default

# Run BeeAX database setup and migrations
cd /app/packages/beeax-server

has_schema=$(PGPASSWORD=beeax psql -h localhost -U beeax -d default -tAc \
...
has_workspace=$(PGPASSWORD=beeax psql -h localhost -U beeax -d default -tAc \
