resource "kubernetes_deployment" "beeaxcrm_server" {
  metadata {
  ...
  spec {
    replicas = var.beeaxcrm_server_replicas
    selector {
      match_labels = {
        app = "${var.beeaxcrm_app_name}-server"
      }
    ...
    template {
      ...
      spec {
        container {
          ...
          volume_mount {
            name       = "server-data"
            mount_path = var.beeaxcrm_server_data_mount_path
          }

          volume_mount {
            name       = "docker-data"
            mount_path = var.beeaxcrm_docker_data_mount_path
          }
        }
