# Kubernetes for BeeAX CRM (Helm)
DISCLAIMER: The k8s and podman deployments are not maintained by the core team.
These files are provided and maintained by the community. BeeAX core team
maintains support for docker deployment.

## Overview

This repository contains a world-class Helm chart and (legacy) raw Kubernetes manifests plus Terraform files to help you deploy and manage the BeeAX CRM application. The chart is located at `packages/beeax-docker/helm/beeax` and supersedes the manifests in `packages/beeax-docker/k8s/manifests`.
...
helm install my-beeax packages/beeax-docker/helm/beeax \
  --namespace beeaxcrm --create-namespace --wait \
...
See [QUICKSTART](../helm/beeax/QUICKSTART.md) and [chart README](../helm/beeax/README.md) for details.
...
cd beeax/packages/beeax-docker/k8s
```
...
2. Create Server Secret
    ...
    ``` bash
    kubectl create secret generic -n beeaxcrm tokens --from-literal accessToken=changeme --from-literal loginToken="changeme" --from-literal refreshToken="changeme" --from-literal fileToken="changeme"
    ```
