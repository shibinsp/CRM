---
title: Overview
description: Place your app inside BeeAX's UI — sidebar entries, saved views, record page tabs, and sandboxed React components.
icon: "table-columns"
---

A BeeAX app's **layout layer** is everything the user sees: where the app surfaces in the sidebar, which list views it ships, how its record detail pages are arranged, and which custom React components render inside those pages.
...
<CardGroup cols={2}>
  ...
  <Card title="Front Components" icon="window-maximize" href="/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — sandboxed React components that render inside BeeAX.
  </Card>
...
Front components run inside an isolated Web Worker using Remote DOM — they render *natively* in the page (not inside an iframe), but cannot reach the host page or DOM directly. Communication with BeeAX happens through a message-passing host API.
