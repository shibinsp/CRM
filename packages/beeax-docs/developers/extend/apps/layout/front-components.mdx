...
description: Build React components that render inside BeeAX's UI with sandboxed isolation.
icon: "window-maximize"
---

Front components are React components that render directly inside BeeAX's UI. They run in an **isolated Web Worker** using Remote DOM — your code is sandboxed but renders natively in the page, not in an iframe.

## Where front components can be used

Front components can render in two locations within BeeAX:
...
After syncing with `yarn beeax dev` (or running a one-shot `yarn beeax dev --once`), the quick action appears in the top-right corner of the page:
...
Because the component returns `null`, BeeAX skips rendering a container for it — no empty space appears in the layout. The component still has access to all hooks and the host communication API.

## SDK Command components

The `beeax-sdk` package provides four Command helper components designed for headless front components. Each component executes an action on mount, handles errors by showing a snackbar notification, and automatically unmounts the front component when done.

Import them from `beeax-sdk/command`:
...
A logic function declared with `httpRouteTriggerSettings` is reachable over HTTP at its route path. BeeAX injects the base URL your functions are served from into the worker as `TWENTY_FUNCTIONS_URL`, together with the `TWENTY_APP_ACCESS_TOKEN` that authenticates the call. There is no dedicated SDK client for invoking your own functions yet, so call them with a plain `fetch`:

> **On BeeAX Cloud, HTTP-triggered logic functions are served on a dedicated per-workspace domain** at `https://<your-workspace-subdomain>.beeax.com<path>` — this is exactly what `TWENTY_FUNCTIONS_URL` resolves to. For external callers, copy the exact URL from the function's **HTTP trigger** settings or the application's **Settings** tab.
...
The path appended to `TWENTY_FUNCTIONS_URL` is the logic function's `httpRouteTriggerSettings.path`. Keep `isAuthRequired: true`; the `TWENTY_APP_ACCESS_TOKEN` BeeAX mints for your component authenticates the request:
...
### Calling the BeeAX REST API

To read or write BeeAX records from a front component, use `RestApiClient` from `beeax-client-sdk/rest`. It belongs to the same client family as `CoreApiClient` and `MetadataApiClient`, but targets the BeeAX REST API (`/rest/...`) instead of the GraphQL API, reading its base URL from `TWENTY_API_URL`.
...
| `TWENTY_API_URL` | Base URL of the BeeAX core API |
| `TWENTY_APP_ACCESS_TOKEN` | Short-lived token scoped to your app's role |

## Host communication API

Front components can trigger navigation, modals, and notifications using functions from `beeax-sdk`:
...
- **BeeAX UI components** — import from `beeax-sdk/ui` (Button, Tag, Status, Chip, Avatar, and more)
