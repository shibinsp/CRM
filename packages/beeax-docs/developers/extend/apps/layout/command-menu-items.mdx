...
A **command menu item** is the bridge between the user and a [front component](/developers/extend/apps/layout/front-components). It registers the component in BeeAX's command menu (Cmd+K) and, optionally, as a pinned quick-action button in the top-right corner of the page.
...
The `conditionalAvailabilityExpression` field lets you control when a command is visible based on the current page context. Import typed variables and operators from `beeax-sdk` to build expressions:
