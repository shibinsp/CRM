...
Wrap the object behind a few small helpers so the rest of your code reads like a key-value API — `get`, `set`, and `del`. They use [`CoreApiClient`](/developers/extend/apps/logic/logic-functions#typed-api-clients-beeax-client-sdk), which is generated from your workspace schema and fully typed against the `kvStore` object.
