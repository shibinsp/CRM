...
- Use `serverVariables` (not `applicationVariables`) — OAuth credentials are server-wide and one OAuth app per BeeAX server.
...
4. Open the installed app in BeeAX as a server admin → set the values on the corresponding `serverVariables`.
