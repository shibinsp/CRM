...
title: Publishing
icon: "upload"
description: Distribute your BeeAX app to the marketplace or deploy it internally.
---
...
- **Deploy a tarball** — upload your app directly to a specific BeeAX server for internal or private use.
- **Publish to npm** — list your app in the BeeAX marketplace for any workspace to discover and install.
...
This compiles TypeScript sources, transpiles logic functions and front components, and writes everything to `.beeax/output/`. Add `--tarball` to also produce a `.tgz` package for manual distribution or the publish command.

## Deploying to a server (tarball)

For apps you don't want publicly available — proprietary tools, enterprise-only integrations, or experimental builds — you can deploy a tarball directly to a BeeAX server.

### Prerequisites

Before deploying, you need a configured remote pointing to the target server. Remotes store the server URL and authentication credentials locally in `~/.beeax/config.json`.
...
2. Run `yarn beeax app:publish --private` (or `yarn beeax app:publish --private --remote production`)
...
If your app uses a feature introduced in a specific BeeAX server version (for example, OAuth providers added in v2.3.0), you should declare the minimum server version your app requires using the `engines.beeax` field in `package.json`:
...
- If `engines.beeax` is set and the target server's version does not satisfy the range, the deploy (tarball upload) or install is rejected with a `SERVER_VERSION_INCOMPATIBLE` error and a message indicating both the required range and the actual server version.
- If `engines.beeax` is **not set**, the app is accepted on any server version (backward-compatible with existing apps).
- If the server has no `APP_VERSION` configured, the check is skipped.

<Note>
The server is the authoritative check — it validates `engines.beeax` on both tarball upload and workspace install. If you deploy a tarball out-of-band or install from the marketplace, the server still enforces compatibility.
</Note>

## Automated CI/CD (scaffolded workflows)

Apps generated with `create-beeax-app` ship with two GitHub Actions workflows out of the box, under `.github/workflows/`. They are ready to run as soon as you push the repo to GitHub — no extra setup is needed for CI, and CD only requires a single secret.
...
2. Spawns an isolated BeeAX test instance using the `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` composite action (the CI equivalent of `yarn beeax docker:start --test`).
...
- `TWENTY_VERSION` (env, defaults to `latest`) — pin the BeeAX server version used in CI by editing this in `ci.yml`.
...
Deploys your app to a configured BeeAX server on every push to `main`, and optionally from a pull request when the `deploy` label is applied.

**What it does:**

1. Checks out the PR head (for labeled PRs) or the pushed commit.
2. Runs `beeax/beeax/.github/actions/deploy-beeax-app@main` — the CI equivalent of `yarn beeax app:publish --private`.
3. Runs `beeax/beeax/.github/actions/install-beeax-app@main` so the newly deployed version is installed into the target workspace.

**Required configuration:**

| Setting | Where | Purpose |
|---------|-------|---------|
| `TWENTY_DEPLOY_URL` | `env` in `cd.yml` (defaults to `http://localhost:3000`) | The BeeAX server to deploy to. Change this to your real server URL before first use. |
...
Both workflows reference reusable actions at `@main`, so action updates in the `beeax/beeax` repo are picked up automatically. If you want deterministic builds, replace `@main` with a commit SHA or release tag on each `uses:` line.

## Publishing to npm

Publishing to npm makes your app discoverable in the BeeAX marketplace. Any BeeAX workspace can browse, install, and upgrade marketplace apps directly from the UI.

### Requirements

- An [npm](https://www.npmjs.com) account
- The `beeax-app` keyword in your `package.json` `keywords` array (add it manually — it is not included by default in the `create-beeax-app` template)
...
The BeeAX server syncs its marketplace catalog from the npm registry **every hour**.
...
<Note>
If your app does not define an `aboutDescription` in `defineApplication()`, the marketplace will automatically use your package's `README.md` from npm as the about page content. This means you can maintain a single README for both npm and the BeeAX marketplace. If you want a different description in the marketplace, explicitly set `aboutDescription`.
</Note>
...
For other CI systems (GitLab CI, CircleCI, etc.), the same three commands apply: `yarn install`, `yarn beeax dev:build`, then `npm publish` from `.beeax/output`.
...
Go to the **Settings > Applications** page in BeeAX, where both marketplace and tarball-deployed apps can be browsed and installed.
...
To install a newer version, deploy or publish it first, then re-run `yarn beeax app:install`.
</Note>
