---
title: Testing
description: Vitest setup, integration tests against a real BeeAX server, type checking, and CI with GitHub Actions.
icon: "flask"
---

The SDK provides programmatic APIs that let you build, deploy, install, and uninstall your app from test code. Combined with [Vitest](https://vitest.dev/) and the typed API clients, you can write integration tests that verify your app works end-to-end against a real BeeAX server.
...
Both environments have `beeax-client-sdk/core` and `beeax-client-sdk/metadata` available as pre-provided modules — these are not bundled but resolved at runtime by the server.
...
The `beeax-sdk/cli` subpath exports functions you can call directly from test code:
...
Make sure your local BeeAX server is running, then:
...
2. Spins up a temporary BeeAX server using the `beeax/beeax/.github/actions/spawn-beeax-docker-image` action
...
You don't need to configure any secrets — the `spawn-beeax-docker-image` action starts an ephemeral BeeAX server directly in the runner and outputs the connection details. The `GITHUB_TOKEN` secret is provided automatically by GitHub.

To pin a specific BeeAX version instead of `latest`, change the `TWENTY_VERSION` environment variable at the top of the workflow.
