...
<Note>
For day-to-day local iteration you almost always want `yarn beeax dev`. Deploying and publishing are for shipping releases, **not** for the local loop.
</Note>

| You want to… | Command | Notes |
| --- | --- | --- |
| Iterate locally with live sync | `yarn beeax dev` | Watches your files and syncs on every change. |
| Sync once and exit (CI, scripts, hooks) | `yarn beeax dev --once` | One build + sync, then exits. |
| Preview changes **without applying them** | `yarn beeax dev --once --dry-run` | Computes and prints the diff; writes nothing. |
| Remove the app from the workspace | `yarn beeax app:uninstall` | Add `--yes` to skip the prompt. |
| Ship a tarball to a server | `yarn beeax app:publish --private` | Requires a **strictly higher** `package.json` version — see [Publishing](/developers/extend/apps/operations/publishing). |
| Publish to the marketplace (npm) | `yarn beeax app:publish` | — |
| Install / upgrade a deployed version | `yarn beeax app:install` | Installs the version currently deployed. |
| Wipe the local server and start clean | `yarn beeax docker:reset` | Deletes **all** local data — last resort. |

### Local sync does not need a version bump

The strictly-increasing `version` rule (`VERSION_ALREADY_EXISTS` on deploy, `APP_ALREADY_INSTALLED` / `CANNOT_DOWNGRADE_APPLICATION` on install) applies to **`app:publish` / `app:install`** — the release path. `yarn beeax dev` syncs your manifest in place and never requires a version change, so you don't need to touch `package.json` to iterate. If you find yourself bumping the version to test a local change, you're using the release path when you want the dev loop.
...
`yarn beeax dev --once --dry-run` builds your manifest, asks the server for the migration plan, and prints it — **without applying anything**. It's the safe way to answer "what would this sync change?" before committing to it.
...
<Note>
A dry run only previews **metadata** changes, and it requires the app to have been synced at least once (so the workspace knows about it). If you run it against an app that was never synced, the server reports that the app is not installed — run `yarn beeax dev` once first.
</Note>
...
1. **Re-sync.** Run `yarn beeax dev --once` again. Syncs are idempotent — re-running a clean manifest is safe and often resolves a transient hiccup.
2. **Preview the plan.** Run `yarn beeax dev --once --dry-run` to see exactly what the next sync intends to change, without applying it.
...
4. **Uninstall and reinstall.** `yarn beeax app:uninstall`, then sync again (`yarn beeax dev`). This rebuilds the app's metadata from a clean slate while keeping the rest of your workspace intact.
5. **Full reset (last resort).** `yarn beeax docker:reset`, then re-seed and re-sync.

<Warning>
`yarn beeax docker:reset` deletes **all** data in your local instance — every workspace, record, and app. Only use it once the earlier steps have failed.
</Warning>

<Note>
Hit a metadata error? Please [open an issue](https://github.com/beeax/beeax/issues/new/choose) and include the failing migration message (with its metadata type and `universalIdentifier`), the `Metadata changes` output from the sync, and the commands you ran.
</Note>
