...
Relations connect two objects together. In BeeAX, relations are always **bidirectional** — every relation has two sides, and each side is declared as a field that references the other.
...
To create a relation with a built-in BeeAX object (Person, Company, etc.), use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
