...
description: Add fields to standard BeeAX objects (Person, Company, …) or to objects from other apps using defineField.
icon: "wand-magic-sparkles"
---

Use `defineField()` to add a field to an object you don't own — a standard BeeAX object like Person or Company, or an object shipped by another installed app. Unlike inline fields declared inside [`defineObject`](/developers/extend/apps/data/objects), standalone fields require an `objectUniversalIdentifier` to specify which object they extend.
...
- `objectUniversalIdentifier` identifies the target object. For standard BeeAX objects, import the constant from `beeax-sdk`:
...
- To add a tab to a standard page layout (e.g. the Task or Company detail page), use [`definePageLayoutTab`](/developers/extend/apps/layout/page-layouts#definepagelayouttab) with `STANDARD_PAGE_LAYOUT_UNIVERSAL_IDENTIFIERS` from `beeax-sdk/define`.
