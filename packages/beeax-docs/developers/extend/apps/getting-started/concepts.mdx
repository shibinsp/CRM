---
title: Concepts
description: How BeeAX apps work — entity model, sandboxing, and the install lifecycle.
icon: "sitemap"
---

BeeAX apps are TypeScript packages that extend your workspace with custom objects, logic, UI components, and AI capabilities. They run on the BeeAX platform with full sandboxing and permission controls.

## How apps work

An app is a collection of **entities** declared using `defineEntity()` functions from the `beeax-sdk` package. The SDK detects these declarations via AST analysis at build time and produces a **manifest** — a complete description of what your app adds to a workspace. These functions validate your configuration at build time and provide IDE autocompletion and type safety.
...
| **Front Component** | Sandboxed React UI inside BeeAX | [Front Components](/developers/extend/apps/layout/front-components) |
...
- **Front components** run in Web Workers using Remote DOM — sandboxed from the main page but rendering native DOM elements (not iframes). They communicate with BeeAX via a message-passing host API.
...
- **`yarn beeax dev`** — watches your source files and live-syncs changes to a connected BeeAX server. The typed API client is regenerated automatically when the schema changes.
- **`yarn beeax dev:build`** — compiles TypeScript, bundles logic functions and front components with esbuild, and produces a manifest.
