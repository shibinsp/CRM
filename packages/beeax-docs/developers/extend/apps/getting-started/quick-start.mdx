...
title: Quick Start
icon: "rocket"
description: Create your first BeeAX app in minutes.
---
...
- **Docker** — [Download](https://www.docker.com/products/docker-desktop/). Needed to run a local BeeAX server. Skip if you already have BeeAX running elsewhere.

Building a BeeAX app has three phases. The scaffolder collapses them into one happy-path command, but each phase is a separate concept — when something fails, knowing which phase you're in tells you what to fix.

| Phase | What you do | Tool | Result |
|---|---|---|---|
| **1. Scaffold** | Generate the app's source code | `npx create-beeax-app` | A TypeScript project on disk |
| **2. Run a server** | Start a BeeAX server to sync into | Docker + `yarn beeax server` | A running BeeAX instance |
| **3. Sync** | Live-sync your code to the server | `yarn beeax dev` | Your changes appear in the UI |
...
You'll be prompted for a name and description — press **Enter** for the defaults. This generates a TypeScript project in `my-beeax-app/` with a starter `application-config.ts`, a default role, a CI workflow, and an integration test.

**After this phase:** you have an app's source code on your machine. It isn't running yet — that's Phase 2.

---

## Phase 2 — Run a local BeeAX server

Your app needs a BeeAX server to sync into. The server is a full BeeAX instance — UI, GraphQL API, PostgreSQL — running locally in Docker. Your local code uploads its definitions to that server, which makes them appear in the UI.

The scaffolder offers to start one for you:

> **Would you like to set up a local BeeAX instance?**

- **Yes (recommended)** — pulls the `beeaxcrm/beeax-app-dev` Docker image and starts it on port `2020`. Make sure Docker is running first.
- **No** — choose this if you already have a BeeAX server you want to connect to. You can wire it up later with `yarn beeax remote:add`.
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="BeeAX login screen" />
</div>

Click **Authorize** on the next screen — this gives the CLI access to your workspace.

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="BeeAX CLI authorization screen" />
</div>
...
**After this phase:** you have a running BeeAX server at [http://localhost:2020](http://localhost:2020) with your CLI authorized to sync to it.

<Note>
If Docker isn't installed or running, the scaffolder will tell you the right start command for your OS. Once Docker is up, you can resume with `yarn beeax docker:start` — no need to re-scaffold.
</Note>
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/app-in-ui-1.png" alt="Your Apps list showing My beeax app" />
</div>

Click **My beeax app** to see its **application registration** — a server-level record describing your app (name, identifier, OAuth credentials, source). One registration can be installed across multiple workspaces on the same server.
...
|---------|----------|-------------|
| `yarn beeax dev` | Watches and re-syncs on every change. Runs until you stop it. | Interactive local development. |
| `yarn beeax dev --once` | Single build + sync, exits `0` on success, `1` on failure. | CI, pre-commit hooks, AI agents, scripted workflows. |
| `yarn beeax dev --once --dry-run` | Builds and prints the metadata changes **without applying them**. | Inspecting what a sync would change before committing to it. |
...
| **Front components** | React components that render inside BeeAX's UI (side panel, widgets, command menu) |
