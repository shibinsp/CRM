...
- The environment variables `APPLICATION_ID`, `APP_ACCESS_TOKEN`, and `API_URL` are available inside the handler (same as any other logic function), so you can call the BeeAX API with an application access token scoped to your app.
...
- **Not executed in dev mode**: when an app is registered locally (via `yarn beeax dev`), the server skips the install flow entirely and syncs files directly through the CLI watcher — so post-install never runs in dev mode, regardless of `shouldRunSynchronously`. Use `yarn beeax dev:function:exec --postInstall` to trigger it manually against a running workspace.
...
- **Not executed in dev mode**: same as post-install — the install flow is skipped entirely for locally-registered apps, so pre-install never runs under `yarn beeax dev`. Use `yarn beeax dev:function:exec --preInstall` to trigger it manually.
