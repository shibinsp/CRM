...
- **Included in builds** — `yarn beeax dev:build` bundles all public assets into the distribution output.

## Accessing public assets with `getPublicAssetUrl`

Use the `getPublicAssetUrl` helper from `beeax-sdk` to get the full URL of a file in your `public/` directory. It works in both **logic functions** and **front components**.
