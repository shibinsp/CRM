...
Webhooks push data to your systems in real-time when events occur in BeeAX — no polling required. Use them to keep external systems in sync, trigger automations, or send alerts.
...
BeeAX sends webhooks for these event types:
...
BeeAX signs each webhook request for security. Validate signatures to ensure requests are authentic.

### Headers

| Header | Description |
|--------|-------------|
| `X-BeeAX-Webhook-Signature` | HMAC SHA256 signature |
| `X-BeeAX-Webhook-Timestamp` | Request timestamp |

### Validation Steps

1. Get the timestamp from `X-BeeAX-Webhook-Timestamp`
2. Create the string: `{timestamp}:{JSON payload}`
3. Compute HMAC SHA256 using your webhook secret
4. Compare with `X-BeeAX-Webhook-Signature`
...
| **Workflow Webhook Trigger** | IN | Receive data into BeeAX from external systems |
