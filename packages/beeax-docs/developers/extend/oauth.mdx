...
BeeAX implements OAuth 2.0 with authorization code + PKCE for user-facing apps and client credentials for server-to-server access. Clients are registered dynamically via [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591) — no manual setup in a dashboard.
...
| BeeAX App with UI extensions | [Apps](/developers/extend/apps/getting-started) (OAuth is handled automatically) |

## Register a Client

BeeAX supports **dynamic client registration** per [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591). No manual setup needed — register programmatically:
...
Use this flow when your app acts on behalf of a BeeAX user.
...
After authorization, BeeAX redirects back to your `redirect_uri`:
...
BeeAX publishes its OAuth configuration at a standard discovery endpoint:
