...
If you have an existing BeeAX instance with production data, **DO NOT** follow these steps as they will permanently delete your database!

While installing BeeAX for the first time, you might want to change the default database password.
...
During BeeAX installation, you need to provision your postgres database with the right schemas, extensions, and users.
...
#### Missing beeax-x package

Make sure to run yarn in the root directory and then run `npx nx server:dev beeax-server`. If this still doesn't work try building the missing package manually.
...
#### While running `npx nx start` or `npx nx start beeax-front`, Out of memory error is thrown

Run only the services you need, instead of `npx nx start`. For instance, if you work on the server, run only `npx nx worker beeax-server`
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. At BeeAX, we noticed that some VScode extensions were taking a lot of RAM so we temporarily disable them.
...
- Open your BeeAX app, navigate to `/settings`, and enable the **Advanced** toggle at the bottom left of the screen.
- Create a new webhook.
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in BeeAX. Set the **Filters** to `Companies` and `Created`.
<div style={{textAlign: 'center'}}>
...
Comment out checker plugin in `packages/beeax-ui/vite-config.ts` like in example below
```
plugins: [
...
If you're running BeeAX behind a reverse proxy and experiencing connection issues:
...
2. **Check Reverse Proxy Settings:**
    ...
    - Confirm that your reverse proxy is correctly forwarding requests to the BeeAX server.
    - Ensure headers like `X-Forwarded-For` and `X-Forwarded-Proto` are properly set.

3. **Restart Services:**

    After making changes, restart both the reverse proxy and BeeAX containers.
...
- Community Support:
    ...
    Reach out to the [BeeAX community](https://github.com/beeax/beeax/issues) or [support channels](https://discord.gg/cx5n4Jzs57) for assistance.
