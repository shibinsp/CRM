...
This guide provides step-by-step instructions to install and configure the BeeAX application using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls that could break your setup.
...
Install the latest stable version of BeeAX with a single command:
```bash
...
If you host beeaxCRM on your own computer, open your browser and navigate to [http://localhost:3000](http://localhost:3000).
...
### Expose BeeAX to External Access

By default, BeeAX runs on `localhost` at port `3000`. To access it via an external domain or IP address, you need to configure the `SERVER_URL` in your `.env` file.
...
SSL (HTTPS) is required for certain browser features to work properly. While these features might work during local development (as browsers treat localhost differently), a proper SSL setup is needed when hosting BeeAX on a regular domain.
...
We strongly recommend setting up BeeAX behind a reverse proxy with SSL termination for optimal security and functionality.
