...
Objects are structures that allow you to store data (records, attributes, and values) specific to an organization. BeeAX provides both standard and custom objects.

Standard objects are in-built objects with a set of attributes available for all users. Examples of standard objects in BeeAX include Company and Person. Standard objects have standard fields that are also available for all BeeAX users, like Company.displayName.
