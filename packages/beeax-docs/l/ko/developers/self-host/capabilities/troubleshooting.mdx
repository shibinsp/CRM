...
BeeAX를 처음 설치할 때, 기본 데이터베이스 비밀번호를 변경하고 싶을 수 있습니다.
...
BeeAX 설치 중에 적절한 스키마, 확장, 사용자를 사용하여 postgres 데이터베이스를 프로비저닝해야 합니다.
프로비저닝이 성공적으로 이루어지면 데이터베이스에 `기본` 및 `메타데이터` 스키마가 있어야 합니다.
그렇지 않을 경우, 컴퓨터에서 여러 개의 postgres 인스턴스가 실행되지 않도록 하십시오.

#### 모듈 'beeax-emails' 또는 해당 형식 선언을 찾을 수 없습니다.

데이터베이스 초기화하기 전에 `beeax-emails` 패키지를 빌드해야 합니다: `npx nx run beeax-emails:build`

#### beeax-x 패키지가 누락됨

루트 디렉토리에서 yarn을 실행한 다음 `npx nx server:dev beeax-server` 를 실행하십시오. 여전히 작동하지 않으면 누락된 패키지를 수동으로 빌드해보세요.
...
#### `npx nx start` 또는 `npx nx start beeax-front` 실행 시 메모리 부족 오류 발생

`npx nx start` 대신 필요한 서비스만 실행하십시오. 예를 들어, 서버에서 작업하는 경우, `npx nx worker beeax-server`만 실행하십시오.

**작동하지 않는 경우:**
WSL에서 `npx nx run beeax-server:start`만 실행하려고 했으나 아래의 메모리 오류로 실패할 경우:
...
**작동하지 않는 경우:**
어떤 프로세스가 대부분의 머신 RAM을 소모하고 있는지 조사하십시오. BeeAX에서는 일부 VScode 확장이 많은 RAM을 소모하고 있음을 발견하여 임시로 비활성화했습니다.
...
* BeeAX 앱을 열고 `/settings`로 이동하여 화면 왼쪽 하단에 있는 **고급** 토글을 활성화하십시오.
* 새 웹훅 생성.
* **고유한 웹훅 URL**을 BeeAX의 **엔드포인트 URL** 필드에 붙여넣습니다. **필터**를 `회사`와 `생성됨`으로 설정하십시오.
...
`packages/beeax-ui/vite-config.ts`의 체크 플러그인을 아래 예와 같이 주석 처리하십시오
...
BeeAX를 리버스 프록시 뒤에서 실행 중이며 연결 문제가 있는 경우:
...
2. **리버스 프록시 설정 확인:**
   ...
   * 리버스 프록시가 BeeAX 서버로 요청을 정확히 전달하는지 확인하십시오.
   * `X-Forwarded-For`, `X-Forwarded-Proto`와 같은 헤더가 적절히 설정되어 있는지 확인하십시오.

3. **서비스 재시작:**

   변경 후, 리버스 프록시와 BeeAX 컨테이너를 모두 다시 시작하십시오.
...
* 커뮤니티 지원:
  ...
  [BeeAX 커뮤니티](https://github.com/beeax/beeax/issues)나 [지원 채널](https://discord.gg/cx5n4Jzs57)에 문의하십시오.
