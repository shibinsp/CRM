---
title: 프런트 컴포넌트
description: BeeAX의 UI 내부에서 샌드박스 환경으로 격리된 채 렌더링되는 React 컴포넌트를 빌드하세요.
icon: window-maximize
---

프런트 컴포넌트는 BeeAX의 UI 내부에서 직접 렌더링되는 React 컴포넌트입니다. 이들은 Remote DOM을 사용하는 격리된 Web Worker에서 실행됩니다 — 코드는 샌드박스 처리되지만 iframe이 아닌 페이지 내에서 네이티브로 렌더링됩니다.

## 프런트 컴포넌트를 사용할 수 있는 위치

프런트 컴포넌트는 BeeAX 내에서 두 위치에 렌더링될 수 있습니다:
...
`yarn beeax dev`로 동기화한 후(또는 일회성으로 `yarn beeax dev --once`를 실행한 경우), 페이지 우측 상단에 빠른 작업이 표시됩니다:
...
컴포넌트가 `null`을 반환하기 때문에, BeeAX는 해당 컴포넌트에 대한 컨테이너 렌더링을 생략합니다 — 레이아웃에 빈 공간이 생기지 않습니다. 컴포넌트는 여전히 모든 훅과 호스트 통신 API에 접근할 수 있습니다.

## SDK Command 컴포넌트

`beeax-sdk` 패키지는 헤드리스 프런트 컴포넌트를 위해 설계된 네 가지 Command 헬퍼 컴포넌트를 제공합니다. 각 컴포넌트는 마운트 시 동작을 실행하고, 스낵바 알림을 표시하여 오류를 처리하며, 완료되면 프런트 컴포넌트를 자동으로 언마운트합니다.

`beeax-sdk/command`에서 임포트하세요:
...
`httpRouteTriggerSettings`로 선언된 로직 함수는 HTTP를 통해 해당 라우트 경로에서 액세스할 수 있습니다. BeeAX는 워커에 함수들이 제공되는 기본 URL을 `TWENTY_FUNCTIONS_URL`로 주입하고, 호출을 인증하는 `TWENTY_APP_ACCESS_TOKEN`도 함께 주입합니다. 아직 자체 함수를 호출하기 위한 전용 SDK 클라이언트는 없으므로, 일반 `fetch`로 호출하세요:

> **BeeAX Cloud에서 HTTP로 트리거되는 로직 함수는 작업공간별 전용 도메인에서 제공됩니다**: `https://\<your-workspace-subdomain>.beeax.com\<path>` — 이는 `TWENTY_FUNCTIONS_URL`이 정확히 가리키는 주소입니다. 외부 호출자의 경우, 함수의 **HTTP trigger** 설정 또는 애플리케이션의 **Settings** 탭에서 정확한 URL을 복사하세요.
...
`TWENTY_FUNCTIONS_URL`에 추가되는 경로는 로직 함수의 `httpRouteTriggerSettings.path`입니다. `isAuthRequired: true`로 유지하세요. 컴포넌트를 위해 BeeAX가 발급하는 `TWENTY_APP_ACCESS_TOKEN`이 요청을 인증합니다:
...
프론트 컴포넌트에서 BeeAX 레코드를 읽거나 쓰려면 `beeax-client-sdk/rest`의 `RestApiClient`를 사용하세요. 이는 `CoreApiClient` 및 `MetadataApiClient`와 동일한 클라이언트 패밀리에 속하지만, GraphQL API 대신 BeeAX REST API(`/rest/...`)를 대상으로 하며, 기본 URL은 `TWENTY_API_URL`에서 읽어 옵니다.
...
| `TWENTY_API_URL`          | BeeAX 코어 API의 기본 URL      |
| `TWENTY_APP_ACCESS_TOKEN` | 앱의 역할 범위로 제한된 단기간 유효한 토큰   |

## 호스트 통신 API

프런트 컴포넌트는 `beeax-sdk`의 함수를 사용해 내비게이션, 모달, 알림을 트리거할 수 있습니다:
...
* **BeeAX UI 컴포넌트** — `beeax-sdk/ui`에서 임포트(버튼, 태그, 상태, 칩, 아바타 등)
