...
* **빌드에 포함** — `yarn beeax dev:build`는 모든 퍼블릭 에셋을 배포 출력물에 번들합니다.

## `getPublicAssetUrl`로 퍼블릭 에셋에 접근하기

`beeax-sdk`의 `getPublicAssetUrl` 헬퍼를 사용해 `public/` 디렉터리의 파일 전체 URL을 가져오세요. 이는 로직 함수와 프런트 컴포넌트 모두에서 동작합니다.
