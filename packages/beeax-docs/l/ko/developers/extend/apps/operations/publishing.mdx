...
title: 게시
icon: 업로드
description: BeeAX 앱을 마켓플레이스에 배포하거나 내부에 배포하세요.
---
...
* **타르볼 배포** — 내부 또는 비공개 사용을 위해 특정 BeeAX 서버에 앱을 직접 업로드합니다.
* **npm에 게시** — 모든 워크스페이스가 검색하고 설치할 수 있도록 BeeAX 마켓플레이스에 앱을 등재합니다.
...
이는 TypeScript 소스를 컴파일하고, 로직 함수와 프런트 컴포넌트를 트랜스파일하며, 모든 항목을 `.beeax/output/`에 기록합니다. 수동 배포 또는 publish 명령을 위한 `.tgz` 패키지도 생성하려면 `--tarball`을 추가하세요.

## 서버에 배포(타르볼)

공개하고 싶지 않은 앱 — 독점 도구, 엔터프라이즈 전용 통합 또는 실험적 빌드 — 의 경우, 타르볼을 BeeAX 서버에 직접 배포할 수 있습니다.

### 사전 준비

배포 전에 대상 서버를 가리키도록 구성된 원격이 필요합니다. 원격은 서버 URL과 인증 자격 증명을 로컬의 `~/.beeax/config.json`에 저장합니다.
...
2. `yarn beeax app:publish --private`(또는 `yarn beeax app:publish --private --remote production`)를 실행합니다.
...
앱이 특정 BeeAX 서버 버전에서 도입된 기능(예: v2.3.0에 추가된 OAuth 공급자)을 사용한다면, `package.json`의 `engines.beeax` 필드를 사용하여 앱에 필요한 최소 서버 버전을 선언해야 합니다:
...
* `engines.beeax`가 설정되어 있고 대상 서버의 버전이 해당 범위를 충족하지 않으면, 배포(tarball 업로드) 또는 설치가 `SERVER_VERSION_INCOMPATIBLE` 오류와 함께 거부되며 필요한 범위와 실제 서버 버전을 모두 나타내는 메시지가 표시됩니다.
* `engines.beeax`가 **설정되어 있지 않으면**, 앱은 모든 서버 버전에서 허용됩니다(기존 앱과 하위 호환).
* 서버에 `APP_VERSION`이 구성되어 있지 않으면, 검사가 생략됩니다.

<Note>
서버가 최종 검증을 수행합니다 — 타르볼 업로드와 워크스페이스 설치 모두에서 `engines.beeax`를 검증합니다. 타르볼을 별도 경로로 배포하거나 마켓플레이스에서 설치하더라도, 서버는 여전히 호환성을 강제합니다.
</Note>

## 자동화된 CI/CD (스캐폴드된 워크플로)

`create-beeax-app`으로 생성된 앱은 `.github/workflows/` 아래에 기본으로 두 개의 GitHub Actions 워크플로가 포함되어 있습니다. 저장소를 GitHub에 푸시하는 즉시 실행할 준비가 되어 있습니다 — CI에는 추가 설정이 필요 없고, CD에는 단 하나의 시크릿만 필요합니다.
...
1. 앱의 소스 코드를 체크아웃합니다.
2. `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` 컴포지트 액션을 사용해 격리된 BeeAX 테스트 인스턴스를 생성합니다(CI에서 `yarn beeax docker:start --test`와 동등합니다).
...
* `TWENTY_VERSION`(환경 변수, 기본값 `latest`) — `ci.yml`에서 이 값을 수정하여 CI에서 사용하는 BeeAX 서버 버전을 고정합니다.
...
매번 `main`으로 푸시될 때 구성된 BeeAX 서버에 앱을 배포하며, `deploy` 라벨이 적용된 경우 풀 리퀘스트에서도 선택적으로 배포합니다.

**하는 일:**

1. 라벨이 지정된 PR의 경우 PR 헤드, 아니면 푸시된 커밋을 체크아웃합니다.
2. `beeax/beeax/.github/actions/deploy-beeax-app@main`를 실행합니다 — CI에서의 `yarn beeax app:publish --private`에 해당합니다.
3. `beeax/beeax/.github/actions/install-beeax-app@main`를 실행하여 새로 배포된 버전을 대상 워크스페이스에 설치합니다.

**필수 구성:**

| 설정                      | 위치                                                        | 목적                                               |
| ----------------------- | --------------------------------------------------------- | ------------------------------------------------ |
| `TWENTY_DEPLOY_URL`     | `cd.yml`의 `env`(기본값 `http://localhost:3000`)              | 배포 대상 BeeAX 서버입니다. 처음 사용하기 전에 실제 서버 URL로 변경하세요. |
...
두 워크플로 모두 `@main`의 재사용 가능한 액션을 참조하므로, `beeax/beeax` 저장소의 액션 업데이트가 자동으로 반영됩니다. 결정적 빌드를 원한다면, 각 `uses:` 줄에서 `@main`을 커밋 SHA 또는 릴리스 태그로 바꾸세요.

## npm에 게시

npm에 게시하면 BeeAX 마켓플레이스에서 앱을 찾을 수 있게 됩니다. 모든 BeeAX 워크스페이스는 UI에서 바로 마켓플레이스 앱을 탐색, 설치 및 업그레이드할 수 있습니다.

### 요구 사항

* [npm](https://www.npmjs.com) 계정
* `package.json`의 `keywords` 배열에 있는 `beeax-app` 키워드(수동으로 추가하세요 — `create-beeax-app` 템플릿에 기본적으로 포함되어 있지 않습니다)
...
BeeAX 서버는 npm 레지스트리에서 마켓플레이스 카탈로그를 **매시간** 동기화합니다.
...
<Note>
앱에서 `defineApplication()`에 `aboutDescription`을 정의하지 않으면, 마켓플레이스는 소개 페이지 콘텐츠로 npm에 게시된 패키지의 `README.md`를 자동으로 사용합니다. 즉, npm과 BeeAX 마켓플레이스 모두에서 하나의 README만 관리하면 됩니다. 마켓플레이스에서 다른 설명을 사용하려면 `aboutDescription`을 명시적으로 설정하세요.
</Note>
...
다른 CI 시스템(GitLab CI, CircleCI 등)에서도 동일한 세 가지 명령을 사용합니다: `yarn install`, `yarn beeax dev:build`, 그리고 `.beeax/output`에서 `npm publish`.
...
BeeAX UI의 **설정 > 애플리케이션** 페이지로 이동하면 마켓플레이스 앱과 타르볼로 배포된 앱을 모두 탐색하고 설치할 수 있습니다.
...
더 최신 버전을 설치하려면 먼저 배포하거나 게시한 다음 `yarn beeax app:install`을 다시 실행하세요.
</Note>
