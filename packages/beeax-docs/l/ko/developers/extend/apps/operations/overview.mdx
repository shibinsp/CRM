...
\*\*운영 계층(operations layer)\*\*은 앱을 *사용하는 것*이 아니라 앱에 *수행하는* 모든 작업을 의미합니다. 예를 들어 CLI 명령 실행, 실제 BeeAX 서버를 대상으로 한 통합 테스트 실행, CI 구성, 그리고 단일 서버에 배포되는 tarball로 또는 마켓플레이스에 등록되는 npm 패키지로 릴리스를 배포하는 작업 등이 여기에 포함됩니다.
...
<CardGroup cols={2}>
  <Card title="CLI" icon="터미널" href="/l/ko/developers/extend/apps/operations/cli">
    `yarn beeax` 참고 — exec, logs, uninstall, remotes.
  </Card>
