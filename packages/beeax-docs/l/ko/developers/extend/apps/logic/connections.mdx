...
* `serverVariables`(`applicationVariables` 아님)을 사용하세요 — OAuth 자격 증명은 서버 전체에 적용되며 BeeAX 서버마다 하나의 OAuth 앱만 사용합니다.
...
4. 서버 관리자 권한으로 BeeAX에서 설치된 앱을 열고 → 해당 `serverVariables`에 값을 설정하세요.
