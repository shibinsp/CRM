...
웹훅은 BeeAX에서 이벤트가 발생하면 데이터를 실시간으로 귀하의 시스템으로 푸시합니다 — 폴링이 필요 없습니다. 이를 사용하여 외부 시스템을 동기화 상태로 유지하고, 자동화를 트리거하거나, 알림을 보낼 수 있습니다.
...
BeeAX는 다음 이벤트 유형에 대해 웹훅을 전송합니다:
...
BeeAX는 보안을 위해 각 웹훅 요청에 서명합니다. 요청의 진위를 보장하기 위해 서명을 검증하세요.

### 헤더

| 헤더                           | 설명             |
| ---------------------------- | -------------- |
| `X-BeeAX-Webhook-Signature` | HMAC SHA256 서명 |
| `X-BeeAX-Webhook-Timestamp` | 요청 타임스탬프       |

### 검증 단계

1. `X-BeeAX-Webhook-Timestamp`에서 타임스탬프를 가져옵니다
2. 다음 문자열을 생성합니다: `{timestamp}:{JSON payload}`
3. 웹훅 비밀을 사용하여 HMAC SHA256을 계산합니다
4. `X-BeeAX-Webhook-Signature`와 비교합니다
...
| **워크플로 웹훅 트리거**    | IN  | 외부 시스템에서 BeeAX로 데이터를 수신합니다        |
