---
title: BeeAX 앱
description: BeeAX 맞춤설정을 코드로 구축하고 관리하세요.
---
...
앱을 사용하면 BeeAX 맞춤설정을 **코드로** 구축하고 관리할 수 있습니다. 모든 것을 UI에서 구성하는 대신, 데이터 모델과 로직 함수를 코드로 정의합니다 — 이를 통해 더 빠르게 구축·유지 관리하고 여러 워크스페이스에 배포할 수 있습니다.
...
* Node.js 24+ 및 Yarn 4
* BeeAX 워크스페이스와 API 키(https://app.beeax.com/settings/api-webhooks에서 생성)
...
참고: [create-beeax-app](https://www.npmjs.com/package/create-beeax-app) 및 [beeax-sdk CLI](https://www.npmjs.com/package/beeax-sdk)의 CLI 참고 페이지도 확인하세요.

## 프로젝트 구조(스캐폴딩됨)

`npx create-beeax-app@latest my-beeax-app`를 실행하면, 스캐폴더가 다음을 수행합니다:

* `my-beeax-app/`에 최소한의 기본 애플리케이션을 복사합니다
* 로컬 `beeax-sdk` 종속성과 Yarn 4 구성을 추가합니다
* `beeax` CLI와 연결된 설정 파일과 스크립트를 생성합니다
...
* **package.json**: 앱 이름, 버전, 엔진(Node 24+, Yarn 4)을 선언하고, `beeax-sdk`와 함께 `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall`, `auth:login` 같은 스크립트를 추가합니다. 이 스크립트들은 로컬 `beeax` CLI에 위임됩니다.
...
* `yarn app:dev`는 `node_modules/beeax-sdk/generated`에 타입드 BeeAX 클라이언트를 자동으로 생성합니다.
...
자격 증명은 사용자별로 `~/.beeax/config.json`에 저장됩니다. 여러 프로필을 유지하고 프로필 간에 전환할 수 있습니다.
...
beeax-sdk는 앱 내부에서 사용하는 타입드 빌딩 블록과 헬퍼 함수를 제공합니다. 가장 자주 사용하게 될 핵심 요소는 다음과 같습니다.
...
<Note>
  **기본 필드는 자동으로 생성됩니다.** 사용자 정의 객체를 정의하면 BeeAX가 `name`, `createdAt`, `updatedAt`, `createdBy`, `position`, `deletedAt` 등의 표준 필드를 자동으로 추가합니다. 이 필드들은 `fields` 배열에 정의할 필요가 없습니다. 사용자 정의 필드만 추가하세요.
</Note>
...
* Hello World 앱의 동작 예제를 참조하세요: [`packages/beeax-apps/hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
라우트 트리거가 로직 함수를 호출하면, AWS HTTP API v2 형식을 따르는 `RoutePayload` 객체를 받습니다. `beeax-sdk`에서 해당 타입을 임포트하세요:
...
`yarn app:dev`는 `node_modules/beeax-sdk/generated`에 타입드 BeeAX 클라이언트를 자동으로 생성합니다. 함수에서 사용하세요:
...
함수가 BeeAX에서 실행될 때, 플랫폼은 코드가 실행되기 전에 자격 증명을 환경 변수로 주입합니다:

* `TWENTY_API_URL`: 앱이 대상으로 하는 BeeAX API의 기본 URL.
...
객체, 함수, 여러 트리거를 보여주는 최소한의 엔드투엔드 예제를 [여기](https://github.com/beeax/beeax/tree/main/packages/beeax-apps/hello-world)에서 살펴보세요:

## 수동 설정(스캐폴더 없이)

최적의 시작 경험을 위해 `create-beeax-app` 사용을 권장하지만, 프로젝트를 수동으로 설정할 수도 있습니다. CLI를 전역으로 설치하지 마세요. 대신 `beeax-sdk`를 로컬 종속성으로 추가하고 package.json에 스크립트를 연결하세요:
...
* 서버에 연결할 수 없음: API URL과 BeeAX 서버에 접근 가능한지 확인하세요.
