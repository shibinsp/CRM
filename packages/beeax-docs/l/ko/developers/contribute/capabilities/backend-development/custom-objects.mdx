...
개체는 조직 고유의 데이터(레코드, 속성 및 값)를 저장할 수 있는 구조입니다. BeeAX에는 표준 개체와 사용자 지정 개체가 모두 제공됩니다.

표준 개체는 모든 사용자에게 제공되는 속성이 있는 내장 개체입니다. BeeAX의 표준 개체 예시로는 Company(회사)와 Person(사람)이 있습니다. 표준 개체는 모든 BeeAX 사용자에게 제공되는 표준 필드를 가지고 있으며, 예를 들어 Company.displayName과 같습니다.
