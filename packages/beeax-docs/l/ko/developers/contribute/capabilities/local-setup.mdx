...
title: 로컬 설정
icon: laptop-code
description: 콘트리뷰터(기여자)나 호기심 많은 개발자를 위한 가이드로서, BeeAX를 로컬에서 실행하고자 하는 분들에게 드리는 안내입니다.
---
...
BeeAX를 설치하고 사용하기 전에, 먼저 컴퓨터에 다음을 설치하세요:
...
BeeAX는 최상의 성능을 제공하기 위해 Redis 캐시가 필요합니다.
...
<Info>
**멀티 워크스페이스 모드:** 기본적으로 BeeAX는 하나의 워크스페이스만 생성할 수 있는 단일 워크스페이스 모드로 실행됩니다. 멀티 워크스페이스 지원을 활성화하려면(서브도메인 기반 기능을 테스트할 때 유용합니다), 서버의 `.env` 파일에서 `IS_MULTIWORKSPACE_ENABLED=true`로 설정하세요. 자세한 내용은 [멀티 워크스페이스 모드](/l/ko/developers/self-host/capabilities/setup#multi-workspace-mode)를 참조하세요.
</Info>

## 6단계: 의존성 설치

BeeAX 서버를 빌드하고 데이터베이스에 몇 가지 데이터를 시드하기 위해, 다음 명령어를 실행하세요:
...
BeeAX의 프론트엔드는 [http://localhost:3001](http://localhost:3001)에서 실행됩니다.
기본 데모 계정을 사용하여 로그인할 수 있습니다: `tim@apple.dev` (비밀번호: `tim@apple.dev`)

**백엔드**

* BeeAX의 서버는 [http://localhost:3000](http://localhost:3000)에서 실행됩니다.
