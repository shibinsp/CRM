---
title: MCP 서버
description: Model Context Protocol을 사용하여 AI 어시스턴트를 BeeAX 워크스페이스에 연결하세요.
---
...
BeeAX는 Claude Desktop, Claude Code, Cursor, ChatGPT 등 AI 어시스턴트가 자연어로 CRM 데이터를 읽고 쓸 수 있도록 [MCP](https://modelcontextprotocol.io/) 서버를 제공합니다.

MCP 엔드포인트로 **워크스페이스 URL**(BeeAX에 접속할 때 사용하는 URL)을 사용하세요. BeeAX Cloud에서는 워크스페이스 URL이 `https://{mycompany}.beeax.com` 또는 커스텀 도메인일 수 있습니다. 서버는 다음 위치에서 사용할 수 있습니다:

| 환경         | MCP 엔드포인트                                                                  |
| ---------- | -------------------------------------------------------------------------- |
| **클라우드**   | `https://{your-workspace-url}/mcp` (예: `https://mycompany.beeax.com/mcp`) |
...
다음을 MCP 클라이언트 구성에 추가하고 `{your-workspace-url}`를 워크스페이스 호스트로 바꾸세요(예: `mycompany.beeax.com`):
...
1. `/.well-known/oauth-protected-resource` 및 `/.well-known/oauth-authorization-server`를 통해 BeeAX의 OAuth 메타데이터를 발견합니다
...
BeeAX에서 **Settings > AI > More > MCP Server**로 이동하세요. 인증 방법(OAuth 또는 API Key)을 선택하고, JSON 스니펫(이미 워크스페이스 URL이 포함되어 있습니다)을 복사하여 MCP 클라이언트의 구성 파일에 붙여넣으세요.
...
MCP 클라이언트를 다시 시작하세요(또는 구성을 다시 로드하세요). OAuth를 사용 중이라면 액세스 권한 부여를 위해 BeeAX로 리디렉션됩니다. API 키를 사용하는 경우 즉시 연결됩니다.
...
연결되면 MCP 서버는 BeeAX API를 미러링하는 도구들을 제공합니다. 권장 워크플로는 다음과 같습니다:
...
자가 호스팅 인스턴스의 경우 `{your-workspace-url}`을 서버 URL로 바꾸세요. 환경의 `SERVER_URL`이 BeeAX 인스턴스의 공개 URL과 일치하는지 확인하세요 — 이는 OAuth 디스커버리 메타데이터를 생성하는 데 사용됩니다.
