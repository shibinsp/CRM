...
파일을 업로드하면 BeeAX가 열을 분석하여 기존 필드와 일치시키려고 시도합니다.

### 자동 매핑

BeeAX는 다음을 기준으로 열을 일치시키려고 시도합니다:

* 열 헤더 이름(정확 또는 유사 일치)
* 데이터 유형 감지(날짜, 숫자, 이메일)
* 일반적인 필드 패턴

<Note>**빠른 팁:** 가져오려는 객체에서 몇 행을 내보내세요. 내보낸 파일에는 BeeAX가 기대하는 정확한 열 이름이 포함되어, 가져오는 동안 자동 매핑이 원활해집니다.</Note>

### 수동 매핑 옵션

각 열에 대해 다음을 수행할 수 있습니다:

* **필드에 매핑**: 드롭다운에서 일치하는 BeeAX 필드를 선택
...
가져오기 중 `id`를 지정하는 것은 선택 사항입니다. 제공하지 않으면 BeeAX가 ID를 자동으로 생성합니다.
