---
title: CSV로 회사 가져오기
description: BeeAX로 회사를 가져오는 포괄적인 단계별 가이드.
---

## 개요

이 가이드는 회사를 BeeAX로 가져오는 과정을 단계별로 안내합니다. **회사는 먼저 가져와야 합니다**. People과 Opportunities가 회사에 연결되기 때문입니다.
...
<Note>
**전문가 팁:** 가져오기 중에 **Download sample file**을 클릭하여 BeeAX가 기대하는 정확한 열 이름을 확인하세요.
</Note>
...
3. BeeAX가 파일을 분석할 때까지 기다리세요

## 4단계: 열 매핑

BeeAX가 열을 필드에 자동으로 매칭하려고 시도합니다. 검토 및 조정:
...
1. BeeAX에서 사용자의 값을 기존 옵션과 나란히 표시합니다
2. 파일의 각 값을 BeeAX 옵션과 매칭하세요
3. 필요하면 새 옵션을 만드세요
...
가져오기를 완료하기 전에 BeeAX가 데이터를 검증합니다:
...
| 오류             | 원인                  | 해결책                         |
| -------------- | ------------------- | --------------------------- |
| **중복 도메인**     | 도메인이 BeeAX에 이미 존재함 | 파일에서 제거하거나 기존 레코드를 업데이트하세요  |
...
<AccordionGroup>
  <Accordion title="왜 두 회사가 동일한 도메인을 가질 수 없나요?">
    도메인은 BeeAX에서 고유 식별자입니다. 이는 중복 회사를 방지하고 이메일 동기화가 올바른 회사에 이메일을 정확히 연결되도록 보장합니다.
  </Accordion>
  ...
  <Accordion title="이미 존재하는 도메인을 가져오면 어떻게 되나요?">
    기존 회사와 일치하는 고유 식별자(도메인 또는 id)를 포함하면, BeeAX는 중복을 생성하는 대신 해당 회사를 업데이트합니다.
  </Accordion>
