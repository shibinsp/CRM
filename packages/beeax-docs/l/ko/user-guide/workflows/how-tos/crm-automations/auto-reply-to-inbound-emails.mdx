...
BeeAX에서는 그 지문이 Message 객체의 `headerMessageId`로 저장됩니다. 워크플로우가 이를 가져와 Send Email 작업의 In-Reply-To 필드에 전달합니다.
...
이메일이 BeeAX에 도착할 때마다 이 트리거가 실행됩니다.
...
* **VIP에게만 회신** — 발신자의 도메인을 확인하거나 BeeAX의 연락처로 존재하는지 확인하는 분기를 추가하세요
