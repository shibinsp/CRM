...
Das übergeordnete Objekt ("Eins"-Seite) muss in BeeAX existieren, bevor Sie darauf verweisen können.
...
1. Exportieren Sie die übergeordneten Datensätze aus BeeAX
...
Verwenden Sie `https://domain.com` (nicht nur `domain.com`). Dies entspricht der Art, wie BeeAX Unternehmensdomains speichert, und verhindert Zuordnungsfehler.
</Warning>
...
BeeAX verknüpft automatisch jeden untergeordneten Datensatz mit dem passenden übergeordneten Datensatz.
...
| **Nicht vorhandene Datensätze**            | Der übergeordnete Datensatz existiert in BeeAX nicht     | Importieren Sie zunächst übergeordnete Datensätze oder prüfen Sie auf Tippfehler |
| **Unterschiedliche Groß-/Kleinschreibung** | `Acme.com` in der Datei, aber `acme.com` in BeeAX        | Achten Sie auf exakt übereinstimmende Groß-/Kleinschreibung                      |
...
<AccordionGroup>
  <Accordion title="Was ist, wenn mein übergeordneter Datensatz keine Domain oder E-Mail hat?">
    ...
    1. Verwenden Sie die BeeAX-`id` (exportieren Sie übergeordnete Datensätze, um ihre IDs zu erhalten)
