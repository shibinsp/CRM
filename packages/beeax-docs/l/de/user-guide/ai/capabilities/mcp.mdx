---
title: MCP-Server
description: Verbinden Sie KI-Assistenten mit Ihrem BeeAX-Workspace über das Model Context Protocol.
---
...
BeeAX stellt einen [MCP](https://modelcontextprotocol.io/)-Server bereit, damit KI-Assistenten — Claude Desktop, Claude Code, Cursor, ChatGPT und andere — Ihre CRM-Daten in natürlicher Sprache lesen und schreiben können.

Verwenden Sie Ihre **Workspace-URL** (die URL, mit der Sie auf BeeAX zugreifen) als MCP-Endpunkt. In BeeAX Cloud kann Ihre Workspace-URL `https://{mycompany}.beeax.com` oder eine benutzerdefinierte Domain sein. Der Server ist verfügbar unter:

| Umgebung          | MCP-Endpunkt                                                                  |
| ----------------- | ----------------------------------------------------------------------------- |
| **Cloud**         | `https://{your-workspace-url}/mcp` (z. B. `https://mycompany.beeax.com/mcp`) |
...
Fügen Sie dies zu Ihrer MCP-Client-Konfiguration hinzu und ersetzen Sie `{your-workspace-url}` durch den Host Ihrer Workspace-URL (z. B. `mycompany.beeax.com`):
...
1. Die OAuth-Metadaten von BeeAX über `/.well-known/oauth-protected-resource` und `/.well-known/oauth-authorization-server` ermitteln
...
Gehen Sie in BeeAX zu **Settings > AI > More > MCP Server**. Wählen Sie Ihre Authentifizierungsmethode (OAuth oder API-Schlüssel), kopieren Sie das JSON-Snippet (es verwendet bereits Ihre Workspace-URL) und fügen Sie es in die Konfigurationsdatei Ihres MCP-Clients ein.
...
Starten Sie Ihren MCP-Client neu (oder laden Sie die Konfiguration neu). Bei Verwendung von OAuth werden Sie zu BeeAX weitergeleitet, um den Zugriff zu autorisieren. Bei Verwendung eines API-Schlüssels wird die Verbindung sofort hergestellt.
...
Nach der Verbindung stellt der MCP-Server Tools bereit, die die BeeAX-API widerspiegeln. Der empfohlene Workflow ist:
...
Für selbstgehostete Instanzen ersetzen Sie `{your-workspace-url}` durch die URL Ihres Servers. Stellen Sie sicher, dass `SERVER_URL` in Ihrer Umgebung der öffentlichen URL Ihrer BeeAX-Instanz entspricht — dieser Wert wird verwendet, um die OAuth-Discovery-Metadaten zu generieren.

```bash
SERVER_URL=https://beeax.yourcompany.com
```
