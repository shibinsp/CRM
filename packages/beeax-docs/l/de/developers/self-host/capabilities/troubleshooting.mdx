...
Wenn Sie bereits eine bestehende BeeAX-Instanz mit Produktionsdaten haben, **NICHT** diesen Schritten folgen, da diese permanent Ihre Datenbank löschen!

Bei der ersten Installation von BeeAX möchten Sie möglicherweise das Standard-Datenbankpasswort ändern.
...
Während der BeeAX-Installation müssen Sie Ihre Postgres-Datenbank mit den richtigen Schemata, Erweiterungen und Benutzern bereitstellen.
...
#### Modul 'beeax-emails' oder seine entsprechenden Typdeklarationen nicht gefunden.

Sie müssen das Paket `beeax-emails` erstellen, bevor Sie die Initialisierung der Datenbank mit `npx nx run beeax-emails:build` ausführen.

#### Fehlendes BeeAX-x Paket

Stellen Sie sicher, dass Sie `yarn` im Root-Verzeichnis ausführen und dann `npx nx server:dev beeax-server` ausführen. Wenn das immer noch nicht funktioniert, versuchen Sie, das fehlende Paket manuell zu erstellen.
...
#### Beim Ausführen von `npx nx start` oder `npx nx start beeax-front` wird ein Speicherfehler angezeigt

Führen Sie nur die Services aus, die Sie benötigen, anstatt `npx nx start`. Wenn Sie zum Beispiel am Server arbeiten, führen Sie nur `npx nx worker beeax-server` aus.

**Wenn es nicht funktioniert:**
Wenn Sie versucht haben, nur `npx nx run beeax-server:start` auf WSL zu starten und es mit dem untenstehenden Speicherfehler fehlschlägt:
...
**Wenn es nicht funktioniert:**
Untersuchen Sie, welche Prozesse den größten Teil Ihres RAMs beanspruchen. Bei BeeAX haben wir festgestellt, dass einige vscode-Erweiterungen viel RAM verwenden, daher deaktivieren wir sie vorübergehend.
...
* Öffnen Sie Ihre BeeAX-App, navigieren Sie zu `/settings`, und aktivieren Sie den **Erweitert**-Schalter unten links auf dem Bildschirm.
* Erstellen Sie einen neuen Webhook.
* Fügen Sie **Ihre einzigartige Webhook-URL** in das Feld **Endpoint-Url** in BeeAX ein. Stellen Sie die **Filter** auf `Companies` und `Created` ein.
...
Kommentieren Sie das Checker-Plugin in `packages/beeax-ui/vite-config.ts` wie im folgenden Beispiel aus
...
Wenn Sie BeeAX hinter einem Reverse-Proxy ausführen und Verbindungsprobleme haben:
...
2. **Reverse-Proxy-Einstellungen prüfen:**
   ...
   * Stellen Sie sicher, dass Ihr Reverse-Proxy Anfragen korrekt an den BeeAX-Server weiterleitet.
   * Stellen Sie sicher, dass Header wie `X-Forwarded-For` und `X-Forwarded-Proto` korrekt gesetzt sind.

3. **Dienste neu starten:**

   Starten Sie nach Änderungen sowohl den Reverse-Proxy als auch die BeeAX-Container neu.
...
* Unterstützung aus der Community:
  ...
  Wenden Sie sich an die [BeeAX Community](https://github.com/beeax/beeax/issues) oder [Support-Kanäle](https://discord.gg/cx5n4Jzs57) für Unterstützung.
