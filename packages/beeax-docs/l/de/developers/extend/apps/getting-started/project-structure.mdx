...
description: Was in einer generierten BeeAX-App enthalten ist – Dateien, Ordner und was jede einzelne davon macht.
icon: folder-tree
---

Eine neue App, die mit `npx create-beeax-app` generiert wurde, sieht so aus:
...
Beide BeeAX-SDK-Pakete gehören unter `devDependencies`, nicht unter `dependencies`:
...
* **`beeax-sdk`** stellt die `beeax`-CLI sowie die Build-/Scaffolding-Tools bereit. Es läuft nur während der Entwicklung und beim Build und wird zur Laufzeit der veröffentlichten App niemals importiert.
* **`beeax-client-sdk`** *wird* hingegen von deinem App-Code importiert (`CoreApiClient`, `MetadataApiClient`, `RestApiClient`), aber BeeAX stellt es zur Laufzeit bereit – Logikfunktionen beziehen es aus einer generierten SDK-Schicht, und Frontend-Komponenten lösen es aus serverseitig ausgelieferten Modulen auf. Deine installierte Kopie wird nur für die Typprüfung und den Build zum Zeitpunkt des Deployments verwendet, daher muss sie niemals im ausgelieferten Bundle enthalten sein.

Wenn eines der Pakete unter `dependencies` bleibt, wird es in das Runtime-Bundle der installierten App gezogen, wo es nur Ballast ist. `beeax build` gibt eine Warnung aus, wenn eines von beiden weiterhin unter `dependencies` aufgeführt ist.
