...
icon: hochladen
description: Veröffentlichen Sie Ihre BeeAX-App auf dem BeeAX-Marktplatz oder stellen Sie sie intern bereit.
---
...
* **Einen Tarball bereitstellen** — Laden Sie Ihre App direkt auf einen bestimmten BeeAX-Server für die interne oder private Nutzung hoch.
* **Auf npm veröffentlichen** — führen Sie Ihre App im BeeAX-Marktplatz auf, damit jeder Arbeitsbereich sie entdecken und installieren kann.
...
Dabei werden TypeScript-Quelltexte kompiliert, Logikfunktionen und Frontend-Komponenten transpiliert und alles in `.beeax/output/` geschrieben. Fügen Sie `--tarball` hinzu, um zusätzlich ein `.tgz`-Paket für die manuelle Verteilung oder den Publish-Befehl zu erzeugen.

## Bereitstellung auf einem Server (Tarball)

Für Apps, die Sie nicht öffentlich verfügbar machen möchten — proprietäre Tools, ausschließlich für Unternehmen bestimmte Integrationen oder experimentelle Builds — können Sie einen Tarball direkt auf einem BeeAX-Server bereitstellen.

### Voraussetzungen

Bevor Sie bereitstellen, benötigen Sie ein konfiguriertes Remote, das auf den Zielserver zeigt. Remotes speichern die Server-URL und Anmeldeinformationen lokal in `~/.beeax/config.json`.
...
2. Führen Sie `yarn beeax app:publish --private` aus (oder `yarn beeax app:publish --private --remote production`)
...
Wenn Ihre App eine Funktion verwendet, die in einer bestimmten BeeAX-Serverversion eingeführt wurde (z. B. OAuth-Anbieter, die in v2.3.0 hinzugefügt wurden), sollten Sie die minimale Serverversion, die Ihre App benötigt, mithilfe des Felds `engines.beeax` in `package.json` angeben:
...
* Wenn `engines.beeax` gesetzt ist und die Version des Zielservers den Bereich nicht erfüllt, wird die Bereitstellung (Tarball-Upload) oder Installation mit dem Fehler `SERVER_VERSION_INCOMPATIBLE` abgelehnt, zusammen mit einer Meldung, die sowohl den erforderlichen Bereich als auch die tatsächliche Serverversion angibt.
* Wenn `engines.beeax` nicht gesetzt ist, wird die App auf jeder Serverversion akzeptiert (abwärtskompatibel mit bestehenden Apps).
* Wenn auf dem Server keine `APP_VERSION` konfiguriert ist, wird die Prüfung übersprungen.

<Note>
Der Server ist die maßgebliche Prüfinstanz — er validiert `engines.beeax` sowohl beim Tarball-Upload als auch bei der Workspace-Installation. Auch wenn Sie einen Tarball außerhalb des regulären Prozesses bereitstellen oder aus dem Marktplatz installieren, erzwingt der Server weiterhin die Kompatibilität.
</Note>

## Automatisiertes CI/CD (vorgefertigte Workflows)

Apps, die mit `create-beeax-app` erzeugt wurden, enthalten von Haus aus zwei GitHub-Actions-Workflows unter `.github/workflows/`. Sie sind einsatzbereit, sobald Sie das Repository zu GitHub pushen — für CI ist keine zusätzliche Einrichtung erforderlich, und für CD ist nur ein einziges Secret nötig.
...
2. Startet eine isolierte BeeAX-Testinstanz mithilfe der Composite-Action `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (das CI-Äquivalent zu `yarn beeax docker:start --test`).
...
* `TWENTY_VERSION` (env, standardmäßig `latest`) — fixieren Sie die in CI verwendete BeeAX-Server-Version, indem Sie dies in `ci.yml` anpassen.
...
Stellt Ihre App bei jedem Push auf `main` auf einem konfigurierten BeeAX-Server bereit und optional aus einem Pull Request, wenn das Label `deploy` gesetzt ist.

**Was sie macht:**

1. Checkt den PR-Head (bei PRs mit Label) oder den gepushten Commit aus.
2. Führt `beeax/beeax/.github/actions/deploy-beeax-app@main` aus — das CI-Äquivalent zu `yarn beeax app:publish --private`.
3. Führt `beeax/beeax/.github/actions/install-beeax-app@main` aus, damit die neu bereitgestellte Version in den Ziel-Workspace installiert wird.

**Erforderliche Konfiguration:**

| Einstellung             | Wo                                                               | Zweck                                                                                                                      |
| ----------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` in `cd.yml` (standardmäßig `http://localhost:3000`)        | Der BeeAX-Server, auf den bereitgestellt werden soll. Ändern Sie dies vor der ersten Verwendung auf die echte Server-URL. |
...
Beide Workflows verweisen auf wiederverwendbare Actions mit `@main`, sodass Aktualisierungen der Actions im Repository `beeax/beeax` automatisch übernommen werden. Wenn Sie deterministische Builds möchten, ersetzen Sie `@main` in jeder `uses:`-Zeile durch eine Commit-SHA oder einen Release-Tag.

## Auf npm veröffentlichen

Die Veröffentlichung auf npm macht Ihre App im BeeAX-Marktplatz auffindbar. Jeder BeeAX-Arbeitsbereich kann Marktplatz-Apps direkt über die Benutzeroberfläche durchsuchen, installieren und aktualisieren.

### Anforderungen

* Ein [npm](https://www.npmjs.com)-Konto
* Das Schlüsselwort `beeax-app` in Ihrem `package.json`-Array `keywords` (manuell hinzufügen — es ist in der `create-beeax-app`-Vorlage standardmäßig nicht enthalten)
...
Der BeeAX-Server synchronisiert seinen Marktplatzkatalog **stündlich** aus der npm-Registry.
...
<Note>
Wenn Ihre App keine `aboutDescription` in `defineApplication()` definiert, verwendet der Marktplatz automatisch die `README.md` Ihres Pakets von npm als Inhalt der Über-uns-Seite. Das bedeutet, dass Sie eine einzige README sowohl für npm als auch für den BeeAX-Marktplatz pflegen können. Wenn Sie im Marktplatz eine andere Beschreibung möchten, setzen Sie `aboutDescription` explizit.
</Note>
...
Für andere CI-Systeme (GitLab CI, CircleCI usw.) gelten die gleichen drei Befehle: `yarn install`, `yarn beeax dev:build` und anschließend `npm publish` aus `.beeax/output`.
...
Gehen Sie zur Seite **Einstellungen > Anwendungen** in BeeAX, auf der sowohl Marktplatz- als auch per Tarball bereitgestellte Apps durchsucht und installiert werden können.
...
Um eine neuere Version zu installieren, stellen Sie sie zuerst bereit oder veröffentlichen Sie sie und führen Sie dann `yarn beeax app:install` erneut aus.
</Note>
