...
Dabei werden TypeScript-Quelltexte kompiliert, Logikfunktionen und Frontend-Komponenten transpiliert und alles in `.beeax/output/` geschrieben. Fügen Sie `--tarball` hinzu, um zusätzlich ein `.tgz`-Paket für die manuelle Verteilung oder den Deploy-Befehl zu erzeugen.
...
2. Führen Sie `yarn beeax deploy` aus (oder `yarn beeax deploy --remote production`)
...
<Note>
Der Server ist die maßgebliche Prüfinstanz — er validiert `engines.beeax` sowohl beim Tarball-Upload als auch bei der Workspace-Installation. Auch wenn Sie ein Tarball außerhalb des regulären Prozesses bereitstellen oder aus dem Marketplace installieren, erzwingt der Server weiterhin die Kompatibilität.
</Note>
...
2. Startet eine isolierte BeeAX-Testinstanz mithilfe der Composite-Action `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (das CI-Äquivalent zu `yarn beeax server start --test`).
...
2. Führt `beeax/beeax/.github/actions/deploy-beeax-app@main` aus — das CI-Äquivalent zu `yarn beeax deploy`.
...
Für andere CI-Systeme (GitLab CI, CircleCI usw.) gelten die gleichen drei Befehle: `yarn install`, `yarn beeax build` und anschließend `npm publish` aus `.beeax/output`.
...
Um eine neuere Version zu installieren, stellen Sie sie zuerst bereit oder veröffentlichen Sie sie und führen Sie dann `yarn beeax install` erneut aus.
</Note>
