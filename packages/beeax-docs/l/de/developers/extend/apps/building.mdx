---
title: Architektur
description: Wie BeeAX-Apps funktionieren — Sandboxing, Lebenszyklus und Bausteine.
icon: sitemap
---

BeeAX-Apps sind TypeScript-Pakete, die Ihren Arbeitsbereich mit benutzerdefinierten Objekten, Logik, UI-Komponenten und KI-Funktionen erweitern. Sie laufen auf der BeeAX-Plattform mit vollständigem Sandboxing und Berechtigungsverwaltung.

## Wie Apps funktionieren

Eine App ist eine Sammlung von **Entitäten**, die mithilfe von `defineEntity()`-Funktionen aus dem Paket `beeax-sdk` deklariert werden. Das SDK erkennt diese Deklarationen zur Build-Zeit per AST-Analyse und erzeugt ein **Manifest** — eine vollständige Beschreibung dessen, was Ihre App zu einem Arbeitsbereich hinzufügt.
...
| **Frontend-Komponente**    | Sandboxed React-UI auf der BeeAX-Seite                           | [Frontend-Komponenten](/l/de/developers/extend/apps/front-components) |
...
* **Frontend-Komponenten** laufen in Web Workers mit Remote DOM — von der Hauptseite isoliert, rendern aber native DOM-Elemente (keine iframes). Sie kommunizieren über eine Message-Passing-Host-API mit BeeAX.
...
* **`yarn beeax dev`** — überwacht Ihre Quelldateien und synchronisiert Änderungen in Echtzeit mit einem verbundenen BeeAX-Server. Der typisierte API-Client wird automatisch neu erzeugt, wenn sich das Schema ändert.
* **`yarn beeax build`** — kompiliert TypeScript, bündelt Logikfunktionen und Frontend-Komponenten mit esbuild und erzeugt ein Manifest.
...
<CardGroup cols={2}>
  ...
  <Card title="Frontend-Komponenten" icon="window-maximize" href="/l/de/developers/extend/apps/front-components">
    Sandboxed React-Komponenten innerhalb der UI von BeeAX.
  </Card>
