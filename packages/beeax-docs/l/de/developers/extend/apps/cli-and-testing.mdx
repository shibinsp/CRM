...
* **In Builds enthalten** — `yarn beeax build` bündelt alle öffentlichen Assets in der Distributionsausgabe.

### Zugriff auf öffentliche Assets mit `getPublicAssetUrl`

Verwenden Sie den Helper `getPublicAssetUrl` aus `beeax-sdk`, um die vollständige URL einer Datei in Ihrem `public/`-Verzeichnis zu erhalten. Dies funktioniert sowohl in Logikfunktionen als auch in Frontend-Komponenten.
...
In beiden Umgebungen stehen `beeax-client-sdk/core` und `beeax-client-sdk/metadata` als vorab bereitgestellte Module zur Verfügung — sie werden nicht gebündelt, sondern zur Laufzeit vom Server aufgelöst.

## Ihre App testen

Das SDK stellt programmgesteuerte APIs bereit, mit denen Sie Ihre App aus Testcode heraus bauen, bereitstellen, installieren und deinstallieren können. In Kombination mit [Vitest](https://vitest.dev/) und den typisierten API-Clients können Sie Integrationstests schreiben, die prüfen, dass Ihre App End-to-End gegen einen echten BeeAX-Server funktioniert.
...
Der Subpfad `beeax-sdk/cli` exportiert Funktionen, die Sie direkt aus Testcode aufrufen können:
...
Stellen Sie sicher, dass Ihr lokaler BeeAX-Server läuft, und führen Sie dann Folgendes aus:
...
### Funktionen ausführen (`yarn beeax exec`)
...
### Funktionsprotokolle ansehen (`yarn beeax logs`)
...
<Note>
Dies unterscheidet sich von `yarn beeax server logs`, das die Docker-Container-Logs anzeigt. `yarn beeax logs` zeigt die Funktionsausführungsprotokolle Ihrer App vom BeeAX-Server.
</Note>

### Eine App deinstallieren (`yarn beeax uninstall`)
...
2. Startet einen temporären BeeAX-Server mit der Aktion `beeax/beeax/.github/actions/spawn-beeax-docker-image`
...
Sie müssen keine Secrets konfigurieren — die Aktion `spawn-beeax-docker-image` startet einen flüchtigen BeeAX-Server direkt im Runner und gibt die Verbindungsdetails aus. Das Secret `GITHUB_TOKEN` wird automatisch von GitHub bereitgestellt.

Um eine bestimmte BeeAX-Version statt `latest` festzulegen, ändern Sie die Umgebungsvariable `TWENTY_VERSION` oben im Workflow.
