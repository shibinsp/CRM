...
* Ein funktionierendes Beispiel finden Sie unter: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* Sie können mit `yarn beeax add` neue Objekte erzeugen; der Assistent führt Sie durch Benennung, Felder und Beziehungen.

<Note>
**Basisfelder werden automatisch erstellt.** Wenn Sie ein benutzerdefiniertes Objekt definieren, fügt BeeAX automatisch Standardfelder hinzu
...
Verwenden Sie `defineField()`, um Objekten, die Ihnen nicht gehören — etwa Standardobjekten von BeeAX (Person, Company usw.) — Felder hinzuzufügen oder Objekten aus anderen Apps. Im Gegensatz zu Inline-Feldern in `defineObject()` benötigen eigenständige Felder einen `objectUniversalIdentifier`, um anzugeben, welches Objekt sie erweitern:
...
Hauptpunkte:
* Der `objectUniversalIdentifier` identifiziert das Zielobjekt. Für Standardobjekte verwenden Sie `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`, die aus `beeax-sdk` exportiert werden.
...
Relationen verbinden Objekte miteinander. In BeeAX sind Relationen stets **bidirektional** — Sie definieren beide Seiten, und jede Seite referenziert die andere.
...
Um eine Relation mit einem integrierten BeeAX-Objekt (Person, Company usw.) zu erstellen, verwenden Sie `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Entitäten mit `yarn beeax add` erstellen
...
| Feld                   | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| Logikfunktion          | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Frontend-Komponente    | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rolle                  | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| Skill                  | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| Agent                  | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| Ansicht                | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| Navigationsmenüeintrag | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Seitenlayout           | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Was der Scaffolder generiert

Jeder Entitätstyp hat seine eigene Vorlage. Zum Beispiel fragt `yarn beeax add object` nach:
