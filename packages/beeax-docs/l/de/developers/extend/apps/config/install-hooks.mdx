...
* **Nicht im Dev-Modus ausgeführt**: Wenn eine App lokal registriert ist (über `yarn beeax dev`), überspringt der Server den Installationsablauf vollständig und synchronisiert Dateien direkt über den CLI-Watcher — daher läuft Post-Install im Dev-Modus nie, unabhängig von `shouldRunSynchronously`. Verwenden Sie `yarn beeax dev:function:exec --postInstall`, um es manuell gegen einen laufenden Arbeitsbereich auszulösen.
...
* **Nicht im Dev-Modus ausgeführt**: wie bei Post-Install — der Installationsablauf wird für lokal registrierte Apps vollständig übersprungen, daher läuft Pre-Install unter `yarn beeax dev` nie. Verwenden Sie `yarn beeax dev:function:exec --preInstall`, um es manuell auszulösen.
