---
title: Layout
description: Definieren Sie Ansichten, Navigationsmenüeinträge und Seitenlayouts, um das Erscheinungsbild Ihrer App in BeeAX zu gestalten.
icon: table-columns
---

Layout-Entitäten steuern, wie Ihre App innerhalb der Benutzeroberfläche von BeeAX dargestellt wird — was in der Seitenleiste angezeigt wird, welche gespeicherten Ansichten mit der App ausgeliefert werden und wie eine Detailseite eines Datensatzes angeordnet ist.
...
`definePageLayoutTab` ermöglicht es Ihrer App, eine einzelne Registerkarte — mit optionalen Widgets — an ein **bestehendes** Seitenlayout anzuhängen. Der häufigste Anwendungsfall ist das Hinzufügen einer benutzerdefinierten Registerkarte (z. B. einer Analytics- oder KI-Zusammenfassungs-Registerkarte) zu einer der in BeeAX integrierten Datensatzseiten oder zu einem Seitenlayout, das Ihre eigene App bereits mitliefert.

Das Zielseitenlayout muss entweder ein **Standard**-Seitenlayout von BeeAX sein oder eines, das von **Ihrer eigenen App** definiert wird; appübergreifende Verweise auf Seitenlayouts, die einer anderen installierten App gehören, werden derzeit nicht unterstützt.
