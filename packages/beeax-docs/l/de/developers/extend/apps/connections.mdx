...
* Verwenden Sie `serverVariables` (nicht `applicationVariables`) — OAuth-Anmeldedaten gelten serverweit und es gibt eine OAuth-App pro BeeAX-Server.
...
4. Öffnen Sie die installierte App in BeeAX als Serveradministrator → setzen Sie die Werte in den entsprechenden `serverVariables`.
