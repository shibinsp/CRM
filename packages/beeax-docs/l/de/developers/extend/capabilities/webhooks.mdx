...
Webhooks übermitteln Daten in Echtzeit an Ihre Systeme, wenn Ereignisse in BeeAX auftreten — kein Polling erforderlich. Verwenden Sie sie, um externe Systeme synchron zu halten, Automatisierungen auszulösen oder Benachrichtigungen zu senden.
...
BeeAX sendet Webhooks für diese Ereignistypen:
...
BeeAX signiert jede Webhook-Anfrage zu Sicherheitszwecken. Validieren Sie die Signaturen, um sicherzustellen, dass die Anfragen authentisch sind.

### Header

| Kopfzeile                    | Beschreibung            |
| ---------------------------- | ----------------------- |
| `X-BeeAX-Webhook-Signature` | HMAC-SHA256-Signatur    |
| `X-BeeAX-Webhook-Timestamp` | Zeitstempel der Anfrage |

### Validierungsschritte

1. Den Zeitstempel aus `X-BeeAX-Webhook-Timestamp` abrufen
2. Zeichenfolge erstellen: `{timestamp}:{JSON payload}`
3. Den HMAC-SHA256-Hash mit Ihrem Webhook-Geheimnis berechnen
4. Mit `X-BeeAX-Webhook-Signature` vergleichen
...
| **Workflow-Webhook-Trigger** | IN       | Daten aus externen Systemen in BeeAX empfangen                                  |
