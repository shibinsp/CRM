...
<AccordionGroup>
<Accordion title="Quali ruoli sono inclusi in BeeAX per impostazione predefinita?">
BeeAX include per impostazione predefinita i ruoli **Admin** e **Member**. Puoi creare ulteriori ruoli personalizzati in base alle esigenze del tuo team (ad es., Addetto alle vendite, Manager, Utente in sola lettura).
</Accordion>
...
Puoi controllare:
* **Invia email**: Possibilità di inviare email da BeeAX
...
Con il provisioning JIT, gli account utente vengono creati automaticamente in BeeAX quando qualcuno accede tramite SSO per la prima volta. Viene assegnato loro automaticamente il ruolo predefinito.
</Accordion>
