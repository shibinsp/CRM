...
description: Comprendere le funzionalità di integrazione delle email in BeeAX.
---
...
BeeAX collega automaticamente le email dalle tue caselle di posta connesse ai relativi record CRM, mantenendo la cronologia di comunicazione in un unico posto.
...
BeeAX importa email a circa **400 messaggi al minuto**. Questa velocità è attualmente limitata dalle limitazioni di frequenza dell'API di Gmail (e vale in modo analogo per altri provider).
...
* **Email mancanti**: Verifica se:
  ...
  * La creazione automatica dei contatti è disabilitata (le email necessitano di record BeeAX esistenti)
