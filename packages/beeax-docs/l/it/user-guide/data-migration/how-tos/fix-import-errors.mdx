...
Dopo aver caricato il file e mappato le colonne, BeeAX convalida i tuoi dati:

1. **Esecuzione della convalida** — BeeAX controlla ogni riga per individuare errori
...
| ---------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| Valori duplicati evidenziati in giallo                                             | Il valore esiste già in BeeAX o compare due volte nel tuo file |
...
**Opzione 3: Consenti a BeeAX di aggiornare il record esistente**

1. Assicurati che il tuo file includa un identificatore univoco (`email`, `domain` o `id`)
2. Mappa il campo identificatore univoco
3. BeeAX aggiornerà il record esistente invece di creare un duplicato
...
Finché almeno un identificatore univoco corrisponde, BeeAX aggiorna il record.
</Note>
...
3. Verifica se i record esistono già in BeeAX
...
Ciò significa che non esiste alcuna Azienda in BeeAX con quello specifico identificatore.
...
Stai tentando di collegarti a un record che non esiste in BeeAX.
...
Una colonna del tuo file non può essere mappata perché il campo non esiste in BeeAX.
...
1. **L'utente non ha accettato l'invito** — l'utente non esiste ancora in BeeAX
2. **Uso dell'ID utente dal vecchio sistema** — BeeAX non può abbinare gli ID di un altro sistema
3. **Formato email errato** — l'email non corrisponde all'account BeeAX dell'utente

### Come risolvere

1. Assicurati che tutti gli utenti abbiano **accettato il loro invito** al tuo spazio di lavoro BeeAX
2. Usa l'**indirizzo email** dell'utente (non il nome o l'ID del vecchio sistema)
3. Usa la stessa email che hanno utilizzato per aderire a BeeAX
...
<Check>Il formato del dominio corrisponde esattamente al formato di BeeAX</Check>
...
1. **Scarica il file di esempio** — vedi il formato esatto che BeeAX si aspetta
