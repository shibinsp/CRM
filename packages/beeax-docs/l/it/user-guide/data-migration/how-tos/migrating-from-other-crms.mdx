...
description: Guida passo-passo per migrare i tuoi dati da qualsiasi CRM a BeeAX.
---

## Panoramica

Questa guida ti accompagna nella migrazione dei tuoi dati da qualsiasi CRM a BeeAX. Il processo prevede la revisione dei tuoi dati, la preparazione del tuo spazio di lavoro BeeAX, l'esportazione dal sistema attuale e l'importazione in BeeAX.
...
Crea un documento di mappatura tra il tuo CRM attuale e BeeAX:
...
* Il campo di destinazione di BeeAX
...
## Passaggio 3: Configura il tuo spazio di lavoro BeeAX

Prima di importare i dati, prepara il tuo spazio di lavoro BeeAX:
...
Se i tuoi dati includono riferimenti a utenti (Proprietario dell'account, Assegnatario, ecc.), tali utenti devono esistere in BeeAX prima dell'importazione. In caso contrario, tali relazioni non possono essere mappate.
</Warning>
...
Apri ogni file CSV esportato in un'applicazione di fogli di calcolo e preparalo per BeeAX.

### Rimuovi i duplicati

1. Ordina per il campo univoco (email per Persone, dominio per Aziende)
2. Rimuovi o unisci le righe duplicate
3. Verifica che non esistano già duplicati in BeeAX
...
Usa `https://domain.com` (non `domain.com` o `www.domain.com`). Questo corrisponde al formato di BeeAX ed evita duplicati quando colleghi la sincronizzazione di email e calendario.
</Warning>
...
2. Usa gli stessi indirizzi email che gli utenti hanno utilizzato per unirsi al tuo spazio di lavoro BeeAX
...
* BeeAX inizierà a sincronizzare le email con i record dei contatti
...
<AccordionGroup>
  <Accordion title="Posso migrare i flussi di lavoro automaticamente?">
    Al momento no. I flussi di lavoro devono essere ricreati manualmente in BeeAX.
  </Accordion>
...
Contatta [contact@beeax.com](mailto:contact@beeax.com) oppure scopri i nostri [Servizi di implementazione](/l/it/user-guide/getting-started/capabilities/implementation-services).
