---
title: Server MCP
description: Collega gli assistenti AI al tuo spazio di lavoro di BeeAX utilizzando il Model Context Protocol.
---
...
BeeAX espone un server [MCP](https://modelcontextprotocol.io/) affinché gli assistenti AI — Claude Desktop, Claude Code, Cursor, ChatGPT e altri — possano leggere e scrivere i dati del tuo CRM in linguaggio naturale.

Usa l'**URL dello spazio di lavoro** (l'URL che usi per accedere a BeeAX) come endpoint MCP. Su BeeAX Cloud, l'URL del tuo spazio di lavoro potrebbe essere `https://{mycompany}.beeax.com` oppure un dominio personalizzato. Il server è disponibile all'indirizzo:

| Ambiente          | Endpoint MCP                                                                   |
| ----------------- | ------------------------------------------------------------------------------ |
| **Cloud**         | `https://{your-workspace-url}/mcp` (ad es. `https://mycompany.beeax.com/mcp`) |
...
Aggiungi questo alla configurazione del tuo client MCP, sostituendo `{your-workspace-url}` con l'host del tuo spazio di lavoro (ad es. `mycompany.beeax.com`):
...
1. Scoprire i metadati OAuth di BeeAX tramite `/.well-known/oauth-protected-resource` e `/.well-known/oauth-authorization-server`
...
Vai su **Impostazioni > AI > Altro > MCP Server** in BeeAX. Scegli il metodo di autenticazione (OAuth o Chiave API), copia lo snippet JSON (userà già l'URL del tuo spazio di lavoro) e incollalo nel file di configurazione del tuo client MCP.
...
Riavvia il tuo client MCP (o ricarica la configurazione). Se usi OAuth verrai reindirizzato a BeeAX per autorizzare l'accesso. Se usi una chiave API la connessione è immediata.
...
Una volta connesso, il server MCP espone strumenti che rispecchiano l'API di BeeAX. Il flusso di lavoro consigliato è:
...
Per le istanze self-hosted, sostituisci `{your-workspace-url}` con l'URL del tuo server. Assicurati che `SERVER_URL` nel tuo ambiente corrisponda all'URL pubblico della tua istanza BeeAX — viene utilizzato per generare i metadati di discovery OAuth.
