...
Questa guida fornisce istruzioni passo passo per installare e configurare l'applicazione BeeAX usando Docker Compose. L'obiettivo è rendere il processo semplice ed evitare gli errori comuni che potrebbero compromettere il tuo setup.
...
Installa l'ultima versione stabile di BeeAX con un unico comando:
...
```bash
VERSION=vx.y.z BRANCH=nome-ramo bash <(curl -sL https://raw.githubusercontent.com/beeax/beeax/main/packages/beeax-docker/scripts/install.sh)
```
...
Se ospiti beeaxCRM sul tuo computer, apri il browser e naviga a [http://localhost:3000](http://localhost:3000).
...
### Esponi BeeAX per Accesso Esterno

Per impostazione predefinita, BeeAX gira su `localhost` alla porta `3000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
Raccomandiamo fortemente di configurare BeeAX dietro un proxy inverso con terminazione SSL per una sicurezza e funzionalità ottimali.
