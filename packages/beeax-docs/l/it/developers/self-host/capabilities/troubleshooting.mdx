...
Se hai un'istanza di BeeAX esistente con dati di produzione, **NON** seguire questi passaggi poiché elimineranno permanentemente il tuo database!

Durante l'installazione di BeeAX per la prima volta, potresti voler cambiare la password del database predefinita.
...
Durante l'installazione di BeeAX, devi fornire il tuo database postgres con gli schemi, le estensioni e gli utenti corretti.
...
#### Cannot find module 'beeax-emails' or its corresponding type declarations.

You have to build the package `beeax-emails` before running the initialization of the database with `npx nx run beeax-emails:build`

#### Pacchetto beeax-x mancante

Assicurati di eseguire yarn nella directory principale e poi esegui `npx nx server:dev beeax-server`. Se ancora non funziona prova a costruire manualmente il pacchetto mancante.
...
#### Durante l'esecuzione di `npx nx start` o `npx nx start beeax-front`, viene generato un errore di memoria insufficiente

Esegui solo i servizi di cui hai bisogno, invece di `npx nx start`. Ad esempio, se lavori sul server, esegui solo `npx nx worker beeax-server`

**If it does not work:**
If you tried to run only `npx nx run beeax-server:start` on WSL and it's failing with the below memory error:
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. In BeeAX, abbiamo notato che alcune estensioni di VScode stavano occupando molta RAM quindi le disabilitiamo temporaneamente.
...
* Apri la tua app BeeAX, naviga su `/settings` e abilita il toggle **Avanzate** in basso a sinistra dello schermo.
* Crea un nuovo webhook.
* Incolla **Your Unique Webhook URL** nel campo **Endpoint Url** in BeeAX. Imposta i **Filtri** su `Companies` e `Created`.
...
Commenta il plugin checker in `packages/beeax-ui/vite-config.ts` come mostrato nell'esempio sotto
...
Se stai eseguendo BeeAX dietro un reverse proxy e stai riscontrando problemi di connessione:
...
2. **Controlla le impostazioni del reverse proxy:**
   ...
   * Conferma che il tuo reverse proxy sta inoltrando correttamente le richieste al server BeeAX.
   * Assicurati che le intestazioni come `X-Forwarded-For` e `X-Forwarded-Proto` siano impostate correttamente.

3. **Riavvia i servizi:**

   Dopo aver apportato modifiche, riavvia sia il reverse proxy che i container BeeAX.
...
* Supporto comunitario:
  ...
  Contatta la [comunità BeeAX](https://github.com/beeax/beeax/issues) o [canali di supporto](https://discord.gg/cx5n4Jzs57) per assistenza.
