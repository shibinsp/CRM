...
icon: laptop-code
description: La guida per i collaboratori (o sviluppatori curiosi) che vogliono eseguire BeeAX localmente.
---
...
Prima di poter installare e usare BeeAX, assicurati di installare quanto segue sul tuo computer:
...
BeeAX richiede una cache Redis per offrire le migliori prestazioni.
...
## Passo 8: Utilizza BeeAX

**Frontend**

Il frontend di BeeAX sarà in esecuzione su [http://localhost:3001](http://localhost:3001).
Puoi accedere utilizzando l'account demo predefinito: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

* Il server di BeeAX sarà attivo e funzionante su [http://localhost:3000](http://localhost:3000)
