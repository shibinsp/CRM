...
I webhook inviano dati ai tuoi sistemi in tempo reale quando si verificano eventi in BeeAX — senza necessità di polling. Usali per mantenere sincronizzati i sistemi esterni, attivare automazioni o inviare avvisi.
...
BeeAX invia webhook per questi tipi di eventi:
...
BeeAX firma ogni richiesta webhook per motivi di sicurezza. Convalida le firme per assicurarti che le richieste siano autentiche.
...
| `X-BeeAX-Webhook-Timestamp` | Timestamp della richiesta |

### Passaggi di convalida

1. Ottieni il timestamp da `X-BeeAX-Webhook-Timestamp`
2. Crea la stringa: `{timestamp}:{JSON payload}`
3. Calcola HMAC SHA256 usando il segreto del tuo webhook
4. Confronta con `X-BeeAX-Webhook-Signature`
...
| **Trigger webhook del Workflow** | IN        | Ricevi dati in BeeAX da sistemi esterni                                 |
