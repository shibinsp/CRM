...
BeeAX implementa OAuth 2.0 con codice di autorizzazione + PKCE per le app rivolte agli utenti e credenziali client per l'accesso da server a server. I client vengono registrati dinamicamente tramite [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591) — nessuna configurazione manuale in una dashboard.
...
| App BeeAX con estensioni dell'interfaccia utente | [App](/l/it/developers/extend/apps/getting-started) (OAuth è gestito automaticamente) |

## Registrare un client

BeeAX supporta la **registrazione dinamica dei client** secondo [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591). Non è necessaria alcuna configurazione manuale — registra a livello di codice:
...
Usa questo flusso quando la tua app agisce per conto di un utente BeeAX.
...
Dopo l'autorizzazione, BeeAX reindirizza al tuo `redirect_uri`:
...
BeeAX pubblica la propria configurazione OAuth in un endpoint di discovery standard:
