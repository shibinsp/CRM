...
Questo compila i sorgenti TypeScript, transpila le funzioni di logica e i componenti front-end e scrive tutto in `.beeax/output/`. Aggiungi `--tarball` per produrre anche un pacchetto `.tgz` per la distribuzione manuale o il comando di deploy.
...
2. Esegui `yarn beeax deploy` (oppure `yarn beeax deploy --remote production`)
...
2. Avvia un'istanza di test isolata di BeeAX utilizzando l'azione composita `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (l'equivalente per la CI di `yarn beeax server start --test`).
...
2. Esegue `beeax/beeax/.github/actions/deploy-beeax-app@main` — l'equivalente per la CI di `yarn beeax deploy`.
...
Per altri sistemi CI (GitLab CI, CircleCI, ecc.), si applicano gli stessi tre comandi: `yarn install`, `yarn beeax build`, quindi `npm publish` da `.beeax/output`.
...
Per installare una versione più recente, effettua prima il deploy o la pubblicazione, quindi riesegui `yarn beeax install`.
</Note>
