---
title: Panoramica
description: TypeScript lato server che viene eseguito all'interno di BeeAX — attivato da route HTTP, pianificazioni cron, eventi del database, strumenti di AI o azioni di workflow.
icon: bolt
---

Il **livello logico** di un'app BeeAX è il codice che *viene eseguito* — handler TypeScript lato server che reagiscono a richieste HTTP, pianificazioni cron e modifiche ai record; skill e agenti di AI che vivono all'interno dello spazio di lavoro; e connessioni OAuth che permettono alle tue funzioni di agire per conto dell'utente in servizi di terze parti.
...
| **Strumento AI**        | Una funzionalità AI di BeeAX decide di chiamare la tua funzione      | `toolTriggerSettings`           |
