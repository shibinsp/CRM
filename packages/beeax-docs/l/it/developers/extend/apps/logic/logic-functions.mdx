...
Le funzioni logiche sono funzioni TypeScript lato server che vengono eseguite sulla piattaforma BeeAX. Possono essere attivate da richieste HTTP, pianificazioni cron o eventi del database — e possono anche essere esposte come strumenti per agenti di IA.
...
import type { RoutePayload } from 'beeax-sdk/logic-function';
...
> ad es. `path: '/post-card/create'` è invocabile su `https://your-beeax-server.com/s/post-card/create`
...
yarn beeax dev:function:exec -n create-new-post-card -p '{"key": "value"}'
```
...
Importa il tipo `RoutePayload` da `beeax-sdk/logic-function`:
...
Per impostazione predefinita, restituire un valore semplice dal tuo handler lo invia come risposta `200` (JSON per gli oggetti, `text/plain` per le stringhe). Per controllare il codice di stato e le intestazioni della risposta, restituisci un oggetto `Response` da `beeax-sdk/logic-function`:

```ts
import { Response } from 'beeax-sdk/logic-function';
...
const handler = async (event: RoutePayload) => {
  ...
  const body = (event.body ?? {}) as {
    metadata?: { beeaxWorkspaceId?: string };
    type?: string;
  };

  return {
    workspaceId: body.metadata?.beeaxWorkspaceId ?? '',
...
```
POST https://your-beeax-server.com/webhooks/server/:resolverLogicFunctionUniversalIdentifier
```
...
* **`toolTriggerSettings`** — rende la funzione individuabile dalle funzionalità di IA di BeeAX (chat, MCP, function calling). Usa lo standard JSON Schema, il formato che gli LLM comprendono nativamente.
* **`workflowActionTriggerSettings`** — fa apparire la funzione come un passaggio nel builder visivo dei flussi di lavoro. Usa il ricco `InputSchema` di BeeAX affinché il builder possa visualizzare correttamente editor di campi, selettori di variabili ed etichette.
...
* `toolTriggerSettings.inputSchema` e `workflowActionTriggerSettings.inputSchema` sono entrambi opzionali. Se omessi, il builder del manifest li deduce dal codice sorgente dell'handler (JSON Schema per lo strumento di IA, `InputSchema` di BeeAX per l'azione del flusso di lavoro). Forniscine uno esplicitamente quando desideri una tipizzazione più ricca — ad esempio, con campi compatibili con `FieldMetadataType` come `CURRENCY` o `RELATION` per il builder dei flussi di lavoro, oppure con campi `description` che l'agente di IA può leggere:
...
Per dichiarare i parametri **una sola volta** e servire entrambi i contesti, definisci un unico JSON Schema (`InputJsonSchema`) e convertilo per l'azione del flusso di lavoro con `jsonSchemaToInputSchema` da `beeax-sdk/logic-function`. `toolTriggerSettings.inputSchema` accetta direttamente il JSON Schema, mentre `workflowActionTriggerSettings.inputSchema` si aspetta l'`InputSchema` di BeeAX:

```ts
import { defineLogicFunction } from 'beeax-sdk/define';
import { jsonSchemaToInputSchema, type InputJsonSchema } from 'beeax-sdk/logic-function';
...
<Note>
**Helper di runtime.** `beeax-sdk/utils` riesporta piccoli helper di runtime in modo che gli handler non importino mai direttamente da `beeax-shared`. Per esempio, `isDefined(value)` restituisce `false` sia per `null` che per `undefined` — usalo per restringere in modo sicuro gli input opzionali degli handler, che possono arrivare come `null` a runtime anche quando sono tipizzati come `T | undefined`:

```ts
import { isDefined } from 'beeax-sdk/utils';
...
## Client API tipizzati (beeax-client-sdk)

Il pacchetto `beeax-client-sdk` fornisce due client GraphQL tipizzati per interagire con l'API di BeeAX dalle tue funzioni logiche e dai componenti front-end.

| Client              | Importa                      | Endpoint                                                                 | Generato?                  |
| ------------------- | ---------------------------- | ------------------------------------------------------------------------ | -------------------------- |
| `CoreApiClient`     | `beeax-client-sdk/core`     | `/graphql` — dati dello spazio di lavoro (record, oggetti)               | Sì, in fase di dev/build   |
| `MetadataApiClient` | `beeax-client-sdk/metadata` | `/metadata` — configurazione dello spazio di lavoro, caricamenti di file | No, fornito pronto all'uso |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Esegui query e modifica i dati dello spazio di lavoro (record, oggetti)">

`CoreApiClient` è il client principale per interrogare e modificare i dati dello spazio di lavoro. Viene **generato dallo schema del tuo spazio di lavoro** durante `yarn beeax dev` o `yarn beeax dev:build`, quindi è completamente tipizzato per corrispondere ai tuoi oggetti e campi.
...
<Note>
**CoreApiClient viene generato in fase di dev/build.** Se lo usi senza eseguire prima `yarn beeax dev` o `yarn beeax dev:build`, genera un errore. La generazione avviene automaticamente — la CLI esegue l'introspezione dello schema GraphQL del tuo spazio di lavoro e genera un client tipizzato usando `@genql/cli`.
</Note>
...
```ts
import { CoreApiClient, CoreSchema } from 'beeax-client-sdk/core';
...
<Note>
  Quando il tuo codice viene eseguito su BeeAX (funzioni logiche o componenti front-end), la piattaforma inietta le credenziali come variabili d'ambiente:

  * `TWENTY_API_URL` — URL di base dell'API di BeeAX
