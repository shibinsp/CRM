...
* Usa `serverVariables` (non `applicationVariables`) — le credenziali OAuth sono a livello di server e c'è una sola app OAuth per ogni server BeeAX.
...
4. Apri l'app installata in BeeAX come amministratore del server → imposta i valori nelle corrispondenti `serverVariables`.
