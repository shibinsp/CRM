...
description: Crea componenti React che vengono renderizzati all'interno della UI di BeeAX con isolamento in sandbox.
icon: window-maximize
---

I componenti front-end sono componenti React che vengono renderizzati direttamente all'interno della UI di BeeAX. Vengono eseguiti in un **Web Worker** isolato utilizzando Remote DOM — il tuo codice è in sandbox ma viene renderizzato in modo nativo nella pagina, non in un iframe.

## Dove possono essere utilizzati i componenti front-end

I componenti front-end possono essere renderizzati in due posizioni all'interno di BeeAX:
...
Dopo la sincronizzazione con `yarn beeax dev` (o eseguendo una volta sola `yarn beeax dev --once`), l'azione rapida appare nell'angolo in alto a destra della pagina:
...
Poiché il componente restituisce `null`, BeeAX evita di renderizzare un contenitore per esso — non appare alcuno spazio vuoto nel layout. Il componente ha comunque accesso a tutti gli hook e all'API di comunicazione con l'host.

## Componenti Command dell'SDK

Il pacchetto `beeax-sdk` fornisce quattro componenti di supporto Command progettati per i componenti front-end headless. Ogni componente esegue un'azione al montaggio, gestisce gli errori mostrando una notifica snackbar e smonta automaticamente il componente front-end al termine.

Importali da `beeax-sdk/command`:
...
Una funzione logica dichiarata con `httpRouteTriggerSettings` è raggiungibile tramite HTTP al relativo percorso della route. BeeAX inietta nel worker l'URL di base da cui vengono servite le tue funzioni come `TWENTY_FUNCTIONS_URL`, insieme al `TWENTY_APP_ACCESS_TOKEN` che autentica la chiamata. Non esiste ancora un client SDK dedicato per invocare le proprie funzioni, quindi chiamale con un semplice `fetch`:

> **Su BeeAX Cloud, le funzioni logiche attivate tramite HTTP sono servite su un dominio dedicato per ogni workspace** in `https://\<your-workspace-subdomain>.beeax.com\<path>` — questo è esattamente ciò in cui viene risolto `TWENTY_FUNCTIONS_URL`. Per i chiamanti esterni, copia l’URL esatto dalle impostazioni del **trigger HTTP** della funzione o dalla scheda **Settings** dell’applicazione.
...
Il percorso aggiunto a `TWENTY_FUNCTIONS_URL` è il `httpRouteTriggerSettings.path` della funzione logica. Mantieni `isAuthRequired: true`; il `TWENTY_APP_ACCESS_TOKEN` che BeeAX genera per il tuo componente autentica la richiesta:
...
### Chiamare le BeeAX REST API

Per leggere o scrivere record di BeeAX da un front component, usa `RestApiClient` da `beeax-client-sdk/rest`. Appartiene alla stessa famiglia di client di `CoreApiClient` e `MetadataApiClient`, ma si rivolge alle BeeAX REST API (`/rest/...`) invece della GraphQL API, leggendo il suo URL di base da `TWENTY_API_URL`.
...
| ------------------------- | --------------------------------------------------------------------------- |
| `TWENTY_FUNCTIONS_URL`    | URL di base da cui vengono servite le funzioni di logica HTTP della tua app |
| `TWENTY_API_URL`          | URL di base dell'API Core di BeeAX                                         |
| `TWENTY_APP_ACCESS_TOKEN` | Token di breve durata con ambito limitato al ruolo della tua app            |

## API di comunicazione con l'host

I componenti front-end possono attivare navigazione, modali e notifiche utilizzando funzioni da `beeax-sdk`:
...
* **Componenti BeeAX UI** — importali da `beeax-sdk/ui` (Button, Tag, Status, Chip, Avatar e altro)
