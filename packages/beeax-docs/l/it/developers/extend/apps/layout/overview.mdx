---
title: Panoramica
description: "Inserisci la tua app all'interno dell'interfaccia utente di BeeAX: voci della barra laterale, viste salvate, schede nella pagina di dettaglio dei record e componenti React isolati."
icon: table-columns
---

Il **livello di layout** di un'app BeeAX è tutto ciò che l'utente vede: dove l'app viene visualizzata nella barra laterale, quali viste elenco fornisce, come sono organizzate le sue pagine di dettaglio dei record e quali componenti React personalizzati vengono renderizzati all'interno di quelle pagine.
...
<CardGroup cols={2}>
  ...
  <Card title="Componenti front-end" icon="window-maximize" href="/l/it/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — componenti React isolati che vengono renderizzati all'interno di BeeAX.
  </Card>
...
I componenti front-end vengono eseguiti all'interno di un Web Worker isolato usando Remote DOM: vengono renderizzati in modo nativo nella pagina (non all'interno di un iframe), ma non possono accedere direttamente alla pagina host o al DOM. La comunicazione con BeeAX avviene tramite un'API host basata sul passaggio di messaggi.
