...
* **Docker** — [Scarica](https://www.docker.com/products/docker-desktop/). Necessario per eseguire un server BeeAX locale. Salta se hai già BeeAX in esecuzione altrove.

La creazione di un'app BeeAX ha tre fasi. Lo strumento di scaffolding le combina in un unico comando per il percorso ottimale, ma ogni fase è un concetto distinto — quando qualcosa fallisce, sapere in quale fase ti trovi indica cosa correggere.

| Fase                    | Cosa fai                                               | Strumento                     | Risultato                          |
| ----------------------- | ------------------------------------------------------ | ----------------------------- | ---------------------------------- |
| **1. Crea struttura**   | Genera il codice sorgente dell'app                     | `npx create-beeax-app`       | Un progetto TypeScript sul disco   |
| **2. Esegui un server** | Avvia un server BeeAX con cui sincronizzare           | Docker + `yarn beeax server` | Un'istanza BeeAX in esecuzione    |
| **3. Sincronizza**      | Sincronizza in tempo reale il tuo codice con il server | `yarn beeax dev`             | Le tue modifiche compaiono nell'UI |
...
Ti verrà chiesto un nome e una descrizione — premi **Invio** per usare i valori predefiniti. Questo genera un progetto TypeScript in `my-beeax-app/` con un `application-config.ts` iniziale, un ruolo predefinito, un workflow CI e un test di integrazione.

**Dopo questa fase:** hai il codice sorgente dell'app sulla tua macchina. Non è ancora in esecuzione — questa è la Fase 2.

---

## Fase 2 — Esegui un server BeeAX locale

La tua app ha bisogno di un server BeeAX con cui sincronizzarsi. Il server è un'istanza BeeAX completa — UI, API GraphQL, PostgreSQL — in esecuzione in locale su Docker. Il tuo codice locale carica le sue definizioni su quel server, che le rende visibili nell'UI.

Lo strumento di scaffolding ti propone di avviarne uno per te:

> **Vuoi configurare un'istanza locale di BeeAX?**

* **Sì (consigliato)** — scarica l'immagine Docker `beeaxcrm/beeax-app-dev` e la avvia sulla porta `2020`. Assicurati prima che Docker sia in esecuzione.
* **No** — scegli questa opzione se hai già un server BeeAX a cui vuoi connetterti. Puoi collegarlo in seguito con `yarn beeax remote:add`.
...
**Dopo questa fase:** hai un server BeeAX in esecuzione su [http://localhost:2020](http://localhost:2020) con la tua CLI autorizzata a sincronizzarsi con esso.

<Note>
Se Docker non è installato o in esecuzione, lo strumento di scaffolding ti indicherà il comando di avvio corretto per il tuo sistema operativo. Quando Docker è attivo, puoi riprendere con `yarn beeax docker:start` — non è necessario rifare lo scaffolding.
</Note>
...
Fai clic su **My beeax app** per vedere la sua **registrazione dell'applicazione** — un record a livello di server che descrive la tua app (nome, identificatore, credenziali OAuth, origine). Una registrazione può essere installata in più spazi di lavoro sullo stesso server.
...
| ---------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `yarn beeax dev`                  | Monitora e risincronizza a ogni modifica. Rimane in esecuzione finché non lo interrompi.             | Sviluppo locale interattivo.                                                                   |
| `yarn beeax dev --once`           | Singola build + sincronizzazione, termina con codice `0` in caso di successo, `1` in caso di errore. | CI, hook pre-commit, agenti IA, flussi di lavoro scriptati.                                    |
| `yarn beeax dev --once --dry-run` | Crea e stampa le modifiche ai metadati **senza applicarle**.                                         | Ispezionare quali modifiche verrebbero apportate da una sincronizzazione prima di confermarla. |
