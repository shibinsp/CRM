---
title: Architettura
description: Come funzionano le app di BeeAX — sandboxing, ciclo di vita e componenti di base.
icon: sitemap
---

Le app di BeeAX sono pacchetti TypeScript che estendono il tuo spazio di lavoro con oggetti personalizzati, logica, componenti dell'interfaccia utente (UI) e funzionalità di IA. Vengono eseguite sulla piattaforma BeeAX con sandboxing completo e controlli delle autorizzazioni.

## Come funzionano le app

Un'app è una raccolta di **entità** dichiarate utilizzando le funzioni `defineEntity()` del pacchetto `beeax-sdk`. L'SDK rileva queste dichiarazioni tramite analisi dell'AST in fase di build e produce un **manifest** — una descrizione completa di ciò che la tua app aggiunge a uno spazio di lavoro.
...
| **Componente front-end**         | UI React in sandbox nella pagina di BeeAX             | [Componenti front-end](/l/it/developers/extend/apps/front-components) |
...
* **I componenti front-end** vengono eseguiti in Web Workers utilizzando il Remote DOM — isolati dalla pagina principale ma renderizzando elementi DOM nativi (non iframe). Comunicano con BeeAX tramite un'API host basata sul passaggio di messaggi.
...
* **`yarn beeax dev`** — monitora i file sorgente e sincronizza in tempo reale le modifiche su un server BeeAX connesso. Il client API tipizzato viene rigenerato automaticamente quando lo schema cambia.
* **`yarn beeax build`** — compila TypeScript, crea i bundle delle funzioni logiche e dei componenti front-end con esbuild e produce un manifest.
...
<CardGroup cols={2}>
  ...
  <Card title="Componenti front-end" icon="window-maximize" href="/l/it/developers/extend/apps/front-components">
    Componenti React in sandbox nell'UI di BeeAX.
  </Card>
