...
title: Pubblicazione
icon: carica
description: Distribuisci la tua app BeeAX nel marketplace oppure distribuiscila internamente.
---
...
* **Distribuisci un tarball** — carica la tua app direttamente su un server BeeAX specifico per uso interno o privato.
* **Pubblica su npm** — elenca la tua app nel marketplace di BeeAX affinché qualsiasi spazio di lavoro possa scoprirla e installarla.
...
Questo compila i sorgenti TypeScript, transpila le funzioni di logica e i componenti front-end e scrive tutto in `.beeax/output/`. Aggiungi `--tarball` per produrre anche un pacchetto `.tgz` per la distribuzione manuale o per il comando di publish.

## Distribuzione su un server (tarball)

Per le app che non vuoi rendere pubbliche — strumenti proprietari, integrazioni solo aziendali o build sperimentali — puoi distribuire un tarball direttamente su un server BeeAX.

### Prerequisiti

Prima della distribuzione, ti serve un remote configurato che punti al server di destinazione. I remote memorizzano localmente l'URL del server e le credenziali di autenticazione in `~/.beeax/config.json`.
...
2. Esegui `yarn beeax app:publish --private` (oppure `yarn beeax app:publish --private --remote production`)
...
Se la tua app utilizza una funzionalità introdotta in una specifica versione del server BeeAX (ad esempio, i provider OAuth aggiunti nella v2.3.0), dovresti dichiarare la versione minima del server richiesta dalla tua app utilizzando il campo `engines.beeax` in `package.json`:
...
* Se `engines.beeax` è impostato e la versione del server di destinazione non soddisfa l'intervallo, il deploy (caricamento del tarball) o l'installazione vengono rifiutati con un errore `SERVER_VERSION_INCOMPATIBLE` e un messaggio che indica sia l'intervallo richiesto sia la versione effettiva del server.
* Se `engines.beeax` **non è impostato**, l'app è accettata su qualsiasi versione del server (retrocompatibile con le app esistenti).
* Se sul server non è configurata `APP_VERSION`, il controllo viene ignorato.

<Note>
Il server è l'autorità di controllo — convalida `engines.beeax` sia durante il caricamento del tarball sia durante l'installazione nello spazio di lavoro. Se distribuisci un tarball out-of-band o installi dal marketplace, il server continua comunque a imporre la compatibilità.
</Note>

## CI/CD automatizzati (workflow preconfigurati)

Le app generate con `create-beeax-app` includono due workflow di GitHub Actions pronti all'uso, nella cartella `.github/workflows/`. Sono pronti all'esecuzione non appena esegui il push del repository su GitHub — non è necessaria alcuna configurazione aggiuntiva per la CI e la CD richiede solo un singolo secret.
...
2. Avvia un'istanza di test isolata di BeeAX utilizzando l'azione composita `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (l'equivalente per la CI di `yarn beeax docker:start --test`).
...
* `TWENTY_VERSION` (variabile di ambiente, predefinito `latest`) — fissa la versione del server BeeAX usata nella CI modificando questo valore in `ci.yml`.
...
Esegue il deploy della tua app su un server BeeAX configurato a ogni push su `main` e, facoltativamente, da una pull request quando viene applicata l'etichetta `deploy`.

**Cosa fa:**

1. Esegue il checkout della testa della PR (per le PR etichettate) oppure del commit inviato.
2. Esegue `beeax/beeax/.github/actions/deploy-beeax-app@main` — l'equivalente per la CI di `yarn beeax app:publish --private`.
3. Esegue `beeax/beeax/.github/actions/install-beeax-app@main` in modo che la versione appena distribuita venga installata nello spazio di lavoro di destinazione.

**Configurazione richiesta:**

| Impostazione            | Dove                                                             | Scopo                                                                                                             |
| ----------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` in `cd.yml` (predefinito `http://localhost:3000`)          | Il server BeeAX su cui effettuare il deploy. Modificalo con l'URL reale del tuo server prima del primo utilizzo. |
...
Entrambi i workflow fanno riferimento ad azioni riutilizzabili a `@main`, quindi gli aggiornamenti delle azioni nel repository `beeax/beeax` vengono recepiti automaticamente. Se desideri build deterministiche, sostituisci `@main` con uno SHA di commit o un tag di release in ciascuna riga `uses:`.

## Pubblicazione su npm

La pubblicazione su npm rende la tua app scopribile nel marketplace di BeeAX. Qualsiasi spazio di lavoro BeeAX può sfogliare, installare e aggiornare le app del marketplace direttamente dall'interfaccia utente.

### Requisiti

* Un account [npm](https://www.npmjs.com)
* La parola chiave `beeax-app` nell'array `keywords` del tuo `package.json` (aggiungila manualmente — non è inclusa per impostazione predefinita nel template `create-beeax-app`)
...
Il server BeeAX sincronizza il proprio catalogo del marketplace dal registro npm **ogni ora**.
...
<Note>
Se la tua app non definisce un `aboutDescription` in `defineApplication()`, il marketplace userà automaticamente il `README.md` del tuo pacchetto su npm come contenuto della pagina Informazioni. Questo significa che puoi mantenere un unico README sia per npm sia per il marketplace di BeeAX. Se desideri una descrizione diversa nel marketplace, imposta esplicitamente `aboutDescription`.
</Note>
...
Per altri sistemi CI (GitLab CI, CircleCI, ecc.), si applicano gli stessi tre comandi: `yarn install`, `yarn beeax dev:build`, quindi `npm publish` da `.beeax/output`.
...
Vai alla pagina **Impostazioni > Applicazioni** in BeeAX, dove è possibile sfogliare e installare sia le app del marketplace sia quelle distribuite tramite tarball.
...
Per installare una versione più recente, effettua prima il deploy o la pubblicazione, quindi riesegui `yarn beeax app:install`.
</Note>
