...
<Note>
Per l’iterazione locale quotidiana vuoi quasi sempre `yarn beeax dev`. Il deploy e la pubblicazione servono a distribuire le release, **non** per il ciclo locale.
</Note>

| Vuoi…                                                       | Comando                             | Note                                                                                                                                   |
| ----------------------------------------------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Iterare in locale con sincronizzazione in tempo reale       | `yarn beeax dev`                   | Monitora i file e sincronizza a ogni modifica.                                                                                         |
| Sincronizzare una volta e uscire (CI, script, hook)         | `yarn beeax dev --once`            | Esegue una build + sincronizzazione, poi termina.                                                                                      |
| Visualizzare in anteprima le modifiche **senza applicarle** | `yarn beeax dev --once --dry-run`  | Calcola e stampa il diff; non scrive nulla.                                                                                            |
| Rimuovere l'app dallo spazio di lavoro                      | `yarn beeax app:uninstall`         | Aggiungi `--yes` per saltare il prompt.                                                                                                |
| Inviare un tarball a un server                              | `yarn beeax app:publish --private` | Richiede una versione di `package.json` **strettamente superiore** — vedi [Publishing](/l/it/developers/extend/apps/operations/publishing). |
| Pubblicare nel marketplace (npm)                            | `yarn beeax app:publish`           | —                                                                                                                                      |
| Installare / aggiornare una versione distribuita            | `yarn beeax app:install`           | Installa la versione attualmente distribuita.                                                                                          |
| Pulire il server locale e ripartire da zero                 | `yarn beeax docker:reset`          | Elimina **tutti** i dati locali — ultima risorsa.                                                                                      |

### La sincronizzazione locale non richiede un incremento di versione

La regola della `version` strettamente crescente (`VERSION_ALREADY_EXISTS` in fase di deploy, `APP_ALREADY_INSTALLED` / `CANNOT_DOWNGRADE_APPLICATION` in fase di installazione) si applica a **`app:publish` / `app:install`** — il percorso di release. `yarn beeax dev` sincronizza il tuo manifest in-place e non richiede mai una modifica di versione, quindi non devi toccare `package.json` per iterare. Se ti ritrovi ad aumentare la versione per testare una modifica locale, stai usando il percorso di release quando invece vuoi il ciclo di sviluppo.
...
`yarn beeax dev --once --dry-run` crea il tuo manifest, chiede al server il piano di migrazione e lo stampa — **senza applicare nulla**. È il modo sicuro per rispondere a "cosa cambierebbe questa sincronizzazione?" prima di impegnarti ad applicarla.
...
<Note>
Un dry run mostra in anteprima solo le modifiche ai **metadati** e richiede che l'app sia stata sincronizzata almeno una volta (così lo spazio di lavoro la conosce). Se lo esegui su un'app che non è mai stata sincronizzata, il server segnala che l'app non è installata — esegui prima `yarn beeax dev` una volta.
</Note>
...
1. **Nuova sincronizzazione.** Esegui di nuovo `yarn beeax dev --once`. Le sincronizzazioni sono idempotenti — rieseguire un manifest pulito è sicuro e spesso risolve un problema temporaneo.
2. **Visualizza in anteprima il piano.** Esegui `yarn beeax dev --once --dry-run` per vedere esattamente cosa intende cambiare la prossima sincronizzazione, senza applicarlo.
...
4. **Disinstalla e reinstalla.** `yarn beeax app:uninstall`, poi sincronizza di nuovo (`yarn beeax dev`). Questo ricostruisce i metadati dell'app partendo da zero, lasciando intatto il resto del tuo spazio di lavoro.
5. **Ripristino completo (ultima risorsa).** `yarn beeax docker:reset`, poi esegui di nuovo il seeding e la sincronizzazione.

<Warning>
`yarn beeax docker:reset` elimina **tutti** i dati nella tua istanza locale — ogni workspace, record e app. Usalo solo quando i passaggi precedenti hanno fallito.
</Warning>

<Note>
Hai riscontrato un errore di metadati? Per favore [apri una issue](https://github.com/beeax/beeax/issues/new/choose) e includi il messaggio di migrazione non riuscita (con il tipo di metadato e lo `universalIdentifier`), l'output `Metadata changes` della sincronizzazione e i comandi che hai eseguito.
</Note>
