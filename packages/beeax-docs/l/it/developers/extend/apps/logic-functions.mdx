...
Importa il tipo `RoutePayload` da `beeax-sdk`:
...
Le funzioni logiche possono essere esposte come **strumenti** per gli agenti di IA e i flussi di lavoro. Quando una funzione è contrassegnata come strumento, diventa individuabile dalle funzionalità di IA di BeeAX e può essere utilizzata nelle automazioni dei flussi di lavoro.
...
* Le variabili d'ambiente `APPLICATION_ID`, `APP_ACCESS_TOKEN` e `API_URL` sono disponibili all'interno dell'handler (come in qualsiasi altra funzione logica), quindi puoi chiamare le API di BeeAX con un token di accesso applicativo con ambito sulla tua app.
...
* **Non eseguito in modalità dev**: quando un'app è registrata in locale (tramite `yarn beeax dev`), il server salta completamente il flusso di installazione e sincronizza i file direttamente tramite il watcher della CLI — quindi il post-install non viene mai eseguito in modalità dev, indipendentemente da `shouldRunSynchronously`. Usa `yarn beeax exec --postInstall` per attivarlo manualmente su un workspace in esecuzione.
...
* **Non eseguito in modalità dev**: come per il post-install — il flusso di installazione viene completamente saltato per le app registrate localmente, quindi il pre-install non viene mai eseguito con `yarn beeax dev`. Usa `yarn beeax exec --preInstall` per attivarlo manualmente.
...
`CoreApiClient` è il client principale per interrogare e modificare i dati dello spazio di lavoro. Viene **generato dallo schema del tuo spazio di lavoro** durante `yarn beeax dev` o `yarn beeax build`, quindi è completamente tipizzato per corrispondere ai tuoi oggetti e campi.
...
<Note>
**CoreApiClient viene generato in fase di dev/build.** Se lo usi senza eseguire prima `yarn beeax dev` o `yarn beeax build`, genera un errore. La generazione avviene automaticamente — la CLI esegue l'introspezione dello schema GraphQL del tuo spazio di lavoro e genera un client tipizzato usando `@genql/cli`.
</Note>
