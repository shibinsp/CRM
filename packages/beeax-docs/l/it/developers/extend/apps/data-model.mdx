---
title: Modello dati
description: Definisci oggetti, campi, ruoli e metadati dell'applicazione con BeeAX SDK.
icon: database
---

Il pacchetto `beeax-sdk` fornisce le funzioni `defineEntity` per dichiarare il modello di dati della tua applicazione. Devi usare `export default defineEntity({...})` affinché l'SDK rilevi le tue entità. Queste funzioni convalidano la configurazione in fase di build e offrono il completamento automatico nell'IDE e la sicurezza dei tipi.
...
* Vedi un esempio funzionante: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* Puoi generare nuovi oggetti con `yarn beeax add`, che ti guida nella denominazione, nei campi e nelle relazioni.

<Note>
**I campi base vengono creati automaticamente.** Quando definisci un oggetto personalizzato, BeeAX aggiunge automaticamente i campi standard
...
Usa `defineField()` per aggiungere campi a oggetti che non possiedi — come gli oggetti standard di BeeAX (Person, Company, ecc.) o oggetti di altre app. A differenza dei campi inline in `defineObject()`, i campi autonomi richiedono un `objectUniversalIdentifier` per specificare quale oggetto estendono:
...
Punti chiave:
* `objectUniversalIdentifier` identifica l'oggetto di destinazione. Per gli oggetti standard, usa `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` esportati da `beeax-sdk`.
...
Le relazioni collegano gli oggetti tra loro. In BeeAX, le relazioni sono sempre **bidirezionali** — definisci entrambi i lati e ciascun lato fa riferimento all'altro.
...
Per creare una relazione con un oggetto BeeAX integrato (Person, Company, ecc.), usa `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Creazione di entità con lo scaffolding tramite `yarn beeax add`
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Oggetto                      | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| Campo                        | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| Funzione logica              | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente front-end         | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Ruolo                        | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| Abilità                      | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| Agente                       | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| Vista                        | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| Voce del menu di navigazione | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Layout di pagina             | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Cosa genera lo scaffolder

Ogni tipo di entità ha il proprio template. Ad esempio, `yarn beeax add object` richiede:
