---
title: BeeAX から PDF を生成する
description: PDF（見積書など）を生成してレコードに添付するワークフローを作成します。
---

BeeAX で PDF を自動生成または取得し、レコードに添付します。 これは、会社、商談、またはその他のオブジェクトに関連付けられた見積書、請求書、レポートを作成する用途で一般的に使用されます。
...
export const main = async (
  ...
  // Replace with your BeeAX GraphQL endpoint
  // Cloud: https://api.beeax.com/graphql
  // Self-hosted: https://your-domain.com/graphql
  const graphqlEndpoint = 'https://api.beeax.com/graphql';
...
* [BeeAX から見積書または請求書を生成する](/l/ja/user-guide/workflows/how-tos/connect-to-other-tools/generate-quote-or-invoice-from-beeax)
