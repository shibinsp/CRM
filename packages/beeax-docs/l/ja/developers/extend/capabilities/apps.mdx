---
title: BeeAX アプリ
description: BeeAX のカスタマイズをコードとして構築・管理します。
---
...
Apps を使うと、BeeAX のカスタマイズを**コードとして**構築・管理できます。 Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
* Node.js 24+ と Yarn 4
* BeeAX のワークスペースと API キー（https://app.beeax.com/settings/api-webhooks で作成）
...
参考: [create-beeax-app](https://www.npmjs.com/package/create-beeax-app) および [beeax-sdk CLI](https://www.npmjs.com/package/beeax-sdk) の CLI リファレンスページをご覧ください。

## プロジェクト構成（スキャフォルド作成）

`npx create-beeax-app@latest my-beeax-app` を実行すると、スキャフォルダーは次を行います：

* 最小限のベースアプリケーションを `my-beeax-app/` にコピーします
* ローカルの `beeax-sdk` 依存関係と Yarn 4 の設定を追加します
* `beeax` CLI と連携する設定ファイルとスクリプトを作成します
...
* **package.json**: Declares the app name, version, engines (Node 24+, Yarn 4), and adds `beeax-sdk` plus scripts like `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall`, and `auth:login` that delegate to the local `beeax` CLI.
...
* `yarn app:dev` は `node_modules/beeax-sdk/generated` に型付き BeeAX クライアントを自動生成します。
...
認証情報はユーザーごとに `~/.beeax/config.json` に保存されます。 複数のプロファイルを管理し、相互に切り替えることができます。
...
beeax-sdk は、アプリ内で使用する型付きのビルディングブロックとヘルパー関数を提供します。 以下は、最も頻繁に扱う主要な構成要素です。
...
<Note>
  **ベースフィールドは自動作成されます。** カスタムオブジェクトを定義すると、BeeAX は `name`、`createdAt`、`updatedAt`、`createdBy`、`position`、`deletedAt` などの標準フィールドを自動的に追加します。 これらを `fields` 配列で定義する必要はありません。カスタムフィールドのみを追加してください。
</Note>
...
* 動作例は Hello World アプリにあります: [packages/beeax-apps/hello-world/src/roles/function-role.ts](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts)。
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. 型を `beeax-sdk` からインポートします:
...
`yarn app:dev` は `node_modules/beeax-sdk/generated` に型付き BeeAX クライアントを自動生成します。 関数内で使用します：
...
関数が BeeAX 上で実行されると、コードが実行される前に、プラットフォームが認証情報を環境変数として注入します：

* `TWENTY_API_URL`: アプリが対象とする BeeAX API のベース URL。
...
オブジェクト、関数、複数のトリガーを示す最小のエンドツーエンド例は[こちら](https://github.com/beeax/beeax/tree/main/packages/beeax-apps/hello-world)をご覧ください。

## 手動セットアップ（スキャフォルダーなし）

最適な導入体験のために `create-beeax-app` の使用を推奨しますが、手動でプロジェクトをセットアップすることもできます。 CLI をグローバルにインストールしないでください。 代わりに、`beeax-sdk` をローカル依存関係として追加し、package.json にスクリプトを設定します：
...
* サーバーに接続できません: API URL と、BeeAX サーバーに到達可能であることを確認してください。
