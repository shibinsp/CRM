...
<Warning>
  **初めてインストールしますか？** [Docker Compose インストールガイド](/l/ja/developers/self-host/capabilities/docker-compose)に従ってBeeAXを起動し、その後はここに戻り構成してください。
</Warning>

BeeAXは、異なる展開ニーズに合わせて**2つの構成モード**を提供します:
...
1. BeeAXインスタンスにアクセスしてください (通常は `http://localhost:3000`)
2. **設定 / 管理パネル / 構成変数**に移動
...
<Warning>
  ...
  [ 完全な技術リファレンス →](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/beeax-config/config-variables.ts)
</Warning>
...
デフォルトでは、BeeAX は**シングルワークスペースモード**で動作します—組織向けに 1 つの CRM インスタンスが必要な多くのセルフホスト型デプロイメントに最適です。
...
同一の BeeAX インスタンス上で複数の独立したチームがそれぞれのワークスペースを必要とする、SaaS のようなデプロイメント向けにマルチワークスペースモードを有効にします。
...
**必要なスコープ** (自動的に構成される):
[関連ソースコードを見る](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  カレンダーおよびメッセージングAPIを使用するためには、[Microsoft 365 ライセンス](https://admin.microsoft.com/Adminportal/Home)が必要です。 They will not be able to sync their account on BeeAX without one. それがない場合、BeeAX でアカウントを同期できません。
</Warning>
...
[関連ソースコードを見る](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
BeeAX supports logic functions for workflows and custom logic. 実行環境は、`SERVERLESS_TYPE` 環境変数で設定されます。
...
<Note>
  When using `SERVERLESS_TYPE=DISABLED`, any attempt to execute a logic function will return an error. This is useful if you want to run BeeAX without logic function capabilities.
</Note>
