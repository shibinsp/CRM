...
1. BeeAXを実行しているホスト上のターミナルで、BeeAXを停止します：`docker compose down`
...
3. `docker compose up -d` でBeeAXを再起動します。
...
アップグレード後にランタイムの問題が発生した場合は、キャッシュをフラッシュして最新の変更と同期させる必要があるかもしれません。 beeax-serverコンテナ内で次のコマンドを実行します： beeax-serverコンテナ内で次のコマンドを実行します：
...
BeeAXインスタンスをv0.55イメージにアップグレードします。
...
あなたの `beeax-server` コンテナ内で次のコマンドを実行します：

```bash
yarn command:prod cache:flush
```

この問題は特定のBeeAXバージョンに固有であり、今後のアップグレードでは不要なはずです。
...
それは常にあなたの現在のBeeAXのインスタンスの `major.minor` バージョンの範囲内である必要があります。あなたのインスタンスバージョンは、管理パネル（ `/settings/admin-panel` にあり、データベース内でユーザーが `canAccessFullAdminPanel` プロパティを持っている場合にアクセス可能）にまたは `beeax-server` コンテナ内で `echo $APP_VERSION` を実行することで確認できます。

非同期化されたワークスペースバージョンを修正するには、対応するBeeAXのバージョンから関連するアップグレードガイドに従って順次アップグレードし、それが目的のバージョンに到達するまで続ける必要があります。
...
BeeAXインスタンスをv0.52イメージにアップグレードします。
...
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、beeaxバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、beeaxバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。

### v0.50からv0.51

BeeAXインスタンスをv0.51イメージにアップグレードします。
...
BeeAXインスタンスをv0.50.0イメージにアップグレードします。
...
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/beeax/beeax/blob/v0.50.0/packages/beeax-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/beeax/beeax/blob/v0.50.0/packages/beeax-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/beeax/beeax/blob/v0.50.0/packages/beeax-docker/docker-compose.yml)に更新してください。

### v0.43.0からv0.44.0

BeeAXインスタンスをv0.44.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.43.0イメージにアップグレードします。
...
```
docker exec -it beeax-db-1 sh
pg_dump -U {YOUR_POSTGRES_USER} -d {YOUR_POSTGRES_DB} > databases_backup.sql
exit
docker cp beeax-db-1:/home/postgres/databases_backup.sql .
```

ダンプファイルが空でないことを確認してください。

2. docker-compose.ymlを、新しいpostgres:16イメージを使用するようにアップグレードします。[docker-compose.yml](https://raw.githubusercontent.com/beeax/beeax/main/packages/beeax-docker/docker-compose.yml)を参照してください。

3. データベースを新しいpostgres:16コンテナへ復元します。

```
docker cp databases_backup.sql beeax-db-1:/databases_backup.sql
docker exec -it beeax-db-1 sh
...
BeeAXインスタンスをv0.42.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.41.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.40.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.35.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.34.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.33.0イメージにアップグレードします。
...
このバージョンから、DB用のbeeax-postgresイメージは廃止され、beeax-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のbeeax-postgresイメージは廃止され、beeax-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のbeeax-postgresイメージは廃止され、beeax-postgres-spiloが代わりに使用されるようになりました。
beeax-postgresイメージを使い続けたい場合は、docker-compose.yml内の`beeaxcrm/beeax-postgres:${TAG}`を`beeaxcrm/beeax-postgres`に置き換えるだけで済みます。

### v0.31.0からv0.32.0

BeeAXインスタンスをv0.32.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.31.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.30.0イメージにアップグレードします。

**破壊的な変更**:
パフォーマンスを向上させるために、BeeAXはRedisキャッシュの設定を必要とするようになりました。 これを反映した[docker-compose.yml](https://raw.githubusercontent.com/beeax/beeax/main/packages/beeax-docker/docker-compose.yml)を更新しました。
...
BeeAXインスタンスをv0.24.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.23.0イメージにアップグレードします。
...
BeeAXインスタンスをv0.22.0イメージにアップグレードします。
