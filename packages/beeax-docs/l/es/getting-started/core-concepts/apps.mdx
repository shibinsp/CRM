...
title: Aplicaciones
icon: cube
description: "Amplía BeeAX con código: objetos personalizados, lógica del lado del servidor, componentes de IU y agentes de IA, todo como paquetes de TypeScript."
---

La mayoría de los CRM te dan un panel de configuración. BeeAX te da una plataforma. Las aplicaciones son la forma en que los desarrolladores amplían BeeAX más allá de lo que ofrece la IU: definen modelos de datos, lógica del lado del servidor, componentes de IU y capacidades de IA como código, y luego los implementan en uno o más espacios de trabajo.
...
Una aplicación es un paquete de TypeScript que declara **entidades** usando `beeax-sdk`:
...
| **Componentes de frontend** | Componentes de React aislados (sandboxed) que se renderizan dentro de la interfaz de BeeAX (panel lateral, widgets, menú de comandos) |
...
Escribes tu app como un proyecto de TypeScript en tu máquina. La CLI observa tus archivos fuente y los sincroniza en tiempo real con un servidor de BeeAX en ejecución: edita un archivo y ve el cambio en la interfaz en cuestión de un segundo. El cliente de API tipado se regenera automáticamente cuando cambia el esquema. Cuando estés listo, `yarn beeax app:publish --private` envía la app a un servidor de producción, o `yarn beeax app:publish` publica tu app en npm y en el marketplace de BeeAX.
