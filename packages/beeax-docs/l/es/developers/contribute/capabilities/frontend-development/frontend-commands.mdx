...
```bash
npx nx run beeax-front:lint # pasar --fix para corregir errores de lint
```
...
```bash
npx nx run beeax-front:test # ejecutar pruebas con Jest
npx nx run beeax-front:storybook:serve:dev # ejecutar Storybook
npx nx run beeax-front:storybook:test # ejecutar pruebas # (requiere que yarn storybook:serve:dev esté en ejecución)
npx nx run beeax-front:storybook:coverage # (requiere que yarn storybook:serve:dev esté en ejecución)
```
