...
icon: laptop-code
description: La guía para los colaboradores (o desarrolladores curiosos) que quieren ejecutar BeeAX localmente.
---
...
Antes de que puedas instalar y usar BeeAX, asegúrate de instalar lo siguiente en tu computadora:
...
BeeAX requiere un caché de Redis para proporcionar el mejor rendimiento.
...
El frontend de BeeAX estará ejecutándose en [http://localhost:3001](http://localhost:3001).
Puede iniciar sesión usando la cuenta demo por defecto: `tim@apple.dev` (contraseña: `tim@apple.dev`)

**Backend**

* El servidor de BeeAX estará operativo en [http://localhost:3000](http://localhost:3000)
