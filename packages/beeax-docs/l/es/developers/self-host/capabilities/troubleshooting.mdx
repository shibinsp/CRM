...
Si tiene una instancia de BeeAX existente con datos de producción, **NO** siga estos pasos ya que borrarán permanentemente su base de datos.

Al instalar BeeAX por primera vez, es posible que desee cambiar la contraseña predeterminada de la base de datos.
...
Durante la instalación de BeeAX, debe aprovisionar su base de datos postgres con los esquemas, extensiones y usuarios correctos.
...
#### No se puede encontrar el módulo 'beeax-emails' o sus declaraciones de tipo correspondientes.

Tienes que construir el paquete `beeax-emails` antes de ejecutar la inicialización de la base de datos con `npx nx run beeax-emails:build`

#### Falta el paquete beeax-x

Asegúrese de ejecutar yarn en el directorio raíz y luego ejecute `npx nx server:dev beeax-server`. Si esto aún no funciona, intente construir el paquete faltante manualmente.
...
#### Al ejecutar `npx nx start` o `npx nx start beeax-front`, se produce un error de falta de memoria

Ejecute solo los servicios que necesite, en lugar de `npx nx start`. Por ejemplo, si trabaja en el servidor, ejecute solo `npx nx worker beeax-server`

**Si no funciona:**
Si intentó ejecutar solo `npx nx run beeax-server:start` en WSL y falla con el siguiente error de memoria:
...
**Si no funciona:**
Investigue qué procesos están ocupando la mayor parte de la RAM de su máquina. En BeeAX, notamos que algunas extensiones de VScode estaban ocupando mucha RAM, por lo que las desactivamos temporalmente.
...
* Abra la aplicación BeeAX, navegue a `/settings` y active el interruptor **Avanzado** en la parte inferior izquierda de la pantalla.
* Cree un nuevo webhook.
* Pegue **Su URL de Webhook Única** en el campo **Url de EndPoint** en BeeAX. Establezca los **Filtros** en `Companies` y `Created`.
...
Comente el plugin checker en `packages/beeax-ui/vite-config.ts` como en el ejemplo a continuación
...
Si está ejecutando BeeAX detrás de un proxy inverso y experimenta problemas de conexión:
...
2. **Verifique la configuración del Proxy Reverso:**
   ...
   * Confirme que su proxy reverso está reenviando correctamente las solicitudes al servidor de BeeAX.
...
3. **Reinicie los Servicios:**
   ...
   Después de hacer cambios, reinicie tanto el proxy inverso como los contenedores de BeeAX.
...
* Soporte Comunitario:
  ...
  Póngase en contacto con la [comunidad de BeeAX](https://github.com/beeax/beeax/issues) o [los canales de soporte](https://discord.gg/cx5n4Jzs57) para obtener asistencia.
