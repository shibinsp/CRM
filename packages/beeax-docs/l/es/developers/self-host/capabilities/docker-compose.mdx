...
Esta guía proporciona instrucciones paso a paso para instalar y configurar la aplicación BeeAX utilizando Docker Compose. El objetivo es simplificar el proceso y prevenir errores comunes que podrían arruinar tu configuración.
...
Instala la última versión estable de BeeAX con un solo comando:
...
Si alojas beeaxCRM en tu propia computadora, abre tu navegador y navega a [http://localhost:3000](http://localhost:3000).
...
### Exponer BeeAX para acceso externo

Por defecto, BeeAX se ejecuta en `localhost` en el puerto `3000`. Para acceder a él mediante un dominio externo o dirección IP, necesitas configurar `SERVER_URL` en tu archivo `.env`.
...
Recomendamos encarecidamente configurar BeeAX detrás de un proxy inverso con terminación SSL para una seguridad y funcionalidad óptimas.
