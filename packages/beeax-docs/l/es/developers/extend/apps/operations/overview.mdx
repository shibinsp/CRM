...
La **capa de operaciones** es todo lo que haces *a* tu aplicación en lugar de *con* ella: invocar comandos de CLI, ejecutar pruebas de integración contra un servidor BeeAX real, configurar CI y enviar versiones, ya sea como un tarball implementado en un único servidor o como un paquete de npm listado en el marketplace.
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/l/es/developers/extend/apps/operations/cli">
    Referencia de `yarn beeax` — exec, logs, uninstall, remotes.
  </Card>
