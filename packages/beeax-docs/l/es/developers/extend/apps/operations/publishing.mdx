...
title: Publicación
icon: subir
description: Distribuye tu aplicación de BeeAX en el marketplace o despliégala internamente.
---
...
* **Desplegar un paquete tar** — sube tu aplicación directamente a un servidor BeeAX específico para uso interno o privado.
* **Publicar en npm** — incluye tu aplicación en el marketplace de BeeAX para que cualquier espacio de trabajo la descubra e instale.
...
Esto compila el código fuente de TypeScript, transpila las funciones de lógica y los componentes de frontend, y escribe todo en `.beeax/output/`. Agrega `--tarball` para generar también un paquete `.tgz` para la distribución manual o para el comando `publish`.

## Despliegue en un servidor (tarball)

Para aplicaciones que no quieres que estén disponibles públicamente — herramientas propietarias, integraciones solo para empresas o compilaciones experimentales — puedes desplegar un tarball directamente en un servidor de BeeAX.

### Prerrequisitos

Antes de desplegar, necesitas un remoto configurado que apunte al servidor de destino. Los remotos almacenan la URL del servidor y las credenciales de autenticación localmente en `~/.beeax/config.json`.

Agrega un remoto:

```bash filename="Terminal"
yarn beeax remote:add --url https://your-beeax-server.com --as production
```
...
```bash filename="Terminal"
yarn beeax app:publish --private
...
2. Ejecuta `yarn beeax app:publish --private` (o `yarn beeax app:publish --private --remote production`)
...
Si tu app usa una función introducida en una versión específica del servidor BeeAX (por ejemplo, proveedores de OAuth agregados en v2.3.0), debes declarar la versión mínima del servidor que tu app requiere usando el campo `engines.beeax` en `package.json`:
...
* Si `engines.beeax` está configurado y la versión del servidor de destino no cumple el rango, la implementación (carga del tarball) o la instalación se rechaza con un error `SERVER_VERSION_INCOMPATIBLE` y un mensaje que indica tanto el rango requerido como la versión real del servidor.
* Si `engines.beeax` **no está configurado**, la app se acepta en cualquier versión del servidor (retrocompatible con las apps existentes).
* Si el servidor no tiene `APP_VERSION` configurado, se omite la comprobación.

<Note>
El servidor es la autoridad en la comprobación — valida `engines.beeax` tanto en la carga del tarball como en la instalación en el espacio de trabajo. Si implementas un tarball fuera de banda o instalas desde el marketplace, el servidor sigue garantizando la compatibilidad.
</Note>

## CI/CD automatizado (flujos de trabajo preconfigurados)

Las aplicaciones generadas con `create-beeax-app` incluyen de forma predeterminada dos flujos de trabajo de GitHub Actions, en `.github/workflows/`. Están listas para ejecutarse en cuanto hagas push del repositorio a GitHub — no se necesita configuración adicional para CI, y CD solo requiere un único secreto.
...
2. Inicia una instancia de prueba aislada de BeeAX usando la acción compuesta `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (el equivalente en CI de `yarn beeax docker:start --test`).
...
* `TWENTY_VERSION` (variable de entorno; por defecto `latest`) — fija la versión del servidor de BeeAX usada en CI editando esto en `ci.yml`.
...
Despliega tu aplicación en un servidor de BeeAX configurado en cada push a `main` y, opcionalmente, desde un pull request cuando se aplica la etiqueta `deploy`.

**Qué hace:**

1. Obtiene el head del PR (para PR etiquetados) o el commit enviado.
2. Ejecuta `beeax/beeax/.github/actions/deploy-beeax-app@main` — el equivalente en CI de `yarn beeax app:publish --private`.
3. Ejecuta `beeax/beeax/.github/actions/install-beeax-app@main` para que la versión recién desplegada se instale en el espacio de trabajo de destino.

**Configuración necesaria:**

| Configuración           | Dónde                                                                | Propósito                                                                                                     |
| ----------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` en `cd.yml` (por defecto `http://localhost:3000`)              | El servidor de BeeAX al que se va a desplegar. Cámbialo por la URL real de tu servidor antes del primer uso. |
...
Ambos flujos de trabajo hacen referencia a acciones reutilizables en `@main`, por lo que las actualizaciones de acciones en el repositorio `beeax/beeax` se aplican automáticamente. Si quieres compilaciones deterministas, reemplaza `@main` por un SHA de commit o una etiqueta de versión en cada línea `uses:`.

## Publicación en npm

Publicarla en npm hace que tu aplicación sea visible en el marketplace de BeeAX. Cualquier espacio de trabajo de BeeAX puede explorar, instalar y actualizar aplicaciones del marketplace directamente desde la interfaz de usuario.

### Requisitos

* Una cuenta de [npm](https://www.npmjs.com)
* La palabra clave `beeax-app` en la matriz `keywords` de tu `package.json` (agrégala manualmente — no se incluye de forma predeterminada en la plantilla `create-beeax-app`)
...
```bash filename="Terminal"
yarn beeax app:publish --tag beta
```

### Cómo funciona el descubrimiento en el marketplace

El servidor de BeeAX sincroniza su catálogo del marketplace desde el registro de npm **cada hora**.

Puedes activar la sincronización de inmediato en lugar de esperar:

```bash filename="Terminal"
yarn beeax dev:catalog-sync
# To target a specific remote:
# yarn beeax dev:catalog-sync --remote production
```
...
<Note>
Si tu aplicación no define un `aboutDescription` en `defineApplication()`, el marketplace usará automáticamente el `README.md` de tu paquete en npm como el contenido de la página Acerca de. Esto significa que puedes mantener un único README tanto para npm como para el marketplace de BeeAX. Si quieres una descripción diferente en el marketplace, establece explícitamente `aboutDescription`.
</Note>
...
jobs:
  publish:
    ...
    steps:
      ...
      - run: npx beeax dev:build
      - run: npm publish --provenance --access public
        working-directory: .beeax/output
```

Para otros sistemas de CI (GitLab CI, CircleCI, etc.), se aplican los mismos tres comandos: `yarn install`, `yarn beeax dev:build` y luego `npm publish` desde `.beeax/output`.
...
Ve a la página **Configuración > Aplicaciones** en BeeAX, donde se pueden explorar e instalar tanto las aplicaciones del marketplace como las desplegadas mediante tarball.
...
Para instalar una versión más reciente, primero despliégala o publícala y luego vuelve a ejecutar `yarn beeax app:install`.
</Note>
