...
Ambos entornos tienen `beeax-client-sdk/core` y `beeax-client-sdk/metadata` disponibles como módulos preproporcionados — estos no se incluyen en el bundle sino que se resuelven en tiempo de ejecución por el servidor.
...
La subruta `beeax-sdk/cli` exporta funciones que puedes invocar directamente desde el código de pruebas:
...
Asegúrate de que tu servidor local de BeeAX esté en ejecución y luego:
...
1. Obtiene tu código
2. Inicia un servidor temporal de BeeAX usando la acción `beeax/beeax/.github/actions/spawn-beeax-docker-image`
...
No necesitas configurar secretos: la acción `spawn-beeax-docker-image` inicia un servidor efímero de BeeAX directamente en el runner y devuelve los detalles de conexión. El secreto `GITHUB_TOKEN` lo proporciona GitHub automáticamente.

Para fijar una versión específica de BeeAX en lugar de `latest`, cambia la variable de entorno `TWENTY_VERSION` al inicio del flujo de trabajo.
