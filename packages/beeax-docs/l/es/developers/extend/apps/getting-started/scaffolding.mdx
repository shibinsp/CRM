---
title: Andamiaje
description: Genera archivos de entidad de forma interactiva con yarn beeax dev:add — objetos, campos, vistas, funciones de lógica y más.
...
| ------------------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Objeto                          | `yarn beeax dev:add object`             | `src/objects/\<name>.ts`               |
| Campo                           | `yarn beeax dev:add field`              | `src/fields/\<name>.ts`                |
| Función de lógica               | `yarn beeax dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente de frontend          | `yarn beeax dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                             | `yarn beeax dev:add role`               | `src/roles/\<name>.ts`                 |
| Habilidad                       | `yarn beeax dev:add skill`              | `src/skills/\<name>.ts`                |
| Agente                          | `yarn beeax dev:add agent`              | `src/agents/\<name>.ts`                |
| Vista                           | `yarn beeax dev:add view`               | `src/views/\<name>.ts`                 |
| Elemento del menú de navegación | `yarn beeax dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Diseño de página                | `yarn beeax dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## Qué genera el generador

Cada tipo de entidad tiene su propia plantilla. Por ejemplo, `yarn beeax dev:add object` solicita:
