...
* Usa `serverVariables` (no `applicationVariables`) — las credenciales de OAuth son a nivel de servidor y hay una aplicación OAuth por servidor de BeeAX.
...
4. Abre la aplicación instalada en BeeAX como administrador del servidor → establece los valores en los `serverVariables` correspondientes.
