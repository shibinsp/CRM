...
* **Incluidos en las compilaciones** — `yarn beeax dev:build` agrupa todos los recursos públicos en la salida de distribución.

## Acceder a recursos públicos con `getPublicAssetUrl`

Usa el helper `getPublicAssetUrl` de `beeax-sdk` para obtener la URL completa de un archivo en tu directorio `public/`. Funciona tanto en **funciones de lógica** como en **componentes de frontend**.
