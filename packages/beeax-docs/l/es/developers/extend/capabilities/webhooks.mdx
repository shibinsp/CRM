...
Los webhooks envían datos a tus sistemas en tiempo real cuando ocurren eventos en BeeAX — no se requiere sondeo. Úsalos para mantener sincronizados los sistemas externos, activar automatizaciones o enviar alertas.
...
BeeAX envía webhooks para estos tipos de eventos:
...
BeeAX firma cada solicitud de webhook por seguridad. Valida las firmas para garantizar que las solicitudes sean auténticas.

### Encabezados

| Encabezado                   | Descripción                     |
| ---------------------------- | ------------------------------- |
| `X-BeeAX-Webhook-Signature` | Firma HMAC SHA256               |
| `X-BeeAX-Webhook-Timestamp` | Marca de tiempo de la solicitud |

### Pasos de validación

1. Obtén la marca de tiempo de `X-BeeAX-Webhook-Timestamp`
2. Crea la cadena: `{timestamp}:{JSON payload}`
3. Calcula HMAC SHA256 usando tu secreto de webhook
4. Compara con `X-BeeAX-Webhook-Signature`
...
| **Disparador de webhook de flujo de trabajo** | ENTRADA   | Recibir datos en BeeAX desde sistemas externos                                   |
