---
title: Aplicaciones de BeeAX
description: Crea y gestiona personalizaciones de BeeAX como código.
---
...
Las aplicaciones te permiten crear y administrar personalizaciones de BeeAX **como código**. En lugar de configurar todo a través de la interfaz de usuario, defines tu modelo de datos y funciones de lógica en código, lo que hace más rápido crear, mantener y desplegar en múltiples espacios de trabajo.
...
* Node.js 24+ y Yarn 4
* Un espacio de trabajo de BeeAX y una clave de API (créala en https://app.beeax.com/settings/api-webhooks)
...
Consulta también: las páginas de referencia de la CLI para [create-beeax-app](https://www.npmjs.com/package/create-beeax-app) y [beeax-sdk CLI](https://www.npmjs.com/package/beeax-sdk).

## Estructura del proyecto (generada)

Cuando ejecutas `npx create-beeax-app@latest my-beeax-app`, el generador:

* Copia una aplicación base mínima en `my-beeax-app/`
* Añade una dependencia local de `beeax-sdk` y la configuración de Yarn 4
* Crea archivos de configuración y scripts vinculados a la CLI `beeax`
...
* **package.json**: Declara el nombre de la aplicación, la versión, los entornos (Node 24+, Yarn 4) y agrega `beeax-sdk` además de scripts como `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` y `auth:login` que delegan en la CLI local `beeax`.
...
* `yarn app:dev` genera automáticamente el cliente BeeAX tipado en `node_modules/beeax-sdk/generated`.
...
Tus credenciales se almacenan por usuario en `~/.beeax/config.json`. Puedes mantener varios perfiles y cambiar entre ellos.
...
El beeax-sdk proporciona bloques de construcción tipados y funciones auxiliares que utilizas dentro de tu aplicación. A continuación, las partes clave que usarás con más frecuencia.
...
<Note>
  **Los campos base se crean automáticamente.** Cuando defines un objeto personalizado, BeeAX añade automáticamente campos estándar como `name`, `createdAt`, `updatedAt`, `createdBy`, `position` y `deletedAt`. No necesitas definir estos en tu matriz `fields` — solo agrega tus campos personalizados.
</Note>
...
* Consulta un ejemplo funcional en la aplicación Hello World: [`packages/beeax-apps/hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
Cuando un disparador de ruta invoca tu función de lógica, esta recibe un objeto `RoutePayload` que sigue el formato de AWS HTTP API v2. Importa el tipo desde `beeax-sdk`:
...
`yarn app:dev` genera automáticamente el cliente BeeAX tipado en `node_modules/beeax-sdk/generated`. Úsalo en tus funciones:
...
Cuando tu función se ejecuta en BeeAX, la plataforma inyecta credenciales como variables de entorno antes de que tu código se ejecute:

* `TWENTY_API_URL`: URL base de la API de BeeAX a la que apunta tu aplicación.
...
Explora un ejemplo mínimo de extremo a extremo que demuestra objetos, funciones y múltiples desencadenadores [aquí](https://github.com/beeax/beeax/tree/main/packages/beeax-apps/hello-world):

## Configuración manual (sin el generador)

Aunque recomendamos usar `create-beeax-app` para la mejor experiencia de inicio, también puedes configurar un proyecto manualmente. No instales la CLI globalmente. En su lugar, agrega `beeax-sdk` como dependencia local y conecta scripts en tu package.json:
...
* No se puede conectar al servidor: verifica la URL de la API y que el servidor de BeeAX sea accesible.
