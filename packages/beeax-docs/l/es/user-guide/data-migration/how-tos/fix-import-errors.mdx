...
Después de cargar tu archivo y asignar las columnas, BeeAX valida tus datos:

1. **Se ejecuta la validación** — BeeAX verifica cada fila en busca de errores
...
| ----------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Valores duplicados resaltados en amarillo                                     | El valor ya existe en BeeAX o aparece dos veces en tu archivo |
...
**Opción 3: deja que BeeAX actualice el registro existente**

1. Asegúrate de que tu archivo incluya un identificador único (`email`, `domain` o `id`)
2. Asigna el campo de identificador único
3. BeeAX actualizará el registro existente en lugar de crear un duplicado
...
Siempre que uno de los identificadores únicos coincida, BeeAX actualiza el registro.
</Note>
...
3. Comprueba si los registros ya existen en BeeAX
...
Esto significa que no hay ninguna empresa en BeeAX con ese identificador específico.
...
Estás intentando vincular a un registro que no existe en BeeAX.
...
No se puede asignar una columna de tu archivo porque el campo no existe en BeeAX.
...
1. **El usuario no ha aceptado su invitación** — el usuario aún no existe en BeeAX
2. **Uso del ID de usuario del sistema anterior** — BeeAX no puede hacer coincidir IDs de otro sistema
3. **Formato de correo electrónico incorrecto** — el correo no coincide con la cuenta del usuario en BeeAX

### Cómo solucionarlo

1. Asegúrate de que todos los usuarios hayan **aceptado su invitación** a tu espacio de trabajo de BeeAX
2. Usa la **dirección de correo electrónico** del usuario (no su nombre ni el ID del sistema anterior)
3. Usa el mismo correo que utilizaron para unirse a BeeAX
...
<Check>El formato de dominio coincide exactamente con el formato de BeeAX</Check>
...
1. **Descarga el archivo de ejemplo** — ve el formato exacto que espera BeeAX
