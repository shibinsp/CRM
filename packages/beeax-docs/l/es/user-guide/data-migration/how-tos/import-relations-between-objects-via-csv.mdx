...
El padre (lado "uno") debe existir en BeeAX antes de poder hacer referencia a él.
...
1. Exporte los registros padre desde BeeAX
...
Use `https://domain.com` (no solo `domain.com`). Esto coincide con cómo BeeAX almacena los dominios de las Empresas y evita errores de coincidencia.
</Warning>
...
BeeAX vinculará automáticamente cada registro hijo con el padre correspondiente.
...
| **Registros inexistentes**                | El registro padre no existe en BeeAX                | Importe primero los registros padre o verifique si hay errores tipográficos |
| **Diferencia de mayúsculas y minúsculas** | `Acme.com` en el archivo pero `acme.com` en BeeAX   | Asegure una coincidencia exacta de mayúsculas y minúsculas                  |
...
<AccordionGroup>
  <Accordion title="¿Qué pasa si mi registro padre no tiene dominio ni correo electrónico?">
    Tiene dos opciones:
    1. Use el `id` de BeeAX (exporte los registros padre para obtener sus IDs)
