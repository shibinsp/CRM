...
description: Guía paso a paso para migrar tus datos desde cualquier CRM a BeeAX.
---

## Resumen

Esta guía te lleva paso a paso para migrar tus datos desde cualquier CRM a BeeAX. El proceso implica auditar tus datos, preparar tu espacio de trabajo de BeeAX, exportar desde tu sistema actual e importar en BeeAX.
...
Crea un documento de mapeo entre tu CRM actual y BeeAX:
...
## Paso 3: Configura tu espacio de trabajo en BeeAX

Antes de importar datos, prepara tu espacio de trabajo en BeeAX:
...
Si tus datos incluyen referencias a usuarios (propietario de la cuenta, asignado, etc.), esos usuarios deben existir en BeeAX antes de la importación. De lo contrario, esas relaciones no se podrán asignar.
</Warning>
...
Abre cada CSV exportado en una aplicación de hojas de cálculo y prepáralo para BeeAX.

### Elimina duplicados

1. Ordena por el campo único (correo electrónico para Personas, dominio para Empresas)
2. Elimina o combina filas duplicadas
3. Verifica que no existan duplicados ya en BeeAX
...
Usa `https://domain.com` (no `domain.com` ni `www.domain.com`). Esto coincide con el formato de BeeAX y evita duplicados cuando conectas la sincronización de correo/calendario.
</Warning>
...
2. Usa las mismas direcciones de correo electrónico que los usuarios utilizaron para unirse a tu espacio de trabajo en BeeAX
...
1. Navega hasta el objeto en BeeAX
...
* BeeAX comenzará a sincronizar los correos electrónicos con los registros de contactos
...
<AccordionGroup>
  <Accordion title="¿Puedo migrar los flujos de trabajo automáticamente?">
    Actualmente no. Los flujos de trabajo deben recrearse manualmente en BeeAX.
  </Accordion>
...
Contacta con [contact@beeax.com](mailto:contact@beeax.com) o explora nuestros [Servicios de implementación](/l/es/user-guide/getting-started/capabilities/implementation-services).
