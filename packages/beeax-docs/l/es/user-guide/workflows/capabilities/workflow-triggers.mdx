...
* **No se recomienda para registros creados manualmente**: con este activador, los flujos de trabajo comienzan tan pronto como se crea el registro. Dado que BeeAX UI ofrece guardado automático sobre la marcha (no hay un modo de edición y luego una validación para guardar registros), el flujo de trabajo se activará antes de que el usuario complete todos los campos.
...
* Para solicitudes POST, define la estructura del cuerpo esperada para que BeeAX sepa qué datos esperar. Añade aquí los campos que recibirás y que se necesitarán más abajo en tu flujo de trabajo.
