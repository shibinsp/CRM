---
title: Servidor MCP
description: Conecta asistentes de IA a tu espacio de trabajo de BeeAX mediante el Model Context Protocol.
---
...
BeeAX expone un servidor [MCP](https://modelcontextprotocol.io/) para que los asistentes de IA — Claude Desktop, Claude Code, Cursor, ChatGPT y otros — puedan leer y escribir tus datos de CRM mediante lenguaje natural.

Usa tu **URL del espacio de trabajo** (la URL que utilizas para acceder a BeeAX) como el punto de acceso MCP. En BeeAX Cloud, la URL de tu espacio de trabajo puede ser `https://{mycompany}.beeax.com` o un dominio personalizado. El servidor está disponible en:

| Entorno             | Punto de acceso MCP                                                             |
| ------------------- | ------------------------------------------------------------------------------- |
| **Nube**            | `https://{your-workspace-url}/mcp` (p. ej., `https://mycompany.beeax.com/mcp`) |
...
Agrega esto a la configuración de tu cliente MCP, reemplazando `{your-workspace-url}` por el host de tu espacio de trabajo (p. ej., `mycompany.beeax.com`):
...
1. Descubrir los metadatos de OAuth de BeeAX a través de `/.well-known/oauth-protected-resource` y `/.well-known/oauth-authorization-server`
...
Ve a **Settings > AI > More > MCP Server** en BeeAX. Elige tu método de autenticación (OAuth o API Key), copia el fragmento JSON (ya usará la URL de tu espacio de trabajo) y pégalo en el archivo de configuración de tu cliente MCP.
...
Reinicia tu cliente MCP (o recarga la configuración). Si usas OAuth, se te redirigirá a BeeAX para autorizar el acceso. Si usas una clave de API, la conexión es inmediata.
...
Una vez conectado, el servidor MCP expone herramientas que reflejan la API de BeeAX. El flujo de trabajo recomendado es:
...
Para instancias autohospedadas, reemplaza `{your-workspace-url}` por la URL de tu servidor. Asegúrate de que `SERVER_URL` en tu entorno coincida con la URL pública de tu instancia de BeeAX — se utiliza para generar los metadatos de descubrimiento de OAuth.
