---
title: MCP Sunucusu
description: Model Context Protocol kullanarak yapay zeka asistanlarını BeeAX çalışma alanınıza bağlayın.
---
...
BeeAX, yapay zeka asistanlarının — Claude Desktop, Claude Code, Cursor, ChatGPT ve diğerlerinin — doğal dil aracılığıyla CRM verilerinizi okuması ve yazması için bir [MCP](https://modelcontextprotocol.io/) sunucusu sunar.

MCP uç noktası olarak **çalışma alanı URL'nizi** (BeeAX'ye erişmek için kullandığınız URL) kullanın. BeeAX Cloud'da, çalışma alanı URL'niz `https://{mycompany}.beeax.com` ya da özel bir alan adı olabilir. Sunucu şu adreste kullanılabilir:

| Ortam                        | MCP Uç Noktası                                                               |
| ---------------------------- | ---------------------------------------------------------------------------- |
| **Bulut**                    | `https://{your-workspace-url}/mcp` (örn. `https://mycompany.beeax.com/mcp`) |
...
Bunu MCP istemci yapılandırmanıza ekleyin; `{your-workspace-url}` öğesini çalışma alanınızın ana makine adıyla değiştirin (örn. `mycompany.beeax.com`):
...
1. BeeAX'nin OAuth üst verilerini `/.well-known/oauth-protected-resource` ve `/.well-known/oauth-authorization-server` üzerinden keşfeder
...
BeeAX içinde **Settings > AI > More > MCP Server** bölümüne gidin. Kimlik doğrulama yöntemini (OAuth veya API Anahtarı) seçin, JSON parçacığını kopyalayın (çalışma alanı URL'nizi zaten kullanacaktır) ve MCP istemcinizin yapılandırma dosyasına yapıştırın.
...
MCP istemcinizi yeniden başlatın (veya yapılandırmayı yeniden yükleyin). OAuth kullanıyorsanız, erişimi yetkilendirmek için BeeAX'ye yönlendirileceksiniz. API anahtarı kullanıyorsanız, bağlantı anında kurulur.
...
Bağlandıktan sonra, MCP sunucusu BeeAX API'sini yansıtan araçlar sunar. Önerilen iş akışı:
...
Öz barındırmalı kurulumlarda `{your-workspace-url}` değerini sunucunuzun URL'siyle değiştirin. Ortamınızdaki `SERVER_URL` değerinin BeeAX örneğinizin genel URL'siyle eşleştiğinden emin olun — bu, OAuth keşif üst verilerini oluşturmak için kullanılır.
