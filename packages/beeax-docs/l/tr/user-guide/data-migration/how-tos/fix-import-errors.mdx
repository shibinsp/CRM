...
Dosyanızı yükledikten ve sütunları eşledikten sonra, BeeAX verilerinizi doğrular:

1. **Doğrulama çalışır** — BeeAX her satırı hatalar için kontrol eder
...
| ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| Yinelenen değerler sarı renkle vurgulanır                                      | Değer BeeAX'de zaten mevcut veya dosyanızda iki kez görünüyor |
...
**Seçenek 3: Mevcut kaydı BeeAX'nin güncellemesine izin verin**
...
3. BeeAX, yinelenen bir kayıt oluşturmak yerine mevcut kaydı güncelleyecektir
...
Benzersiz tanımlayıcılardan biri eşleştiği sürece BeeAX kaydı günceller.
</Note>
...
3. Kayıtların BeeAX'de zaten mevcut olup olmadığını kontrol edin
...
Bu, BeeAX'de o özel tanımlayıcıya sahip bir Şirket olmadığı anlamına gelir.
...
BeeAX'de mevcut olmayan bir kayda bağlanmaya çalışıyorsunuz.
...
Dosyanızdaki bir sütun eşlenemiyor çünkü alan BeeAX'de mevcut değil.
...
1. **Kullanıcı davetini kabul etmedi** — kullanıcı henüz BeeAX'de mevcut değil
2. **Eski sistemdeki kullanıcı kimliğini kullanma** — BeeAX başka bir sistemin kimliklerini eşleştiremez
3. **Yanlış e-posta biçimi** — e-posta, kullanıcının BeeAX hesabıyla eşleşmiyor

### Nasıl Düzeltilir

1. Tüm kullanıcıların BeeAX çalışma alanınıza **davetlerini kabul ettiğinden** emin olun
2. Kullanıcının **e-posta adresini** kullanın (adlarını veya eski sistem kimliğini değil)
3. BeeAX'ye katılmak için kullandıkları aynı e-postayı kullanın
...
<Check>Alan adı biçimi, BeeAX'nin biçimiyle birebir eşleşir</Check>
...
1. **Örnek dosyayı indirin** — BeeAX'nin beklediği tam biçimi görün
