...
description: Verilerinizi herhangi bir CRM'den BeeAX'ye taşımak için adım adım kılavuz.
---

## Genel Bakış

Bu kılavuz, verilerinizi herhangi bir CRM'den BeeAX'ye taşıma sürecinde size yol gösterir. Süreç, verilerinizi denetlemeyi, BeeAX çalışma alanınızı hazırlamayı, mevcut sisteminizden dışa aktarmayı ve BeeAX'ye içe aktarmayı içerir.
...
Mevcut CRM'iniz ile BeeAX arasında bir eşleme belgesi oluşturun:
...
## Adım 3: BeeAX Çalışma Alanınızı Kurun

Verileri içe aktarmadan önce BeeAX çalışma alanınızı hazırlayın:
...
Verileriniz kullanıcı referansları içeriyorsa (Hesap Sahibi, Atanan vb.), bu kullanıcıların içe aktarmadan önce BeeAX'de mevcut olması gerekir. Aksi halde bu ilişkiler eşlenemez.
</Warning>
...
Dışa aktarılan her CSV'yi bir hesap tablosu uygulamasında açın ve BeeAX için hazırlayın.
...
3. BeeAX'de halihazırda yinelenen olmadığını doğrulayın
...
`https://domain.com` kullanın (`domain.com` veya `www.domain.com` değil). Bu, BeeAX'nin biçimiyle eşleşir ve e-posta/takvim eşzamanlamasını bağladığınızda yinelenmeleri önler.
</Warning>
...
2. Kullanıcıların BeeAX çalışma alanınıza katılmak için kullandıklarıyla aynı e-posta adreslerini kullanın
...
* BeeAX, e-postaları kişi kayıtlarıyla eşzamanlamaya başlayacaktır
...
<AccordionGroup>
  <Accordion title="İş akışlarını otomatik olarak taşıyabilir miyim?">
    Şu anda mümkün değil. İş akışları BeeAX'de manuel olarak yeniden oluşturulmalıdır.
  </Accordion>
...
[contact@beeax.com](mailto:contact@beeax.com) adresinden bizimle iletişime geçin veya [Uygulama Hizmetlerimizi](/l/tr/user-guide/getting-started/capabilities/implementation-services) keşfedin.
