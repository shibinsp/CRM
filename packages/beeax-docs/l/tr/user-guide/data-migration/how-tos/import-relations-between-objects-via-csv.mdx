...
Ana ("bir" taraf) kaydı, ona referans verebilmeden önce BeeAX'de mevcut olmalıdır.
...
1. Ana kayıtları BeeAX'den dışa aktarın
...
`https://domain.com` kullanın (yalnızca `domain.com` değil). Bu, BeeAX'nin Şirket alan adlarını saklama biçimiyle eşleşir ve eşleştirme hatalarını önler.
</Warning>
...
BeeAX, her alt kaydı otomatik olarak eşleşen ana kayda bağlar.
...
| **Mevcut olmayan kayıtlar**      | Ana kayıt BeeAX'de mevcut değil               | Önce ana kayıtları içe aktarın veya yazım hatalarını kontrol edin |
| **Büyük/küçük harf uyumsuzluğu** | Dosyada `Acme.com`, BeeAX'de ise `acme.com`   | Bire bir büyük/küçük harf eşleşmesini sağlayın                    |
...
<AccordionGroup>
  <Accordion title="Ana kaydımın alan adı veya e-postası yoksa ne olur?">
    İki seçeneğiniz var:
    1. BeeAX `id` değerini kullanın (kimlikleri almak için ana kayıtları dışa aktarın)
