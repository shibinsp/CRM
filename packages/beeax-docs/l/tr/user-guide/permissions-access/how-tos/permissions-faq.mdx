...
<AccordionGroup>
<Accordion title="What roles come with BeeAX by default?">
BeeAX comes with an **Admin** and **Member** roles by default. You can create additional custom roles based on your team's needs (e.g., Sales Rep, Manager, Read-Only User).
</Accordion>
...
You can control:
* **Send Email**: Ability to send emails from BeeAX
...
With JIT provisioning, user accounts are automatically created in BeeAX when someone logs in via SSO for the first time. They're assigned the default role automatically.
</Accordion>
