...
Mevcut bir BeeAX örneğiniz varsa ve üretim verileri varsa, bu adımları **TAKİP ETMEYİN**, çünkü bu veritabanınızı kalıcı olarak silecektir!

BeeAX'yi ilk kez kurarken, varsayılan veritabanı parolasını değiştirmek isteyebilirsiniz.
...
BeeAX kurulumu sırasında, postgres veritabanınızı doğru şemalar, uzantılar ve kullanıcılarla hazırlamanız gerekir.
...
#### Modül 'beeax-emails' veya buna karşılık gelen tür açıklamaları bulunamıyor.

`beeax-emails` paketini veritabanını `npx nx run beeax-emails:build` ile başlatmadan önce derlemeniz gerekiyor.

#### Eksik beeax-x paketi

Kök dizinde yarn çalıştırdığınızdan ve ardından `npx nx server:dev beeax-server` çalıştırdığınızdan emin olun. Bu hala çalışmıyorsa eksik paketi manuel olarak derlemeyi deneyin.
...
#### `npx nx start` veya `npx nx start beeax-front` çalıştırırken, bellek hatası verildi

`npx nx start` komutu yerine yalnızca ihtiyaç duyduğunuz servisleri çalıştırın. Örneğin, sunucuda çalışıyorsanız yalnızca `npx nx worker beeax-server` çalıştırın.

**Bu işe yaramazsa:**
WSL'de yalnızca `npx nx run beeax-server:start` çalıştırmayı denediyseniz ve aşağıdaki bellek hatasıyla karşılaşıyorsanız:
...
**Bu işe yaramazsa:**
Hangi işlemlerin makinenizin RAM'inin çoğunu kullandığını araştırın. BeeAX'de bazı VScode uzantılarının çok fazla RAM aldığını fark ettik, bu yüzden onları geçici olarak devre dışı bırakıyoruz.
...
* BeeAX uygulamanızı açın, `/settings` sayfasına gidin ve ekranın sol alt köşesinde bulunan **Gelişmiş** seçeneğini etkinleştirin.
* Yeni bir webhook oluşturun.
* **Benzersiz Webhook URL'nizi** BeeAX'deki **Endpoint URL** alanına yapıştırın. **Filtreleri** `Şirketler` ve `Oluşturuldu` olarak ayarlayın.
...
Aşağıdaki örnekte olduğu gibi `packages/beeax-ui/vite-config.ts` içindeki checker eklentisini yorum satırına alın
...
Eğer BeeAX'yi ters proxy'nin arkasında çalıştırıyorsanız ve bağlantı sorunları yaşıyorsanız:
...
2. **Ters Proxy Ayarlarını Kontrol Edin:**
   ...
   * Ters proxy'nizin BeeAX sunucusuna talepleri düzgün bir şekilde yönlendirdiğinden emin olun.
...
3. **Servisleri Yeniden Başlatın:**
   ...
   Değişiklik yaptıktan sonra hem ters proxy'yi hem de BeeAX konteynerlerini yeniden başlatın.
...
* Topluluk Desteği:
  ...
  [BeeAX topluluğuna](https://github.com/beeax/beeax/issues) veya [destek kanallarına](https://discord.gg/cx5n4Jzs57) yardım için ulaşın.
