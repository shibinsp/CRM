...
BeeAX, **v1.22** sürümünden itibaren sürümler arası yükseltmeleri destekler. Desteklenen herhangi bir sürümden, ara sürümlerin her birine tek tek geçmeden doğrudan en son sürüme atlayabilirsiniz.

Örneğin, v1.22'den doğrudan v2.0'a yükseltme tamamen desteklenir.

## v2.5+ sürümüne yükseltme — bekleme hâlindeki şifreleme zarfı

**v2.5** ile birlikte, BeeAX beklemede olan gizli verileri (OAuth belirteçleri, uygulama değişkenleri, imzalama için kullanılan özel anahtarlar, hassas yapılandırma değerleri, TOTP sırları) sürümlendirilmiş ve `ENCRYPTION_KEY` (veya `ENCRYPTION_KEY` ayarlı değilse `APP_SECRET`) ile şifrelenmiş bir `enc:v2:` zarfının içinde saklar.
