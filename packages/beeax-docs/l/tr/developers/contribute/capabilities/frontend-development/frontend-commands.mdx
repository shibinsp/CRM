...
```bash
npx nx run beeax-front:lint # lint hatalarını düzeltmek için --fix'i geç
```
...
```bash
npx nx run beeax-front:test # jest testlerini çalıştır
npx nx run beeax-front:storybook:serve:dev # storybook'u çalıştır
npx nx run beeax-front:storybook:test # testleri çalıştır # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
npx nx run beeax-front:storybook:coverage # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
```
