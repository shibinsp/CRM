...
title: Yerel Kurulum
icon: laptop-code
description: BeeAX'i yerel olarak çalıştırmak isteyen katkıda bulunanlar (veya meraklı geliştiriciler) için kılavuz.
---
...
BeeAX'i yüklemeden ve kullanmadan önce bilgisayarınıza aşağıdakileri yüklediğinizden emin olun:
...
BeeAX, en iyi performansı sağlamak için bir Redis önbelleği gerektirir.
...
## Adım 8: BeeAX'yi kullanın

**Ön Uç**

BeeAX'nin ön ucu [http://localhost:3001](http://localhost:3001) adresinde çalışıyor olacak.
Varsayılan demo hesabıyla giriş yapabilirsiniz: `tim@apple.dev` (şifre: `tim@apple.dev`)

**Arka Uç**

* BeeAX'nin sunucusu [http://localhost:3000](http://localhost:3000) adresinde çalışır durumda olacak
