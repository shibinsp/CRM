...
* **Derlemelere dahil edilir** — `yarn beeax build`, tüm genel varlıkları dağıtım çıktısına paketler.

### `getPublicAssetUrl` ile genel varlıklara erişme

`beeax-sdk` içindeki `getPublicAssetUrl` yardımcı işlevini kullanarak `public/` dizininizdeki bir dosyanın tam URL'sini alın. Hem **mantık işlevlerinde** hem de **ön uç bileşenlerinde** çalışır.
...
Her iki ortamda da `beeax-client-sdk/core` ve `beeax-client-sdk/metadata` önceden sağlanmış modüller olarak mevcuttur — bunlar paketlenmez, ancak çalışma zamanında sunucu tarafından çözülür.

## Uygulamanızı test etme

SDK, test kodundan uygulamanızı derlemenize, dağıtmanıza, yüklemenize ve kaldırmanıza olanak tanıyan programatik API'ler sağlar. Tiplenmiş API istemcileriyle birlikte [Vitest](https://vitest.dev/) kullanarak, uygulamanızın gerçek bir BeeAX sunucusunda uçtan uca çalıştığını doğrulayan entegrasyon testleri yazabilirsiniz.
...
`beeax-sdk/cli` alt yolu, test kodundan doğrudan çağırabileceğiniz fonksiyonları dışa aktarır:
...
Yerel BeeAX sunucunuzun çalıştığından emin olun, ardından:
...
### Fonksiyonları çalıştırma (`yarn beeax exec`)
...
### Fonksiyon günlüklerini görüntüleme (`yarn beeax logs`)
...
<Note>
Bu, Docker konteyner günlüklerini gösteren `yarn beeax server logs` komutundan farklıdır. `yarn beeax logs`, uygulamanızın fonksiyon yürütme günlüklerini BeeAX sunucusundan gösterir.
</Note>

### Bir uygulamayı kaldırma (`yarn beeax uninstall`)
...
2. `beeax/beeax/.github/actions/spawn-beeax-docker-image` eylemini kullanarak geçici bir BeeAX sunucusu başlatır
...
Herhangi bir gizli değişken yapılandırmanız gerekmez — `spawn-beeax-docker-image` eylemi, koşucu içinde doğrudan geçici bir BeeAX sunucusu başlatır ve bağlantı ayrıntılarını çıktı olarak verir. `GITHUB_TOKEN` gizli değişkeni GitHub tarafından otomatik olarak sağlanır.

`latest` yerine belirli bir BeeAX sürümünü sabitlemek için iş akışının başındaki `TWENTY_VERSION` ortam değişkenini değiştirin.
