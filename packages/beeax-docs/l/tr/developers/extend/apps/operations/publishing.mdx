...
title: Yayımlama
icon: yükle
description: BeeAX uygulamanızı pazaryerine sunun ya da dahili olarak dağıtın.
---
...
* **Bir tar arşivi dağıtın** — uygulamanızı dahili veya özel kullanım için doğrudan belirli bir BeeAX sunucusuna yükleyin.
* **npm’ye yayımlama** — uygulamanızı BeeAX pazaryerinde listeleyin; böylece herhangi bir çalışma alanı keşfedip yükleyebilir.
...
Bu işlem TypeScript kaynaklarını derler, mantık işlevlerini ve ön uç bileşenlerini transpile eder ve her şeyi `.beeax/output/` konumuna yazar. El ile dağıtım veya yayımlama komutu için bir `.tgz` paketini de üretmek amacıyla `--tarball` ekleyin.

## Sunucuya dağıtım (tarball)

Genel kullanıma açık olmasını istemediğiniz uygulamalar — sahipli araçlar, yalnızca kurumsal entegrasyonlar veya deneysel derlemeler — için bir tarball’ı doğrudan bir BeeAX sunucusuna dağıtabilirsiniz.

### Ön Gereksinimler

Dağıtmadan önce, hedef sunucuyu işaret eden yapılandırılmış bir remote’a ihtiyacınız vardır. Remote’lar sunucu URL’sini ve kimlik doğrulama bilgilerini yerel olarak `~/.beeax/config.json` içinde saklar.
...
2. `yarn beeax app:publish --private` (veya `yarn beeax app:publish --private --remote production`) komutunu çalıştırın
...
Uygulamanız belirli bir BeeAX sunucu sürümünde sunulan bir özelliği kullanıyorsa (örneğin, v2.3.0'da eklenen OAuth sağlayıcıları), uygulamanızın gerektirdiği en düşük sunucu sürümünü `package.json` içindeki `engines.beeax` alanını kullanarak belirtmelisiniz:
...
* `engines.beeax` ayarlanmışsa ve hedef sunucunun sürümü aralığı karşılamıyorsa, dağıtım (tarball yüklemesi) veya yükleme, gerekli aralığı ve gerçek sunucu sürümünü belirten bir mesajla birlikte `SERVER_VERSION_INCOMPATIBLE` hatasıyla reddedilir.
* `engines.beeax` **ayarlı değilse**, uygulama herhangi bir sunucu sürümünde kabul edilir (mevcut uygulamalarla geriye dönük uyumludur).
* Sunucuda `APP_VERSION` yapılandırılmamışsa, denetim atlanır.

<Note>
Nihai denetim sunucudadır — hem tarball yüklemesinde hem de çalışma alanı (workspace) kurulumunda `engines.beeax`'yi doğrular. Bir tarball'ı bant dışı dağıtırsanız veya marketplace'ten kurarsanız, sunucu yine de uyumluluğu zorunlu kılar.
</Note>

## Otomatik CI/CD (hazır şablonlu iş akışları)

`create-beeax-app` ile oluşturulan uygulamalar, kutudan çıktığı gibi `.github/workflows/` altında iki GitHub Actions iş akışıyla gelir. Depoyu GitHub’a iter itmez çalışmaya hazırdır — CI için ek bir kurulum gerekmez ve CD yalnızca tek bir gizli anahtar gerektirir.
...
2. `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` bileşik eylemini kullanarak yalıtılmış bir BeeAX test örneği başlatır (CI'daki `yarn beeax docker:start --test` eşdeğeridir).
...
* `TWENTY_VERSION` (ortam, varsayılanı `latest`) — CI’da kullanılan BeeAX sunucu sürümünü `ci.yml` içinde bunu düzenleyerek sabitleyin.
...
`main` dalına yapılan her itmede uygulamanızı yapılandırılmış bir BeeAX sunucusuna dağıtır ve isteğe bağlı olarak `deploy` etiketi uygulandığında bir çekme isteğinden de dağıtım yapar.

**Ne yapar:**

1. Etiketli PR'ler için PR'in head commit'ini ya da itilen commit'i alır.
2. `beeax/beeax/.github/actions/deploy-beeax-app@main` çalıştırır — `yarn beeax app:publish --private` komutunun CI eşdeğeridir.
3. `beeax/beeax/.github/actions/install-beeax-app@main` eylemini çalıştırır; böylece yeni dağıtılan sürüm hedef çalışma alanına kurulur.

**Gerekli yapılandırma:**

| Ayar                    | Nerede                                                       | Amaç                                                                                                |
| ----------------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `cd.yml` içinde `env` (varsayılan: `http://localhost:3000`)  | Dağıtımın yapılacağı BeeAX sunucusu. İlk kullanımdan önce bunu gerçek sunucu URL'nizle değiştirin. |
...
Her iki iş akışı da `@main` üzerindeki yeniden kullanılabilir eylemlere başvurur; bu nedenle `beeax/beeax` deposundaki eylem güncellemeleri otomatik olarak alınır. Deterministik derlemeler istiyorsanız, her `uses:` satırında `@main` ifadesini bir commit SHA'sı veya sürüm etiketiyle değiştirin.

## npm’ye yayımlama

npm’ye yayımlamak, uygulamanızın BeeAX pazaryerinde keşfedilebilir olmasını sağlar. Herhangi bir BeeAX çalışma alanı, pazaryeri uygulamalarına doğrudan arayüzden göz atabilir, yükleyebilir ve güncelleyebilir.

### Gereksinimler

* Bir [npm](https://www.npmjs.com) hesabı
* `package.json` içindeki `keywords` dizinizdeki `beeax-app` anahtar sözcüğü (elle ekleyin — varsayılan olarak `create-beeax-app` şablonunda yer almaz)
...
BeeAX sunucusu pazar yeri kataloğunu npm kayıt defterinden **her saat** eşitler.
...
Diğer CI sistemleri (GitLab CI, CircleCI, vb.) için de aynı üç komut geçerlidir: `yarn install`, `yarn beeax dev:build` ve ardından `.beeax/output` dizininden `npm publish`.
...
BeeAX içinde **Ayarlar > Uygulamalar** sayfasına gidin; burada hem pazar yerindeki hem de tarball ile dağıtılmış uygulamalar görüntülenip yüklenebilir.
...
Daha yeni bir sürümü kurmak için önce onu dağıtın veya yayımlayın, ardından `yarn beeax app:install` komutunu yeniden çalıştırın.
</Note>
