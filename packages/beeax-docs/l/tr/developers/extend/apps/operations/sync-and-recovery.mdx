...
<Note>
Günlük yerel yinelemelerde neredeyse her zaman `yarn beeax dev` kullanmak istersiniz. Dağıtım ve yayımlama, sürümleri göndermek içindir, yerel döngü için **değil**.
</Note>

| Şunu yapmak istiyorsunuz…                                  | Komut                               | Notlar                                                                                                                                      |
| ---------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Canlı senkronizasyonla yerelde yineleyin                   | `yarn beeax dev`                   | Dosyalarınızı izler ve her değişiklikte senkronize eder.                                                                                    |
| Bir kez senkronize et ve çık (CI, betikler, kancalar)      | `yarn beeax dev --once`            | Tek bir derleme + senkronizasyon yapar ve ardından çıkar.                                                                                   |
| Değişiklikleri **uygulamadan** önizleyin                   | `yarn beeax dev --once --dry-run`  | Farkı hesaplar ve yazdırır; hiçbir şey yazmaz.                                                                                              |
| Uygulamayı çalışma alanından kaldırın                      | `yarn beeax app:uninstall`         | İstemi atlamak için `--yes` ekleyin.                                                                                                        |
| Bir tarball'ı sunucuya gönderin                            | `yarn beeax app:publish --private` | `package.json` içinde **kesin olarak daha yüksek** bir sürüm gerektirir — bkz. [Publishing](/l/tr/developers/extend/apps/operations/publishing). |
| Pazaryerine (npm) yayımlayın                               | `yarn beeax app:publish`           | —                                                                                                                                           |
| Dağıtılmış bir sürümü yükleyin / yükseltin                 | `yarn beeax app:install`           | Şu anda dağıtılmış olan sürümü yükler.                                                                                                      |
| Yerel sunucuyu silin ve temiz bir şekilde yeniden başlatın | `yarn beeax docker:reset`          | Yerel verilerin **tamamını** siler — son çare.                                                                                              |

### Yerel senkronizasyon için sürüm artırmaya gerek yoktur

Sıkı artan `version` kuralı (dağıtımda `VERSION_ALREADY_EXISTS`, yüklemede `APP_ALREADY_INSTALLED` / `CANNOT_DOWNGRADE_APPLICATION`) **`app:publish` / `app:install`** için — yani yayın yolu için — geçerlidir. `yarn beeax dev`, manifestinizi yerinde senkronize eder ve hiçbir zaman bir sürüm değişikliği gerektirmez, bu yüzden yineleme yapmak için `package.json` dosyasına dokunmanız gerekmez. Yerel bir değişikliği test etmek için kendinizi sürümü artırırken buluyorsanız, ihtiyacınız olan geliştirme döngüsü yerine yayın yolunu kullanıyorsunuz demektir.
...
`yarn beeax dev --once --dry-run`, manifestinizi derler, sunucudan geçiş planını ister ve onu yazdırır — **hiçbir şeyi uygulamadan**. Bu, ona taahhüt etmeden önce "Bu senkronizasyon neyi değiştirir?" sorusunu yanıtlamanın güvenli yoludur.

```bash filename="Terminal"
yarn beeax dev --once --dry-run
```
...
<Note>
Bir dry run yalnızca **üst veri** değişikliklerini önizler ve uygulamanın en az bir kez senkronize edilmiş olmasını gerektirir (böylece çalışma alanı ondan haberdar olur). Hiç senkronize edilmemiş bir uygulamaya karşı çalıştırırsanız, sunucu uygulamanın yüklü olmadığını bildirir — önce bir kez `yarn beeax dev` çalıştırın.
</Note>
...
1. **Yeniden senkronize edin.** `yarn beeax dev --once` komutunu tekrar çalıştırın. Senkronizasyonlar idempotenttir — temiz bir manifesti yeniden çalıştırmak güvenlidir ve çoğu zaman geçici bir aksaklığı giderir.
2. **Planı önizleyin.** Bir sonraki senkronizasyonun tam olarak neyi değiştirmeyi amaçladığını, uygulamadan görmek için `yarn beeax dev --once --dry-run` çalıştırın.
...
4. **Kaldırın ve yeniden yükleyin.** `yarn beeax app:uninstall` komutunu çalıştırın, ardından yeniden senkronize edin (`yarn beeax dev`). Bu, uygulamanın üst verilerini temiz bir başlangıçtan yeniden oluşturur ve çalışma alanınızın geri kalanını olduğu gibi bırakır.
5. **Tam sıfırlama (son çare).** `yarn beeax docker:reset` komutunu çalıştırın, ardından yeniden tohumlayın ve yeniden senkronize edin.

<Warning>
`yarn beeax docker:reset`, yerel örneğinizdeki verilerin **tamamını** siler — tüm çalışma alanlarını, kayıtları ve uygulamaları. Yalnızca önceki adımlar başarısız olduktan sonra kullanın.
</Warning>

<Note>
Bir üst veri hatasıyla mı karşılaştınız? Lütfen [issue açın](https://github.com/beeax/beeax/issues/new/choose) ve başarısız olan geçiş iletisini (üst veri türü ve `universalIdentifier` ile birlikte), senkronizasyondan gelen `Metadata changes` çıktısını ve çalıştırdığınız komutları ekleyin.
</Note>
