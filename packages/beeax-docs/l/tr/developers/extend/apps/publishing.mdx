...
Bu işlem TypeScript kaynaklarını derler, mantık işlevlerini ve ön uç bileşenlerini transpile eder ve her şeyi `.beeax/output/` konumuna yazar. El ile dağıtım veya deploy komutu için bir `.tgz` paketini de üretmek amacıyla `--tarball` ekleyin.
...
2. `yarn beeax deploy` (veya `yarn beeax deploy --remote production`) komutunu çalıştırın
...
2. `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` bileşik eylemini kullanarak yalıtılmış bir BeeAX test örneği başlatır (CI'daki `yarn beeax server start --test` eşdeğeri).
...
2. `beeax/beeax/.github/actions/deploy-beeax-app@main` çalıştırır — `yarn beeax deploy` komutunun CI eşdeğeridir.
...
Diğer CI sistemleri (GitLab CI, CircleCI, vb.) için de aynı üç komut geçerlidir: `yarn install`, `yarn beeax build` ve ardından `.beeax/output` dizininden `npm publish`.
...
Daha yeni bir sürümü kurmak için önce onu dağıtın veya yayımlayın, ardından `yarn beeax install` komutunu yeniden çalıştırın.
</Note>
