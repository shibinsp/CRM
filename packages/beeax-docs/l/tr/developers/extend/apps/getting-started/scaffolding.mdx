...
description: Nesne, alan, görünüm, mantık işlevi ve daha fazlasına ait varlık dosyalarını `yarn beeax dev:add` komutuyla etkileşimli olarak oluşturun.
...
| -------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Nesne                | `yarn beeax dev:add object`             | `src/objects/\<name>.ts`               |
| Alan                 | `yarn beeax dev:add field`              | `src/fields/\<name>.ts`                |
| Mantık işlevi        | `yarn beeax dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Ön uç bileşeni       | `yarn beeax dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                  | `yarn beeax dev:add role`               | `src/roles/\<name>.ts`                 |
| Beceri               | `yarn beeax dev:add skill`              | `src/skills/\<name>.ts`                |
| Temsilci             | `yarn beeax dev:add agent`              | `src/agents/\<name>.ts`                |
| Görünüm              | `yarn beeax dev:add view`               | `src/views/\<name>.ts`                 |
| Gezinme menüsü öğesi | `yarn beeax dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Sayfa düzeni         | `yarn beeax dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## İskelet oluşturucunun ürettikleri

Her varlık türünün kendi şablonu vardır. Örneğin, `yarn beeax dev:add object` şunları sorar:
