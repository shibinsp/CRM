...
description: BeeAX'nin UI'si içinde, korumalı alan (sandbox) izolasyonu ile görüntülenen React bileşenleri oluşturun.
icon: window-maximize
---

Ön uç bileşenler, BeeAX'nin UI'si içinde doğrudan görüntülenen React bileşenleridir. Remote DOM kullanan izole bir Web Worker içinde çalışırlar — kodunuz izole bir ortamda (sandbox) çalışır ancak bir iframe içinde değil, sayfada yerel olarak işlenir.

## Ön uç bileşenlerinin kullanılabileceği yerler

Ön uç bileşenler, BeeAX içinde iki konumda işlenebilir:
...
const HelloWorld = () => {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <p>This component renders inside BeeAX.</p>
    </div>
  );
};
...
`yarn beeax dev` ile senkronize ettikten sonra (veya tek seferlik bir `yarn beeax dev --once` çalıştırdıktan sonra), hızlı işlem sayfanın sağ üst köşesinde görünür:
...
import { defineFrontComponent } from 'beeax-sdk/define';
import { useRecordId, enqueueSnackbar } from 'beeax-sdk/front-component';
...
Bileşen `null` döndürdüğü için, BeeAX bunun için bir kapsayıcı oluşturmayı atlar — düzende boş alan görünmez. Bileşen yine de tüm hook'lara ve host iletişim API'sine erişime sahiptir.

## SDK Command bileşenleri

`beeax-sdk` paketi, headless ön uç bileşenler için tasarlanmış dört Command yardımcı bileşeni sağlar. Her bileşen bağlandığında bir eylem yürütür, hataları bir snackbar bildirimi göstererek ele alır ve tamamlandığında ön bileşeni otomatik olarak kaldırır.

Bunları `beeax-sdk/command` içinden içe aktarın:
...
import { defineFrontComponent } from 'beeax-sdk/define';
import { Command } from 'beeax-sdk/command';
import { CoreApiClient } from 'beeax-sdk/clients';
...
import { defineFrontComponent } from 'beeax-sdk/define';
import { CommandModal } from 'beeax-sdk/command';
...
`httpRouteTriggerSettings` ile bildirilen bir mantık işlevi, rota yolunda HTTP üzerinden erişilebilir durumdadır. BeeAX, işlevlerinizin sunulduğu temel URL’yi, çağrıyı kimlik doğrulayan `TWENTY_APP_ACCESS_TOKEN` ile birlikte, worker’a `TWENTY_FUNCTIONS_URL` olarak enjekte eder. Kendi işlevlerinizi çağırmak için henüz özel bir SDK istemcisi yoktur, bu yüzden onları basit bir `fetch` ile çağırın:

> **BeeAX Cloud üzerinde, HTTP ile tetiklenen mantık işlevleri, çalışma alanı başına ayrılmış özel bir etki alanında** `https://\<your-workspace-subdomain>.beeax.com\<path>` adresinde sunulur — bu, `TWENTY_FUNCTIONS_URL`'ün tam olarak çözümlendiği değerdir. Harici çağrıcılar için, tam URL’yi işlevin **HTTP trigger** ayarlarından veya uygulamanın **Settings** sekmesinden kopyalayın.
...
const SyncPrs = () => {
  const execute = async () => {
    await fetch(`${process.env.TWENTY_FUNCTIONS_URL}/github/fetch-prs`, {
      ...
      body: JSON.stringify({ owner: 'beeax', repo: 'beeax' }),
    });
  };
...
`TWENTY_FUNCTIONS_URL` değerine eklenen yol (path), mantık fonksiyonunun `httpRouteTriggerSettings.path` değeridir. `isAuthRequired: true` ayarını koruyun; bileşeniniz için BeeAX'nin oluşturduğu `TWENTY_APP_ACCESS_TOKEN` isteği kimlik doğrular:
...
### BeeAX REST API'sini çağırma

Bir front bileşeninden BeeAX kayıtlarını okumak veya yazmak için, `beeax-client-sdk/rest` paketinden `RestApiClient` kullanın. `CoreApiClient` ve `MetadataApiClient` ile aynı istemci ailesine aittir, ancak GraphQL API yerine BeeAX REST API'sini (`/rest/...`) hedefler ve temel URL'sini `TWENTY_API_URL` değerinden okur.
...
const client = new RestApiClient({
  baseUrl: 'https://myworkspace.beeax.com',
  token: 'my-token',
});
```
...
```tsx
import { RestApiClient, RestApiClientError } from 'beeax-client-sdk/rest';
...
| Değişken                  | Açıklama                                                       |
| ------------------------- | -------------------------------------------------------------- |
| `TWENTY_FUNCTIONS_URL`    | Uygulamanızın HTTP mantık fonksiyonlarının sunulduğu temel URL |
| `TWENTY_API_URL`          | BeeAX çekirdek API'sinin temel URL'si                         |
| `TWENTY_APP_ACCESS_TOKEN` | Uygulamanızın rolüyle sınırlanan kısa ömürlü bir belirteç      |

## Host iletişim API'si

Ön uç bileşenleri, `beeax-sdk`'deki işlevleri kullanarak gezinmeyi, modalları ve bildirimleri tetikleyebilir:
...
import { defineFrontComponent } from 'beeax-sdk/define';
import { useRecordId } from 'beeax-sdk/front-component';
import { enqueueSnackbar, closeSidePanel } from 'beeax-sdk/front-component';
import { CoreApiClient } from 'beeax-sdk/clients';
...
import { defineFrontComponent, numberOfSelectedRecords } from 'beeax-sdk/define';
import { useSelectedRecordIds } from 'beeax-sdk/front-component';
import { enqueueSnackbar, closeSidePanel } from 'beeax-sdk/front-component';
import { CoreApiClient } from 'beeax-sdk/clients';
...
```tsx
import { defineFrontComponent } from 'beeax-sdk/define';
import { getPublicAssetUrl } from 'beeax-sdk/utils';
...
* **BeeAX UI bileşenleri** — `beeax-sdk/ui` içinden içe aktarın (Button, Tag, Status, Chip, Avatar ve daha fazlası)
...
```tsx
import { defineFrontComponent } from 'beeax-sdk/define';
import { Button, Tag, Status } from 'beeax-sdk/ui';
