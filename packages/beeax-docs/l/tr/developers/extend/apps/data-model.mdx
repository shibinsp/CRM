---
title: Veri modeli
description: BeeAX SDK ile nesneleri, alanları, rolleri ve uygulama meta verilerini tanımlayın.
icon: database
---

`beeax-sdk` paketi, uygulamanızın veri modelini tanımlamak için `defineEntity` işlevlerini sağlar. SDK'nin varlıklarınızı algılayabilmesi için `export default defineEntity({...})` kullanmanız gerekir. Bu fonksiyonlar, derleme zamanında yapılandırmanızı doğrular ve IDE otomatik tamamlama ile tür güvenliği sağlar.
...
* Çalışan bir örnek için bkz.: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* `yarn beeax add` kullanarak, adlandırma, alanlar ve ilişkiler konusunda sizi yönlendirerek yeni nesneler oluşturabilirsiniz.

<Note>
**Temel alanlar otomatik olarak oluşturulur.** Özel bir nesne tanımladığınızda BeeAX, standart alanları otomatik olarak ekler
...
Önemli noktalar:
* `objectUniversalIdentifier` hedef nesneyi tanımlar. Standart nesneler için, `beeax-sdk`'den dışa aktarılan `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`'ı kullanın.
...
İlişkiler nesneleri birbirine bağlar. BeeAX'de ilişkiler her zaman **çift yönlüdür** — her iki tarafı da tanımlarsınız ve her taraf diğerine başvurur.
...
Yerleşik bir BeeAX nesnesiyle (Person, Company, vb.) ilişki oluşturmak için `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` kullanın:
...
## `yarn beeax add` ile varlıklar için iskelet oluşturma
...
| -------------------- | ------------------------------------ | ------------------------------------------------------- |
| Nesne                | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| Alan                 | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| Mantık işlevi        | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Ön uç bileşeni       | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                  | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| Beceri               | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| Temsilci             | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| Görünüm              | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| Gezinme menüsü öğesi | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Sayfa düzeni         | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### İskelet oluşturucunun ürettikleri

Her varlık türünün kendi şablonu vardır. Örneğin, `yarn beeax add object` şunları sorar:
