---
title: Mimari
description: BeeAX uygulamaları nasıl çalışır — korumalı alan, yaşam döngüsü ve yapı taşları.
icon: sitemap
---

BeeAX uygulamaları, çalışma alanınızı özel nesneler, mantık, UI bileşenleri ve yapay zekâ yetenekleriyle genişleten TypeScript paketleridir. Tam korumalı alan ve izin kontrolleriyle BeeAX platformunda çalışırlar.

## Uygulamalar nasıl çalışır

Bir uygulama, `beeax-sdk` paketindeki `defineEntity()` işlevleri kullanılarak bildirilen **varlıklar** koleksiyonudur. SDK, bu bildirimleri derleme sırasında AST analiziyle algılar ve bir **manifest** üretir — uygulamanızın bir çalışma alanına neler eklediğinin eksiksiz bir açıklaması.
...
| **Ön Uç Bileşeni**       | BeeAX'nin sayfasında korumalı alanda React kullanıcı arayüzü | [Ön Uç Bileşenleri](/l/tr/developers/extend/apps/front-components)         |
...
* **Ön uç bileşenleri**, Remote DOM kullanan Web Worker'larda çalışır — ana sayfadan yalıtılmıştır ancak yerel DOM öğelerini (iframe'ler değil) oluşturur. BeeAX ile mesaj iletimi yapan bir ana makine API'si aracılığıyla iletişim kurarlar.
...
* **`yarn beeax dev`** — kaynak dosyalarınızı izler ve bağlı bir BeeAX sunucusuna değişiklikleri canlı olarak senkronize eder. Şema değiştiğinde tipli API istemcisi otomatik olarak yeniden oluşturulur.
* **`yarn beeax build`** — TypeScript'i derler, mantık işlevlerini ve ön uç bileşenlerini esbuild ile paketler ve bir manifest üretir.
...
<CardGroup cols={2}>
  ...
  <Card title="Ön uç bileşenleri" icon="window-maximize" href="/l/tr/developers/extend/apps/front-components">
    BeeAX'nin kullanıcı arayüzünde korumalı alanda React bileşenleri.
  </Card>
