...
* **Geliştirme modunda çalıştırılmaz**: kurulum sonrasında olduğu gibi — yerel olarak kaydedilen uygulamalarda kurulum akışı tamamen atlanır, bu nedenle `yarn beeax dev` altında kurulum öncesi hiç çalışmaz. Bunu elle tetiklemek için `yarn beeax dev:function:exec --preInstall` kullanın.
