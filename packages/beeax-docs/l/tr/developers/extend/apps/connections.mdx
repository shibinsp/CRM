...
* `serverVariables` kullanın (`applicationVariables` değil) — OAuth kimlik bilgileri sunucu genelidir ve her BeeAX sunucusu için bir OAuth uygulaması vardır.
...
4. Yüklü uygulamayı BeeAX'de bir sunucu yöneticisi olarak açın → karşılık gelen `serverVariables` üzerinde değerleri ayarlayın.
