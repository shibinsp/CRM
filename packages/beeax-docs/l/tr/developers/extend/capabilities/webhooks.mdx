...
Webhook'lar, BeeAX'de olaylar gerçekleştiğinde verileri sistemlerinize gerçek zamanlı olarak iletir — sürekli sorgulamaya gerek yok. Harici sistemleri senkron tutmak, otomasyonları tetiklemek veya uyarılar göndermek için bunları kullanın.
...
BeeAX, şu olay türleri için webhook'lar gönderir:
...
BeeAX, güvenlik için her webhook isteğini imzalar. İsteklerin gerçekliğini sağlamak için imzaları doğrulayın.

### Üstbilgiler

| Başlık                       | Açıklama            |
| ---------------------------- | ------------------- |
| `X-BeeAX-Webhook-Signature` | HMAC SHA256 imzası  |
| `X-BeeAX-Webhook-Timestamp` | İstek zaman damgası |

### Doğrulama Adımları

1. `X-BeeAX-Webhook-Timestamp` değerinden zaman damgasını alın
2. Dizeyi oluşturun: `{timestamp}:{JSON payload}`
3. Webhook gizli anahtarınızı kullanarak HMAC SHA256 hesaplayın
4. `X-BeeAX-Webhook-Signature` ile karşılaştırın
...
| **İş Akışı Webhook Tetikleyicisi** | IN  | Harici sistemlerden BeeAX'ye veri alın                                     |
