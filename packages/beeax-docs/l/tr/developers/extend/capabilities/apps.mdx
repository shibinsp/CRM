---
title: BeeAX Uygulamaları
description: BeeAX özelleştirmelerini kod olarak oluşturun ve yönetin.
---
...
* **Ön yüz bileşenleri** — BeeAX'nin kullanıcı arayüzünde (kayıt sayfaları, komut menüsü, yan paneller) görüntülenen React bileşenleri
* **Yapay zekâ yetenekleri ve ajanları** — BeeAX'nin yapay zekâsını özel yeteneklerle genişletin
...
Bu, yeni bir uygulamanın iskeletini oluşturur, isteğe bağlı olarak yerel bir BeeAX sunucusunu başlatır ve dosyalarınızdaki değişiklikleri izlemeye başlar. Tam adım adım anlatım için [Başlarken](/l/tr/developers/extend/apps/getting-started) kılavuzuna bakın.
...
| `defineFrontComponent()`           | BeeAX'nin kullanıcı arayüzündeki React bileşenleri        |
...
1. **`yarn beeax dev`** — kaynak dosyaları izler, değişiklikte yeniden derler, sunucuyla senkronize eder, tipli API istemcileri üretir
2. **`yarn beeax build`** — dağıtılabilir bir derleme üretir
3. **`yarn beeax deploy`** — uzak bir BeeAX sunucusuna dağıtır
4. **`yarn beeax add`** — etkileşimli olarak yeni bir varlık iskeleti oluşturur

### CLI başvurusu

```bash filename="Terminal"
yarn beeax help          # Tüm komutları listele
yarn beeax server start  # Yerel geliştirme sunucusunu başlat
yarn beeax remote add    # Bir BeeAX sunucusuna bağlan
yarn beeax exec -n fn    # Bir mantık fonksiyonunu çalıştır
yarn beeax logs -n fn    # Fonksiyon günlüklerini izle
```
