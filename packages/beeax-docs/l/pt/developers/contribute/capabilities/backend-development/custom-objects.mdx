...
Os objetos são estruturas que permitem armazenar dados (registros, atributos e valores) específicos de uma organização. O BeeAX vem com objetos padrão e personalizados.

Objetos padronizados são objetos embutidos com um conjunto de atributos disponíveis para todos os usuários. Exemplos de objetos padronizados na BeeAX incluem Empresa e Pessoa. Os objetos padrão têm campos padrão que também estão disponíveis para todos os usuários do BeeAX, como Company.displayName.
