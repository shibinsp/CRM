...
Se você tiver uma instância existente do BeeAX com dados em produção, **NÃO** siga estes passos, pois eles excluirão permanentemente seu banco de dados!

Ao instalar o BeeAX pela primeira vez, você pode querer alterar a senha padrão do banco de dados.
...
Durante a instalação do BeeAX, é necessário configurar seu banco de dados postgres com os esquemas, extensões e usuários corretos.
...
#### Não é possível encontrar o módulo 'beeax-emails' ou suas declarações de tipo correspondentes.

É preciso compilar o pacote `beeax-emails` antes de iniciar a inicialização do banco de dados com `npx nx run beeax-emails:build`

#### Pacote beeax-x ausente

Certifique-se de executar o yarn no diretório raiz e depois executar `npx nx server:dev beeax-server`. Se isso ainda não funcionar, tente compilar manualmente o pacote ausente.
...
#### Ao executar `npx nx start` ou `npx nx start beeax-front`, é lançada uma mensagem de erro de falta de memória

Run only the services you need, instead of `npx nx start`. Por exemplo, se estiver trabalhando no servidor, execute apenas `npx nx worker beeax-server`

**Se não funcionar:**
Se você tentou executar apenas `npx nx run beeax-server:start` no WSL e está falhando com o erro de memória abaixo:
...
**Se não funcionar:**
Investigue quais processos estão consumindo a maior parte da RAM do seu computador. No BeeAX, percebemos que algumas extensões do VScode estavam consumindo muita RAM, então as desativamos temporariamente.
...
* Abra seu aplicativo BeeAX, navegue até `/settings`, e ative a alternância **Avançado** na parte inferior esquerda da tela.
* Crie um novo webhook.
* Cole **Sua URL Webhook Única** no campo **Endpoint Url** no BeeAX. Defina os **Filtros** para `Companies` e `Created`.
...
Comente o plugin checker em `packages/beeax-ui/vite-config.ts` como no exemplo abaixo
...
Se você está executando o BeeAX por trás de um proxy reverso e está enfrentando problemas de conexão:
...
2. **Verifique as configurações do Proxy Reverso:**
   ...
   * Confirme que seu proxy reverso está encaminhando corretamente as solicitações para o servidor BeeAX.
   * Certifique-se de que cabeçalhos como `X-Forwarded-For` e `X-Forwarded-Proto` estão configurados corretamente.

3. **Reinicie os Serviços:**

   Após fazer as alterações, reinicie tanto o proxy reverso quanto os contêineres do BeeAX.
...
* Suporte Comunitário:
  ...
  Entre em contato com a [comunidade BeeAX](https://github.com/beeax/beeax/issues) ou [canais de suporte](https://discord.gg/cx5n4Jzs57) para obter assistência.
