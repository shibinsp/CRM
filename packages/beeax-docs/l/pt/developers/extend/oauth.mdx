...
A BeeAX implementa OAuth 2.0 com código de autorização + PKCE para aplicações voltadas ao utilizador e credenciais de cliente para acesso servidor a servidor. Os clientes são registados dinamicamente via [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591) — sem configuração manual num painel.
...
| Aplicação BeeAX com extensões de UI               | [Aplicações](/l/pt/developers/extend/apps/getting-started) (OAuth é tratado automaticamente) |

## Registar um cliente

A BeeAX suporta **registo dinâmico de clientes** conforme [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591). Sem necessidade de configuração manual — registe programaticamente:
...
Use este fluxo quando a sua aplicação atuar em nome de um utilizador da BeeAX.
...
Após a autorização, a BeeAX redireciona de volta para o seu `redirect_uri`:
...
A BeeAX publica a sua configuração OAuth num endpoint padrão de descoberta:
