...
description: Crie componentes React que renderizam dentro da UI do BeeAX com isolamento em sandbox.
icon: window-maximize
---

Componentes de front-end são componentes React que renderizam diretamente dentro da UI do BeeAX. Eles são executados em um Web Worker isolado usando Remote DOM — seu código é sandboxed, mas renderiza nativamente na página, não em um iframe.

## Onde os componentes de front-end podem ser usados

Os componentes de front-end podem ser renderizados em dois locais dentro do BeeAX:
...
Após sincronizar com `yarn beeax dev` (ou executando uma única vez o `yarn beeax dev --once`), a ação rápida aparece no canto superior direito da página:
...
Como o componente retorna `null`, o BeeAX ignora renderizar um contêiner para ele — nenhum espaço vazio aparece no layout. O componente ainda tem acesso a todos os hooks e à API de comunicação do host.

## Componentes Command do SDK

O pacote `beeax-sdk` fornece quatro componentes auxiliares Command projetados para componentes de front-end headless. Cada componente executa uma ação ao montar, trata erros exibindo uma notificação de snackbar e desmonta automaticamente o componente de front-end ao concluir.

Importe-os de `beeax-sdk/command`:
...
O caminho anexado a `TWENTY_FUNCTIONS_URL` é o `httpRouteTriggerSettings.path` da função lógica. Mantenha `isAuthRequired: true`; o `TWENTY_APP_ACCESS_TOKEN` que a BeeAX gera para o seu componente autentica a solicitação:
...
### Chamando a API REST da BeeAX

Para ler ou gravar registros da BeeAX a partir de um front component, use `RestApiClient` de `beeax-client-sdk/rest`. Ele pertence à mesma família de clientes que `CoreApiClient` e `MetadataApiClient`, mas tem como alvo a API REST da BeeAX (`/rest/...`) em vez da API GraphQL, lendo sua URL base de `TWENTY_API_URL`.
...
| `TWENTY_API_URL`          | URL base da API principal da BeeAX                                         |
| `TWENTY_APP_ACCESS_TOKEN` | Token de curta duração limitado ao escopo do papel do seu app               |

## API de comunicação do host

Componentes de front-end podem acionar navegação, modais e notificações usando funções de `beeax-sdk`:
...
* **Componentes de UI do BeeAX** — importe de `beeax-sdk/ui` (Button, Tag, Status, Chip, Avatar e mais)
