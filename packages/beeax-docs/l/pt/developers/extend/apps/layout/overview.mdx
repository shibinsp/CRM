---
title: Visão Geral
description: Coloque seu app dentro da interface do BeeAX — entradas na barra lateral, visualizações salvas, abas na página de registro e componentes React em sandbox.
icon: table-columns
---

A **camada de layout** de um app do BeeAX é tudo o que o usuário vê: onde o app aparece na barra lateral, quais visualizações de lista ele fornece, como suas páginas de detalhes de registro são organizadas e quais componentes React personalizados são renderizados dentro dessas páginas.
...
<CardGroup cols={2}>
  ...
  <Card title="Componentes de front-end" icon="window-maximize" href="/l/pt/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — componentes React em sandbox que são renderizados dentro do BeeAX.
  </Card>
