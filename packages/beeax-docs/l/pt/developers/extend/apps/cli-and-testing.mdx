...
* **Incluídos nas compilações** — `yarn beeax build` agrupa todos os recursos públicos na saída de distribuição.

### Acessando recursos públicos com `getPublicAssetUrl`

Use o helper `getPublicAssetUrl` de `beeax-sdk` para obter a URL completa de um arquivo no seu diretório `public/`. Funciona tanto em **funções lógicas** quanto em **componentes de front-end**.
...
Ambos os ambientes têm `beeax-client-sdk/core` e `beeax-client-sdk/metadata` disponíveis como módulos pré-fornecidos — eles não são empacotados, mas resolvidos em tempo de execução pelo servidor.

## Testando seu aplicativo

O SDK fornece APIs programáticas que permitem compilar, implantar, instalar e desinstalar seu aplicativo a partir de código de teste. Em conjunto com [Vitest](https://vitest.dev/) e os clientes de API tipados, você pode escrever testes de integração que verificam que seu aplicativo funciona de ponta a ponta em um servidor BeeAX real.
...
O subcaminho `beeax-sdk/cli` exporta funções que você pode chamar diretamente a partir do código de teste:
...
Certifique-se de que seu servidor BeeAX local esteja em execução e, em seguida:
...
### Executando funções (`yarn beeax exec`)
...
### Visualizando logs de funções (`yarn beeax logs`)
...
<Note>
Isso é diferente de `yarn beeax server logs`, que mostra os logs do contêiner Docker. `yarn beeax logs` mostra os logs de execução de funções do seu aplicativo a partir do servidor BeeAX.
</Note>

### Desinstalando um aplicativo (`yarn beeax uninstall`)
...
2. Inicializa um servidor BeeAX temporário usando a ação `beeax/beeax/.github/actions/spawn-beeax-docker-image`
...
Você não precisa configurar nenhum segredo — a ação `spawn-beeax-docker-image` inicia um servidor BeeAX efêmero diretamente no runner e fornece os detalhes de conexão. O segredo `GITHUB_TOKEN` é fornecido automaticamente pelo GitHub.

Para fixar uma versão específica do BeeAX em vez de `latest`, altere a variável de ambiente `TWENTY_VERSION` no topo do workflow.
