...
As funções de lógica são funções TypeScript no lado do servidor que são executadas na plataforma BeeAX. Elas podem ser acionadas por solicitações HTTP, agendamentos cron ou eventos de banco de dados — e também podem ser expostas como ferramentas para agentes de IA.
...
import type { DatabaseEventPayload, ObjectRecordCreateEvent, CronPayload, RoutePayload } from 'beeax-sdk/define';
import { CoreApiClient, type Person } from 'beeax-client-sdk/core';
...
> por exemplo, `path: '/post-card/create'` é acessível em `https://your-beeax-server.com/s/post-card/create`
...
yarn beeax exec -n create-new-post-card -p '{"key": "value"}'
```

```bash filename="Terminal"
yarn beeax exec -y e56d363b-0bdc-4d8a-a393-6f0d1c75bdcf
```
...
Importe o tipo `RoutePayload` de `beeax-sdk`:

```ts
import { defineLogicFunction, type RoutePayload } from 'beeax-sdk/define';
...
* **`toolTriggerSettings`** — torna a função disponível para os recursos de IA do BeeAX (chat, MCP, chamadas de função). Usa o JSON Schema padrão, o formato que os LLMs entendem nativamente.
* **`workflowActionTriggerSettings`** — torna a função visível como uma etapa no construtor visual de fluxos de trabalho. Usa o `InputSchema` avançado do BeeAX para que o construtor possa renderizar editores de campo adequados, seletores de variáveis e rótulos.
...
* `toolTriggerSettings.inputSchema` e `workflowActionTriggerSettings.inputSchema` são opcionais. Quando omitidos, o construtor de manifestos os infere a partir do código-fonte do handler (JSON Schema para a ferramenta de IA, `InputSchema` do BeeAX para a ação de fluxo de trabalho). Forneça um explicitamente quando quiser uma tipagem mais rica — por exemplo, com campos compatíveis com `FieldMetadataType`, como `CURRENCY` ou `RELATION` para o construtor de fluxos de trabalho, ou com campos `description` que o agente de IA pode ler:
...
import { definePostInstallLogicFunction, type InstallPayload } from 'beeax-sdk/define';
...
* As variáveis de ambiente `APPLICATION_ID`, `APP_ACCESS_TOKEN` e `API_URL` estão disponíveis dentro do manipulador (assim como em qualquer outra função de lógica), então você pode chamar a API da BeeAX com um token de acesso de aplicativo com escopo para o seu app.
...
* **Não executado no modo de desenvolvimento**: quando um app é registrado localmente (via `yarn beeax dev`), o servidor pula completamente o fluxo de instalação e sincroniza arquivos diretamente pelo watcher da CLI — portanto, a pós-instalação nunca é executada no modo de desenvolvimento, independentemente de `shouldRunSynchronously`. Use `yarn beeax exec --postInstall` para acioná-lo manualmente em um workspace em execução.
...
import { definePreInstallLogicFunction, type InstallPayload } from 'beeax-sdk/define';
...
* **Não é executada no modo de desenvolvimento**: igual à pós-instalação — o fluxo de instalação é totalmente ignorado para apps registrados localmente, portanto a pré-instalação nunca é executada com `yarn beeax dev`. Use `yarn beeax exec --preInstall` para acioná-lo manualmente.
...
## Clientes de API tipados (beeax-client-sdk)

O pacote `beeax-client-sdk` fornece dois clientes GraphQL tipados para interagir com a API do BeeAX a partir das suas funções de lógica e componentes de front-end.

| Cliente             | Importar                     | Endpoint                                                             | Gerado?                    |
| ------------------- | ---------------------------- | -------------------------------------------------------------------- | -------------------------- |
| `CoreApiClient`     | `beeax-client-sdk/core`     | `/graphql` — dados do espaço de trabalho (registros, objetos)        | Sim, em tempo de dev/build |
| `MetadataApiClient` | `beeax-client-sdk/metadata` | `/metadata` — configuração do espaço de trabalho, upload de arquivos | Não, vem pré-compilado     |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Consultar e modificar dados do espaço de trabalho (registros, objetos)">

`CoreApiClient` é o cliente principal para consultar e mutar dados do espaço de trabalho. Ele é **gerado a partir do schema do seu espaço de trabalho** durante `yarn beeax dev` ou `yarn beeax build`, então é totalmente tipado para corresponder aos seus objetos e campos.
...
<Note>
**CoreApiClient é gerado em tempo de dev/build.** Se você usá-lo sem executar primeiro `yarn beeax dev` ou `yarn beeax build`, ele lançará um erro. A geração ocorre automaticamente — a CLI analisa o schema GraphQL do seu espaço de trabalho e gera um cliente tipado usando `@genql/cli`.
</Note>
...
<Note>
  Quando seu código é executado no BeeAX (funções de lógica ou componentes de front-end), a plataforma injeta credenciais como variáveis de ambiente:

  * `TWENTY_API_URL` — URL base da API do BeeAX
