---
title: Conceitos
description: Como as aplicações BeeAX funcionam — modelo de entidade, sandboxing e ciclo de vida da instalação.
icon: sitemap
---
...
Uma aplicação é uma coleção de **entidades** declaradas usando funções `defineEntity()` do pacote `beeax-sdk`. O SDK detecta essas declarações via análise de AST no momento da compilação e produz um **manifesto** — uma descrição completa do que seu aplicativo adiciona a um espaço de trabalho. Essas funções validam sua configuração em tempo de compilação e oferecem autocompletar na IDE e segurança de tipos.
...
| **Componente de front-end**   | UI React em sandbox dentro do BeeAX                | [Componentes de front-end](/l/pt/developers/extend/apps/layout/front-components)        |
...
* **`yarn beeax dev:build`** — compila TypeScript, empacota funções de lógica e componentes de front-end com o esbuild e produz um manifesto.
