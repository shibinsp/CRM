---
title: Scaffolding
description: Gere arquivos de entidade de forma interativa com `yarn beeax dev:add` — objetos, campos, visualizações, funções de lógica e mais.
...
| Função lógica             | `yarn beeax dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente de front-end   | `yarn beeax dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Função                    | `yarn beeax dev:add role`               | `src/roles/\<name>.ts`                 |
| Habilidade                | `yarn beeax dev:add skill`              | `src/skills/\<name>.ts`                |
| Agente                    | `yarn beeax dev:add agent`              | `src/agents/\<name>.ts`                |
| Vista                     | `yarn beeax dev:add view`               | `src/views/\<name>.ts`                 |
| Item do menu de navegação | `yarn beeax dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Layout da página          | `yarn beeax dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## O que o scaffolder gera

Cada tipo de entidade tem seu próprio modelo. Por exemplo, `yarn beeax dev:add object` solicita:
