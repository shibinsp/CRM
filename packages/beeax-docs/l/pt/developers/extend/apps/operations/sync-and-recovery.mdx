...
<Note>
Para a iteração local do dia a dia, quase sempre você vai querer `yarn beeax dev`. Fazer deploy e publicar servem para entregar releases, **não** para o ciclo local.
</Note>

| Você quer…                                        | Comando                             | Notas                                                                                                                          |
| ------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Iterar localmente com sincronização em tempo real | `yarn beeax dev`                   | Monitora seus arquivos e sincroniza a cada alteração.                                                                          |
| Sincronizar uma vez e sair (CI, scripts, hooks)   | `yarn beeax dev --once`            | Um build + sincronização, depois encerra.                                                                                      |
| Prever mudanças **sem aplicá-las**                | `yarn beeax dev --once --dry-run`  | Calcula e imprime o diff; não grava nada.                                                                                      |
| Remover o app do workspace                        | `yarn beeax app:uninstall`         | Adicione `--yes` para pular o prompt.                                                                                          |
| Enviar um tarball para um servidor                | `yarn beeax app:publish --private` | Requer uma versão **estritamente maior** em `package.json` — veja [Publicação](/l/pt/developers/extend/apps/operations/publishing). |
| Publicar no marketplace (npm)                     | `yarn beeax app:publish`           | —                                                                                                                              |
| Instalar / atualizar uma versão implantada        | `yarn beeax app:install`           | Instala a versão atualmente implantada.                                                                                        |
| Limpar o servidor local e começar do zero         | `yarn beeax docker:reset`          | Exclui **todos** os dados locais — último recurso.                                                                             |

### A sincronização local não precisa de incremento de versão

A regra de `version` estritamente crescente (`VERSION_ALREADY_EXISTS` no deploy, `APP_ALREADY_INSTALLED` / `CANNOT_DOWNGRADE_APPLICATION` na instalação) se aplica a **`app:publish` / `app:install`** — o caminho de release. `yarn beeax dev` sincroniza seu manifesto no lugar e nunca exige mudança de versão, então você não precisa mexer em `package.json` para iterar. Se você se pegar aumentando a versão para testar uma mudança local, está usando o caminho de release quando o que quer é o ciclo de desenvolvimento.
...
`yarn beeax dev --once --dry-run` compila seu manifesto, pede ao servidor o plano de migração e o imprime — **sem aplicar nada**. É a forma segura de responder "o que esta sincronização mudaria?" antes de se comprometer com ela.
...
<Note>
Um dry run só antevê mudanças de **metadados**, e exige que o app tenha sido sincronizado ao menos uma vez (para que o workspace o conheça). Se você rodar isso em um app que nunca foi sincronizado, o servidor informa que o app não está instalado — rode `yarn beeax dev` uma vez antes.
</Note>
...
1. **Ressincronizar.** Rode `yarn beeax dev --once` novamente. Sincronizações são idempotentes — rodar novamente um manifesto limpo é seguro e frequentemente resolve um problema transitório.
2. **Prever o plano.** Rode `yarn beeax dev --once --dry-run` para ver exatamente o que a próxima sincronização pretende mudar, sem aplicá-la.
...
4. **Desinstalar e reinstalar.** `yarn beeax app:uninstall`, depois sincronize novamente (`yarn beeax dev`). Isso reconstrói os metadados do app a partir do zero, mantendo o restante do seu workspace intacto.
5. **Reset completo (último recurso).** `yarn beeax docker:reset`, depois faça o seeding e a sincronização novamente.

<Warning>
`yarn beeax docker:reset` exclui **todos** os dados da sua instância local — todos os workspaces, registros e apps. Use isso somente depois que as etapas anteriores tiverem falhado.
</Warning>

<Note>
Encontrou um erro de metadados? Por favor, [abra uma issue](https://github.com/beeax/beeax/issues/new/choose) e inclua a mensagem de migração que falhou (com seu tipo de metadado e `universalIdentifier`), a saída de `Metadata changes` da sincronização e os comandos que você rodou.
</Note>
