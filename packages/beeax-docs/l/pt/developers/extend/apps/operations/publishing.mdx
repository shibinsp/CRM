...
Isso compila seu código-fonte em TypeScript, transpila funções de lógica e componentes de front-end e grava tudo em `.beeax/output/`. Adicione `--tarball` para também gerar um pacote `.tgz` para distribuição manual ou para o comando de publish.
...
2. Execute `yarn beeax app:publish --private` (ou `yarn beeax app:publish --private --remote production`)
...
<Note>
O servidor realiza a verificação definitiva — ele valida `engines.beeax` tanto no upload do tarball quanto na instalação no espaço de trabalho. Se você implantar um tarball fora de banda ou instalar a partir do marketplace, o servidor ainda impõe a compatibilidade.
</Note>
...
2. Inicia uma instância de teste do BeeAX isolada usando a ação composta `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (o equivalente em CI de `yarn beeax docker:start --test`).
...
2. Executa `beeax/beeax/.github/actions/deploy-beeax-app@main` — o equivalente em CI de `yarn beeax app:publish --private`.
3. Executa `beeax/beeax/.github/actions/install-beeax-app@main` para que a versão recém-implantada seja instalada no espaço de trabalho de destino.
...
Para outros sistemas de CI (GitLab CI, CircleCI etc.), aplicam-se os mesmos três comandos: `yarn install`, `yarn beeax dev:build` e, em seguida, `npm publish` a partir de `.beeax/output`.
...
Para instalar uma versão mais recente, implante ou publique-a primeiro e, em seguida, execute novamente `yarn beeax app:install`.
</Note>
