...
A **camada de operações** é tudo o que você faz *para* o seu app em vez de *com* ele: invocar comandos de CLI, executar testes de integração contra um servidor BeeAX real, configurar CI e enviar versões — seja como um tarball implantado em um único servidor ou como um pacote npm listado no marketplace.
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/l/pt/developers/extend/apps/operations/cli">
    Referência do `yarn beeax` — exec, logs, uninstall, remotes.
  </Card>
