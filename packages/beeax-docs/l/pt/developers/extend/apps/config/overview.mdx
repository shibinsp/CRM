...
A **camada de configuração** de uma aplicação BeeAX é o que descreve a aplicação *para a plataforma* — a sua identidade, as permissões que detém e o código que é executado durante a instalação ou atualização. Estas declarações não adicionam novos formatos de dados nem comportamento em tempo de execução; dizem à BeeAX *quem é a aplicação* e *como configurá-la*.
