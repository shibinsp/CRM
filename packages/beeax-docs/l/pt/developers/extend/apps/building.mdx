---
title: Arquitetura
description: Como as aplicações BeeAX funcionam — sandboxing, ciclo de vida e os blocos de construção.
icon: sitemap
---

As aplicações BeeAX são pacotes TypeScript que estendem seu espaço de trabalho com objetos personalizados, lógica, componentes de UI e recursos de IA. Elas são executadas na plataforma BeeAX com sandboxing completo e controles de permissão.

## Como as aplicações funcionam

Uma aplicação é uma coleção de **entidades** declaradas usando funções `defineEntity()` do pacote `beeax-sdk`. O SDK detecta essas declarações via análise de AST no momento da compilação e produz um **manifesto** — uma descrição completa do que seu aplicativo adiciona a um espaço de trabalho.
...
| **Componente de front-end**   | UI React em sandbox na página do BeeAX               | [Componentes de front-end](/l/pt/developers/extend/apps/front-components) |
...
* **Componentes de front-end** executam em Web Workers usando Remote DOM — isolados da página principal, mas renderizando elementos DOM nativos (não iframes). Eles se comunicam com o BeeAX por meio de uma API de host com passagem de mensagens.
...
* **`yarn beeax dev`** — observa seus arquivos-fonte e sincroniza ao vivo as alterações com um servidor BeeAX conectado. O cliente de API tipado é regenerado automaticamente quando o esquema muda.
* **`yarn beeax build`** — compila TypeScript, empacota funções de lógica e componentes de front-end com o esbuild e produz um manifesto.
...
<CardGroup cols={2}>
  ...
  <Card title="Componentes de front-end" icon="window-maximize" href="/l/pt/developers/extend/apps/front-components">
    Componentes React em sandbox dentro da UI do BeeAX.
  </Card>
