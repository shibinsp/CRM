...
description: Defina objetos, campos, papéis e metadados da aplicação com o BeeAX SDK.
icon: database
---

O pacote `beeax-sdk` fornece funções `defineEntity` para declarar o modelo de dados da sua aplicação. Você deve usar `export default defineEntity({...})` para que o SDK detecte suas entidades. Essas funções validam sua configuração em tempo de compilação e oferecem autocompletar na IDE e segurança de tipos.
...
* Veja um exemplo funcional: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* Você pode criar novos objetos usando `yarn beeax add`, que orienta você sobre nomeação, campos e relacionamentos.

<Note>
**Os campos base são criados automaticamente.** Quando você define um objeto personalizado, o BeeAX adiciona automaticamente campos padrão
...
Use `defineField()` para adicionar campos a objetos que não são seus — como objetos padrão do BeeAX (Person, Company, etc.). ou a objetos de outros apps. Ao contrário dos campos inline em `defineObject()`, os campos independentes exigem um `objectUniversalIdentifier` para especificar qual objeto eles estendem:
...
Pontos-chave:
* `objectUniversalIdentifier` identifica o objeto de destino. Para objetos padrão, use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exportado de `beeax-sdk`.
...
As relações conectam objetos entre si. No BeeAX, as relações são sempre **bidirecionais** — você define ambos os lados, e cada lado faz referência ao outro.
...
Para criar uma relação com um objeto integrado do BeeAX (Person, Company, etc.), use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Gerando entidades com `yarn beeax add`
...
| ------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Objeto                    | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| Campo                     | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| Função lógica             | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componente de front-end   | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Papel                     | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| Habilidade                | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| Agente                    | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| Vista                     | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| Item do menu de navegação | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Layout da página          | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### O que o scaffolder gera

Cada tipo de entidade tem seu próprio modelo. Por exemplo, `yarn beeax add object` solicita:
