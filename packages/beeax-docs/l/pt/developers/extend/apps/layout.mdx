---
title: Layout
description: Defina vistas, itens do menu de navegação e layouts de página para moldar como seu app aparece no BeeAX.
icon: table-columns
---

As entidades de layout controlam como seu app aparece na UI do BeeAX — o que fica na barra lateral, quais vistas salvas acompanham o app e como uma página de detalhes de registro é organizada.
...
`definePageLayoutTab` permite que seu app adicione uma única aba — com widgets opcionais — a um layout de página **existente**. O caso de uso mais comum é adicionar uma aba personalizada (por exemplo, uma aba de análises ou de resumo por IA) a uma das páginas de registro nativas do BeeAX, ou a um layout de página que o seu próprio app já fornece.

O layout de página de destino deve ser um layout de página **padrão** do BeeAX ou um definido pelo **seu próprio app**; referências entre apps a layouts de página pertencentes a outro app instalado não são compatíveis no momento.
