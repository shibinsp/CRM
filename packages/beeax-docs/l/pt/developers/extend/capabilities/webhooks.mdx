...
Os webhooks enviam dados para seus sistemas em tempo real quando eventos ocorrem no BeeAX — sem necessidade de polling. Use-os para manter sistemas externos em sincronia, acionar automações ou enviar alertas.
...
O BeeAX envia webhooks para estes tipos de eventos:
...
O BeeAX assina cada solicitação de webhook por segurança. Valide as assinaturas para garantir que as solicitações sejam autênticas.

### Headers

| Cabeçalho                    | Descrição                |
| ---------------------------- | ------------------------ |
| `X-BeeAX-Webhook-Signature` | Assinatura HMAC SHA256   |
| `X-BeeAX-Webhook-Timestamp` | Timestamp da solicitação |

### Etapas de validação

1. Obtenha o timestamp de `X-BeeAX-Webhook-Timestamp`
2. Crie a string: `{timestamp}:{JSON payload}`
3. Calcule o HMAC SHA256 usando o segredo do seu webhook
4. Compare com `X-BeeAX-Webhook-Signature`
...
| **Gatilho de webhook de fluxo de trabalho** | ENTRADA | Receber dados no BeeAX a partir de sistemas externos                            |
