...
description: Como o BeeAX impõe a unicidade dos dados durante a importação.
---
...
O BeeAX impõe unicidade em determinados campos para evitar registos duplicados e garantir a integridade dos dados. Compreender estas restrições é essencial para importações bem-sucedidas.
...
<Note>O campo `id` é o identificador interno do BeeAX, gerado automaticamente para cada registo. Utiliza o formato UUID (por exemplo, `c776ee49-f608-4a77-8cc8-6fe96ae1e43f`).</Note>
...
<Note>O nome do campo `id` é reservado para o ID interno do BeeAX. Utilize um nome diferente, como `externalId` ou `legacyId`, para identificadores externos.</Note>
...
2. **Verifique se existem registos** no BeeAX antes de importar
