...
Depois de carregar o seu ficheiro e mapear as colunas, o BeeAX valida os seus dados:

1. **A validação é executada** — o BeeAX verifica cada linha quanto a erros
...
| --------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| Valores duplicados realçados a amarelo                                      | O valor já existe no BeeAX ou aparece duas vezes no seu ficheiro |
...
**Opção 3: Deixe o BeeAX atualizar o registo existente**

1. Garanta que o seu ficheiro inclui um identificador único (`email`, `domain` ou `id`)
2. Mapeie o campo do identificador único
3. O BeeAX atualizará o registo existente em vez de criar um duplicado
...
Desde que um identificador único corresponda, o BeeAX atualiza o registo.
</Note>
...
3. Verifique se os registos já existem no BeeAX
...
Isto significa que não existe nenhuma Empresa no BeeAX com esse identificador específico.
...
Está a tentar ligar a um registo que não existe no BeeAX.
...
Uma coluna no seu ficheiro não pode ser mapeada porque o campo não existe no BeeAX.
...
1. **O utilizador não aceitou o convite** — o utilizador ainda não existe no BeeAX
2. **A usar o ID de utilizador do sistema antigo** — o BeeAX não consegue corresponder IDs de outro sistema
3. **Formato de email incorreto** — o email não corresponde à conta do utilizador no BeeAX

### Como corrigir

1. Garanta que todos os utilizadores aceitaram o convite para o seu espaço de trabalho no BeeAX
2. Use o **endereço de email** do utilizador (não o nome nem o ID do sistema antigo)
3. Use o mesmo email que utilizaram para aderir ao BeeAX
...
<Check>O formato de domínio corresponde exatamente ao formato do BeeAX</Check>
...
1. **Transfira o ficheiro de exemplo** — veja o formato exato que o BeeAX espera
