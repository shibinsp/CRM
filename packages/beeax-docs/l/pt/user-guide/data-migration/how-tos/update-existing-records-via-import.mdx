...
Quando importar um ficheiro que contenha um **identificador único** que corresponde a um registo existente, a BeeAX atualiza esse registo em vez de criar um duplicado.

| Se o identificador único...        | A BeeAX irá...                                           |
| ---------------------------------- | --------------------------------------------------------- |
...
4. **Não altere os valores do identificador único** — caso contrário a BeeAX criará novos registos
...
Se alterar `john@acme.com` para `john.smith@acme.com`, a BeeAX criará um novo registo em vez de atualizar o existente.
</Warning>
...
A BeeAX faz a correspondência dos registos pelo identificador único e atualiza-os com novos valores.
...
3. A BeeAX fará a correspondência com base nesse campo
...
| **Incompatibilidade de maiúsculas/minúsculas** | O ficheiro tem `John@acme.com`, mas a BeeAX tem `john@acme.com` | Cria novo registo                        | Exporte da BeeAX para obter os valores exatos |
...
<AccordionGroup>
  ...
  <Accordion title="Como sei quais os registos que serão atualizados vs. criados?">
    Durante a etapa de revisão da importação, a BeeAX mostra quantos registos serão atualizados vs. criados com base nas correspondências do identificador único.
  </Accordion>
