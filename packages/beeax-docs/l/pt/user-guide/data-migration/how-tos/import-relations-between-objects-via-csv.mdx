...
O pai (lado "um") deve existir no BeeAX antes de poder referenciá-lo.
...
1. Exporte os registos pai do BeeAX
...
Use `https://domain.com` (não apenas `domain.com`). Isto corresponde à forma como a BeeAX armazena domínios de Empresas e evita erros de correspondência.
</Warning>
...
A BeeAX ligará automaticamente cada registo filho ao pai correspondente.
...
| **Registos inexistentes**                      | O registo pai não existe no BeeAX               | Importe primeiro os registos pai ou verifique se há erros ortográficos |
| **Incompatibilidade de maiúsculas/minúsculas** | `Acme.com` no ficheiro mas `acme.com` no BeeAX  | Garanta correspondência exata de maiúsculas/minúsculas                 |
...
<AccordionGroup>
  <Accordion title="E se o meu registo pai não tiver um domínio ou email?">
    Tem duas opções:
    1. Use o `id` da BeeAX (exporte os registos pai para obter os respetivos IDs)
