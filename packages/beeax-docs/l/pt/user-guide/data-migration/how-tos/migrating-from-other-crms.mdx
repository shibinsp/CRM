...
description: Guia passo a passo para migrar os seus dados de qualquer CRM para o BeeAX.
---

## Visão Geral

Este guia explica como migrar os seus dados de qualquer CRM para o BeeAX. O processo envolve auditar os seus dados, preparar o seu espaço de trabalho no BeeAX, exportar do seu sistema atual e importar para o BeeAX.
...
Crie um documento de mapeamento entre o seu CRM atual e o BeeAX:
...
## Passo 3: Configure o seu espaço de trabalho no BeeAX

Antes de importar dados, prepare o seu espaço de trabalho no BeeAX:
...
Se os seus dados incluírem referências a utilizadores (Proprietário da conta, Responsável, etc.), esses utilizadores devem existir no BeeAX antes da importação. Caso contrário, essas relações não podem ser mapeadas.
</Warning>
...
Abra cada CSV exportado numa aplicação de folha de cálculo e prepare-o para o BeeAX.

### Remova duplicados

1. Ordene pelo campo único (e-mail para Pessoas, domínio para Empresas)
2. Remova ou una linhas duplicadas
3. Verifique se não existem já duplicados no BeeAX
...
Use `https://domain.com` (não `domain.com` nem `www.domain.com`). Isto corresponde ao formato do BeeAX e evita duplicados quando ligar a sincronização de e-mail/calendário.
</Warning>
...
2. Use os mesmos endereços de e-mail que os utilizadores usaram para aderir ao seu espaço de trabalho no BeeAX
...
1. Navegue até ao objeto no BeeAX
...
* O BeeAX começará a sincronizar e-mails com os registos de contactos
...
<AccordionGroup>
  <Accordion title="Posso migrar fluxos de trabalho automaticamente?">
    De momento, não. Os fluxos de trabalho devem ser recriados manualmente no BeeAX.
  </Accordion>
...
Contacte [contact@beeax.com](mailto:contact@beeax.com) ou explore os nossos [Serviços de Implementação](/l/pt/user-guide/getting-started/capabilities/implementation-services).
