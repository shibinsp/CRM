...
description: Guia completo passo a passo para importar pessoas/contactos para o BeeAX.
---

## Visão Geral

Este guia explica como importar os seus contactos (People) para o BeeAX. **Importe Empresas primeiro** se quiser associar Pessoas a Empresas.
...
* **Deve ser único** — sem duplicados no seu ficheiro ou no BeeAX
...
<Note>
**Use o formato de URL de domínio** (`https://acme.com`), não o rótulo. Isto corresponde à forma como as Empresas são armazenadas no BeeAX.
</Note>
...
<Note>
**Dica profissional:** Clique em **Download sample file** durante a importação ou exporte algumas Pessoas existentes para ver os nomes de colunas exatos que o BeeAX espera.
</Note>
...
3. Aguarde que o BeeAX analise o seu ficheiro

## Passo 4: Mapeie as suas colunas

O BeeAX tenta automaticamente corresponder as suas colunas aos campos. Revise e ajuste:
...
3. O BeeAX irá associar cada Pessoa à Empresa correspondente
...
1. O BeeAX exibe seus valores ao lado das opções existentes
2. Relacione cada valor no seu arquivo a uma opção do BeeAX
...
Antes de concluir a importação, o BeeAX valida seus dados:
...
| Erro                           | Causa                                      | Solução                                              |
| ------------------------------ | ------------------------------------------ | ---------------------------------------------------- |
| **E-mail duplicado**           | O e-mail já existe no BeeAX ou no arquivo | Remova o duplicado ou atualize o registro existente  |
...
Seus contatos agora estão no BeeAX! Próximos passos:
...
2. O BeeAX faz a correspondência dos registros por esse identificador único
...
<AccordionGroup>
  <Accordion title="Por que duas Pessoas não podem ter o mesmo e-mail?">
    O e-mail é um identificador único no BeeAX. Isso evita contatos duplicados e garante que a sincronização de e-mail vincule corretamente os e-mails à pessoa certa.
  </Accordion>
  ...
  <Accordion title="Posso importar o ID do meu CRM anterior?">
    Sim! Crie um campo personalizado marcado como "único" no seu modelo de dados para armazenar o ID externo. Observação: o nome do campo `id` é reservado para o ID interno do BeeAX.
  </Accordion>
