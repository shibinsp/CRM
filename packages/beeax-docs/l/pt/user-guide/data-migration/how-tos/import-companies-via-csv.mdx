...
description: Guia completo passo a passo para importar empresas para o BeeAX.
---

## Visão Geral

Este guia explica como importar as suas empresas para o BeeAX. **As empresas devem ser importadas primeiro** porque Pessoas e Oportunidades se ligam a Empresas.
...
<Note>
**Dica profissional:** Clique em **Download sample file** durante a importação para ver os nomes de colunas exatos que o BeeAX espera.
</Note>
...
1. O BeeAX mostra os seus valores ao lado das opções existentes
2. Relacione cada valor no seu ficheiro a uma opção do BeeAX
...
Antes de concluir a importação, o BeeAX valida os seus dados:
...
| Erro                            | Causa                          | Solução                                            |
| ------------------------------- | ------------------------------ | -------------------------------------------------- |
| **Domínio duplicado**           | O domínio já existe no BeeAX  | Remova do ficheiro ou atualize o registo existente |
...
2. O BeeAX faz a correspondência dos registos por este identificador único
...
<AccordionGroup>
  <Accordion title="Porque é que duas Empresas não podem ter o mesmo domínio?">
    O domínio é um identificador único no BeeAX. Isto evita empresas duplicadas e garante que a sincronização de e-mail associe corretamente os e-mails à empresa certa.
  </Accordion>
  ...
  <Accordion title="O que acontece se importar um domínio que já existe?">
    Se incluir um identificador único (domínio ou id) que corresponda a uma empresa existente, a BeeAX atualiza essa empresa em vez de criar um duplicado.
  </Accordion>
