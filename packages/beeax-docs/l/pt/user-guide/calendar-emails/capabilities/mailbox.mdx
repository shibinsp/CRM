...
description: Entendendo os recursos de integração de e-mail no BeeAX.
---
...
BeeAX vincula automaticamente os emails de suas caixas de correio conectadas aos registros de CRM relevantes, mantendo todo o histórico de comunicação em um só lugar.
...
BeeAX importa emails a cerca de **400 mensagens por minuto**. Atualmente, essa taxa é limitada pela limitação de taxa da API do Gmail (e aplica-se de forma semelhante a outros provedores).
...
* **Emails Ausentes**: Verifique se:
  ...
  * A criação automática de contatos está desativada (os emails precisam de registros existentes no BeeAX)
