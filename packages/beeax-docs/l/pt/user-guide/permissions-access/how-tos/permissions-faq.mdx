...
<AccordionGroup>
<Accordion title="Quais funções o BeeAX traz por padrão?">
O BeeAX vem com as funções **Admin** e **Member** por padrão. Você pode criar funções personalizadas adicionais com base nas necessidades da sua equipe (por exemplo, Representante de vendas, Gerente, Usuário somente leitura).
</Accordion>
...
Você pode controlar:
* **Enviar e-mail**: Capacidade de enviar e-mails a partir do BeeAX
...
Com o provisionamento JIT, as contas de usuário são criadas automaticamente no BeeAX quando alguém faz login via SSO pela primeira vez. A função padrão é atribuída automaticamente.
</Accordion>
