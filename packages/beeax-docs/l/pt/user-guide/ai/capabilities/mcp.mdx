---
title: Servidor MCP
description: Conecte assistentes de IA ao seu espaço de trabalho do BeeAX usando o Model Context Protocol.
---
...
O BeeAX expõe um servidor [MCP](https://modelcontextprotocol.io/) para que assistentes de IA — Claude Desktop, Claude Code, Cursor, ChatGPT e outros — possam ler e escrever seus dados de CRM por meio de linguagem natural.

Use o **URL do espaço de trabalho** (o URL que você usa para acessar o BeeAX) como o endpoint do MCP. Na BeeAX Cloud, o URL do seu espaço de trabalho pode ser `https://{mycompany}.beeax.com` ou um domínio personalizado. O servidor está disponível em:

| Ambiente           | Endpoint do MCP                                                                      |
| ------------------ | ------------------------------------------------------------------------------------ |
| **Nuvem**          | `https://{your-workspace-url}/mcp` (por exemplo, `https://mycompany.beeax.com/mcp`) |
...
Adicione isto à configuração do seu cliente MCP, substituindo `{your-workspace-url}` pelo host do seu espaço de trabalho (por exemplo, `mycompany.beeax.com`):
...
1. Descobrir os metadados de OAuth do BeeAX por meio de `/.well-known/oauth-protected-resource` e `/.well-known/oauth-authorization-server`
...
Vá até **Settings > AI > More > MCP Server** no BeeAX. Escolha seu método de autenticação (OAuth ou Chave de API), copie o trecho de JSON (ele já usará o URL do seu espaço de trabalho) e cole-o no arquivo de configuração do seu cliente MCP.
...
Reinicie seu cliente MCP (ou recarregue a configuração). Se estiver usando OAuth, você será redirecionado ao BeeAX para autorizar o acesso. Se estiver usando uma chave de API, a conexão é imediata.
...
Depois de conectado, o servidor MCP expõe ferramentas que refletem a API do BeeAX. O fluxo de trabalho recomendado é:
...
Para instâncias auto-hospedadas, substitua `{your-workspace-url}` pelo URL do seu servidor. Certifique-se de que `SERVER_URL` no seu ambiente corresponda ao URL público da sua instância do BeeAX — isso é usado para gerar os metadados de descoberta do OAuth.
