...
🚨 **重要：此解决方案仅适用于全新安装** 🚨
如果您有一个现有的 BeeAX 实例和生产数据，请**不要**按照这些步骤操作，因为这些步骤会永久删除您的数据库！

在首次安装 BeeAX 时，您可能想更改默认的数据库密码。
...
在安装 BeeAX 时，您需要为您的 postgres 数据库提供正确的模式、扩展和用户。
如果您成功进行了此配置，数据库中应有 `default` 和 `metadata` 模式。
如果没有，请确保您的计算机上没有运行多个 postgres 实例。

#### 找不到模块 'beeax-emails' 或其相应的类型声明。

在运行 `npx nx run beeax-emails:build`来初始化数据库之前，您必须构建 `beeax-emails` 包。

#### 缺少 beeax-x 包

确保在根目录运行 yarn，然后运行 `npx nx server:dev beeax-server`。 如果这依然不工作，尝试手动构建缺失的包。
...
#### 运行 `npx nx start` 或 `npx nx start beeax-front` 时，出现内存不足错误

仅运行您需要的服务，而不是 `npx nx start`。 例如，如果您在服务器上工作，仅运行 `npx nx worker beeax-server`

**如果不起作用：**
如果您尝试在 WSL 上仅运行 `npx nx run beeax-server:start` 但因以下内存错误而失败：
...
**如果不起作用：**
调查哪些进程消耗了您机器的大部分内存。 在 BeeAX，我们注意到一些 VScode 扩展占用了大量的内存，因此我们暂时禁用了它们。
...
* 打开您的 BeeAX 应用，导航到 `/settings`，并在屏幕左下角启用 **高级** 切换。
* 创建一个新的 webhook。
* 把 **您的唯一 Webhook URL** 粘贴在 BeeAX 的 **Endpoint Url** 字段中。 将 **筛选器** 设置为 `Companies` 和 `Created`。
...
注释掉 `packages/beeax-ui/vite-config.ts` 中的 checker 插件，如下面示例所示
...
2. **检查反向代理设置：**
   ...
   * 确认您的反向代理正确地将请求转发到 BeeAX 服务器。
   * 确保正确设置了像 `X-Forwarded-For` 和 `X-Forwarded-Proto` 这样的标头。

3. **重启服务：**

   在进行更改后，重启反向代理和 BeeAX 容器。
...
* 社区支持：
  ...
  联系 [BeeAX 社区](https://github.com/beeax/beeax/issues) 或 [支持渠道](https://discord.gg/cx5n4Jzs57) 以获得帮助。
