...
BeeAX 对面向用户的应用采用授权码 + PKCE 实现 OAuth 2.0，对服务器到服务器访问采用客户端凭证。 客户端通过 [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591) 动态注册——无需在仪表板中手动设置。
...
| 带有 UI 扩展的 BeeAX 应用 | [应用](/l/zh/developers/extend/apps/getting-started) (OAuth 将自动处理) |

## 注册客户端

根据 [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591)，BeeAX 支持**动态客户端注册**。 无需手动设置——以编程方式注册：
...
当你的应用代表某个 BeeAX 用户执行操作时，请使用此流程。
...
授权后，BeeAX 会重定向回你的 `redirect_uri`：
