...
当 BeeAX 中发生事件时，Webhook 会实时将数据推送到您的系统 — 无需轮询。 使用它们来保持外部系统同步、触发自动化或发送警报。
...
为了安全起见，BeeAX 会对每个 Webhook 请求进行签名。 请验证签名以确保请求真实有效。

### Headers

| 标题                           | 描述             |
| ---------------------------- | -------------- |
| `X-BeeAX-Webhook-Signature` | HMAC SHA256 签名 |
| `X-BeeAX-Webhook-Timestamp` | 请求时间戳          |

### 验证步骤

1. 从 `X-BeeAX-Webhook-Timestamp` 获取时间戳
2. 构造字符串：`{timestamp}:{JSON payload}`
3. 使用您的 Webhook 密钥计算 HMAC SHA256
4. 与 `X-BeeAX-Webhook-Signature` 进行比较
...
| **工作流 Webhook 触发器** | IN  | 从外部系统接收数据到 BeeAX    |
