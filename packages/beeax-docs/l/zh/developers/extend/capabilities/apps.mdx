---
title: BeeAX 应用
description: 以代码的形式构建并管理 BeeAX 自定义项。
---
...
应用可通过自定义对象、字段、逻辑函数、前端组件、AI 技能等来扩展 BeeAX——全部以代码进行管理。 无需通过 UI 配置所有内容，你可以用 TypeScript 定义数据模型和逻辑，并将其部署到一个或多个工作空间。

**你可以构建的内容:**

* **自定义对象和字段** — 使用新实体扩展你的数据模型，或为现有对象（如 Company 或 Person）添加字段
* **逻辑函数** — 由数据库事件、定时任务（cron）或 HTTP 路由触发的服务端函数
* **前端组件** — 在 BeeAX 的 UI 中渲染的 React 组件（记录页面、命令菜单、侧边面板）
* **AI 技能和智能体** — 通过自定义能力扩展 BeeAX 的 AI
...
这将搭建一个新的应用，可选地启动本地 BeeAX 服务器，并开始监视你的文件变更。 请参阅[开始使用](/l/zh/developers/extend/apps/getting-started)指南，查看完整演练。
...
| `defineFrontComponent()`           | BeeAX 的 UI 中的 React 组件 |
| `defineRole()`                     | 权限角色                    |
| `defineView()`                     | 已保存视图配置                 |
...
1. **`yarn beeax dev`** — 监视源文件、更改时重新构建、同步到服务器、生成类型化的 API 客户端
2. **`yarn beeax build`** — 生成可分发的构建产物
3. **`yarn beeax deploy`** — 部署到远程 BeeAX 服务器
4. **`yarn beeax add`** — 交互式生成一个新实体

### CLI 参考

```bash filename="Terminal"
yarn beeax help          # 列出所有命令
yarn beeax server start  # 启动本地开发服务器
yarn beeax remote add    # 连接到 BeeAX 服务器
yarn beeax exec -n fn    # 执行逻辑函数
yarn beeax logs -n fn    # 实时查看函数日志
```
