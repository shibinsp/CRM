---
title: 概览
description: 在 BeeAX 内运行的服务端 TypeScript，可由 HTTP 路由、Cron 调度、数据库事件、AI 工具或工作流动作触发。
icon: bolt
---

BeeAX 应用的 **逻辑层** 是实际*运行*的代码——用于响应 HTTP 请求、cron 调度和记录变更的服务端 TypeScript 处理程序；存在于工作区内的 AI 技能和智能体；以及让你的函数能够代表用户在第三方服务中执行操作的 OAuth 连接。
...
| **AI 工具**   | 某个 BeeAX AI 功能决定调用你的函数时                | `toolTriggerSettings`           |
