...
这会编译 TypeScript 源码，转译逻辑函数和前端组件，并将所有内容写入 `.beeax/output/`。 添加 `--tarball`，还可以生成用于手动分发或 publish 命令的 `.tgz` 包。
...
2. 运行 `yarn beeax app:publish --private`（或 `yarn beeax app:publish --private --remote production`）
...
1. 检出你的应用源代码。
2. 使用 `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` 组合 action 启动一个隔离的 BeeAX 测试实例（相当于 CI 中的 `yarn beeax docker:start --test`）。
...
2. 运行 `beeax/beeax/.github/actions/deploy-beeax-app@main`——相当于 CI 中的 `yarn beeax app:publish --private`。
...
BeeAX 服务器会**每小时**从 npm 注册表同步其市场目录。
...
对于其他 CI 系统（GitLab CI、CircleCI 等），同样适用以下三条命令：`yarn install`、`yarn beeax dev:build`，然后在 `.beeax/output` 目录下执行 `npm publish`。
...
在 BeeAX 的 **Settings > Applications** 页面中，既可以浏览也可以安装来自市场或通过 tarball 部署的应用。
...
若要安装较新的版本，请先部署或发布它，然后重新运行 `yarn beeax app:install`。
</Note>
