---
title: 数据模型
description: 使用 BeeAX SDK 定义对象、字段、角色和应用程序元数据。
icon: database
---

`beeax-sdk` 包提供 `defineEntity` 函数，用于声明应用的数据模型。 你必须使用 `export default defineEntity({...})`，这样 SDK 才能检测到你的实体。 这些函数会在构建时校验你的配置，并提供 IDE 自动补全和类型安全。
...
* 查看一个可运行示例：[`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts)。
...
* 你可以使用 `yarn beeax add` 脚手架创建新对象，它会引导你完成命名、字段和关系。

<Note>
**基础字段会自动创建。** 当你定义自定义对象时，BeeAX 会自动添加标准字段
...
使用 `defineField()` 向你不拥有的对象添加字段——例如标准的 BeeAX 对象（Person、Company 等）。 或来自其他应用的对象。 与在 `defineObject()` 中的内联字段不同，独立字段需要一个 `objectUniversalIdentifier` 来指定它们要扩展的对象：
...
关键点：
* `objectUniversalIdentifier` 用于标识目标对象。 对于标准对象，请使用从 `beeax-sdk` 导出的 `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`。
...
关系用于将对象彼此连接。 在 BeeAX 中，关系始终是双向的——你需要定义两侧，每一侧都引用另一侧。
...
要与内置的 BeeAX 对象（Person、Company 等）建立关系，请使用 `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`：
...
| 实体类型  | 命令                                   | 生成的文件                                                   |
| ----- | ------------------------------------ | ------------------------------------------------------- |
| 对象    | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| 字段    | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| 逻辑函数  | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| 前端组件  | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| 角色    | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| 技能    | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| 代理    | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| 视图    | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| 导航菜单项 | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| 页面布局  | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### 脚手架生成的内容

每种实体类型都有其自己的模板。 例如，`yarn beeax add object` 会询问：
