...
title: 发布
icon: 上传
description: 将你的 BeeAX 应用分发到应用市场，或进行内部部署。
---
...
* **部署 tar 包** — 直接将你的应用上传到特定的 BeeAX 服务器，以供内部或私有使用。
* **发布到 npm** — 将你的应用在 BeeAX 应用市场上架，供任何工作区发现并安装。
...
This compiles TypeScript sources, transpiles logic functions and front components, and writes everything to `.beeax/output/`. Add `--tarball` to also produce a `.tgz` package for manual distribution or the deploy command.

## 部署到服务器（tar 包）

对于你不希望公开的应用（专有工具、仅供企业使用的集成或实验性构建），你可以将 tar 包直接部署到某台 BeeAX 服务器。

### 先决条件

在部署之前，你需要配置一个指向目标服务器的远程。 远程会将服务器 URL 和身份验证凭据本地存储在 `~/.beeax/config.json` 中。
...
2. Run `yarn beeax deploy` (or `yarn beeax deploy --remote production`)
...
如果你的应用使用了特定 BeeAX 服务器版本中引入的功能（例如在 v2.3.0 中新增的 OAuth 提供方），应当在 `package.json` 的 `engines.beeax` 字段中声明应用所需的最低服务器版本：
...
* 如果已设置 `engines.beeax`，且目标服务器的版本不满足该范围，则部署（tarball 上传）或安装将被拒绝，并返回 `SERVER_VERSION_INCOMPATIBLE` 错误以及一条同时指明所需范围和实际服务器版本的消息。
* 如果 `engines.beeax` **未设置**，则该应用可在任何服务器版本上被接受（与现有应用向后兼容）。
* 如果服务器未配置 `APP_VERSION`，则跳过该检查。

<Note>
服务器是权威校验方——它会在 tarball 上传和工作区安装时验证 `engines.beeax`。 即使你通过带外方式部署 tarball 或从应用市场安装，服务器仍会强制执行兼容性要求。
</Note>

## 自动化 CI/CD（脚手架生成的工作流）

使用 `create-beeax-app` 生成的应用开箱即带有两个 GitHub Actions 工作流，位于 `.github/workflows/`。 当你将仓库推送到 GitHub 后即可运行——CI 无需额外设置，CD 只需要一个机密。
...
1. 检出你的应用源代码。
2. 使用 `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` 组合 action 启动一个隔离的 BeeAX 测试实例（相当于 CI 中的 `yarn beeax server start --test`）。
...
* `TWENTY_VERSION`（环境变量，默认 `latest`）— 通过在 `ci.yml` 中编辑它来固定 CI 使用的 BeeAX 服务器版本。
...
在每次向 `main` 推送时将你的应用部署到已配置的 BeeAX 服务器；当为拉取请求添加 `deploy` 标签时，也可从该拉取请求进行部署。

**作用:**

1. 检出 PR 的 head（针对已加标签的 PR），或被推送的提交。
2. 运行 `beeax/beeax/.github/actions/deploy-beeax-app@main`——相当于 CI 中的 `yarn beeax deploy`。
3. 运行 `beeax/beeax/.github/actions/install-beeax-app@main`，将新部署的版本安装到目标工作区。

**必需的配置:**

| 设置                      | 位置                                                       | 目的                                       |
| ----------------------- | -------------------------------------------------------- | ---------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `cd.yml` 中的 `env`（默认值为 `http://localhost:3000`）          | 要部署到的 BeeAX 服务器。 首次使用前将其更改为你真实的服务器 URL。 |
...
两个工作流都引用了位于 `@main` 的可复用 actions，因此会自动获取 `beeax/beeax` 仓库中的 action 更新。 如果你希望构建具有确定性，请在每个 `uses:` 行中将 `@main` 替换为某个提交的 SHA 或发行标签。

## 发布到 npm

发布到 npm 可让你的应用在 BeeAX 应用市场中被发现。 任何 BeeAX 工作区都可以直接通过 UI 浏览、安装和升级应用市场中的应用。

### 要求

* 一个 [npm](https://www.npmjs.com) 账户
* 你在 `package.json` 的 `keywords` 数组中的 `beeax-app` 关键字（需要手动添加 — 在 `create-beeax-app` 模板中默认不包含）
...
<Note>
如果您的应用未在 `defineApplication()` 中定义 `aboutDescription`，市场将自动使用 npm 上您的软件包的 `README.md` 作为关于页面内容。 这意味着您可以为 npm 和 BeeAX 市场维护同一个 README。 如果您希望在市场中使用不同的描述，请显式设置 `aboutDescription`。
</Note>
...
对于其他 CI 系统（GitLab CI、CircleCI 等），同样适用以下三条命令：`yarn install`、`yarn beeax build`，然后在 `.beeax/output` 目录下执行 `npm publish`。
...
若要安装较新的版本，请先部署或发布它，然后重新运行 `yarn beeax install`。
</Note>
