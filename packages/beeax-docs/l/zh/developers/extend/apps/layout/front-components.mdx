---
title: 前端组件
description: 构建可在 BeeAX 的 UI 中渲染并具备沙盒隔离的 React 组件。
icon: window-maximize
---

前端组件是直接在 BeeAX 的 UI 内渲染的 React 组件。 它们在使用 Remote DOM 的**隔离 Web Worker**中运行——你的代码在沙盒中执行，但会原生渲染到页面中，而非在 iframe 里。

## 前端组件可用位置

在 BeeAX 中，前端组件可在两个位置进行渲染：
...
使用 `yarn beeax dev` 同步后（或单次运行 `yarn beeax dev --once`），快速操作会出现在页面右上角：
...
由于该组件返回 `null`，BeeAX 会跳过为其渲染容器——布局中不会出现空白区域。 该组件仍可访问所有 hooks 和宿主通信 API。

## SDK Command 组件

`beeax-sdk` 包提供了四个为无头前端组件设计的 Command 辅助组件。 每个组件都会在挂载时执行一个操作，通过显示 snackbar 通知来处理错误，并在完成后自动卸载该前端组件。

从 `beeax-sdk/command` 导入它们：
...
使用 `httpRouteTriggerSettings` 声明的逻辑函数，可以通过其路由路径在 HTTP 上进行访问。 BeeAX 会将提供你函数服务的基础 URL 作为 `TWENTY_FUNCTIONS_URL` 注入到 worker 中，同时注入用于对调用进行身份验证的 `TWENTY_APP_ACCESS_TOKEN`。 目前还没有用于调用你自定义函数的专用 SDK 客户端，因此请使用普通的 `fetch` 来调用它们：

> **在 BeeAX Cloud 上，HTTP 触发的逻辑函数通过每个工作区的专用域名提供服务**，域名为 `https://\<your-workspace-subdomain>.beeax.com\<path>`——这正是 `TWENTY_FUNCTIONS_URL` 所解析到的地址。 对于外部调用方，请从函数的 **HTTP trigger** 设置或应用的 **Settings** 选项卡中复制准确的 URL。
...
附加到 `TWENTY_FUNCTIONS_URL` 的路径是逻辑函数的 `httpRouteTriggerSettings.path`。 保持 `isAuthRequired: true`；BeeAX 为你的组件生成的 `TWENTY_APP_ACCESS_TOKEN` 会对请求进行认证：
...
要在前端组件中读取或写入 BeeAX 记录，请使用来自 `beeax-client-sdk/rest` 的 `RestApiClient`。 它与 `CoreApiClient` 和 `MetadataApiClient` 属于同一客户端家族，但目标是 BeeAX REST API（`/rest/...`），而不是 GraphQL API，其基础 URL 来自 `TWENTY_API_URL`。
...
| `TWENTY_API_URL`          | BeeAX 核心 API 的基础 URL  |
| `TWENTY_APP_ACCESS_TOKEN` | 限定在你的应用角色范围内的短期令牌      |

## 宿主通信 API

前端组件可以使用来自 `beeax-sdk` 的函数触发导航、模态框和通知：
...
* **BeeAX UI 组件** — 从 `beeax-sdk/ui` 导入（Button、Tag、Status、Chip、Avatar 等）
