---
title: 概览
description: 将你的应用嵌入 BeeAX 的 UI 中——侧边栏条目、已保存视图、记录页面选项卡以及沙箱化的 React 组件。
icon: table-columns
---

BeeAX 应用的**布局层**是用户看到的一切：应用在侧边栏中的呈现位置、它随附的列表视图、记录详情页的排布方式，以及在这些页面中渲染的自定义 React 组件。
...
<CardGroup cols={2}>
  ...
  <Card title="前端组件" icon="window-maximize" href="/l/zh/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — 在 BeeAX 内部渲染的沙箱化 React 组件。
  </Card>
...
前端组件在使用 Remote DOM 的隔离 Web Worker 内运行——它们在页面中*原生*渲染（而不是在 iframe 中），但无法直接访问宿主页面或 DOM。 与 BeeAX 的通信通过消息传递的宿主 API 完成。
