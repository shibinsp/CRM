...
* **开发模式下不执行**：当应用在本地注册（通过 `yarn beeax dev`）时，服务器会完全跳过安装流程，并通过 CLI 监视器直接同步文件——因此无论 `shouldRunSynchronously` 如何，安装后在开发模式下都不会运行。 使用 `yarn beeax dev:function:exec --postInstall` 在运行中的工作区上手动触发它。
...
* **开发模式下不执行**：与安装后相同——对于本地注册的应用将完全跳过安装流程，因此在 `yarn beeax dev` 下不会运行安装前。 使用 `yarn beeax dev:function:exec --preInstall` 手动触发它。
