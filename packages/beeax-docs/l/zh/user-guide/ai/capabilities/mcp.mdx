---
title: MCP服务器
description: 使用 Model Context Protocol 将 AI 助手连接到你的 BeeAX 工作区。
---
...
BeeAX 提供一个 [MCP](https://modelcontextprotocol.io/) 服务器，使 AI 助手——Claude Desktop、Claude Code、Cursor、ChatGPT 等——能够通过自然语言读取和写入您的 CRM 数据。

将您的**工作区 URL**（用于访问 BeeAX 的 URL）用作 MCP 端点。 在 BeeAX Cloud 上，您的工作区 URL 可能是 `https://{mycompany}.beeax.com`，也可能是自定义域名。 服务器可通过以下地址访问：

| 环境      | MCP 端点                                                                     |
| ------- | -------------------------------------------------------------------------- |
| **云端**  | `https://{your-workspace-url}/mcp` (例如 `https://mycompany.beeax.com/mcp`) |
...
将以下内容添加到 MCP 客户端配置中，并将 `{your-workspace-url}` 替换为您的工作区主机（例如 `mycompany.beeax.com`）：
...
1. 通过 `/.well-known/oauth-protected-resource` 和 `/.well-known/oauth-authorization-server` 发现 BeeAX 的 OAuth 元数据
...
在 BeeAX 中前往 **Settings > AI > More > MCP Server**。 选择您的身份验证方式（OAuth 或 API 密钥），复制 JSON 片段（其中已包含您的工作区 URL），并将其粘贴到 MCP 客户端的配置文件中。
...
重启您的 MCP 客户端（或重新加载配置）。 如果使用 OAuth，您将被重定向到 BeeAX 以授权访问。 如果使用 API 密钥，连接会立即建立。
...
连接后，MCP 服务器会提供与 BeeAX API 一一对应的工具。 推荐的工作流程是：
...
对于自托管实例，请将 `{your-workspace-url}` 替换为您的服务器 URL。 请确保您环境中的 `SERVER_URL` 与 BeeAX 实例的公共 URL 一致——该值用于生成 OAuth 发现元数据。
