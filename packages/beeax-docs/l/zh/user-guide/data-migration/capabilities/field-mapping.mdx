...
当您上传文件时，BeeAX 会分析您的列，并尝试将其与现有字段匹配。
...
<Note>**快速提示：** 从您要导入的对象中导出几行数据。 导出的文件将包含 BeeAX 期望的精确列名，从而在导入时实现无缝的自动映射。</Note>

### 手动映射选项

对于每一列，您可以：

* **映射到字段**：从下拉列表中选择匹配的 BeeAX 字段
* **不进行映射**：完全跳过该列（不会导入数据）
...
在导入过程中指定 `id` 是可选的。 如未提供，BeeAX 将自动生成 ID。
