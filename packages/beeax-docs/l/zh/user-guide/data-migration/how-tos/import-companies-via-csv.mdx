---
title: 通过 CSV 导入公司
description: 将公司导入 BeeAX 的完整分步指南。
---

## 概览

本指南将引导您将公司导入 BeeAX。 **应先导入公司**，因为 People 和 Opportunities 会关联到公司。
...
<Note>
**专业提示：** 在导入过程中单击 **Download sample file**，以查看 BeeAX 期望的确切列名。
</Note>
...
| 错误         | 原因              | 解决方案                    |
| ---------- | --------------- | ----------------------- |
| **域名重复**   | 域名已存在于 BeeAX 中 | 从文件中删除或更新现有记录           |
...
<AccordionGroup>
  <Accordion title="为什么两个公司不能使用相同的域名？">
    在 BeeAX 中，域名是唯一标识符。 这可防止重复公司，并确保电子邮件同步将邮件正确关联到对应的公司。
  </Accordion>
  ...
  <Accordion title="如果我导入了一个已存在的域名，会发生什么？">
    如果您包含与现有公司匹配的唯一标识符（域名或 id），BeeAX 会更新该公司，而不是创建重复项。
  </Accordion>
