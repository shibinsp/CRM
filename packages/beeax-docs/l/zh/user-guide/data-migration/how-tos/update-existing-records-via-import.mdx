...
当您导入的文件包含与现有记录匹配的**唯一标识符**时，BeeAX 会更新该记录，而不是创建重复项。
...
如果您将 `john@acme.com` 更改为 `john.smith@acme.com`，BeeAX 将创建新记录，而不是更新现有记录。
</Warning>
...
| **大小写不匹配**  | 文件中为 `John@acme.com`，但 BeeAX 中为 `john@acme.com` | 创建新记录      | 从 BeeAX 导出以获取精确值            |
...
<AccordionGroup>
  ...
  <Accordion title="我如何知道哪些记录会被更新、哪些会被创建？">
    在导入审核步骤中，BeeAX 会根据唯一标识符的匹配情况，显示将被更新与被创建的记录数量。
  </Accordion>
