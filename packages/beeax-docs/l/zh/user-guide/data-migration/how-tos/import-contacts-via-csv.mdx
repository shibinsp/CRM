---
title: 通过 CSV 导入联系人
description: 将人员/联系人导入 BeeAX 的完整分步指南。
---

## 概览

本指南将引导您将联系人（People）导入 BeeAX。 **请先导入公司**，如果您想将 People 关联到公司。
...
<Note>
**使用域名 URL 格式**（`https://acme.com`），而不是标签。 这与公司在 BeeAX 中的存储方式一致。
</Note>
...
<Note>
**专业提示：** 在导入过程中单击 **Download sample file**，或导出现有的一些 People，以查看 BeeAX 期望的确切列名。
</Note>
...
3. BeeAX 会将每个 Person 关联到匹配的 Company
...
1. BeeAX 会将您的值与现有选项并列显示
2. 将文件中的每个值与一个 BeeAX 选项匹配
3. 或在需要时创建新选项
...
| 错误           | 原因                   | 解决方案                  |
| ------------ | -------------------- | --------------------- |
| **电子邮件重复**   | 电子邮件在 BeeAX 或文件中已存在 | 删除重复项或更新现有记录          |
...
<AccordionGroup>
  <Accordion title="为什么两个 People 不能使用相同的电子邮件？">
    在 BeeAX 中，电子邮件是唯一标识符。 这可防止重复联系人，并确保电子邮件同步将邮件正确关联到对应的人。
  </Accordion>
  ...
  <Accordion title="我可以导入之前 CRM 中的 ID 吗？">
    是! 在您的数据模型中创建一个标记为“唯一”的自定义字段，用于存储外部 ID。 注意：字段名 `id` 保留给 BeeAX 的内部 ID。
  </Accordion>
