...
在 BeeAX 中，该指纹作为 `headerMessageId` 存储在 Message 对象中。 您的工作流会获取它，并将其传递到 Send Email 操作的 In-Reply-To 字段。
...
* **仅回复 VIP** — 添加一个分支，用于检查发件人的域名，或他们是否在 BeeAX 中作为联系人存在
