...
<AccordionGroup>
<Accordion title="BeeAX 默认包含哪些角色？">
BeeAX 默认提供 **管理员** 和 **成员** 角色。 您可以根据团队需求创建额外的自定义角色（例如：销售代表、经理、只读用户）。
</Accordion>
...
启用 JIT 预配后，用户首次通过 SSO 登录时，会在 BeeAX 中自动创建其账户。 系统会自动为其分配默认角色。
</Accordion>
