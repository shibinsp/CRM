...
Înregistrarea părinte (partea „unu”) trebuie să existe în BeeAX înainte de a o putea referi.
...
1. Exportați înregistrările părinte din BeeAX
...
Folosiți `https://domain.com` (nu doar `domain.com`). Acest lucru corespunde modului în care BeeAX stochează domeniile companiilor și previne erorile de potrivire.
</Warning>
...
BeeAX va lega automat fiecare înregistrare copil de părintele corespunzător.
...
| **Înregistrări inexistente**           | Înregistrarea părinte nu există în BeeAX            | Importați mai întâi înregistrările părinte sau verificați greșelile de tastare |
| **Nepotrivire de majuscule/minuscule** | `Acme.com` în fișier, dar `acme.com` în BeeAX       | Asigurați potrivirea exactă a majusculelor/minusculelor                        |
...
<AccordionGroup>
  <Accordion title="Ce se întâmplă dacă înregistrarea mea părinte nu are un domeniu sau un email?">
    Aveți două opțiuni:
    1. Folosiți `id`-ul BeeAX (exportați înregistrările părinte pentru a obține ID-urile lor)
