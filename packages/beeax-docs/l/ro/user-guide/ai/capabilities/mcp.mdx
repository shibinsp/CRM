---
title: Server MCP
description: Conectați asistenți AI la spațiul dvs. de lucru BeeAX folosind Model Context Protocol.
---
...
BeeAX expune un server [MCP](https://modelcontextprotocol.io/) astfel încât asistenții AI — Claude Desktop, Claude Code, Cursor, ChatGPT și alții — să poată citi și scrie datele tale din CRM prin limbaj natural.

Folosește **URL-ul spațiului de lucru** (URL-ul pe care îl folosești pentru a accesa BeeAX) drept punct final MCP. Pe BeeAX Cloud, URL-ul spațiului tău de lucru poate fi `https://{mycompany}.beeax.com` sau un domeniu personalizat. Serverul este disponibil la:

| Mediu                | Punct final MCP                                                                |
| -------------------- | ------------------------------------------------------------------------------ |
| **Cloud**            | `https://{your-workspace-url}/mcp` (de ex. `https://mycompany.beeax.com/mcp`) |
...
Adaugă asta în configurația clientului tău MCP, înlocuind `{your-workspace-url}` cu gazda spațiului tău de lucru (de ex. `mycompany.beeax.com`):
...
1. Va descoperi metadatele OAuth ale BeeAX prin `/.well-known/oauth-protected-resource` și `/.well-known/oauth-authorization-server`
...
Mergi la **Settings > AI > More > MCP Server** în BeeAX. Alege metoda de autentificare (OAuth sau Cheie API), copiază fragmentul JSON (va folosi deja URL-ul spațiului tău de lucru) și lipește-l în fișierul de configurare al clientului tău MCP.
...
Repornește clientul tău MCP (sau reîncarcă configurația). Dacă folosești OAuth, vei fi redirecționat către BeeAX pentru a autoriza accesul. Dacă folosești o cheie API, conexiunea este imediată.
...
După conectare, serverul MCP expune instrumente care oglindesc API-ul BeeAX. Fluxul de lucru recomandat este:
...
Pentru instanțele auto-găzduite, înlocuiește `{your-workspace-url}` cu URL-ul serverului tău. Asigură-te că `SERVER_URL` din mediul tău corespunde URL-ului public al instanței tale BeeAX — acesta este folosit pentru a genera metadatele de descoperire OAuth.
