...
Acest ghid oferă instrucțiuni pas cu pas pentru a instala și configura aplicația BeeAX folosind Docker Compose. Scopul este de a face procesul direct și de a preveni capcanele comune care ar putea deteriora configurarea dvs.
...
Instalați cea mai recentă versiune stabilă a BeeAX cu o singură comandă:
...
Dacă găzduiți beeaxCRM pe propriul computer, deschideți browserul și navigați la [http://localhost:3000](http://localhost:3000).
...
### Expuneți BeeAX pentru Acces Extern

Implicit, BeeAX rulează pe `localhost` la portul `3000`. Pentru a-l accesa printr-un domeniu extern sau adresă IP, trebuie să configurați `SERVER_URL` în fișierul dvs. `.env`.
...
SSL (HTTPS) este necesar pentru funcționarea corespunzătoare a anumitor funcții ale browserului. Deși aceste funcții pot funcționa în timpul dezvoltării locale (deoarece browserele tratează localhost diferit), o configurare SSL corespunzătoare este necesară când găzduiți BeeAX pe un domeniu obișnuit.
...
Recomandăm cu tărie configurarea BeeAX din spatele unui proxy revers cu terminare SSL pentru securitate și funcționalitate optimă.
