...
Dacă aveți deja o instanță BeeAX cu date de producție, **NU** urmați acești pași deoarece vor șterge permanent baza dvs. de date!

În timp ce instalați BeeAX pentru prima dată, este posibil să doriți să schimbați parola implicită a bazei de date.
...
În timpul instalării BeeAX, trebuie să aprovizionați baza de date postgres cu schemele, extensiile și utilizatorii corecți.
...
#### Nu se poate găsi modulul 'beeax-emails' sau declarațiile de tip corespunzătoare.

Trebuie să construiți pachetul `beeax-emails` înainte de a rula inițializarea bazei de date cu `npx nx run beeax-emails:build`

#### Pachetul beeax-x lipsă

Asigurați-vă că rulați yarn în directorul rădăcină și apoi rulați `npx nx server:dev beeax-server`. Dacă acest lucru nu funcționează, încercați să construiți manual pachetul lipsă.
...
#### În timpul rulării `npx nx start` sau `npx nx start beeax-front`, apare o eroare Out of memory

Rulați doar serviciile de care aveți nevoie, în loc de `npx nx start`. De exemplu, dacă lucrați la server, rulați doar `npx nx worker beeax-server`

**Dacă nu funcționează:**
Dacă ați încercat să rulați doar `npx nx run beeax-server:start` pe WSL și nu reușește cu eroarea de memorie de mai jos:
...
**Dacă nu funcționează:**
Investigați care procese consumă cea mai mare parte din RAM-ul mașinii dvs. La BeeAX, am observat că unele extensii VScode consumau multă RAM, așa că le dezactivăm temporar.
...
* Deschideți aplicația BeeAX, navigați la `/settings` și activați comutatorul **Avansat** în colțul din stânga jos al ecranului.
* Creați un nou webhook.
* Lipiți **Your Unique Webhook URL** în câmpul **Endpoint Url** din BeeAX. Setați **Filtrele** pe `Companii` și `Create`.
...
Comentați pluginul checker în `packages/beeax-ui/vite-config.ts`, ca în exemplul de mai jos
...
Dacă rulați BeeAX în spatele unui proxy invers și întâmpinați probleme de conexiune:
...
2. **Verificați Setările Proxy-ului Invers:**
   ...
   * Confirmați că proxy-ul invers transmite corect cererile către serverul BeeAX.
   * Asigurați-vă că headerele precum `X-Forwarded-For` și `X-Forwarded-Proto` sunt setate corect.

3. **Reporniți Serviciile:**

   După ce faceți schimbări, reporniți atât proxy-ul invers, cât și containerele BeeAX.
...
* Suport Comunitar:
  ...
  Contactați [comunitatea BeeAX](https://github.com/beeax/beeax/issues) sau [canalele de suport](https://discord.gg/cx5n4Jzs57) pentru asistență.
