...
Webhook-urile trimit date către sistemele dvs. în timp real atunci când au loc evenimente în BeeAX — nu este necesară interogarea periodică. Folosiți-le pentru a menține sistemele externe sincronizate, a declanșa automatizări sau a trimite alerte.
...
BeeAX trimite webhook-uri pentru aceste tipuri de evenimente:
...
BeeAX semnează fiecare cerere webhook din motive de securitate. Validați semnăturile pentru a vă asigura că cererile sunt autentice.

### Anteturi

| Antet                        | Descriere                  |
| ---------------------------- | -------------------------- |
| `X-BeeAX-Webhook-Signature` | Semnătură HMAC SHA256      |
| `X-BeeAX-Webhook-Timestamp` | Marcaj temporal al cererii |

### Pași de validare

1. Obțineți marcajul temporal din `X-BeeAX-Webhook-Timestamp`
2. Creați șirul: `{timestamp}:{JSON payload}`
3. Calculați HMAC SHA256 folosind secretul webhook-ului dvs.
4. Comparați cu `X-BeeAX-Webhook-Signature`
...
| **Declanșator webhook în fluxul de lucru** | IN       | Primiți date în BeeAX din sisteme externe                                       |
