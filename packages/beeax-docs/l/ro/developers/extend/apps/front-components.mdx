...
Componentele frontale pot fi afișate în două locații în cadrul BeeAX:
...
Pachetul `beeax-sdk` oferă patru componente ajutătoare Command, concepute pentru componente front-end headless. Fiecare componentă execută o acțiune la montare, gestionează erorile afișând o notificare snackbar și demontează automat componenta de interfață la final.

Importă-le din `beeax-sdk/command`:
...
Câmpul `conditionalAvailabilityExpression` vă permite să controlați când este vizibilă o comandă în funcție de contextul paginii curente. Importați variabile tipizate și operatori din `beeax-sdk` pentru a construi expresii:
