...
* Vedeți un exemplu funcțional: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* Puteți genera obiecte noi folosind `yarn beeax add`, care vă ghidează prin denumire, câmpuri și relații.

<Note>
**Câmpurile de bază sunt create automat.** Când definiți un obiect personalizat, BeeAX adaugă automat câmpuri standard
...
Utilizați `defineField()` pentru a adăuga câmpuri la obiecte pe care nu le dețineți — cum ar fi obiectele standard BeeAX (Person, Company etc.). sau obiecte din alte aplicații. Spre deosebire de câmpurile inline din `defineObject()`, câmpurile independente necesită un `objectUniversalIdentifier` pentru a specifica obiectul pe care îl extind:
...
Puncte cheie:
* `objectUniversalIdentifier` identifică obiectul țintă. Pentru obiectele standard, utilizați `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exportați din `beeax-sdk`.
...
Relațiile conectează obiectele între ele. În BeeAX, relațiile sunt întotdeauna bidirecționale — definiți ambele părți, iar fiecare parte o referențiază pe cealaltă.
...
Pentru a crea o relație cu un obiect BeeAX încorporat (Person, Company etc.), utilizați `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Generarea scheletului entităților cu `yarn beeax add`
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Obiect                       | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| Câmp                         | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| Funcție logică               | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Componentă frontend          | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                          | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| Abilitate                    | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| Agent                        | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| Vizualizare                  | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| Element de meniu de navigare | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Machetă de pagină            | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Ce generează scaffolder-ul

Fiecare tip de entitate are propriul său șablon. De exemplu, `yarn beeax add object` solicită:
