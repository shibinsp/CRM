...
title: Publicare
icon: încarcă
description: Distribuie aplicația ta BeeAX în marketplace sau implementeaz-o intern.
---
...
* **Implementează un tarball** — încarcă aplicația direct pe un server BeeAX anume pentru uz intern sau privat.
* **Publică pe npm** — listează aplicația ta în marketplace-ul BeeAX pentru ca orice spațiu de lucru să o poată descoperi și instala.
...
Aceasta compilează sursele TypeScript, transpilează funcțiile de logică și componentele de front-end și scrie totul în `.beeax/output/`. Adaugă `--tarball` pentru a produce și un pachet `.tgz` pentru distribuire manuală sau pentru comanda de publish.

## Implementare pe un server (tarball)

Pentru aplicațiile pe care nu le dorești disponibile public — instrumente proprietare, integrări doar pentru enterprise sau build-uri experimentale — poți implementa un tarball direct pe un server BeeAX.

### Cerințe

Înainte de implementare, ai nevoie de un remote configurat care să indice serverul țintă. Remote-urile stochează local URL-ul serverului și credențialele de autentificare în `~/.beeax/config.json`.
...
2. Rulează `yarn beeax app:publish --private` (sau `yarn beeax app:publish --private --remote production`)
...
Dacă aplicația ta folosește o funcționalitate introdusă într-o anumită versiune de server BeeAX (de exemplu, furnizori OAuth adăugați în v2.3.0), ar trebui să declari versiunea minimă de server necesară aplicației folosind câmpul `engines.beeax` din `package.json`:
...
* Dacă `engines.beeax` este setat și versiunea serverului țintă nu respectă intervalul, implementarea (încărcarea arhivei tarball) sau instalarea este respinsă cu eroarea `SERVER_VERSION_INCOMPATIBLE` și cu un mesaj care indică atât intervalul necesar, cât și versiunea efectivă a serverului.
* Dacă `engines.beeax` nu este setat, aplicația este acceptată pe orice versiune de server (retrocompatibilă cu aplicațiile existente).
* Dacă serverul nu are nicio `APP_VERSION` configurată, verificarea este omisă.

<Note>
Serverul este verificarea autoritativă — validează `engines.beeax` atât la încărcarea arhivei tarball, cât și la instalarea în spațiul de lucru. Dacă implementezi un tarball în afara fluxului standard sau instalezi din marketplace, serverul impune în continuare compatibilitatea.
</Note>

## CI/CD automatizat (fluxuri de lucru preconfigurate)

Aplicațiile generate cu `create-beeax-app` vin, gata de utilizare, cu două fluxuri de lucru GitHub Actions, în `.github/workflows/`. Acestea sunt gata să ruleze imediat ce faci push al repozitoriului pe GitHub — nu este necesară nicio configurare suplimentară pentru CI, iar CD necesită doar un singur secret.
...
2. Pornește o instanță de test BeeAX izolată folosind acțiunea compozită `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (echivalentul din CI al `yarn beeax docker:start --test`).
...
* `TWENTY_VERSION` (variabilă de mediu, implicit `latest`) — fixează versiunea serverului BeeAX folosită în CI editând acest parametru în `ci.yml`.
...
Implementează aplicația pe un server BeeAX configurat la fiecare push pe `main` și, opțional, dintr-un pull request când se aplică eticheta `deploy`.

**Ce face:**

1. Preia head-ul PR-ului (pentru PR-urile etichetate) sau commitul împins.
2. Rulează `beeax/beeax/.github/actions/deploy-beeax-app@main` — echivalentul din CI al `yarn beeax app:publish --private`.
3. Rulează `beeax/beeax/.github/actions/install-beeax-app@main` astfel încât versiunea nou implementată să fie instalată în spațiul de lucru țintă.

**Configurare necesară:**

| Setare                  | Unde                                                       | Scop                                                                                                               |
| ----------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `TWENTY_DEPLOY_URL`     | `env` în `cd.yml` (implicit `http://localhost:3000`)       | Serverul BeeAX la care se face implementarea. Modifică-l la URL-ul real al serverului înainte de prima utilizare. |
...
Ambele fluxuri de lucru fac referire la acțiuni reutilizabile la `@main`, astfel încât actualizările acțiunilor din repo-ul `beeax/beeax` sunt preluate automat. Dacă dorești builduri deterministe, înlocuiește `@main` cu un SHA de commit sau cu un tag de release pe fiecare linie `uses:`.

## Publicarea pe npm

Publicarea pe npm face ca aplicația ta să poată fi descoperită în marketplace-ul BeeAX. Orice spațiu de lucru BeeAX poate răsfoi, instala și actualiza aplicațiile din marketplace direct din interfață.

### Cerințe

* Un cont [npm](https://www.npmjs.com)
* Cuvântul cheie `beeax-app` din array-ul `keywords` al fișierului `package.json` (adaugă-l manual — nu este inclus în mod implicit în șablonul `create-beeax-app`)
...
Serverul BeeAX sincronizează catalogul marketplace-ului din registrul npm **la fiecare oră**.
...
<Note>
Dacă aplicația ta nu definește un `aboutDescription` în `defineApplication()`, piața va folosi automat fișierul `README.md` al pachetului tău de pe npm drept conținut pentru pagina Despre. Acest lucru înseamnă că poți menține un singur README atât pentru npm, cât și pentru piața BeeAX. Dacă vrei o descriere diferită în piață, setează explicit `aboutDescription`.
</Note>
...
Pentru alte sisteme CI (GitLab CI, CircleCI etc.), se aplică aceleași trei comenzi: `yarn install`, `yarn beeax dev:build`, apoi `npm publish` din `.beeax/output`.
...
Mergi la pagina **Setări > Aplicații** din BeeAX, unde pot fi parcurse și instalate atât aplicațiile din marketplace, cât și cele implementate prin tarball.
...
Pentru a instala o versiune mai nouă, fă mai întâi deploy sau public-o, apoi rulează din nou `yarn beeax app:install`.
</Note>
