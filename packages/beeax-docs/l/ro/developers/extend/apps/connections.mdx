...
* Folosește `serverVariables` (nu `applicationVariables`) — acreditările OAuth sunt la nivel de server și există o singură aplicație OAuth pentru fiecare server BeeAX.
...
4. Deschide aplicația instalată în BeeAX ca administrator de server → setează valorile pe `serverVariables` corespunzătoare.
