---
title: Arhitectură
description: Cum funcționează aplicațiile BeeAX — izolarea (sandboxing), ciclul de viață și elementele de bază.
icon: sitemap
---

Aplicațiile BeeAX sunt pachete TypeScript care vă extind spațiul de lucru cu obiecte personalizate, logică, componente UI și capabilități AI. Acestea rulează pe platforma BeeAX, cu izolare completă (sandboxing) și controale de permisiuni.

## Cum funcționează aplicațiile

O aplicație este o colecție de **entități** declarate folosind funcțiile `defineEntity()` din pachetul `beeax-sdk`. SDK-ul detectează aceste declarații prin analiză AST în timpul construirii și produce un **manifest** — o descriere completă a ceea ce aplicația dvs. adaugă unui spațiu de lucru.
...
| **Componentă front-end**         | UI React izolat în pagina BeeAX                            | [Componente front-end](/l/ro/developers/extend/apps/front-components) |
...
* **Componentele front-end** rulează în Web Workers folosind Remote DOM — izolate de pagina principală, dar randând elemente DOM native (nu iframes). Acestea comunică cu BeeAX printr-un API al gazdei bazat pe transmiterea de mesaje.
...
* **`yarn beeax dev`** — monitorizează fișierele sursă și sincronizează în timp real modificările către un server BeeAX conectat. Clientul API tipizat este regenerat automat atunci când schema se schimbă.
* **`yarn beeax build`** — compilează TypeScript, împachetează funcțiile logice și componentele front-end cu esbuild și produce un manifest.
...
<CardGroup cols={2}>
  ...
  <Card title="Componente front-end" icon="window-maximize" href="/l/ro/developers/extend/apps/front-components">
    Componente React izolate în interfața BeeAX.
  </Card>
