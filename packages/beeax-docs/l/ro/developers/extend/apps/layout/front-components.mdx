...
description: Construiți componente React care se afișează în interfața BeeAX, cu izolare în sandbox.
icon: window-maximize
---

Componentele front-end sunt componente React care se afișează direct în interfața BeeAX. Rulează într-un **Web Worker** izolat folosind Remote DOM — codul este izolat (sandboxed), dar se redă nativ în pagină, nu într-un iframe.

## Unde pot fi utilizate componentele front-end

Componentele front-end pot fi afișate în două locații în cadrul BeeAX:
...
După sincronizarea cu `yarn beeax dev` (sau prin rularea comenzii `yarn beeax dev --once` o singură dată), acțiunea rapidă apare în colțul din dreapta sus al paginii:
...
Deoarece componenta returnează `null`, BeeAX omite redarea unui container pentru ea — nu apare spațiu gol în layout. Componenta are în continuare acces la toate hook-urile și la API-ul de comunicare cu gazda.

## Componentele Command din SDK

Pachetul `beeax-sdk` oferă patru componente ajutătoare Command, concepute pentru componente front-end headless. Fiecare componentă execută o acțiune la montare, gestionează erorile afișând o notificare snackbar și demontează automat componenta front-end la final.

Importați-le din `beeax-sdk/command`:
...
O funcție logică declarată cu `httpRouteTriggerSettings` este accesibilă prin HTTP la ruta sa. BeeAX injectează în worker URL-ul de bază de la care sunt deservite funcțiile tale ca `TWENTY_FUNCTIONS_URL`, împreună cu `TWENTY_APP_ACCESS_TOKEN` care autentifică apelul. Nu există încă un client SDK dedicat pentru apelarea propriilor funcții, așa că apelează-le cu un simplu `fetch`:

> **În BeeAX Cloud, funcțiile logice declanșate prin HTTP sunt deservite pe un domeniu dedicat pentru fiecare spațiu de lucru** la `https://\<your-workspace-subdomain>.beeax.com\<path>` — acesta este exact URL-ul la care indică `TWENTY_FUNCTIONS_URL`. Pentru apelanții externi, copiază URL-ul exact din setările **HTTP trigger** ale funcției sau din fila **Settings** a aplicației.
...
Calea adăugată la `TWENTY_FUNCTIONS_URL` este `httpRouteTriggerSettings.path` al funcției logice. Păstrează `isAuthRequired: true`; `TWENTY_APP_ACCESS_TOKEN` pe care BeeAX îl generează pentru componenta ta autentifică cererea:
...
### Apelarea API-ului REST BeeAX

Pentru a citi sau scrie înregistrări BeeAX dintr-un front component, folosește `RestApiClient` din `beeax-client-sdk/rest`. Face parte din aceeași familie de clienți ca `CoreApiClient` și `MetadataApiClient`, dar vizează BeeAX REST API (`/rest/...`) în locul API-ului GraphQL, citindu-și URL-ul de bază din `TWENTY_API_URL`.
...
| `TWENTY_API_URL`          | URL-ul de bază al API-ului de bază BeeAX                                             |
| `TWENTY_APP_ACCESS_TOKEN` | Token cu durată scurtă, limitat la rolul aplicației dvs.                              |

## API-ul de comunicare cu gazda

Componentele front-end pot declanșa navigare, ferestre modale și notificări folosind funcții din `beeax-sdk`:
...
* **Componente BeeAX UI** — import din `beeax-sdk/ui` (Button, Tag, Status, Chip, Avatar și altele)
