...
description: Integrează-ți aplicația în interfața BeeAX — intrări în bara laterală, vizualizări salvate, file în pagina de detalii a înregistrărilor și componente React izolate.
icon: table-columns
---

**Stratul de layout** al unei aplicații BeeAX reprezintă tot ceea ce vede utilizatorul: unde apare aplicația în bara laterală, ce vizualizări de listă oferă, cum sunt aranjate paginile de detalii ale înregistrărilor și ce componente React personalizate sunt afișate în interiorul acelor pagini.
...
<CardGroup cols={2}>
  ...
  <Card title="Componente front-end" icon="window-maximize" href="/l/ro/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — componente React izolate care rulează în interiorul BeeAX.
  </Card>
...
Componentele front rulează în interiorul unui Web Worker izolat folosind Remote DOM — acestea sunt redate *nativ* în pagină (nu într-un iframe), dar nu pot accesa direct pagina gazdă sau DOM-ul. Comunicarea cu BeeAX se face printr-un API al gazdei bazat pe transmiterea de mesaje.
