...
* **Nu se execută în modul dev**: când o aplicație este înregistrată local (prin `yarn beeax dev`), serverul sare complet peste fluxul de instalare și sincronizează fișierele direct prin watcher-ul CLI — astfel încât post-install nu rulează niciodată în modul dev, indiferent de `shouldRunSynchronously`. Folosiți `yarn beeax dev:function:exec --postInstall` pentru a-l declanșa manual într-un workspace care rulează.
...
* **Nu se execută în modul dev**: la fel ca post-install — fluxul de instalare este sărit complet pentru aplicațiile înregistrate local, astfel încât pre-install nu rulează niciodată sub `yarn beeax dev`. Folosiți `yarn beeax dev:function:exec --preInstall` pentru a-l declanșa manual.
