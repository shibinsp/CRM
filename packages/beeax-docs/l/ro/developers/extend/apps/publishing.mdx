...
* **Implementați o arhivă tar** — încărcați aplicația direct pe un server BeeAX anume pentru uz intern sau privat.
...
Aceasta compilează sursele TypeScript, transpilează funcțiile de logică și componentele de front-end și scrie totul în `.beeax/output/`. Adaugă `--tarball` pentru a produce și un pachet `.tgz` pentru distribuire manuală sau pentru comanda de deploy.
...
2. Rulează `yarn beeax deploy` (sau `yarn beeax deploy --remote production`)
...
2. Pornește o instanță de test BeeAX izolată folosind acțiunea compozită `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (echivalentul din CI al `yarn beeax server start --test`).
...
2. Rulează `beeax/beeax/.github/actions/deploy-beeax-app@main` — echivalentul din CI al `yarn beeax deploy`.
...
Pentru alte sisteme CI (GitLab CI, CircleCI etc.), se aplică aceleași trei comenzi: `yarn install`, `yarn beeax build`, apoi `npm publish` din `.beeax/output`.
...
Pentru a instala o versiune mai nouă, fă mai întâi deploy sau public-o, apoi rulează din nou `yarn beeax install`.
</Note>
