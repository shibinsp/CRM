...
icon: laptop-code
description: Ghidul pentru contribuitori (sau dezvoltatori curioși) care doresc să ruleze BeeAX local.
---
...
Înainte de a instala și utiliza BeeAX, asigurați-vă că instalați următoarele pe computerul dvs.:
...
BeeAX necesită un cache Redis pentru a oferi cea mai bună performanță.
...
## Pasul 8: Utilizați BeeAX

**Front-end**

Frontend-ul BeeAX va rula la [http://localhost:3001](http://localhost:3001).
Vă puteți loga folosind contul demo implicit: `tim@apple.dev` (parolă: `tim@apple.dev`)

**Back-end**

* Serverul BeeAX va fi operativ la [http://localhost:3000](http://localhost:3000)
