...
```bash
npx nx run beeax-front:lint # utilizați --fix pentru a corecta erorile de lint
```
...
```bash
npx nx run beeax-front:test # rulați teste jest
npx nx run beeax-front:storybook:serve:dev # rulați storybook
npx nx run beeax-front:storybook:test # rulați testele # (are nevoie de yarn storybook:serve:dev pentru a rula)
npx nx run beeax-front:storybook:coverage # (are nevoie de yarn storybook:serve:dev pentru a rula)
```
