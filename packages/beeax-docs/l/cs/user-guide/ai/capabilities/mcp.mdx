---
title: MCP Server
description: Připojte asistenty AI ke svému pracovnímu prostoru BeeAX pomocí protokolu Model Context Protocol.
---
...
BeeAX zpřístupňuje server [MCP](https://modelcontextprotocol.io/), aby AI asistenti — Claude Desktop, Claude Code, Cursor, ChatGPT a další — mohli číst a zapisovat data vašeho CRM pomocí přirozeného jazyka.

Použijte svou **adresu URL pracovní oblasti** (adresu URL, kterou používáte k přístupu k BeeAX) jako koncový bod MCP. V BeeAX Cloud může vaše adresa URL pracovní oblasti být `https://{mycompany}.beeax.com` nebo vlastní doména. Server je dostupný na:

| Prostředí           | Koncový bod MCP                                                               |
| ------------------- | ----------------------------------------------------------------------------- |
| **Cloud**           | `https://{your-workspace-url}/mcp` (např. `https://mycompany.beeax.com/mcp`) |
...
Přidejte toto do konfigurace svého klienta MCP a nahraďte `{your-workspace-url}` hostitelem vaší pracovní oblasti (např. `mycompany.beeax.com`):
...
1. Získá metadata OAuth služby BeeAX pomocí `/.well-known/oauth-protected-resource` a `/.well-known/oauth-authorization-server`
...
V BeeAX přejděte do **Settings > AI > More > MCP Server**. Vyberte metodu ověření (OAuth nebo klíč API), zkopírujte úryvek JSON (už bude používat adresu URL vaší pracovní oblasti) a vložte jej do konfiguračního souboru klienta MCP.
...
Restartujte svého klienta MCP (nebo znovu načtěte konfiguraci). Pokud používáte OAuth, budete přesměrováni do BeeAX k autorizaci přístupu. Pokud používáte klíč API, připojení je okamžité.
...
Po připojení server MCP zpřístupní nástroje, které odpovídají rozhraní BeeAX API. Doporučený pracovní postup je:
...
U samostatně hostovaných instancí nahraďte `{your-workspace-url}` adresou URL svého serveru. Ujistěte se, že `SERVER_URL` ve vašem prostředí odpovídá veřejné adrese URL vaší instance BeeAX — tato hodnota se používá k vygenerování metadat pro zjišťování OAuth.
