...
Když nahrajete soubor, BeeAX analyzuje vaše sloupce a pokusí se je spárovat s existujícími poli.

### Automatické mapování

BeeAX se snaží spárovat sloupce na základě:

* Názvů záhlaví sloupců (přesné nebo podobné shody)
* Detekce typů dat (datumů, čísel, e‑mailů)
* Běžné vzory polí

<Note>**Rychlý tip:** Exportujte několik řádků z objektu, který chcete importovat. Exportovaný soubor bude mít přesně takové názvy sloupců, jaké BeeAX očekává, což při importu usnadní automatické mapování.</Note>

### Možnosti ručního mapování

U každého sloupce můžete:

* **Namapovat na pole**: Vyberte odpovídající pole BeeAX z rozbalovací nabídky
...
Zadání `id` při importu je volitelné. Pokud není uvedeno, BeeAX jej automaticky vygeneruje.
