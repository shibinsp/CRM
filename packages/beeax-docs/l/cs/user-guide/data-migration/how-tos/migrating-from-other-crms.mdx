...
description: Podrobný průvodce krok za krokem pro migraci vašich dat z libovolného CRM do BeeAX.
---

## Přehled

Tento průvodce vás provede migrací vašich dat z libovolného CRM do BeeAX. Proces zahrnuje audit vašich dat, přípravu vašeho pracovního prostoru BeeAX, export z vašeho současného systému a import do BeeAX.
...
Vytvořte mapovací dokument mezi vaším současným CRM a BeeAX:
...
## Krok 3: Nastavte svůj pracovní prostor BeeAX

Před importem dat připravte svůj pracovní prostor BeeAX:
...
Pokud vaše data obsahují odkazy na uživatele (vlastník účtu, přiřazený uživatel apod.), tito uživatelé musí v BeeAX existovat před importem. Jinak tyto relace nelze namapovat.
</Warning>
...
Otevřete každý exportovaný soubor CSV v tabulkovém editoru a připravte jej pro BeeAX.

### Odstraňte duplicity

1. Seřaďte podle jedinečného pole (e-mail u lidí, doména u společností)
2. Odstraňte nebo sloučte duplicitní řádky
3. Ověřte, že v BeeAX již neexistují žádné duplicity
...
Použijte `https://domain.com` (ne `domain.com` ani `www.domain.com`). To odpovídá formátu BeeAX a předchází duplicitám, když připojíte synchronizaci e-mailu/kalendáře.
</Warning>
...
2. Použijte stejné e-mailové adresy, které uživatelé použili při připojení do vašeho pracovního prostoru BeeAX
...
* BeeAX začne synchronizovat e-maily k záznamům kontaktů
...
<AccordionGroup>
  <Accordion title="Mohu migrovat pracovní postupy automaticky?">
    V tuto chvíli ne. Pracovní postupy je nutné v BeeAX znovu vytvořit ručně.
  </Accordion>
...
Kontaktujte [contact@beeax.com](mailto:contact@beeax.com) nebo si prohlédněte naše [Implementační služby](/l/cs/user-guide/getting-started/capabilities/implementation-services).
