...
Nadřazený („jedna“ strana) musí v BeeAX existovat, než na něj budete moci odkazovat.
...
1. Exportujte nadřazené záznamy z BeeAX
...
Použijte `https://domain.com` (ne pouze `domain.com`). To odpovídá způsobu, jak BeeAX ukládá domény společností, a předchází chybám při párování.
</Warning>
...
BeeAX automaticky propojí každý podřízený záznam s odpovídajícím nadřazeným.
...
| **Neexistující záznamy**     | Nadřazený záznam v BeeAX neexistuje            | Nejprve importujte nadřazené záznamy nebo zkontrolujte překlepy |
| **Rozdílná velikost písmen** | `Acme.com` v souboru, ale v BeeAX `acme.com`   | Zajistěte přesnou shodu velikosti písmen                        |
...
<AccordionGroup>
  <Accordion title="Co když můj nadřazený záznam nemá doménu ani e-mail?">
    Máte dvě možnosti:
    1. Použijte BeeAX `id` (exportujte nadřazené záznamy, abyste získali jejich ID)
