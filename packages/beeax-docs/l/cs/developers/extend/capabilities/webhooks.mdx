...
Webhooky posílají data do vašich systémů v reálném čase, když v BeeAX dojde k událostem — bez potřeby průběžného dotazování. Použijte je k udržování externích systémů v synchronizaci, spouštění automatizací nebo zasílání upozornění.
...
BeeAX odesílá webhooky pro tyto typy událostí:
...
BeeAX kvůli bezpečnosti podepisuje každý požadavek webhooku. Ověřujte podpisy, abyste zajistili, že požadavky jsou autentické.

### Záhlaví

| Hlavička                     | Popis                    |
| ---------------------------- | ------------------------ |
| `X-BeeAX-Webhook-Signature` | Podpis HMAC SHA256       |
| `X-BeeAX-Webhook-Timestamp` | Časové razítko požadavku |

### Kroky ověření

1. Získejte časové razítko z `X-BeeAX-Webhook-Timestamp`
2. Vytvořte řetězec: `{timestamp}:{JSON payload}`
3. Vypočítejte HMAC SHA256 pomocí vašeho tajného klíče webhooku
4. Porovnejte s `X-BeeAX-Webhook-Signature`
...
| **Spouštěč webhooku v pracovním postupu** | IN   | Přijímejte data do BeeAX z externích systémů                   |
