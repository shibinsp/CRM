...
Importujte typ `RoutePayload` z `beeax-sdk`:
...
Logické funkce lze zpřístupnit jako **nástroje** pro agenty AI a pracovní postupy. Když je funkce označena jako nástroj, stane se dohledatelnou funkcemi AI produktu BeeAX a lze ji použít v automatizacích pracovních postupů.
...
* Proměnné prostředí `APPLICATION_ID`, `APP_ACCESS_TOKEN` a `API_URL` jsou dostupné uvnitř obslužné funkce (stejně jako u jakékoli jiné logické funkce), takže můžete volat BeeAX API s aplikačním přístupovým tokenem omezeným na vaši aplikaci.
...
* **Nespouští se v režimu dev**: když je aplikace registrována lokálně (pomocí `yarn beeax dev`), server zcela přeskočí instalační tok a synchronizuje soubory přímo prostřednictvím sledovače CLI — takže se post-install v režimu dev nikdy nespustí bez ohledu na `shouldRunSynchronously`. Použijte `yarn beeax exec --postInstall` k ručnímu spuštění nad běžícím pracovním prostorem.
...
* **Nespouští se v režimu dev**: stejně jako u post-install — u lokálně registrovaných aplikací je instalační tok zcela přeskočen, takže se pre-install pod `yarn beeax dev` nikdy nespustí. Použijte `yarn beeax exec --preInstall` k ručnímu spuštění.
...
`CoreApiClient` je hlavní klient pro dotazování a mutace dat pracovního prostoru. Generuje se z vašeho schématu pracovního prostoru během `yarn beeax dev` nebo `yarn beeax build`, takže je plně typovaný tak, aby odpovídal vašim objektům a polím.
...
<Note>
**CoreApiClient je generován při vývoji/sestavení.** Pokud jej použijete bez předchozího spuštění `yarn beeax dev` nebo `yarn beeax build`, vyvolá chybu. Generování probíhá automaticky — CLI prozkoumá GraphQL schéma vašeho pracovního prostoru a vygeneruje typovaného klienta pomocí `@genql/cli`.
</Note>
