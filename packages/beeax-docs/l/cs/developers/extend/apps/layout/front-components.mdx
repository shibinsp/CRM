...
description: Vytvářejte komponenty Reactu, které se vykreslují uvnitř uživatelského rozhraní BeeAX se sandboxovou izolací.
icon: window-maximize
---

Frontendové komponenty jsou React komponenty, které se vykreslují přímo v uživatelském rozhraní BeeAX. Běží v **izolovaném Web Workeru** s využitím Remote DOM — váš kód je sandboxovaný, ale vykresluje se nativně na stránce, nikoli v iframu.

## Kde lze použít frontendové komponenty

Frontendové komponenty se mohou vykreslovat na dvou místech v rámci BeeAX:
...
Po synchronizaci pomocí `yarn beeax dev` (nebo po jednorázovém spuštění `yarn beeax dev --once`) se rychlá akce zobrazí v pravém horním rohu stránky:
...
Protože komponenta vrací `null`, BeeAX přeskočí vykreslení kontejneru — v rozvržení se neobjeví žádné prázdné místo. Komponenta má však stále přístup ke všem hookům a API komunikace s hostitelem.

## Komponenty SDK Command

Balíček `beeax-sdk` poskytuje čtyři pomocné komponenty Command navržené pro headless front-endové komponenty. Každá komponenta při připojení provede akci, chyby zpracuje zobrazením oznámení ve snackbaru a po dokončení automaticky odpojí front-endovou komponentu.

Importujte je z `beeax-sdk/command`:
...
Logická funkce deklarovaná pomocí `httpRouteTriggerSettings` je přes HTTP dostupná na své cestě (route path). BeeAX vloží do workeru základní URL, ze které jsou vaše funkce poskytovány, jako `TWENTY_FUNCTIONS_URL` spolu s `TWENTY_APP_ACCESS_TOKEN`, který volání autentizuje. Zatím neexistuje žádný specializovaný klient SDK pro volání vlastních funkcí, takže je volejte pomocí prostého `fetch`:

> **V BeeAX Cloud jsou logické funkce spouštěné přes HTTP poskytovány na vyhrazené doméně pro každý workspace** na adrese `https://\<your-workspace-subdomain>.beeax.com\<path>` — právě na tuto adresu `TWENTY_FUNCTIONS_URL` směřuje. Pro externí volající zkopírujte přesnou URL z nastavení funkce **HTTP trigger** nebo z karty **Settings** aplikace.
...
Cesta připojená k `TWENTY_FUNCTIONS_URL` je `httpRouteTriggerSettings.path` logické funkce. Ponechte `isAuthRequired: true`; `TWENTY_APP_ACCESS_TOKEN`, který BeeAX vygeneruje pro vaši komponentu, požadavek autentizuje:
...
### Volání BeeAX REST API

Pro čtení nebo zápis záznamů BeeAX z front komponenty použijte `RestApiClient` z `beeax-client-sdk/rest`. Patří do stejné rodiny klientů jako `CoreApiClient` a `MetadataApiClient`, ale cílí na BeeAX REST API (`/rest/...`) místo na GraphQL API a svou základní URL čte z `TWENTY_API_URL`.
...
| `TWENTY_API_URL`          | Základní URL BeeAX core API                                                                   |
| `TWENTY_APP_ACCESS_TOKEN` | Krátkodobý token s oprávněními omezenými na roli vaší aplikace                                 |

## API komunikace s hostitelem

Frontendové komponenty mohou pomocí funkcí z `beeax-sdk` vyvolávat navigaci, modály a oznámení:
...
* **Komponenty BeeAX UI** — import z `beeax-sdk/ui` (Button, Tag, Status, Chip, Avatar a další)
