---
title: Datový model
description: Definujte objekty, pole, role a metadata aplikace pomocí BeeAX SDK.
icon: database
---

Balíček `beeax-sdk` poskytuje funkce `defineEntity` pro deklaraci datového modelu vaší aplikace. Abyste umožnili SDK detekovat vaše entity, musíte použít `export default defineEntity({...})`. Tyto funkce validují vaši konfiguraci v době sestavení a poskytují automatické doplňování v IDE a typovou bezpečnost.
...
* Podívejte se na funkční příklad: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* Nové objekty můžete vygenerovat pomocí `yarn beeax add`, který vás provede pojmenováním, poli a vztahy.

<Note>
**Základní pole jsou vytvořena automaticky.** Když definujete vlastní objekt, BeeAX automaticky přidá standardní pole
...
Pomocí `defineField()` přidejte pole k objektům, které nevlastníte — například ke standardním objektům BeeAX (Person, Company atd.). nebo k objektům z jiných aplikací. Na rozdíl od inline polí v `defineObject()` vyžadují samostatná pole `objectUniversalIdentifier` k určení, který objekt rozšiřují:
...
Hlavní body:
* `objectUniversalIdentifier` identifikuje cílový objekt. Pro standardní objekty použijte `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exportovaný z `beeax-sdk`.
...
Relace propojují objekty. Ve BeeAX jsou relace vždy obousměrné — definujete obě strany a každá strana odkazuje na tu druhou.
...
Chcete-li vytvořit relaci s vestavěným objektem BeeAX (Person, Company atd.), použijte `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Generování entit pomocí `yarn beeax add`
...
| ------------------------- | ------------------------------------ | ------------------------------------------------------- |
| Objekt                    | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| Pole                      | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| Logická funkce            | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Frontendová komponenta    | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Role                      | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| Dovednost                 | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| Agent                     | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| Pohled                    | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| Položka navigační nabídky | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Rozvržení stránky         | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Co generátor vytváří

Každý typ entity má vlastní šablonu. Například `yarn beeax add object` se zeptá na:
