...
title: Publikování
icon: nahrát
description: Distribuujte svou aplikaci BeeAX do Marketplace nebo ji nasaďte interně.
---
...
* **Nasaďte tarball** — nahrajte svou aplikaci přímo na konkrétní server BeeAX pro interní nebo soukromé použití.
* **Publish to npm** — uveďte svou aplikaci v Marketplace BeeAX, aby ji mohl kterýkoli pracovní prostor objevit a nainstalovat.
...
Tím se zkompilují zdrojové soubory TypeScriptu, transpilují logické funkce a frontendové komponenty a vše se zapíše do `.beeax/output/`. Přidejte `--tarball`, abyste také vytvořili balíček `.tgz` pro ruční distribuci nebo příkaz publish.

## Nasazení na server (tarball)

U aplikací, které nechcete zpřístupnit veřejně — proprietární nástroje, integrace pouze pro enterprise nebo experimentální buildy — můžete nasadit tarball přímo na server BeeAX.

### Předpoklady

Před nasazením potřebujete nakonfigurovaný vzdálený cíl směřující na cílový server. Vzdálené cíle ukládají adresu URL serveru a přihlašovací údaje lokálně v `~/.beeax/config.json`.
...
2. Spusťte `yarn beeax app:publish --private` (nebo `yarn beeax app:publish --private --remote production`)
...
Pokud vaše aplikace používá funkci zavedenou v konkrétní verzi serveru BeeAX (například poskytovatelé OAuth přidaní ve verzi 2.3.0), měli byste deklarovat minimální verzi serveru, kterou vaše aplikace vyžaduje, pomocí pole `engines.beeax` v `package.json`:
...
* Pokud je `engines.beeax` nastaveno a verze cílového serveru nevyhovuje rozsahu, nasazení (nahrání tarballu) nebo instalace je odmítnuto chybou `SERVER_VERSION_INCOMPATIBLE` a zprávou, která uvádí jak požadovaný rozsah, tak skutečnou verzi serveru.
* Pokud `engines.beeax` **není nastaveno**, aplikace je přijata na jakékoli verzi serveru (zpětně kompatibilní se stávajícími aplikacemi).
* Pokud server nemá nakonfigurované `APP_VERSION`, kontrola se přeskočí.

<Note>
Server je rozhodující autoritou — ověřuje `engines.beeax` jak při nahrání tarballu, tak při instalaci do pracovního prostoru. Pokud nasazujete tarball mimo standardní proces nebo instalujete z marketplace, server přesto vynucuje kompatibilitu.
</Note>

## Automatizované CI/CD (předpřipravené workflowy)

Aplikace vygenerované pomocí `create-beeax-app` jsou hned připravené se dvěma workflowy GitHub Actions ve složce `.github/workflows/`. Jsou připravené ke spuštění hned, jakmile repozitář pushnete na GitHub — pro CI není potřeba žádné další nastavení a CD vyžaduje pouze jeden secret.
...
2. Spustí izolovanou testovací instanci BeeAX pomocí složené akce `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (ekvivalent v CI k `yarn beeax docker:start --test`).
...
* `TWENTY_VERSION` (env, výchozí hodnota `latest`) — uzamkněte v CI používanou verzi serveru BeeAX úpravou této hodnoty v `ci.yml`.
...
Nasazuje vaši aplikaci na nakonfigurovaný server BeeAX při každém pushi do `main` a volitelně také z pull requestu, pokud je přidán štítek `deploy`.

**K čemu slouží:**

1. Provede checkout headu PR (u označených PR) nebo pushnutého commitu.
2. Spustí `beeax/beeax/.github/actions/deploy-beeax-app@main` — ekvivalent v CI k `yarn beeax app:publish --private`.
3. Spustí `beeax/beeax/.github/actions/install-beeax-app@main`, aby se nově nasazená verze nainstalovala do cílového pracovního prostoru.

**Požadovaná konfigurace:**

| Nastavení               | Kde                                                             | Účel                                                                                                 |
| ----------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` v `cd.yml` (výchozí `http://localhost:3000`)              | Server BeeAX, na který se nasazuje. Před prvním použitím to změňte na skutečnou URL vašeho serveru. |
...
Obě workflowy odkazují na znovupoužitelné akce na `@main`, takže aktualizace akcí v repozitáři `beeax/beeax` se přeberou automaticky. Pokud chcete deterministická sestavení, nahraďte `@main` v každém řádku `uses:` za commit SHA nebo tag vydání.

## Publikování na npm

Publikování na npm zajistí, že bude vaše aplikace dohledatelná v Marketplace BeeAX. Jakýkoli pracovní prostor BeeAX může procházet, instalovat a aktualizovat aplikace z Marketplace přímo z UI.

### Požadavky

* Účet na [npm](https://www.npmjs.com)
* Klíčové slovo `beeax-app` ve vašem poli `keywords` v souboru `package.json` (přidejte ho ručně — ve výchozím nastavení není zahrnuto v šabloně `create-beeax-app`)
...
Server BeeAX synchronizuje svůj katalog tržiště z registru npm **každou hodinu**.
...
<Note>
Pokud vaše aplikace nedefinuje `aboutDescription` v `defineApplication()`, tržiště automaticky použije soubor `README.md` vašeho balíčku z npm jako obsah stránky O aplikaci. To znamená, že můžete spravovat jediný soubor README jak pro npm, tak pro tržiště BeeAX. Pokud chcete v tržišti jiný popis, explicitně nastavte `aboutDescription`.
</Note>
...
Pro jiné systémy CI (GitLab CI, CircleCI atd.) platí stejné tři příkazy: `yarn install`, `yarn beeax dev:build` a poté `npm publish` z `.beeax/output`.
...
Přejděte na stránku **Nastavení > Aplikace** v BeeAX, kde lze procházet a instalovat jak aplikace z tržiště, tak aplikace nasazené jako tarball.
...
K instalaci novější verze ji nejprve nasaďte nebo publikujte, poté znovu spusťte `yarn beeax app:install`.
</Note>
