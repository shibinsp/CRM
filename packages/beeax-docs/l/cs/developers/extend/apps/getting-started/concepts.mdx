---
title: Pojmy
description: Jak aplikace BeeAX fungují — model entit, sandboxing a životní cyklus instalace.
icon: sitemap
---

Aplikace BeeAX jsou balíčky TypeScriptu, které rozšiřují váš pracovní prostor o vlastní objekty, logiku, komponenty UI a funkce AI. Běží na platformě BeeAX s plnou izolací (sandboxingem) a řízením oprávnění.

## Jak aplikace fungují

Aplikace je kolekce **entit** deklarovaných pomocí funkcí `defineEntity()` z balíčku `beeax-sdk`. SDK tyto deklarace detekuje pomocí analýzy AST při sestavení a vytváří **manifest** — úplný popis toho, co vaše aplikace přidává do pracovního prostoru. Tyto funkce validují vaši konfiguraci v době sestavení a poskytují automatické doplňování v IDE a typovou bezpečnost.
...
| **Frontendová komponenta**    | Izolované React UI uvnitř BeeAX                                | [Frontendové komponenty](/l/cs/developers/extend/apps/layout/front-components)     |
...
* **Frontendové komponenty** běží ve Web Workerech s využitím Remote DOM — jsou oddělené od hlavní stránky, ale vykreslují nativní prvky DOM (nikoli iframy). Komunikují s BeeAX prostřednictvím hostitelského API pro předávání zpráv.
...
* **`yarn beeax dev`** — sleduje vaše zdrojové soubory a průběžně synchronizuje změny s připojeným serverem BeeAX. Typovaný klient API se při změně schématu automaticky znovu vygeneruje.
* **`yarn beeax dev:build`** — zkompiluje TypeScript, zabalí logické funkce a frontendové komponenty pomocí esbuild a vytvoří manifest.
