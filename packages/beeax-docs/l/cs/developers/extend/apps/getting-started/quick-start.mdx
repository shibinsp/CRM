...
* **Docker** — [Stáhnout](https://www.docker.com/products/docker-desktop/). Nutné pro spuštění lokálního serveru BeeAX. Přeskočte, pokud už máte BeeAX spuštěné jinde.

Vytváření aplikace BeeAX má tři fáze. Generátor kostry je spojuje do jediného příkazu pro ideální scénář, ale každá fáze je samostatný koncept — když se něco nepovede, znalost aktuální fáze napoví, co opravit.

| Fáze                   | Co děláte                                                | Nástroj                       | Výsledek                                     |
| ---------------------- | -------------------------------------------------------- | ----------------------------- | -------------------------------------------- |
| **1. Vytvořit kostru** | Vygenerovat zdrojový kód aplikace                        | `npx create-beeax-app`       | Projekt v TypeScriptu na disku               |
| **2. Spustit server**  | Spustit server BeeAX, do kterého se bude synchronizovat | Docker + `yarn beeax server` | Běžící instance BeeAX                       |
| **3. Synchronizovat**  | Živě synchronizovat kód na server                        | `yarn beeax dev`             | Vaše změny se objeví v uživatelském rozhraní |
...
Budete vyzváni k zadání názvu a popisu — výchozí hodnoty potvrdíte klávesou **Enter**. Tím se v `my-beeax-app/` vytvoří projekt v TypeScriptu se startovacím `application-config.ts`, výchozí rolí, CI workflow a integračním testem.

**Po této fázi:** máte na svém počítači zdrojový kód aplikace. Zatím neběží — to je předmětem Fáze 2.

---

## Fáze 2 — Spusťte lokální server BeeAX

Vaše aplikace potřebuje server BeeAX, do kterého se bude synchronizovat. Server je plnohodnotná instance BeeAX — UI, GraphQL API, PostgreSQL — běžící lokálně v Dockeru. Váš lokální kód nahrává své definice na tento server, díky čemuž se objeví v UI.

Generátor kostry nabídne, že vám jej spustí:

> **Chcete nastavit lokální instanci BeeAX?**

* **Ano (doporučeno)** — stáhne Docker image `beeaxcrm/beeax-app-dev` a spustí jej na portu `2020`. Nejprve se ujistěte, že Docker běží.
* **Ne** — zvolte, pokud už máte server BeeAX, ke kterému se chcete připojit. Můžete jej propojit později pomocí `yarn beeax remote:add`.
...
**Po této fázi:** máte spuštěný server BeeAX na [http://localhost:2020](http://localhost:2020) a vaše CLI má oprávnění se k němu synchronizovat.

<Note>
Pokud Docker není nainstalovaný nebo neběží, generátor kostry vám sdělí správný příkaz pro spuštění ve vašem operačním systému. Jakmile Docker poběží, můžete pokračovat pomocí `yarn beeax docker:start` — není potřeba znovu vytvářet kostru.
</Note>
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/app-in-ui-1.png" alt="Seznam Your Apps se zobrazenou aplikací My beeax app" />
</div>

Klikněte na **My beeax app** a zobrazí se jeho **registrace aplikace** — záznam na úrovni serveru, který popisuje vaši aplikaci (název, identifikátor, přihlašovací údaje OAuth, zdroj). Jedna registrace může být nainstalována ve více pracovních prostorech na stejném serveru.
...
| ---------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| `yarn beeax dev`                  | Sleduje změny a při každé změně znovu synchronizuje. Běží, dokud jej nezastavíte.  | Interaktivní lokální vývoj.                                              |
| `yarn beeax dev --once`           | Jedno sestavení + synchronizace, ukončí se s kódem `0` při úspěchu, `1` při chybě. | CI, pre-commit hooky, AI agenti, skriptované pracovní postupy.           |
| `yarn beeax dev --once --dry-run` | Sestaví a vypíše změny metadat **bez jejich použití**.                             | Kontrola toho, co by synchronizace změnila, ještě před jejím potvrzením. |
