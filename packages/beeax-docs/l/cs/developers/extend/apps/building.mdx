---
title: Architektura
description: Jak fungují aplikace BeeAX — sandboxing, životní cyklus a stavební bloky.
icon: sitemap
---
...
Aplikace je kolekce **entit** deklarovaných pomocí funkcí `defineEntity()` z balíčku `beeax-sdk`. SDK tyto deklarace detekuje pomocí analýzy AST při sestavení a vytváří **manifest** — úplný popis toho, co vaše aplikace přidává do pracovního prostoru.
...
| **Frontendová komponenta**    | Izolované React UI na stránce BeeAX            | [Frontendové komponenty](/l/cs/developers/extend/apps/front-components) |
...
* **`yarn beeax build`** — zkompiluje TypeScript, zabalí logické funkce a frontendové komponenty pomocí esbuild a vytvoří manifest.
...
<CardGroup cols={2}>
  ...
  <Card title="Frontendové komponenty" icon="window-maximize" href="/l/cs/developers/extend/apps/front-components">
    Izolované komponenty Reactu v uživatelském rozhraní BeeAX.
  </Card>
