...
* Použijte `serverVariables` (nikoli `applicationVariables`) — pověření OAuth jsou celoserverová a na jeden server BeeAX je jedna aplikace OAuth.
...
4. Otevřete nainstalovanou aplikaci v BeeAX jako správce serveru → nastavte hodnoty na odpovídajících `serverVariables`.
