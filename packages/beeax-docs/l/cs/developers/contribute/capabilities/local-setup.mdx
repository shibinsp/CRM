...
icon: laptop-code
description: Průvodce pro přispěvatele (nebo zvídavé vývojáře), kteří chtějí spustit BeeAX lokálně.
---
...
Než nainstalujete a použijete BeeAX, ujistěte se, že máte na svém počítači nainstalovány následující balíčky:
...
BeeAX vyžaduje ke svému provozu Redis cache pro zajištění nejlepšího výkonu.
...
## Krok 8: Použijte BeeAX

**Frontend**

Frontend BeeAX bude spuštěný na [http://localhost:3001](http://localhost:3001).
Můžete se přihlásit pomocí výchozího demo účtu: `tim@apple.dev` (heslo: `tim@apple.dev`)

**Backend**

* Server BeeAX poběží na [http://localhost:3000](http://localhost:3000)
