...
```bash
npx nx run beeax-front:lint # přidejte --fix pro opravu chyb ve formátování
```
...
```bash
npx nx run beeax-front:test # spusťte jest testy
npx nx run beeax-front:storybook:serve:dev # spusťte storybook
npx nx run beeax-front:storybook:test # spusťte testy # (vyžaduje, aby byl spuštěn yarn storybook:serve:dev)
npx nx run beeax-front:storybook:coverage # (vyžaduje, aby byl spuštěn yarn storybook:serve:dev)
```
