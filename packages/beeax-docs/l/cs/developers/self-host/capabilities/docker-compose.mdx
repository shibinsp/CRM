...
Tato příručka poskytuje podrobný návod k instalaci a konfiguraci aplikace BeeAX pomocí Docker Compose. Cílem je zjednodušit proces a zabránit běžným úskalím, která by mohla narušit vaše nastavení.
...
Nainstalujte nejnovější stabilní verzi BeeAX jedním příkazem:
...
Pokud hostujete beeaxCRM na svém vlastním počítači, otevřete svůj prohlížeč a přejděte na stránku [http://localhost:3000](http://localhost:3000).
...
### Otevření BeeAX pro Externí Přístup

Ve výchozím nastavení běží BeeAX na `localhost` na portu `3000`. Pro přístup přes externí doménu nebo IP adresu musíte nakonfigurovat `SERVER_URL` ve vašem `.env` souboru.
...
SSL (HTTPS) je potřebné pro správné fungování některých funkcí prohlížeče. I když tyto funkce mohou fungovat během místního vývoje (protože prohlížeče zacházejí s localhost jinak), správná konfigurace SSL je nutná při hostování BeeAX na běžné doméně.
...
Důrazně doporučujeme nastavit BeeAX za reverzní proxy se SSL ukončením pro optimální bezpečnost a funkčnost.
