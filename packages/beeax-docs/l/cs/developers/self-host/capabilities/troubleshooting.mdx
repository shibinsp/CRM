...
Pokud máte existující instanci BeeAX s produkčními daty, **NEPROVÁDĚJTE** tyto kroky, protože trvale smažou vaši databázi!

Při první instalaci BeeAX můžete chtít změnit výchozí heslo pro databázi.
...
Během instalace BeeAX musíte nastavit svou postgres databázi s správnými schématy, rozšířeními a uživateli.
...
#### Nelze najít modul 'beeax-emails' ani jeho odpovídající deklarace typů.

Před spuštěním inicializace databáze musíte sestavit balíček `beeax-emails` s `npx nx run beeax-emails:build`

#### Chybí balíček beeax-x

Ujistěte se, že v kořenovém adresáři spouštíte `yarn` a poté spusťte `npx nx server:dev beeax-server`. Pokud stále nefunguje, zkuste chybějící balíček sestavit ručně.
...
#### Při spuštění `npx nx start` nebo `npx nx start beeax-front` došlo k chybě nedostatku paměti

Spusťte pouze služby, které potřebujete, místo `npx nx start`. Například pokud pracujete na serveru, spusťte pouze `npx nx worker beeax-server`

**Pokud to nefunguje:**
Pokud jste se pokusili spustit pouze `npx nx run beeax-server:start` na WSL a selhává to s následující chybou paměti:
...
**Pokud to nefunguje:**
Analyzujte, které procesy vám zabírají většinu paměti na stroji. V BeeAX jsme si všimli, že některá rozšíření VScode zabírala hodně paměti RAM, takže jsme je dočasně deaktivovali.
...
* Otevřete svou BeeAX aplikaci, přejděte na `/settings` a na levé dolní části obrazovky aktivujte přepínač **Pokročilé**.
* Vytvořte nový webhook.
* Vložte **Svou unikátní webhook URL** do pole **Endpoint Url** ve BeeAX. Nastavte **Filtry** na `Companies` a `Created`.
...
Zakomentujte plugin checker v `packages/beeax-ui/vite-config.ts` jako v příkladu níže
...
Pokud provozujete BeeAX za reverzní proxy a máte problémy s připojením:
...
2. **Zkontrolujte nastavení reverzní proxy:**
   ...
   * Potvrďte, že vaše reverzní proxy správně směruje požadavky na server BeeAX.
   * Ujistěte se, že záhlaví jako `X-Forwarded-For` a `X-Forwarded-Proto` jsou správně nastaveny.

3. **Restartujte služby:**

   Po provedení změn restartujte jak reverzní proxy, tak kontejnery BeeAX.
...
* Podpora komunity:
  ...
  Obraťte se na [komunitu BeeAX](https://github.com/beeax/beeax/issues) nebo [podpůrné kanály](https://discord.gg/cx5n4Jzs57) pro pomoc.
