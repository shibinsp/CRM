...
When you upload a file, BeeAX analyzes your columns and attempts to match them to existing fields.

### Automatic Mapping

BeeAX tries to match columns based on:

* Column header names (exact or similar matches)
* Data type detection (dates, numbers, emails)
* Common field patterns

<Note>**Quick tip:** Export a few rows from the object you want to import. The exported file will have the exact column names BeeAX expects, making automatic mapping seamless during import.</Note>

### Manual Mapping Options

For each column, you can:

* **Map to a field**: Select the matching BeeAX field from a dropdown
...
Specifying an `id` during import is optional. BeeAX auto-generates one if not provided.
