...
description: Guide pas à pas pour migrer vos données de n'importe quel CRM vers BeeAX.
---

## Vue d'ensemble

Ce guide vous accompagne pour migrer vos données depuis n'importe quel CRM vers BeeAX. Le processus consiste à auditer vos données, à préparer votre espace de travail sur BeeAX, à exporter depuis votre système actuel et à importer dans BeeAX.
...
Créez un document de correspondance entre votre CRM actuel et BeeAX :
...
## Étape 3 : Configurez votre espace de travail BeeAX

Avant d'importer des données, préparez votre espace de travail BeeAX :
...
Si vos données incluent des références à des utilisateurs (propriétaire du compte, assigné, etc.), ces utilisateurs doivent exister dans BeeAX avant l'importation. Sinon, ces relations ne pourront pas être mises en correspondance.
</Warning>
...
Ouvrez chaque CSV exporté dans une application de tableur et préparez-le pour BeeAX.
...
3. Vérifiez qu'il n'existe pas déjà de doublons dans BeeAX
...
Utilisez `https://domain.com` (pas `domain.com` ni `www.domain.com`). Cela correspond au format de BeeAX et évite les doublons lorsque vous connectez la synchronisation e-mail/calendrier.
</Warning>
...
2. Utilisez les mêmes adresses e-mail que celles utilisées par les utilisateurs pour rejoindre votre espace de travail BeeAX
...
1. Accédez à l'objet dans BeeAX
...
* BeeAX commencera à synchroniser les e-mails avec les enregistrements de contacts
...
<AccordionGroup>
  <Accordion title="Puis-je migrer les flux de travail automatiquement ?">
    Pas pour le moment. Les flux de travail doivent être recréés manuellement dans BeeAX.
  </Accordion>
...
Contactez [contact@beeax.com](mailto:contact@beeax.com) ou découvrez nos [services de mise en œuvre](/l/fr/user-guide/getting-started/capabilities/implementation-services).
