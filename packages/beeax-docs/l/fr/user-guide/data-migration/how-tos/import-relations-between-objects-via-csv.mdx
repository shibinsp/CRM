...
### Relations courantes dans BeeAX
...
Le parent (côté "un") doit exister dans BeeAX avant que vous ne puissiez y faire référence.
...
1. Exportez les enregistrements parents depuis BeeAX
...
Utilisez `https://domain.com` (et non simplement `domain.com`). Cela correspond à la manière dont BeeAX stocke les domaines des Entreprises et évite les erreurs d'association.
</Warning>
...
BeeAX liera automatiquement chaque enregistrement enfant au parent correspondant.
...
| **Enregistrements inexistants** | L'enregistrement parent n'existe pas dans BeeAX            | Importez d'abord les enregistrements parents, ou vérifiez les fautes de frappe |
| **Incohérence de casse**        | `Acme.com` dans le fichier mais `acme.com` dans BeeAX      | Assurez une correspondance exacte de la casse                                  |
...
<AccordionGroup>
  <Accordion title="Que faire si mon enregistrement parent n'a pas de domaine ni d'e-mail ?">
    ...
    1. Utilisez l'`id` BeeAX (exportez les enregistrements parents pour obtenir leurs identifiants)
