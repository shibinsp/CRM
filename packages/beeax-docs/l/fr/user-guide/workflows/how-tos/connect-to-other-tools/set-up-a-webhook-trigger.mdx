...
| **Intégrations personnalisées** | Votre application → automatisation dans BeeAX                 |
...
3. Vous recevrez une URL de webhook unique, par exemple :
   ```
   https://api.beeax.com/webhooks/workflow/abc123...
   ```
...
```bash
curl -X POST https://api.beeax.com/webhooks/workflow/abc123... \
