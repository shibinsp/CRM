...
description: Comprendre les fonctionnalités d'intégration des e-mails dans BeeAX.
---
...
BeeAX lie automatiquement les emails de vos boîtes aux lettres connectées aux enregistrements CRM pertinents, gardant tout l'historique de communication en un seul endroit.
...
BeeAX importe des emails à environ **400 messages par minute**. Ce rythme est actuellement limité par la limitation de débit de l'API Gmail (et s'applique de manière similaire aux autres fournisseurs).
...
* **Emails Manquants** : Vérifiez si :
  ...
  * La création automatique de contacts est désactivée (les e-mails nécessitent des enregistrements existants dans BeeAX)
