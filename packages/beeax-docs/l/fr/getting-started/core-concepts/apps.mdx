...
title: Applications
icon: cube
description: Étendez BeeAX avec du code — des objets personnalisés, de la logique côté serveur, des composants d’interface utilisateur et des agents IA, tous sous forme de packages TypeScript.
---

La plupart des CRM vous donnent un panneau de configuration. BeeAX vous offre une plateforme. Les applications sont le moyen par lequel les développeurs étendent BeeAX au-delà de ce que l’interface utilisateur propose — en définissant des modèles de données, de la logique côté serveur, des composants d’interface utilisateur et des fonctionnalités d’IA sous forme de code, puis en les déployant sur un ou plusieurs espaces de travail.
...
Une application est un package TypeScript qui déclare des **entités** en utilisant le `beeax-sdk` :
...
| **Composants frontaux**   | Composants React isolés qui s’affichent dans l’interface de BeeAX (panneau latéral, widgets, menu de commandes)        |
...
Vous écrivez votre application comme un projet TypeScript sur votre machine. La CLI surveille vos fichiers sources et les synchronise en direct avec un serveur BeeAX en cours d’exécution — modifiez un fichier et voyez le changement dans l’interface en une seconde. Le client d’API typé est régénéré automatiquement lorsque le schéma change. Quand vous êtes prêt, `yarn beeax app:publish --private` envoie l’application vers un serveur de production, ou `yarn beeax app:publish` publie votre application sur npm et sur la place de marché BeeAX.
