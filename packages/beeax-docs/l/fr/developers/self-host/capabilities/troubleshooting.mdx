...
Si vous avez une instance BeeAX existante avec des données de production, **NE SUIVEZ PAS** ces étapes, car elles supprimeront définitivement votre base de données !

Lors de l'installation de BeeAX pour la première fois, vous pouvez vouloir changer le mot de passe par défaut de la base de données.
...
Pendant l'installation de BeeAX, vous devez approvisionner votre base de données postgres avec les bons schémas, extensions et utilisateurs.
...
#### Impossible de trouver le module 'beeax-emails' ou les déclarations de type correspondantes.

Vous devez construire le package `beeax-emails` avant de lancer l'initialisation de la base de données avec `npx nx run beeax-emails:build`

#### Package beeax-x manquant

Assurez-vous d'exécuter yarn dans le répertoire racine, puis d'exécuter `npx nx server:dev beeax-server`. Si cela ne fonctionne toujours pas, essayez de construire le package manquant manuellement.
...
#### Pendant l'exécution de `npx nx start` ou `npx nx start beeax-front`, une erreur de mémoire est renvoyée

Lancez uniquement les services dont vous avez besoin, au lieu de `npx nx start`. Par exemple, si vous travaillez sur le serveur, lancez seulement `npx nx worker beeax-server`

**Si cela ne fonctionne pas :**
Si vous avez essayé de lancer uniquement `npx nx run beeax-server:start` sur WSL et que cela échoue avec l'erreur de mémoire ci-dessous :
...
**Si cela ne fonctionne pas :**
Examinez quels processus consomment le plus de RAM sur votre machine. Chez BeeAX, nous avons remarqué que certaines extensions de VScode consommaient beaucoup de RAM, nous les désactivons donc temporairement.
...
* Ouvrez votre application BeeAX, naviguez jusqu'à `/settings`, et activez le bouton **Avancé** en bas à gauche de l'écran.
* Créez un nouveau webhook.
* Collez **Votre URL Webhook Unique** dans le champ **URL de l'Endpoint** de BeeAX. Réglez les **Filtres** sur `Sociétés` et `Créé`.
...
Commentez le plugin de vérification dans `packages/beeax-ui/vite-config.ts` comme dans l'exemple ci-dessous
...
1. Exécutez les commandes suivantes :
   ```bash
   docker exec -it beeax-server-1 yarn
   docker exec -it beeax-server-1 npx nx database:reset  --configuration=no-seed
   ```
...
Si vous exécutez BeeAX derrière un proxy inverse et rencontrez des problèmes de connexion :
...
2. **Vérifiez les paramètres du proxy inverse :**
   ...
   * Confirmez que votre proxy inverse transfère correctement les requêtes au serveur BeeAX.
   * Assurez-vous que les en-têtes comme `X-Forwarded-For` et `X-Forwarded-Proto` sont correctement configurés.

3. **Redémarrez les services :**

   Après avoir apporté des modifications, redémarrez à la fois le proxy inverse et les conteneurs BeeAX.
...
* Support communautaire :
  ...
  Contactez la [communauté BeeAX](https://github.com/beeax/beeax/issues) ou les [canaux de support](https://discord.gg/cx5n4Jzs57) pour obtenir de l'aide.
