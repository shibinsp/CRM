...
Ce guide fournit des instructions pas-à-pas pour installer et configurer l'application BeeAX à l'aide de Docker Compose. L'objectif est de simplifier le processus et d'éviter les erreurs courantes qui pourraient compromettre votre configuration.
...
Installez la dernière version stable de BeeAX avec une seule commande :
...
Si vous hébergez beeaxCRM sur votre ordinateur, ouvrez votre navigateur et allez à [http://localhost:3000](http://localhost:3000).
...
### Exposer BeeAX à un Accès Externe

Par défaut, BeeAX fonctionne sur `localhost` au port `3000`. Pour y accéder via un domaine externe ou une adresse IP, vous devez configurer le `SERVER_URL` dans votre fichier `.env`.
...
Nous vous recommandons vivement de mettre en place BeeAX derrière un proxy inverse avec terminaison SSL pour une sécurité et une fonctionnalité optimales.
