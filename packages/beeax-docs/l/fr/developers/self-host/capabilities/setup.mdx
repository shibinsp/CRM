...
<Warning>
**Première installation ?** Suivez le [Guide d’installation de Docker Compose](/l/fr/developers/self-host/capabilities/docker-compose) pour faire fonctionner BeeAX, puis revenez ici pour la configuration.
</Warning>

BeeAX offre **deux modes de configuration** pour répondre à différents besoins de déploiement :
...
1. Accédez à votre instance BeeAX (généralement `http://localhost:3000`)
...
[Référence technique complète →](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/beeax-config/config-variables.ts)
</Warning>

## Clés de chiffrement

BeeAX utilise deux clés de chiffrement uniquement définissables via des variables d'environnement :
...
Pour assurer la rétrocompatibilité, si `ENCRYPTION_KEY` n'est pas défini, BeeAX se rabat sur `APP_SECRET` pour le chiffrement au repos, ce qui correspond au comportement historique des anciens déploiements. Les nouvelles installations doivent toujours définir une `ENCRYPTION_KEY` dédiée.
...
Par défaut, BeeAX fonctionne en **mode à espace de travail unique** — idéal pour la plupart des déploiements auto-hébergés où vous avez besoin d'une seule instance CRM pour votre organisation.
...
* Un espace de travail par instance BeeAX
...
Activez le mode multi-espaces de travail pour des déploiements de type SaaS où plusieurs équipes indépendantes ont besoin de leurs propres espaces de travail sur la même instance BeeAX.
...
[Voir le code source pertinent](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
Les utilisateurs doivent avoir une [licence Microsoft 365](https://admin.microsoft.com/Adminportal/Home) pour pouvoir utiliser les API Calendrier et Messagerie. Ils ne pourront pas synchroniser leur compte sur BeeAX sans cela.
</Warning>
...
[Voir le code source pertinent](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
<Warning>
Par défaut, BeeAX stocke les fichiers téléversés sur le système de fichiers local. Pour les déploiements en production, utilisez S3 ou un service compatible S3 (MinIO, DigitalOcean Spaces, etc.). afin de garantir la persistance des fichiers lors des redémarrages des conteneurs et la montée en charge sur plusieurs instances de serveur.
</Warning>

Définissez `STORAGE_TYPE=S_3` et configurez les variables `STORAGE_S3_*` via le panneau d'administration ou `.env`. Voir la [référence config-variables.ts](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/beeax-config/config-variables.ts) pour la liste complète des variables S3.

Lorsque vous utilisez S3 avec des fonctionnalités dépendantes de CORS (par exemple les téléchargements de fichiers dans le navigateur), assurez-vous que votre compartiment S3 autorise l'origine de votre frontend BeeAX dans sa configuration CORS.

## Fonctions logiques et interpréteur de code

BeeAX prend en charge les fonctions logiques pour les flux de travail et l'interpréteur de code pour l'analyse de données par IA. Les deux exécutent du code fourni par l'utilisateur et nécessitent une configuration explicite pour la sécurité.
...
<Note>
Lorsque vous utilisez `LOGIC_FUNCTION_TYPE=DISABLED` ou `CODE_INTERPRETER_TYPE=DISABLED`, toute tentative d'exécution renverra une erreur. Cela est utile si vous souhaitez exécuter BeeAX sans ces capacités.
</Note>
