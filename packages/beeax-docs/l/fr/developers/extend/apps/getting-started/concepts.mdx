---
title: Concepts
description: Fonctionnement des applications BeeAX — modèle d’entité, sandboxing et cycle de vie de l’installation.
icon: sitemap
---

Les applications BeeAX sont des packages TypeScript qui étendent votre espace de travail avec des objets personnalisés, de la logique, des composants d’interface utilisateur et des capacités d’IA. Elles s’exécutent sur la plateforme BeeAX avec une isolation complète en bac à sable et des contrôles d’autorisations.

## Fonctionnement des applications

Une application est un ensemble d’**entités** déclarées à l’aide des fonctions `defineEntity()` du package `beeax-sdk`. Le SDK détecte ces déclarations via une analyse de l’AST au moment de la compilation et produit un **manifeste** — une description complète de ce que votre application ajoute à un espace de travail. Ces fonctions valident votre configuration au moment de la compilation et offrent l’autocomplétion dans l’IDE et une sécurité de typage.
...
| **Composant frontal**             | Interface React isolée en bac à sable dans BeeAX               | [Composants frontaux](/l/fr/developers/extend/apps/layout/front-components)                 |
...
* **Les composants frontaux** s’exécutent dans des Web Workers en utilisant Remote DOM — isolés en bac à sable de la page principale mais rendant des éléments DOM natifs (pas d’iframes). Ils communiquent avec BeeAX via une API hôte de passage de messages.
...
* **`yarn beeax dev`** — surveille vos fichiers source et synchronise en direct les modifications avec un serveur BeeAX connecté. Le client d’API typé est régénéré automatiquement lorsque le schéma change.
* **`yarn beeax dev:build`** — compile TypeScript, regroupe les fonctions logiques et les composants frontaux avec esbuild, et produit un manifeste.
