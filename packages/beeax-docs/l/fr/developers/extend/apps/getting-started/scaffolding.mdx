---
title: Génération
description: Générez des fichiers d'entité de manière interactive avec `yarn beeax dev:add` — objets, champs, vues, fonctions logiques et plus encore.
...
| ----------------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Objet                         | `yarn beeax dev:add object`             | `src/objects/\<name>.ts`               |
| Champ                         | `yarn beeax dev:add field`              | `src/fields/\<name>.ts`                |
| Fonction logique              | `yarn beeax dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Composant frontal             | `yarn beeax dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rôle                          | `yarn beeax dev:add role`               | `src/roles/\<name>.ts`                 |
| Compétence                    | `yarn beeax dev:add skill`              | `src/skills/\<name>.ts`                |
| Agent                         | `yarn beeax dev:add agent`              | `src/agents/\<name>.ts`                |
| Vue                           | `yarn beeax dev:add view`               | `src/views/\<name>.ts`                 |
| Élément de menu de navigation | `yarn beeax dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Mise en page                  | `yarn beeax dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## Ce que génère l'outil de génération

Chaque type d'entité a son propre modèle. Par exemple, `yarn beeax dev:add object` demande :
