...
description: Créez des composants React qui s’affichent dans l’interface utilisateur (UI) de BeeAX avec une isolation en bac à sable.
icon: window-maximize
---

Les composants frontaux sont des composants React qui s'affichent directement dans l'interface utilisateur de BeeAX. Ils s'exécutent dans un **Web Worker isolé** en utilisant Remote DOM — votre code est isolé mais s'affiche nativement dans la page, pas dans un iframe.

## Où les composants frontaux peuvent être utilisés

Les composants frontaux peuvent s'afficher à deux emplacements au sein de BeeAX :
...
Après la synchronisation avec `yarn beeax dev` (ou en exécutant une seule fois `yarn beeax dev --once`), l'action rapide apparaît dans le coin supérieur droit de la page :
...
Comme le composant retourne `null`, BeeAX n'affiche pas de conteneur pour celui-ci — aucun espace vide n'apparaît dans la mise en page. Le composant a toujours accès à tous les hooks et à l'API de communication de l'hôte.

## Composants Command du SDK

Le package `beeax-sdk` fournit quatre composants utilitaires Command conçus pour les composants frontaux headless. Chaque composant exécute une action au montage, gère les erreurs en affichant une notification snackbar et démonte automatiquement le composant frontal une fois terminé.

Importez-les depuis `beeax-sdk/command` :
...
Une fonction logique déclarée avec `httpRouteTriggerSettings` est accessible via HTTP à son chemin de route. BeeAX injecte dans le worker l’URL de base à partir de laquelle vos fonctions sont servies sous la forme de `TWENTY_FUNCTIONS_URL`, ainsi que le `TWENTY_APP_ACCESS_TOKEN` qui authentifie l’appel. Il n’existe pas encore de client SDK dédié pour invoquer vos propres fonctions, donc appelez-les avec un simple `fetch` :

> **Sur BeeAX Cloud, les fonctions logiques déclenchées par HTTP sont servies sur un domaine dédié par espace de travail** à l’adresse `https://\<your-workspace-subdomain>.beeax.com\<path>` — c’est exactement ce à quoi `TWENTY_FUNCTIONS_URL` correspond. Pour les appelants externes, copiez l’URL exacte à partir des paramètres **HTTP trigger** de la fonction ou de l’onglet **Settings** de l’application.
...
Le chemin ajouté à `TWENTY_FUNCTIONS_URL` correspond au `httpRouteTriggerSettings.path` de la fonction logique. Conservez `isAuthRequired: true` ; le `TWENTY_APP_ACCESS_TOKEN` que BeeAX génère pour votre composant authentifie la requête :
...
### Appeler l’API REST de BeeAX

Pour lire ou écrire des enregistrements BeeAX depuis un composant frontal, utilisez `RestApiClient` depuis `beeax-client-sdk/rest`. Il appartient à la même famille de clients que `CoreApiClient` et `MetadataApiClient`, mais cible l’API REST de BeeAX (`/rest/...`) plutôt que l’API GraphQL, en lisant son URL de base depuis `TWENTY_API_URL`.
...
| ------------------------- | ------------------------------------------------------------------------------------------------ |
| `TWENTY_FUNCTIONS_URL`    | URL de base à partir de laquelle les fonctions de logique HTTP de votre application sont servies |
| `TWENTY_API_URL`          | URL de base de l’API principale de BeeAX                                                        |
| `TWENTY_APP_ACCESS_TOKEN` | Jeton de courte durée limité au rôle de votre application                                        |

## API de communication de l'hôte

Les composants frontaux peuvent déclencher la navigation, des modales et des notifications en utilisant des fonctions de `beeax-sdk` :
...
* **Composants BeeAX UI** — à importer depuis `beeax-sdk/ui` (Button, Tag, Status, Chip, Avatar, etc.)
