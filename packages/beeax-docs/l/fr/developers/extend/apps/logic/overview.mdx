...
description: TypeScript côté serveur qui s’exécute dans BeeAX — déclenché par des routes HTTP, des planifications CRON, des événements de base de données, des outils d’IA ou des actions de flux de travail.
icon: bolt
---

La **couche logique** d’une application BeeAX est le code qui *s’exécute* — des gestionnaires TypeScript côté serveur qui réagissent aux requêtes HTTP, aux planifications CRON et aux modifications d’enregistrements ; des compétences et des agents d’IA qui résident dans l’espace de travail ; et des connexions OAuth qui permettent à vos fonctions d’agir au nom d’un utilisateur dans des services tiers.
...
| **Outil IA**                     | Une fonctionnalité IA de BeeAX décide d’appeler votre fonction              | `toolTriggerSettings`           |
