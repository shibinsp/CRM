...
* Utilisez `serverVariables` (pas `applicationVariables`) — les identifiants OAuth sont au niveau du serveur et il n'y a qu'une application OAuth par serveur BeeAX.
...
4. Ouvrez l'application installée dans BeeAX en tant qu'administrateur du serveur → définissez les valeurs sur les `serverVariables` correspondantes.
