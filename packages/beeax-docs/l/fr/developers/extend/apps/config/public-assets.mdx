...
* **Inclus dans les builds** — `yarn beeax dev:build` regroupe toutes les ressources publiques dans la sortie de distribution.

## Accéder aux ressources publiques avec `getPublicAssetUrl`

Utilisez l'utilitaire `getPublicAssetUrl` de `beeax-sdk` pour obtenir l'URL complète d'un fichier dans votre répertoire `public/`. Cela fonctionne à la fois dans les **fonctions logiques** et les **composants frontaux**.
