...
* Les variables d'environnement `APPLICATION_ID`, `APP_ACCESS_TOKEN` et `API_URL` sont disponibles dans le gestionnaire (comme pour toute autre fonction logique), vous pouvez donc appeler l'API BeeAX avec un jeton d'accès d'application limité à votre application.
...
* **Non exécuté en mode dev** : lorsqu'une application est enregistrée localement (via `yarn beeax dev`), le serveur saute complètement le flux d'installation et synchronise les fichiers directement via le watcher de la CLI — ainsi, post-install ne s'exécute jamais en mode dev, quel que soit `shouldRunSynchronously`. Utilisez `yarn beeax dev:function:exec --postInstall` pour le déclencher manuellement sur un espace de travail en cours d'exécution.
...
* **Non exécuté en mode dev** : comme pour post-install — le flux d'installation est entièrement ignoré pour les applications enregistrées localement, donc la pré-installation ne s'exécute jamais sous `yarn beeax dev`. Utilisez `yarn beeax dev:function:exec --preInstall` pour la déclencher manuellement.
