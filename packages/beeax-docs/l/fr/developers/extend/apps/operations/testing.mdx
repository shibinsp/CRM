...
Les deux environnements disposent de `beeax-client-sdk/core` et `beeax-client-sdk/metadata` en tant que modules pré-fournis — ils ne sont pas intégrés au bundle mais résolus à l'exécution par le serveur.
...
Le sous-chemin `beeax-sdk/cli` exporte des fonctions que vous pouvez appeler directement depuis le code de test :
...
Assurez-vous que votre serveur BeeAX local est en cours d'exécution, puis :
...
2. Lance un serveur BeeAX temporaire en utilisant l’action `beeax/beeax/.github/actions/spawn-beeax-docker-image`
...
Vous n’avez pas besoin de configurer de secrets — l’action `spawn-beeax-docker-image` démarre un serveur BeeAX éphémère directement dans le runner et fournit les détails de connexion. Le secret `GITHUB_TOKEN` est fourni automatiquement par GitHub.

Pour épingler une version spécifique de BeeAX au lieu de `latest`, modifiez la variable d’environnement `TWENTY_VERSION` en haut du workflow.
