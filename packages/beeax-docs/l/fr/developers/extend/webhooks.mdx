...
BeeAX envoie une requête HTTP POST vers votre URL chaque fois qu'un enregistrement est créé, mis à jour ou supprimé. Tous les types d'objets sont pris en charge, y compris les objets personnalisés.
...
const receivedSignature = req.headers["x-beeax-webhook-signature"];
