...
Les webhooks envoient des données à vos systèmes en temps réel lorsque des événements se produisent dans BeeAX — aucune interrogation n'est nécessaire. Utilisez-les pour maintenir les systèmes externes synchronisés, déclencher des automatisations ou envoyer des alertes.
...
BeeAX envoie des webhooks pour ces types d'événements :
...
BeeAX signe chaque requête de webhook pour des raisons de sécurité. Validez les signatures pour garantir l'authenticité des requêtes.

### En-têtes

| En-tête                      | Description              |
| ---------------------------- | ------------------------ |
| `X-BeeAX-Webhook-Signature` | Signature HMAC SHA256    |
| `X-BeeAX-Webhook-Timestamp` | Horodatage de la requête |

### Étapes de validation

1. Récupérez l'horodatage depuis `X-BeeAX-Webhook-Timestamp`
2. Créez la chaîne : `{timestamp}:{JSON payload}`
3. Calculez le HMAC SHA256 en utilisant votre secret de webhook
4. Comparez avec `X-BeeAX-Webhook-Signature`
...
const timestamp = req.headers["x-beeax-webhook-timestamp"];
...
const isValid = expectedSignature === req.headers["x-beeax-webhook-signature"];
```
...
| **Déclencheur de webhook de workflow** | ENTRÉE    | Recevoir des données dans BeeAX depuis des systèmes externes                         |
