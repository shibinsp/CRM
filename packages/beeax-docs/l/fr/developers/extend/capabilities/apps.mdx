---
title: Applications BeeAX
description: Créez et gérez les personnalisations BeeAX sous forme de code.
---
...
Les applications vous permettent de créer et de gérer des personnalisations BeeAX **sous forme de code**. Au lieu de tout configurer via l’interface utilisateur, vous définissez votre modèle de données et vos fonctions logiques dans le code — ce qui accélère la création, la maintenance et le déploiement sur plusieurs espaces de travail.
...
* Un espace de travail BeeAX et une clé API (créez-en une sur https://app.beeax.com/settings/api-webhooks)
...
Voir aussi : les pages de référence CLI pour [create-beeax-app](https://www.npmjs.com/package/create-beeax-app) et [beeax-sdk CLI](https://www.npmjs.com/package/beeax-sdk).

## Structure du projet (générée)

Lorsque vous exécutez `npx create-beeax-app@latest my-beeax-app`, l’outil de scaffolding :

* Copie une application de base minimale dans `my-beeax-app/`
* Ajoute une dépendance locale à `beeax-sdk` et la configuration Yarn 4
* Crée des fichiers de configuration et des scripts reliés à la CLI `beeax`
...
* **package.json** : Déclare le nom de l’application, la version, les moteurs (Node 24+, Yarn 4), et ajoute `beeax-sdk` ainsi que des scripts comme `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` et `auth:login` qui délèguent à la CLI locale `beeax`.
...
* `yarn app:dev` génère automatiquement le client BeeAX typé dans `node_modules/beeax-sdk/generated`.
...
Vos identifiants sont stockés par utilisateur dans `~/.beeax/config.json`. Vous pouvez gérer plusieurs profils et basculer entre eux.
...
Le paquet beeax-sdk fournit des blocs de construction typés et des fonctions utilitaires que vous utilisez dans votre application. Voici les éléments clés que vous manipulerez le plus souvent.
...
import { defineObject, FieldType } from 'beeax-sdk';
...
<Note>
  **Les champs de base sont créés automatiquement.** Lorsque vous définissez un objet personnalisé, BeeAX ajoute automatiquement des champs standard tels que `name`, `createdAt`, `updatedAt`, `createdBy`, `position` et `deletedAt`. Vous n'avez pas besoin de les définir dans votre tableau `fields` — ajoutez uniquement vos champs personnalisés.
</Note>
...
import { defineApplication } from 'beeax-sdk';
import { DEFAULT_ROLE_UNIVERSAL_IDENTIFIER } from './default-function.role';

export default defineApplication({
  universalIdentifier: '4ec0391d-18d5-411c-b2f3-266ddc1c3ef7',
  displayName: 'My BeeAX App',
  description: 'My first BeeAX app',
  icon: 'IconWorld',
...
import { defineRole, PermissionFlag } from 'beeax-sdk';
...
* Voir un exemple fonctionnel dans l’application Hello World : [`packages/beeax-apps/hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
import { defineFunction } from 'beeax-sdk';
import type { DatabaseEventPayload, ObjectRecordCreateEvent, CronPayload, RoutePayload } from 'beeax-sdk';
import BeeAX, { type Person } from '~/generated';

const handler = async (params: RoutePayload) => {
  const client = new BeeAX(); // generated typed client
...
Lorsqu’un déclencheur de route appelle votre fonction logique, elle reçoit un objet `RoutePayload` qui suit le format AWS HTTP API v2. Importez le type depuis `beeax-sdk` :

```typescript
import { defineFunction, type RoutePayload } from 'beeax-sdk';
...
`yarn app:dev` génère automatiquement le client BeeAX typé dans `node_modules/beeax-sdk/generated`. Utilisez-le dans vos fonctions :

```typescript
import BeeAX from '~/generated';

const client = new BeeAX();
...
Lorsque votre fonction s’exécute sur BeeAX, la plateforme injecte des identifiants sous forme de variables d’environnement avant l’exécution de votre code :

* `TWENTY_API_URL` : URL de base de l’API BeeAX ciblée par votre application.
...
Découvrez un exemple minimal de bout en bout qui démontre des objets, des fonctions et plusieurs déclencheurs [ici](https://github.com/beeax/beeax/tree/main/packages/beeax-apps/hello-world) :

## Configuration manuelle (sans l’outil de scaffolding)

Même si nous recommandons d’utiliser `create-beeax-app` pour une expérience de démarrage optimale, vous pouvez également configurer un projet manuellement. N’installez pas la CLI globalement. Ajoutez plutôt `beeax-sdk` comme dépendance locale et reliez des scripts dans votre package.json :

```bash filename="Terminal"
yarn add -D beeax-sdk
```
...
{
  "scripts": {
    "auth:login": "beeax auth:login",
    "auth:logout": "beeax auth:logout",
    "auth:status": "beeax auth:status",
    "auth:switch": "beeax auth:switch",
    "auth:list": "beeax auth:list",
    "app:dev": "beeax app:dev",
    "app:uninstall": "beeax app:uninstall",
    "entity:add": "beeax entity:add",
    "function:logs": "beeax function:logs",
    "function:execute": "beeax function:execute",
    "help": "beeax help"
  }
}
```
...
* Impossible de se connecter au serveur : vérifiez l’URL de l’API et que le serveur BeeAX est accessible.
