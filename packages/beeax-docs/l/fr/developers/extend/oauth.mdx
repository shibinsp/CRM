...
BeeAX implémente OAuth 2.0 avec code d’autorisation + PKCE pour les applications destinées aux utilisateurs et des identifiants client pour l’accès serveur à serveur. Les clients sont enregistrés dynamiquement via [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591) — aucune configuration manuelle dans un tableau de bord.
...
| Application BeeAX avec des extensions d’interface utilisateur | [Applications](/l/fr/developers/extend/apps/getting-started) (OAuth est géré automatiquement) |

## Enregistrer un client

BeeAX prend en charge **l’enregistrement dynamique de clients** conformément à [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591). Aucune configuration manuelle requise — enregistrez le client par programmation :
...
Utilisez ce flux lorsque votre application agit au nom d’un utilisateur BeeAX.
...
Après l’autorisation, BeeAX redirige vers votre `redirect_uri` :
...
BeeAX publie sa configuration OAuth à un point de découverte standard :
