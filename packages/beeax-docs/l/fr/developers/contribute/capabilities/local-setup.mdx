...
icon: laptop-code
description: Le guide pour les contributeurs (ou les développeurs curieux) qui souhaitent exécuter BeeAX localement.
---
...
Avant de pouvoir installer et utiliser BeeAX, assurez-vous d'installer ce qui suit sur votre ordinateur :
...
```bash
git clone git@github.com:beeax/beeax.git
```
</Tab>
<Tab title="HTTPS">
...
<Tabs>
  <Tab title="Linux">
    ...
    **Option 2 :** Si vous avez installé Docker :
    ```bash
      make -C packages/beeax-docker postgres-on-docker
    ```
  </Tab>
  <Tab title="Mac OS">
...
BeeAX nécessite un cache Redis pour offrir les meilleures performances.
...
<Tabs>
  <Tab title="Linux">
    ...
    **Option 2 :** Si vous avez installé Docker :
    ```bash
      make -C packages/beeax-docker redis-on-docker
    ```
  </Tab>
  <Tab title="Mac OS">
...
```bash
cp ./packages/beeax-front/.env.example ./packages/beeax-front/.env
cp ./packages/beeax-server/.env.example ./packages/beeax-server/.env
```

<Info>
**Mode multi-espaces de travail:** Par défaut, BeeAX s'exécute en mode mono-espace de travail, où un seul espace de travail peut être créé. Pour activer la prise en charge multi-espaces de travail (utile pour tester des fonctionnalités basées sur des sous-domaines), définissez `IS_MULTIWORKSPACE_ENABLED=true` dans le fichier `.env` de votre serveur. Voir [Mode multi-espaces de travail](/l/fr/developers/self-host/capabilities/setup#multi-workspace-mode) pour plus de détails.
</Info>

## Étape 6 : Installation des dépendances

Pour construire le serveur BeeAX et injecter des données dans votre base de données, exécutez la commande suivante :
...
## Étape 8 : Utiliser BeeAX

**Frontend**

Le frontend de BeeAX sera en cours d'exécution à [http://localhost:3001](http://localhost:3001).
Vous pouvez vous connecter en utilisant le compte démo par défaut : `tim@apple.dev` (mot de passe : `tim@apple.dev`)

**Backend**

* Le serveur de BeeAX sera en fonctionnement à [http://localhost:3000](http://localhost:3000)
