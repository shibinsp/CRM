...
Les objets sont des structures qui vous permettent de stocker des données (enregistrements, attributs et valeurs) spécifiques à une organisation. BeeAX fournit à la fois des objets standard et personnalisés.

Les objets standard sont des objets intégrés avec un ensemble d'attributs disponibles pour tous les utilisateurs. Les exemples d'objets standard dans BeeAX incluent Société et Personne. Les objets standard ont des champs standard également disponibles pour tous les utilisateurs de BeeAX, comme Company.displayName.
