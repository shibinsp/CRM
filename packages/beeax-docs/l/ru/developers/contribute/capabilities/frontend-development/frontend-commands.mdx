...
```bash
npx nx run beeax-front:lint # используйте --fix для исправления ошибок линтинга
```
...
```bash
npx nx run beeax-front:test # запуск тестов jest
npx nx run beeax-front:storybook:serve:dev # запуск storybook
npx nx run beeax-front:storybook:test # запуск тестов # (требуется yarn storybook:serve:dev для работы)
npx nx run beeax-front:storybook:coverage # (требуется yarn storybook:serve:dev для работы)
```
