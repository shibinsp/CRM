...
icon: laptop-code
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать BeeAX локально.
---
...
Прежде чем установить и использовать BeeAX, убедитесь, что у вас установлено следующее:
...
BeeAX требует кэша Redis для обеспечения наилучшей производительности.
...
## Шаг 8: Использовать BeeAX

**Frontend**

Фронтенд BeeAX будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер BeeAX будет работать на [http://localhost:3000](http://localhost:3000).
