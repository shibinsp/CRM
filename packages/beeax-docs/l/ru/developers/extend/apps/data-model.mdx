---
title: Модель данных
description: Определяйте объекты, поля, роли и метаданные приложения с помощью BeeAX SDK.
icon: database
---

Пакет `beeax-sdk` предоставляет функции `defineEntity` для определения модели данных вашего приложения. Вы должны использовать `export default defineEntity({...})`, чтобы SDK обнаруживал ваши сущности. Эти функции проверяют вашу конфигурацию на этапе сборки и обеспечивают автодополнение в IDE и безопасность типов.
...
import { defineRole, PermissionFlag } from 'beeax-sdk/define';
...
* См. рабочий пример: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* Вы можете сгенерировать новые объекты с помощью `yarn beeax add`, который проведёт вас через выбор именования, полей и связей.

<Note>
**Базовые поля создаются автоматически.** Когда вы определяете пользовательский объект, BeeAX автоматически добавляет стандартные поля,
...
Используйте `defineField()` для добавления полей к объектам, которые вам не принадлежат — например, к стандартным объектам BeeAX (Person, Company и т. д.). или к объектам из других приложений. В отличие от встроенных полей в `defineObject()`, отдельные поля требуют `objectUniversalIdentifier`, чтобы указать, какой объект они расширяют:
...
Основные моменты:
* `objectUniversalIdentifier` определяет целевой объект. Для стандартных объектов используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`, экспортируемые из `beeax-sdk`.
...
Отношения связывают объекты между собой. В BeeAX отношения всегда двунаправленные — вы определяете обе стороны, и каждая сторона ссылается на другую.
...
Чтобы создать отношение со встроенным объектом BeeAX (Person, Company и т. д.), используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Создание заготовок сущностей с помощью `yarn beeax add`
...
```bash filename="Terminal"
yarn beeax add object
yarn beeax add logicFunction
yarn beeax add frontComponent
```
...
| -------------------- | ------------------------------------ | ------------------------------------------------------- |
| Объект               | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| Поле                 | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| Логическая функция   | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Компонент фронтенда  | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Роль                 | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| Навык                | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| Агент                | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| Представление        | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| Пункт меню навигации | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Макет страницы       | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Что генерирует скэффолдер

У каждого типа сущности есть свой шаблон. Например, `yarn beeax add object` запрашивает:
...
```bash filename="Terminal"
yarn beeax add logicFunction --path src/custom-folder
```
