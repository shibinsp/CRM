...
description: Создавайте компоненты React, которые отображаются внутри интерфейса BeeAX в изолированной песочнице.
icon: window-maximize
---

Фронтенд-компоненты — это компоненты React, которые отображаются непосредственно внутри интерфейса BeeAX. Они выполняются в изолированном Web Worker с использованием Remote DOM — ваш код изолирован (sandboxed), но рендерится нативно на странице, а не в iframe.

## Где можно использовать фронт-компоненты

Фронт-компоненты могут отображаться в двух местах внутри BeeAX:
...
После синхронизации с помощью `yarn beeax dev` (или однократного запуска `yarn beeax dev --once`) быстрое действие появится в правом верхнем углу страницы:
...
Поскольку компонент возвращает `null`, BeeAX пропускает рендеринг контейнера для него — в макете не появляется пустое место. Компонент по-прежнему имеет доступ ко всем хукам и API взаимодействия с хостом.

## Компоненты SDK Command

Пакет `beeax-sdk` предоставляет четыре вспомогательных компонента Command, предназначенных для headless фронт-компонентов. Каждый компонент выполняет действие при монтировании, обрабатывает ошибки, показывая уведомление snackbar, и автоматически размонтирует фронт-компонент по завершении.

Импортируйте их из `beeax-sdk/command`:
...
Логическая функция, объявленная с `httpRouteTriggerSettings`, доступна по HTTP по своему пути маршрута. BeeAX внедряет в воркер базовый URL, с которого обслуживаются ваши функции, в виде `TWENTY_FUNCTIONS_URL` вместе с `TWENTY_APP_ACCESS_TOKEN`, который аутентифицирует вызов. Пока что нет отдельного клиентского SDK для вызова ваших собственных функций, поэтому вызывайте их с помощью обычного `fetch`:

> **В BeeAX Cloud логические функции с HTTP-триггером обслуживаются на выделенном домене для каждого рабочего пространства** по адресу `https://\<your-workspace-subdomain>.beeax.com\<path>` — именно к этому и разрешается `TWENTY_FUNCTIONS_URL`. Для внешних вызовов скопируйте точный URL из настроек **HTTP trigger** функции или на вкладке **Settings** приложения.
...
Путь, добавляемый к `TWENTY_FUNCTIONS_URL`, — это значение `httpRouteTriggerSettings.path` логической функции. Сохраните `isAuthRequired: true`; `TWENTY_APP_ACCESS_TOKEN`, который BeeAX выпускает для вашего компонента, аутентифицирует запрос:
...
### Вызов REST API BeeAX

Чтобы читать или изменять записи BeeAX из фронт-компонента, используйте `RestApiClient` из `beeax-client-sdk/rest`. Он принадлежит к тому же семейству клиентов, что и `CoreApiClient` и `MetadataApiClient`, но нацелен на REST API BeeAX (`/rest/...`) вместо GraphQL API, считывая базовый URL из `TWENTY_API_URL`.
...
| `TWENTY_API_URL`          | Базовый URL основного API BeeAX                                              |
...
Компоненты фронтенда могут вызывать навигацию, модальные окна и уведомления с помощью функций из `beeax-sdk`:
...
* **Компоненты BeeAX UI** — импорт из `beeax-sdk/ui` (Button, Tag, Status, Chip, Avatar и другие)
