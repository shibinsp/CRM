...
description: Интерактивно генерируйте файлы сущностей с помощью yarn beeax dev:add — объекты, поля, представления, логические функции и многое другое.
...
| -------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Объект               | `yarn beeax dev:add object`             | `src/objects/\<name>.ts`               |
| Поле                 | `yarn beeax dev:add field`              | `src/fields/\<name>.ts`                |
| Логическая функция   | `yarn beeax dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Компонент фронтенда  | `yarn beeax dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Роль                 | `yarn beeax dev:add role`               | `src/roles/\<name>.ts`                 |
| Навык                | `yarn beeax dev:add skill`              | `src/skills/\<name>.ts`                |
| Агент                | `yarn beeax dev:add agent`              | `src/agents/\<name>.ts`                |
| Представление        | `yarn beeax dev:add view`               | `src/views/\<name>.ts`                 |
| Пункт меню навигации | `yarn beeax dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Макет страницы       | `yarn beeax dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## Что генерирует скэффолдер

У каждого типа сущности есть свой шаблон. Например, `yarn beeax dev:add object` запрашивает:
