---
title: Понятия
description: Как работают приложения BeeAX — модель сущностей, изоляция в песочнице и жизненный цикл установки.
icon: sitemap
---
...
Приложение — это набор **сущностей**, объявленных с помощью функций `defineEntity()` из пакета `beeax-sdk`. SDK обнаруживает эти объявления посредством анализа AST на этапе сборки и формирует **манифест** — полное описание того, что ваше приложение добавляет в рабочее пространство. Эти функции проверяют вашу конфигурацию на этапе сборки и обеспечивают автодополнение в IDE и безопасность типов.
...
| **Компонент фронтенда**   | Изолированный в песочнице интерфейс React внутри BeeAX | [Компоненты фронтенда](/l/ru/developers/extend/apps/layout/front-components)         |
...
* **`yarn beeax dev:build`** — компилирует TypeScript, упаковывает логические функции и фронтенд-компоненты с помощью esbuild и формирует манифест.
