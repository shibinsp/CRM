...
Для этого не нужен отдельный примитив хранилища: небольшой **технический объект** с полем `key` и полем `value` даёт вам надёжное key-value-хранилище, ограниченное рабочим пространством и доступное для запросов через тот же [типизированный API-клиент](/l/ru/developers/extend/apps/logic/logic-functions#typed-api-clients-beeax-client-sdk), который вы уже используете для записей.
