...
* **Включены в сборки** — `yarn beeax build` упаковывает все публичные ресурсы в выходной дистрибутив.

### Доступ к публичным ресурсам с помощью `getPublicAssetUrl`

Используйте хелпер `getPublicAssetUrl` из `beeax-sdk`, чтобы получить полный URL файла в каталоге `public/` вашего приложения. Он работает как в **логических функциях**, так и в **компонентах фронтенда**.
...
В обеих средах доступны как предварительно предоставленные модули `beeax-client-sdk/core` и `beeax-client-sdk/metadata` — они не включаются в бандл, а подставляются сервером во время выполнения.

## Тестирование вашего приложения

SDK предоставляет программные API, которые позволяют собирать, разворачивать, устанавливать и удалять ваше приложение из тестового кода. В сочетании с [Vitest](https://vitest.dev/) и типизированными клиентами API вы можете писать интеграционные тесты, которые проверяют, что ваше приложение работает сквозным образом на реальном сервере BeeAX.
...
Подпуть `beeax-sdk/cli` экспортирует функции, которые можно вызывать напрямую из тестового кода:
...
Убедитесь, что ваш локальный сервер BeeAX запущен, затем:
...
### Выполнение функций (`yarn beeax exec`)
...
### Просмотр логов функций (`yarn beeax logs`)
...
<Note>
Это отличается от `yarn beeax server logs`, который показывает логи контейнера Docker. `yarn beeax logs` показывает журналы выполнения функций вашего приложения с сервера BeeAX.
</Note>

### Удаление приложения (`yarn beeax uninstall`)
...
**Remote** — это сервер BeeAX, к которому подключается ваше приложение. Во время настройки скэффолдер автоматически создаст его для вас. Вы можете в любой момент добавлять новые удалённые серверы или переключаться между ними.
...
1. Извлекает ваш код
2. Поднимает временный сервер BeeAX с помощью экшена `beeax/beeax/.github/actions/spawn-beeax-docker-image`
...
Вам не нужно настраивать секреты — экшен `spawn-beeax-docker-image` запускает эфемерный сервер BeeAX прямо в раннере и выводит данные для подключения. Секрет `GITHUB_TOKEN` предоставляется GitHub автоматически.

Чтобы закрепить конкретную версию BeeAX вместо `latest`, измените переменную окружения `TWENTY_VERSION` в начале рабочего процесса.
