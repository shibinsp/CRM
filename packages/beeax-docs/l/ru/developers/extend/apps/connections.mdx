...
import { defineConnectionProvider } from 'beeax-sdk/define';
...
export default defineApplication({
  universalIdentifier: '...',
  displayName: 'Linear',
  description: 'Connect Linear to BeeAX.',
  defaultRoleUniversalIdentifier: '...',
  // OAuth client credentials live on the app registration (one OAuth app per
  // BeeAX server, configured by the admin) — not per-workspace. Declare them
...
* Используйте `serverVariables` (не `applicationVariables`) — учетные данные OAuth являются общими для сервера, и на каждом сервере BeeAX используется одно приложение OAuth.
...
```
https://<your-beeax-server>/apps/oauth/callback
```
...
import { listConnections } from 'beeax-sdk/logic-function';
...
4. Откройте установленное приложение в BeeAX под учетной записью администратора сервера → задайте значения в соответствующих `serverVariables`.
