...
<Warning>
**Устанавливаете впервые?** Следуйте [руководству по установке Docker Compose](/l/ru/developers/self-host/capabilities/docker-compose), чтобы запустить BeeAX, затем вернитесь сюда для настройки.
</Warning>

BeeAX предлагает **два режима конфигурации**, чтобы соответствовать разным потребностям развертывания:
...
1. Получите доступ к вашему экземпляру BeeAX (обычно `http://localhost:3000`)
...
[Полная техническая ссылка →](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/beeax-config/config-variables.ts)
</Warning>

## Ключи шифрования

BeeAX использует два ключа шифрования, задаваемых только через переменные окружения:
...
Для обеспечения обратной совместимости, если `ENCRYPTION_KEY` не задан, BeeAX использует `APP_SECRET` для шифрования данных в состоянии покоя — что соответствует устаревшему поведению для более старых развертываний. Для новых установок всегда следует задавать отдельный `ENCRYPTION_KEY`.
...
По умолчанию BeeAX работает в **режиме одного рабочего пространства** — оптимально для большинства развертываний с самостоятельным размещением, где вашей организации нужен один экземпляр CRM.
...
* Одно рабочее пространство на один экземпляр BeeAX
...
Включите режим нескольких рабочих пространств для развертываний, похожих на SaaS, где нескольким независимым командам нужны собственные рабочие пространства на одном экземпляре BeeAX.
...
соответствующий исходный код](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
Пользователи должны иметь [лицензию Microsoft 365](https://admin.microsoft.com/Adminportal/Home), чтобы иметь возможность использовать API Календаря и Сообщений. Они не смогут синхронизировать свою учетную запись в BeeAX без нее.
</Warning>
...
соответствующий исходный код](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
<Warning>
By default, BeeAX stores uploaded files on the local filesystem. Для продакшн-развертываний используйте S3 или совместимый с S3 сервис (MinIO, DigitalOcean Spaces и т. д.). чтобы файлы сохранялись при перезапусках контейнеров и были доступны на нескольких экземплярах сервера при масштабировании.
</Warning>

Установите `STORAGE_TYPE=S_3` и настройте переменные `STORAGE_S3_*` через админ-панель или `.env`. См. [справочник config-variables.ts](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/beeax-config/config-variables.ts) для полного списка переменных S3.

При использовании S3 с функциями, зависящими от CORS (например, загрузки файлов в браузере), убедитесь, что в конфигурации CORS вашего бакета разрешен origin фронтенда BeeAX.

## Логические функции и интерпретатор кода

BeeAX поддерживает логические функции для рабочих процессов и интерпретатор кода для анализа данных ИИ. Оба запускают предоставленный пользователем код и требуют явной настройки в целях безопасности.
...
<Note>
При использовании `LOGIC_FUNCTION_TYPE=DISABLED` или `CODE_INTERPRETER_TYPE=DISABLED` любая попытка выполнения вернёт ошибку. Это полезно, если вы хотите запускать BeeAX без этих возможностей.
</Note>
