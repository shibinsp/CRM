...
```bash
"npx nx run beeax-front:lint # مرر --fix لإصلاح أخطاء التدقيق"
```
...
```bash
"npx nx run beeax-front:test # تشغيل اختبارات jest\nnpx nx run beeax-front:storybook:serve:dev # تشغيل storybook\nnpx nx run beeax-front:storybook:test # تشغيل الاختبارات # (يحتاج إلى تشغيل yarn storybook:serve:dev)\nnpx nx run beeax-front:storybook:coverage # (يحتاج إلى تشغيل yarn storybook:serve:dev)"
```
