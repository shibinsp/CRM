...
1. أوقف BeeAX: `docker compose down`
2. غيّر قيمة `TAG` في ملف `.env` الموجود بجانب ملف `docker-compose.yml`
3. ابدأ تشغيل BeeAX: `docker compose up -d`
...
بدءًا من **v1.22**، يدعم BeeAX الترقيات عبر الإصدارات. يمكنك الانتقال مباشرةً من أي إصدار مدعوم إلى أحدث إصدار دون المرور بكل إصدار وسيط.

على سبيل المثال، الترقية من v1.22 مباشرةً إلى v2.0 مدعومة بالكامل.

## الترقية إلى v2.5+ — غلاف التشفير للبيانات الساكنة (at-rest)

بدءًا من **v2.5**، يقوم BeeAX بتخزين الأسرار أثناء السكون (at-rest) — مثل رموز OAuth، ومتغيرات التطبيق، ومفاتيح التوقيع الخاصة، وقيم الإعدادات الحساسة، وأسرار TOTP — داخل غلاف ذي إصدار `enc:v2:` ومشفّر باستخدام `ENCRYPTION_KEY` (أو `APP_SECRET` إذا لم يتم تعيين `ENCRYPTION_KEY`).
