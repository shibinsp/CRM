...
<Warning>
**هل هي المرة الأولى التي تقوم فيها بالتثبيت؟** اتبع [دليل تثبيت Docker Compose](/l/ar/developers/self-host/capabilities/docker-compose) لتشغيل BeeAX، ثم عد هنا للإعداد.
</Warning>

يوفر BeeAX **وضعين للإعداد** ليلائم احتياجات النشر المختلفة:
...
1. الوصول إلى نسخة BeeAX الخاصة بك (عادة `http://localhost:3000`)
...
[مرجع تقني كامل →](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/beeax-config/config-variables.ts)
</Warning>

## مفاتيح التشفير

تستخدم BeeAX مفتاحَي تشفير يحددان حصراً عبر متغيرات البيئة:
...
لضمان التوافق مع الإصدارات السابقة، إذا لم يتم تعيين `ENCRYPTION_KEY`، فإن BeeAX تستخدم `APP_SECRET` كبديل لتشفير البيانات أثناء التخزين — بما يطابق سلوك الإصدارات القديمة في عمليات النشر الأقدم. يجب على عمليات التثبيت الجديدة دائماً تعيين قيمة مخصصة لـ `ENCRYPTION_KEY`.
...
افتراضيًا، تعمل BeeAX في **وضع مساحة عمل واحدة** — وهو مثالي لمعظم عمليات النشر المستضافة ذاتيًا حيث تحتاج إلى مثيل واحد من نظام إدارة علاقات العملاء لمؤسستك.
...
* مساحة عمل واحدة لكل مثيل من BeeAX
...
قم بتمكين وضع تعدد مساحات العمل لعمليات النشر المشابهة لـ SaaS حيث تحتاج عدة فرق مستقلة إلى مساحات عمل خاصة بها على نفس مثيل BeeAX.
...
[انظر الشيفرة المصدرية ذات الصلة](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
يجب على المستخدمين الحصول على [ترخيص Microsoft 365](https://admin.microsoft.com/Adminportal/Home) ليتمكنوا من استخدام تقويم API ورسائل. لن يتمكنوا من مزامنة حسابهم في BeeAX دون واحد منها.
</Warning>
...
[انظر الشيفرة المصدرية ذات الصلة](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
<Warning>
افتراضياً، تقوم BeeAX بتخزين الملفات المرفوعة على نظام الملفات المحلي. بالنسبة لعمليات النشر في بيئة الإنتاج، استخدم S3 أو خدمة متوافقة مع S3 (MinIO، DigitalOcean Spaces، إلخ) لضمان بقاء الملفات عبر إعادة تشغيل الحاويات وإتاحة التوسّع عبر مثيلات خوادم متعددة.
</Warning>

عيّن `STORAGE_TYPE=S_3` وقم بتهيئة متغيرات `STORAGE_S3_*` من خلال لوحة الإدارة أو `.env`. راجع [مرجع config-variables.ts](https://github.com/beeax/beeax/blob/main/packages/beeax-server/src/engine/core-modules/beeax-config/config-variables.ts) للاطلاع على القائمة الكاملة لمتغيرات S3.

عند استخدام S3 مع الميزات المعتمدة على CORS (مثل تنزيل الملفات داخل المتصفح)، تأكد من أن حاويتك تسمح بأصل واجهة BeeAX الأمامية ضمن تهيئة CORS الخاصة بها.

## الوظائف المنطقية ومفسر الشيفرة

تدعم BeeAX الوظائف المنطقية لعمليات سير العمل ومفسر الشيفرة لتحليل بيانات الذكاء الاصطناعي. كلاهما يقوم بتشغيل الشيفرة المقدمة من المستخدم ويتطلب تهيئة صريحة لأغراض الأمان.
...
<Note>
عند استخدام `LOGIC_FUNCTION_TYPE=DISABLED` أو `CODE_INTERPRETER_TYPE=DISABLED`، سترجع أي محاولة للتنفيذ خطأً. يكون هذا مفيدًا إذا كنت ترغب في تشغيل BeeAX من دون هذه الإمكانات.
</Note>
