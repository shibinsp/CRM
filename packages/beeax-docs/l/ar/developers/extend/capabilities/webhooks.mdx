...
تدفع خطافات الويب البيانات إلى أنظمتك في الوقت الفعلي عند وقوع أحداث في BeeAX — دون الحاجة إلى الاستطلاع الدوري. استخدمها للحفاظ على تزامن الأنظمة الخارجية، وتشغيل الأتمتة، أو إرسال التنبيهات.
...
يرسل BeeAX خطافات الويب لأنواع الأحداث التالية:
...
يقوم BeeAX بتوقيع كل طلب خطاف ويب لأغراض الأمان. تحقّق من التواقيع للتأكد من أن الطلبات أصيلة.

### الرؤوس

| رأس الصفحة                   | الوصف               |
| ---------------------------- | ------------------- |
| `X-BeeAX-Webhook-Signature` | توقيع HMAC SHA256   |
| `X-BeeAX-Webhook-Timestamp` | الطابع الزمني للطلب |

### خطوات التحقق

1. احصل على الطابع الزمني من `X-BeeAX-Webhook-Timestamp`
2. أنشئ السلسلة: `{timestamp}:{JSON payload}`
3. احسب HMAC SHA256 باستخدام سر خطاف الويب الخاص بك
4. قارِن مع `X-BeeAX-Webhook-Signature`
...
| **مشغّل خطاف ويب لسير العمل** | IN      | استقبال البيانات في BeeAX من الأنظمة الخارجية              |
