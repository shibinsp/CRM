---
title: المفاهيم
description: كيفية عمل تطبيقات BeeAX — نموذج الكيان، العزل (sandboxing)، ودورة حياة التثبيت.
icon: sitemap
---

تطبيقات BeeAX هي حزم TypeScript توسّع مساحة عملك بكائنات مخصّصة، ومنطق، ومكوّنات واجهة مستخدم (UI)، وقدرات ذكاء اصطناعي. تعمل على منصة BeeAX مع عزل كامل وضوابط الأذونات.

## كيف تعمل التطبيقات

التطبيق عبارة عن مجموعة من **الكيانات** يتم إعلانها باستخدام دوال `defineEntity()` من حزمة `beeax-sdk`. يكتشف SDK هذه التصريحات عبر تحليل AST وقت البناء وينتج **ملف بيان** — وصفًا كاملًا لما يضيفه تطبيقك إلى مساحة العمل. تتحقق هذه الدوال من تكوينك وقت البناء وتوفّر إكمالًا تلقائيًا في بيئة التطوير وأمان الأنواع.
...
| **مكوّن أمامي**        | واجهة مستخدم React معزولة داخل BeeAX                            | [المكوّنات الأمامية](/l/ar/developers/extend/apps/layout/front-components)       |
...
* **المكوّنات الأمامية** تعمل ضمن Web Workers باستخدام Remote DOM — معزولة عن الصفحة الرئيسية لكنها تعرض عناصر DOM الأصلية (وليس iframes). تتواصل مع BeeAX عبر واجهة API للمضيف تعتمد تمرير الرسائل.
...
* **`yarn beeax dev`** — يراقب ملفات المصدر لديك ويزامن التغييرات مباشرةً إلى خادم BeeAX متصل. يُعاد توليد عميل API مضبوط الأنواع تلقائيًا عند تغيّر المخطط.
* **`yarn beeax dev:build`** — يجمّع TypeScript، ويضمّن الدوال المنطقية والمكوّنات الأمامية باستخدام esbuild، وينتج ملف بيان.
