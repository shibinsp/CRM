...
description: عرّف الكائنات والحقول والأدوار وبيانات تعريف التطبيق باستخدام SDK الخاصة بـ BeeAX.
icon: database
---

توفر حزمة `beeax-sdk` دوالّ `defineEntity` لتعريف نموذج بيانات تطبيقك. يجب عليك استخدام `export default defineEntity({...})` لكي يكتشف SDK الكيانات الخاصة بك. تتحقق هذه الدوال من تكوينك وقت البناء وتوفّر إكمالًا تلقائيًا في بيئة التطوير وأمان الأنواع.
...
* اطّلع على مثال عملي: [`hello-world/src/roles/function-role.ts`](https://github.com/beeax/beeax/blob/main/packages/beeax-apps/hello-world/src/roles/function-role.ts).
...
* يمكنك إنشاء كائنات جديدة باستخدام `yarn beeax add`، والذي يرشدك خلال التسمية والحقول والعلاقات.

<Note>
**يتم إنشاء الحقول الأساسية تلقائيًا.** عند تعريف كائن مخصص، يضيف BeeAX تلقائيًا حقولًا قياسية
...
استخدم `defineField()` لإضافة حقول إلى كائنات لا تملكها — مثل كائنات BeeAX القياسية (Person, Company, etc.) أو كائنات من تطبيقات أخرى. على خلاف الحقول المضمّنة في `defineObject()`، تتطلّب الحقول المستقلة `objectUniversalIdentifier` لتحديد الكائن الذي تقوم بتوسيعه:
...
النقاط الرئيسية:
* `objectUniversalIdentifier` يحدّد الكائن الهدف. بالنسبة للكائنات القياسية، استخدم `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` المُصدَّر من `beeax-sdk`.
...
تربط العلاقات الكائنات معًا. في BeeAX، تكون العلاقات دائمًا **ثنائية الاتجاه** — حيث تعرّف الجانبين، ويشير كل جانب إلى الآخر.
...
لإنشاء علاقة مع كائن BeeAX مضمّن (Person, Company, etc.)، استخدم `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## توليد قوالب الكيانات باستخدام `yarn beeax add`
...
| ------------------ | ------------------------------------ | ------------------------------------------------------- |
| كائن               | `yarn beeax add object`             | `src/objects/\<name>.ts`               |
| الحقل              | `yarn beeax add field`              | `src/fields/\<name>.ts`                |
| دالة منطقية        | `yarn beeax add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| مكوّن أمامي        | `yarn beeax add frontComponent`     | `src/front-components/\<name>.tsx`     |
| دور                | `yarn beeax add role`               | `src/roles/\<name>.ts`                 |
| مهارة              | `yarn beeax add skill`              | `src/skills/\<name>.ts`                |
| وكيل               | `yarn beeax add agent`              | `src/agents/\<name>.ts`                |
| عرض                | `yarn beeax add view`               | `src/views/\<name>.ts`                 |
| عنصر قائمة التنقّل | `yarn beeax add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| تخطيط الصفحة       | `yarn beeax add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### ما الذي تُنشئه أداة القوالب

لكل نوع كيان قالب خاص به. على سبيل المثال، يسأل `yarn beeax add object` عن:
