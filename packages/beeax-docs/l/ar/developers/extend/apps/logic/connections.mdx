...
* استخدم `serverVariables` (وليس `applicationVariables`) — بيانات اعتماد OAuth على مستوى الخادم، ويوجد تطبيق OAuth واحد لكل خادم BeeAX.
...
4. افتح التطبيق المُثبَّت في BeeAX كمسؤول خادم → عيّن القيم على `serverVariables` المقابلة.
