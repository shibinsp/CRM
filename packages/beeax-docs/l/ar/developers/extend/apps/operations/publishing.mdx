...
يقوم هذا بتجميع مصادر TypeScript، وتحويل دوال المنطق ومكوّنات الواجهة الأمامية، وكتابة كل شيء إلى `.beeax/output/`. أضِف `--tarball` لإنتاج حزمة `.tgz` أيضًا للتوزيع اليدوي أو لأمر publish.
...
2. شغّل `yarn beeax app:publish --private` (أو `yarn beeax app:publish --private --remote production`)
...
إذا كان تطبيقك يستخدم ميزة تم تقديمها في إصدار معيّن من خادم BeeAX (على سبيل المثال، موفّرو OAuth الذين أضيفوا في v2.3.0)، فيجب عليك التصريح بأدنى إصدار من الخادم يتطلبه تطبيقك باستخدام الحقل `engines.beeax` في `package.json`:
...
* إذا تم تعيين `engines.beeax` ولم يستوفِ إصدار الخادم الهدف النطاق، فسيتم رفض النشر (tarball upload) أو التثبيت بخطأ `SERVER_VERSION_INCOMPATIBLE` ورسالة تُشير إلى كلٍ من النطاق المطلوب وإصدار الخادم الفعلي.
* إذا كان `engines.beeax` **غير مُعين**، فسيُقبل التطبيق على أي إصدار من الخادم (متوافق مع الإصدارات السابقة للتطبيقات الحالية).
* إذا لم يكن لدى الخادم قيمة `APP_VERSION` مُكوَّنة، فسيتم تخطي الفحص.

<Note>
الخادم هو الجهة المرجعية للفحص — إذ يتحقق من `engines.beeax` عند كلٍ من رفع tarball وتثبيت مساحة العمل. إذا قمت بنشر tarball خارج القناة المعتادة أو التثبيت من السوق، فسيظل الخادم يفرض التوافق.
</Note>
...
1. يجلب مصدر تطبيقك.
2. ينشئ مثيلاً اختبارياً معزولاً من BeeAX باستخدام الإجراء المركّب `beeax/beeax/.github/actions/spawn-beeax-app-dev-test@main` (المكافئ في CI للأمر `yarn beeax docker:start --test`).
...
2. يشغّل `beeax/beeax/.github/actions/deploy-beeax-app@main` — وهو المكافئ في CI للأمر `yarn beeax app:publish --private`.
...
بالنسبة لأنظمة CI الأخرى (GitLab CI، وCircleCI، إلخ)، تنطبق الأوامر الثلاثة نفسها: `yarn install`، ثم `yarn beeax dev:build`، ثم `npm publish` من `.beeax/output`.
...
لتثبيت إصدار أحدث، انشره (deploy) أو انشره إلى السجل (publish) أولًا، ثم أعد تشغيل `yarn beeax app:install`.
</Note>
