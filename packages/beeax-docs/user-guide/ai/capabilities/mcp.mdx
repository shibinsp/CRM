---
title: MCP Server
description: Connect AI assistants to your BeeAX workspace using the Model Context Protocol.
---
...
BeeAX exposes an [MCP](https://modelcontextprotocol.io/) server so that AI assistants — Claude Desktop, Claude Code, Cursor, ChatGPT, and others — can read and write your CRM data through natural language.

Use your **workspace URL** (the URL you use to access BeeAX) as the MCP endpoint. On BeeAX Cloud, your workspace URL might be `https://{mycompany}.beeax.com` or a custom domain. The server is available at:

| Environment | MCP Endpoint |
|-------------|-------------|
| **Cloud** | `https://{your-workspace-url}/mcp` (e.g. `https://mycompany.beeax.com/mcp`) |
...
Add this to your MCP client configuration, replacing `{your-workspace-url}` with your workspace host (e.g. `mycompany.beeax.com`):
...
1. Discover BeeAX's OAuth metadata via `/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server`
...
Go to **Settings > AI > More > MCP Server** in BeeAX. Choose your authentication method (OAuth or API Key), copy the JSON snippet (it will already use your workspace URL), and paste it into your MCP client's config file.
...
Restart your MCP client (or reload the config). If using OAuth you will be redirected to BeeAX to authorize access. If using an API key the connection is immediate.
...
Once connected, the MCP server exposes tools that mirror the BeeAX API. The recommended workflow is:
...
For self-hosted instances, replace `{your-workspace-url}` with your server URL. Make sure `SERVER_URL` in your environment matches the public URL of your BeeAX instance — this is used to generate the OAuth discovery metadata.
