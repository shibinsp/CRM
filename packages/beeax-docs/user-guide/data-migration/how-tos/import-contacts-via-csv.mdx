...
description: Complete step-by-step guide to importing people/contacts into BeeAX.
---

## Overview

This guide walks you through importing your contacts (People) into BeeAX. **Import Companies first** if you want to link People to Companies.
...
- **Must be unique** — no duplicates in your file or in BeeAX
...
<Note>
**Use Domain URL format** (`https://acme.com`), not the label. This matches how Companies are stored in BeeAX.
</Note>
...
<Note>
**Pro tip:** Click **Download sample file** during import or export a few existing People to see the exact column names BeeAX expects.
</Note>
...
3. Wait for BeeAX to analyze your file

## Step 4: Map Your Columns

BeeAX automatically tries to match your columns to fields. Review and adjust:
...
3. BeeAX will link each Person to the matching Company
...
1. BeeAX shows your values alongside existing options
2. Match each value in your file to a BeeAX option
...
Before completing the import, BeeAX validates your data:
...
|-------|-------|----------|
| **Duplicate email** | Email already exists in BeeAX or file | Remove duplicate or update existing record |
...
Your contacts are now in BeeAX! Next steps:
...
2. BeeAX matches records by this unique identifier
...
<AccordionGroup>
  <Accordion title="Why can't two People have the same email?">
    Email is a unique identifier in BeeAX. This prevents duplicate contacts and ensures email sync correctly links emails to the right person.
  </Accordion>
  ...
  <Accordion title="Can I import the ID from my previous CRM?">
    Yes! Create a custom field marked as "unique" in your data model to store the external ID. Note: the field name `id` is reserved for BeeAX's internal ID.
  </Accordion>
