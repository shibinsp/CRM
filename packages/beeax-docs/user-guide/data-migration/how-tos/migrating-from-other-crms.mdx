...
description: Step-by-step guide to migrate your data from any CRM to BeeAX.
---

## Overview

This guide walks you through migrating your data from any CRM to BeeAX. The process involves auditing your data, preparing your BeeAX workspace, exporting from your current system, and importing into BeeAX.
...
Create a mapping document between your current CRM and BeeAX:
...
## Step 3: Set Up Your BeeAX Workspace

Before importing data, prepare your BeeAX workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in BeeAX before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for BeeAX.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in BeeAX already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches BeeAX's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in BeeAX
...
- BeeAX will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in BeeAX.
  </Accordion>
...
Contact [contact@beeax.com](mailto:contact@beeax.com) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
