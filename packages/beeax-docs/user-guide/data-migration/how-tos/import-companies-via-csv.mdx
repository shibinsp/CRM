...
description: Complete step-by-step guide to importing companies into BeeAX.
---

## Overview

This guide walks you through importing your companies into BeeAX. **Companies should be imported first** because People and Opportunities link to Companies.
...
<Note>
**Pro tip:** Click **Download sample file** during import to see the exact column names BeeAX expects.
</Note>
...
|-------|-------|----------|
| **Duplicate domain** | Domain already exists in BeeAX | Remove from file or update existing record |
...
<AccordionGroup>
  <Accordion title="Why can't two Companies have the same domain?">
    Domain is a unique identifier in BeeAX. This prevents duplicate companies and ensures email sync correctly links emails to the right company.
  </Accordion>
  ...
  <Accordion title="What happens if I import a domain that already exists?">
    If you include a unique identifier (domain or id) that matches an existing company, BeeAX updates that company instead of creating a duplicate.
  </Accordion>
