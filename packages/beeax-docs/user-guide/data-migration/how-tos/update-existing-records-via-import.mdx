...
When you import a file containing a **unique identifier** that matches an existing record, BeeAX updates that record instead of creating a duplicate.

| If unique identifier... | BeeAX will... |
|-------------------------|----------------|
...
4. **Don't change unique identifier values** — or BeeAX will create new records
...
If you change `john@acme.com` to `john.smith@acme.com`, BeeAX will create a new record instead of updating the existing one.
</Warning>
...
BeeAX matches records by the unique identifier and updates them with new values.
...
3. BeeAX will match on that field
...
| **Case mismatch** | File has `John@acme.com` but BeeAX has `john@acme.com` | Creates new record | Export from BeeAX to get exact values |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which records will be updated vs. created?">
    During the import review step, BeeAX shows you how many records will be updated vs. created based on unique identifier matches.
  </Accordion>
