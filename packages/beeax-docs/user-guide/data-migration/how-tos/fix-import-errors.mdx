...
After uploading your file and mapping columns, BeeAX validates your data:

1. **Validation runs** — BeeAX checks each row for errors
...
|---------------|---------------|
| Duplicate values highlighted in yellow | Value already exists in BeeAX or appears twice in your file |
...
**Option 3: Let BeeAX update the existing record**
1. Ensure your file includes a unique identifier (`email`, `domain`, or `id`)
2. Map the unique identifier field
3. BeeAX will update the existing record instead of creating a duplicate
...
As long as one unique identifier matches, BeeAX updates the record.
</Note>
...
3. Check if records already exist in BeeAX
...
This means there is no Company in BeeAX with that specific identifier.
...
### What It Means
You're trying to link to a record that doesn't exist in BeeAX.
...
### What It Means
A column in your file can't be mapped because the field doesn't exist in BeeAX.
...
1. **User hasn't accepted their invitation** — the user doesn't exist in BeeAX yet
2. **Using user ID from old system** — BeeAX can't match IDs from another system
3. **Wrong email format** — the email doesn't match the user's BeeAX account

### How to Fix

1. Ensure all users have **accepted their invitation** to your BeeAX workspace
2. Use the user's **email address** (not their name or old system ID)
3. Use the same email they used to join BeeAX
...
<Check>Domain format matches BeeAX's format exactly</Check>
...
1. **Download the sample file** — see the exact format BeeAX expects
