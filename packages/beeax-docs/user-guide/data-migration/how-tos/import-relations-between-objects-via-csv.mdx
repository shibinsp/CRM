...
The parent ("one" side) must exist in BeeAX before you can reference it.
...
If you need the `id`:
1. Export the parent records from BeeAX
...
Use `https://domain.com` (not just `domain.com`). This matches how BeeAX stores Company domains and prevents matching errors.
</Warning>
...
BeeAX will automatically link each child record to the matching parent.
...
| **Non-existent records** | The parent record doesn't exist in BeeAX | Import parent records first, or check for typos |
| **Case mismatch** | `Acme.com` in file but `acme.com` in BeeAX | Ensure exact case matching |
...
<AccordionGroup>
  <Accordion title="What if my parent record doesn't have a domain or email?">
    ...
    1. Use the BeeAX `id` (export parent records to get their IDs)
