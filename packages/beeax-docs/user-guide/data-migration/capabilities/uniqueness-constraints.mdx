...
description: How BeeAX enforces data uniqueness during import.
---
import { VimeoEmbed } from '/snippets/vimeo-embed.mdx';

## Overview

BeeAX enforces uniqueness on certain fields to prevent duplicate records and ensure data integrity. Understanding these constraints is essential for successful imports.
...
<Note>The `id` field is BeeAX's internal identifier, auto-generated for each record. It uses UUID format (e.g., `c776ee49-f608-4a77-8cc8-6fe96ae1e43f`).</Note>
...
<Note>The field name `id` is reserved for BeeAX's internal ID. Use a different name like `externalId` or `legacyId` for external identifiers.</Note>
...
2. **Check for existing records** in BeeAX before importing
