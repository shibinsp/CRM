---
title: Mailbox
description: Understanding email integration features in BeeAX.
---
...
BeeAX automatically links emails from your connected mailboxes to the relevant CRM records, keeping all communication history in one place.
...
### Sync Rate
BeeAX imports emails at approximately **400 messages per minute**. This rate is currently limited by Gmail API rate limiting (and applies similarly to other providers).
...
- **Missing Emails**: Check if:
  ...
  - Contact auto-creation is disabled (emails need existing BeeAX records)
