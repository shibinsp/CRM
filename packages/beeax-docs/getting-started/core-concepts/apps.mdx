...
title: Apps
icon: "cube"
description: Extend BeeAX with code — custom objects, server-side logic, UI components, and AI agents, all as TypeScript packages.
---


Most CRMs give you a config panel. BeeAX gives you a platform. Apps are how developers extend BeeAX beyond what the UI offers — defining data models, server-side logic, UI components, and AI capabilities as code, then deploying them to one or more workspaces.
...
An app is a TypeScript package that declares **entities** using the `beeax-sdk`:
...
| **Front Components** | Sandboxed React components that render inside BeeAX's UI (side panel, widgets, command menu) |
...
You write your app as a TypeScript project on your machine. The CLI watches your source files and live-syncs them to a running BeeAX server — edit a file, see the change in the UI within a second. The typed API client regenerates automatically when the schema changes. When you're ready, `yarn beeax app:publish --private` pushes to a production server, or `yarn beeax app:publish` lists your app on npm and the BeeAX marketplace.
