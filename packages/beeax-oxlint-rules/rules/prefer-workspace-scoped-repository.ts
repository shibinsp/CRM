  ...
  meta: {
    ...
    messages: {
      preferWorkspaceScopedRepository:
        'Use @InjectWorkspaceScopedRepository({{entityName}}) instead of raw @InjectRepository so the workspaceId guard is enforced. If {{entityName}} genuinely does not fit, add it to EXCLUSIONS or suppress with `// eslint-disable-next-line beeax/prefer-workspace-scoped-repository` and a short reason.',
    },
  },
  create: (context) => {
    return {
