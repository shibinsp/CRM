---
name: beeax-record-presentation
description: "Retrieve and present BeeAX CRM records as readable summaries or tables, using the connected BeeAX MCP server to discover fields, fetch relevant data, format dates and values, build record links, and avoid raw API output."
---

# BeeAX Record Presentation
...
- If no BeeAX MCP tools are available, say that no callable BeeAX MCP server is available in the current thread and ask the user to connect or expose the intended workspace.
...
- For absolute links, combine the workspace origin with that path, for example `https://example.beeax.com/object/person/record-id`.
...
| :--- | :--- | ---: | :--- |
| [Acme renewal](https://example.beeax.com/object/opportunity/record-id-1) | Negotiation | EUR 12,450 | May 5, 2026, 11:43 AM |
| [Globex expansion](https://example.beeax.com/object/opportunity/record-id-2) | Discovery | EUR 8,000 | May 4, 2026, 4:10 PM |
```
