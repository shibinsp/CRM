...
All notable changes to the BeeAX Codex plugin are documented in this file.
...
Entries reference the canonical skills (`create-app`, `develop-app`, `manage-app`, `publish-app`, `use-beeax-mcp`) and the validation script (`scripts/validate.js`).

## [0.1.0] - Unreleased

The first version of the BeeAX Codex plugin.
...
- `references/develop-app/tests.md` and `references/manage-app/cli-and-sync.md`: documented that integration tests must run against the isolated test instance (`yarn beeax docker:start --test`, port `2021`) instead of the dev instance, since the test harness installs and uninstalls the app on its target server.
...
- `.mcp.json` declaring only the public `beeax-docs` MCP server at `https://docs.beeax.com/mcp`.
- `assets/beeax-logo.png` and `assets/beeax-logo.svg` for marketplace branding.

### Skills (`skills/`)
- `create-app` — scaffold a new BeeAX app via `create-beeax-app`, with prompts for the BeeAX instance URL and Docker vs existing-instance choice.
- `develop-app` — add or modify BeeAX app entities (objects, fields, logic functions, roles, views, layouts, skills, agents, connection providers, front components); enforces `yarn beeax dev:add` for entity generation and one-shot `yarn beeax dev --once` sync.
...
- `use-beeax-mcp` — workspace MCP connection, record retrieval, and readable Markdown output with linked record names.
...
- `design/front-component-ui.md` — visual design rules, BeeAX UI defaults, design tokens, component selection.
- `manage-app/cli-and-sync.md` — CLI command semantics, sync modes, build, deploy, logs, CI/CD; explicit ban on watch mode (`yarn beeax dev` without `--once`) for agent use.
...
- `use-beeax-mcp/result-formatting.md` — record link building, date formatting, readable Markdown contract.
