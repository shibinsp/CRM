# BeeAX Codex Plugin — Best Practices Compliance Checklist
...
1. `yarn workspace beeax-codex-plugin validate` exits 0 (all `[automated]` rows green).
...
| M12 | `interface.brandColor` matches `#RRGGBB` and reflects BeeAX brand | Build guide §Marketplace | [automated] `assertInterfaceFields` (planned), [manual] brand alignment |
...
| R2 | `use-beeax-mcp` formatting contract intact across SKILL.md and result-formatting.md | Internal convention | [automated] `assertTwentyMcpFormattingContract` |
...
| # | Requirement | Source | Check |
|---|---|---|---|
| MCP1 | `.mcp.json` declares only the public `beeax-docs` server | Internal convention | [automated] `assertJsonMetadata` |
...
| D2 | Optional repo-level `.agents/plugins/marketplace.json` points at `./packages/beeax-codex-plugin` | Build guide §Distribution | [automated] `assertJsonMetadata` |
...
| 2026-05-28 | M12 | Codex plugin compliance pass | `brandColor: #000000` matches the actual logo background in `assets/beeax-logo.svg`. |
