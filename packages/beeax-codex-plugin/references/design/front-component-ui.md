...
Use this when the user wants to design or polish a BeeAX front component UI.
...
Design front components as compact CRM product UI, not marketing pages. The goal is a surface that scans quickly, feels native inside BeeAX, and keeps actions predictable.

Use Figma measurements as evidence for the current BeeAX rhythm, not as implementation literals. Source code should use BeeAX primitives, component props, and `themeCssVariables` instead of hardcoded visual values.

## BeeAX UI Defaults

Prefer BeeAX UI primitives for CRM-native front component UI:
...
Use local inline styles for layout containers and custom data displays, but keep them aligned with BeeAX tokens. Use `front-components.md` for exact imports and runtime rules.

Use BeeAX UI icons before custom SVG. Use `themeCssVariables.icon.size.md` for default row and action icons, `themeCssVariables.icon.size.sm` for quiet chevrons, and `themeCssVariables.spacing[6]` for icon-only action targets.
...
- Icons use `themeCssVariables.icon.size.*` or native BeeAX UI icon sizing.
- Borders use `themeCssVariables.border.color.*`; avoid raw border colors.
- Shadows use `themeCssVariables.boxShadow.*`; avoid custom shadow stacks unless mirroring an existing BeeAX overlay exactly.
...
- Compact controls: use BeeAX `Button` sizing or `themeCssVariables.spacing[6]` for custom control height.
...
- Checkbox shape and target sizing should come from the native BeeAX checkbox pattern when available.
- Do not use pill or rounded radius for cards, rows, or toolbar buttons unless the existing BeeAX primitive already does.
- Do not mix several custom radii in one component. Pick the matching BeeAX radius token for each element role.
...
BeeAX UI looks precise because repeated edges line up:
...
- Use BeeAX tokens for color and spacing. Use raw values only for semantic data visualization that cannot be represented by existing tokens.

State backgrounds should follow the existing BeeAX pattern:
...
- Selected: same shape and height as the default row, with selected color treatment from the nearest BeeAX primitive.
...
Use actions that feel like BeeAX toolbar controls:

- Use `Button` for actual commands.
- Use BeeAX button `size` and `variant` props before custom button styling.
...
Make text fit in real BeeAX widths:
...
- The component uses BeeAX typography tokens, not custom text sizes.
- Cards use the medium radius token or a smaller existing primitive radius.
- Buttons and action controls use BeeAX `Button` props or compact spacing tokens.
- Table/list rows use a consistent row-height token.
- Icons use `themeCssVariables.icon.size.*` or native BeeAX icon sizing.
- Spacing uses `themeCssVariables.spacing[...]` and `themeCssVariables.betweenSiblingsGap`.
...
- The UI uses BeeAX primitives and `themeCssVariables` before custom styling.
...
2. Lock the structural dimensions with BeeAX primitives and `themeCssVariables`.
3. Align repeated edges and action positions.
4. Apply BeeAX primitives and tokens.
