...
Tests use the `*.spec.ts` extension and live in sibling `__tests__/` folders next to the code they cover, matching BeeAX backend conventions. Write them where `yarn beeax dev --once` does not validate correctness.
...
- Mappers in `src/utils/` — foreign-to-BeeAX mapping, including the "no match" case.
...
The seeded default `TWENTY_API_KEY` works for both instances, so only the URL needs overriding. This mirrors CI, which spawns the same isolated instance via the `spawn-beeax-app-dev-test` action.
