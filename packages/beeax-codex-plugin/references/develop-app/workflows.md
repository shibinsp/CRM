...
BeeAX workflows are workspace records, not app entities — no `define*` primitive. Ship them via `definePostInstallLogicFunction` and the workspace API.
...
Find by deterministic name or slug before creating. Single-record queries throw on absence in some BeeAX versions — treat not-found as "needs create."
...
`yarn beeax dev --once` skips install hooks. Run again after rebuilding to verify idempotency.
