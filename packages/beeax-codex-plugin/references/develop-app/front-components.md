...
Use this reference for BeeAX app front component source files, registration, data access, BeeAX UI imports, runtime imports, and browser verification.
...
- Use `beeax-sdk/front-component` for front component hooks and host APIs.
- Use `beeax-client-sdk/core` or `beeax-client-sdk/metadata` for data access.
- Use `beeax-sdk/ui` for BeeAX UI components, icons, and theme tokens before adding external UI libraries.
- Do not import from `beeax-ui` directly. Use the SDK re-export so build aliases and runtime packaging stay aligned.

The front component renderer provides a BeeAX `ThemeProvider` around the remote root. For isolated examples or local story-style verification, wrapping the component in `ThemeProvider` from `beeax-sdk/ui` is also acceptable.

The canonical BeeAX UI front component example is `packages/beeax-front-component-renderer/src/__stories__/example-sources/beeax-ui-example.front-component.tsx`. It imports `Button`, `Chip`, `H2Title`, `Status`, `Tag`, and `ThemeProvider` from `beeax-sdk/ui`.

When using `themeCssVariables`, compute styles inside the component body or a function called by the component. The SDK mocks `beeax-sdk/ui` during manifest extraction, so module-level constants that dereference `themeCssVariables` can fail before the component renders.

Prefer BeeAX UI icons from `beeax-sdk/ui` when one exists. Use inline SVG only for app-specific marks or icons that are not available through the SDK export.
...
Use the generated or core BeeAX client for reads and writes. Keep loading, empty, error, disabled, and saving states explicit so runtime failures are visible and recoverable.
...
Inside `records`, prefer `id` for the BeeAX record ID because the array name already establishes record context. Do not add flat `recordId` payloads unless the user explicitly asks for a single-record action.
...
A clean typecheck and sync is not runtime verification. After the standard validation in `app-structure.md`, open the relevant BeeAX surface and confirm:
