...
Use this reference for BeeAX app logic functions, skills, agents, and connection providers.
...
- Use `id` inside a `records` array for the BeeAX record ID. Do not add `recordId`, object-specific IDs such as `companyId`, or flat single-record payloads unless the user explicitly requests a single-record contract.
...
Do not write fields BeeAX computes elsewhere (workflow `statuses` is computed from version status — see `workflows.md`).

Dev sync skips install hooks. Invoke locally:

```bash
yarn beeax dev:function:exec
```
