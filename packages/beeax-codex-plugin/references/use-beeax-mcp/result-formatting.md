...
Retrieve the BeeAX records needed to answer the user's question, then present them as a useful answer, not as raw API output. Always translate technical fields, timestamps, IDs, and nested structures into readable summaries that help the user scan, compare, and act.

## Retrieval Workflow

Resolve the intended workspace before selecting a BeeAX MCP server:

- If the user names a workspace, host, MCP server, or URL, use only the matching BeeAX MCP namespace or server.
- If multiple BeeAX MCP namespaces or configured servers are available and the intended workspace is ambiguous, ask one concise clarifying question before retrieving data.
- If exactly one BeeAX MCP namespace is available and the user did not specify a workspace, use it and mention which workspace or server was used when reporting results.
...
- Before querying workspace data, confirm the callable BeeAX MCP namespace or server name corresponds to the intended workspace whenever there is any ambiguity.

Use the selected connected BeeAX MCP server when it is available:
...
- If no BeeAX MCP tools are available, use the setup workflow in `setup.md`; do not invent workspace data.

## Workspace Origin

Record links need a workspace origin, such as `https://example.beeax.com` or `http://workspace.localhost:3001`.
...
Link records back to their original BeeAX context whenever the workspace origin and record identity are known.

- Build record links with the BeeAX show-page path: `/object/:objectNameSingular/:objectRecordId`.
...
- BeeAX AI chat currently renders Markdown with `react-markdown` and `remark-gfm`, without raw HTML rendering. Do not rely on HTML such as `<img width="16" height="16">`.
- Use standard Markdown image syntax only: `![alt](image-url)`.
- The current BeeAX AI chat image CSS preserves intrinsic image size with `height: auto` and only caps `max-width`. Do not place full-size photos or large avatars in tables unless the image URL is already a small thumbnail.
...
Prefer the label users see in BeeAX when it is available from metadata. Otherwise, split camelCase, snake_case, and kebab-case into normal words.
...
- Prefer natural links over extra link columns: link the record name to BeeAX, and link the domain or email only when that external destination is useful.
...
| :---: | :--- | :--- | ---: | :--- |
| ![Acme icon](https://example.com/favicon.ico) | [Acme renewal](https://example.beeax.com/object/opportunity/record-id-1) | Negotiation | EUR 12,450 | May 5, 2026, 11:43 AM |
| ![Globex icon](https://globex.example/favicon.ico) | [Globex expansion](https://example.beeax.com/object/opportunity/record-id-2) | Discovery | EUR 8,000 | May 4, 2026, 4:10 PM |
```
...
```markdown
**[Acme renewal](https://example.beeax.com/object/opportunity/record-id-1)**
