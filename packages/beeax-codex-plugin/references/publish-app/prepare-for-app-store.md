...
Use this reference for the publish-facing documentation and visuals of a BeeAX app. It complements `create-app`; read this when the app is ready for packaging, marketplace listing, npm publishing, private sharing, or user-facing review.

## When To Use

Use this when the user asks to:

- Write or improve a BeeAX app `README.md`.
- Prepare npm package content or BeeAX marketplace copy.
...
BeeAX marketplace metadata comes from `defineApplication()`:
...
One-sentence summary of what the app adds to BeeAX.
...
- BeeAX server/version requirement, if any.
...
4. Open the relevant BeeAX view or app settings page.
...
- Name the BeeAX objects and views users will actually interact with.
...
- Do not use the BeeAX logo as the app logo unless the app is officially owned by BeeAX and the user intends that.

Screenshot guidance:

- Capture real app surfaces after `yarn beeax dev --once` or watch-mode sync.
...
If the app requires a specific BeeAX server version, set `engines.beeax` in `package.json`. If publishing to npm, add the `beeax-app` keyword.
...
- `package.json` has `keywords: ["beeax-app"]` when the app is intended for npm marketplace publishing.
