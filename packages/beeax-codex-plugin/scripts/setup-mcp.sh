...
Examples:
  setup-mcp.sh acme.beeax.com
  setup-mcp.sh --force-login https://crm.example.com
  setup-mcp.sh --name beeax-local acme.localhost:3001
  setup-mcp.sh --name beeax-prod https://crm.example.com/mcp
...
if [[ -z "$url" ]]; then
  echo "A BeeAX workspace URL is required, for example: https://crm.example.com" >&2
  echo
  usage
  exit 1
fi
