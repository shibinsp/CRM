...
const assertTwentyMcpFormattingContract = (fail) => {
  const skillPath = path.join(PLUGIN_ROOT, 'skills/use-beeax-mcp/SKILL.md');
  const resultFormattingPath = path.join(PLUGIN_ROOT, 'references/use-beeax-mcp/result-formatting.md');
  ...
  const requiredSkillFragments = [
    ...
    'MUST link each display name back to BeeAX',
  ...
  for (const fragment of requiredSkillFragments) {
    if (!skill.includes(fragment)) {
      fail(`use-beeax-mcp/SKILL.md is missing formatting contract fragment: ${fragment}`);
    }
  }
...
const assertFrontComponentGuidance = (fail) => {
  ...
  const requiredDevelopSkillFragments = [
    ...
    'BeeAX UI imports',
    'Use `layout.md` for placement, `standalone-pages.md` for full-page custom UI, and `front-component-ui.md` for visual design and BeeAX UI component selection',
  ];
  ...
  const requiredFrontComponentFragments = [
    ...
    'Use `beeax-sdk/front-component`',
    'Use `beeax-client-sdk/core` or `beeax-client-sdk/metadata`',
    'Use `beeax-sdk/ui` for BeeAX UI components',
    'Do not import from `beeax-ui` directly',
    'ThemeProvider',
    'example-sources/beeax-ui-example.front-component.tsx',
    'themeCssVariables',
    'mocks `beeax-sdk/ui` during manifest extraction',
...
const assertCliGuidanceSplit = (fail) => {
  ...
  const forbiddenAppStructureFragments = [
    ...
    'Do not run `yarn beeax dev:typecheck`',
  ...
  const requiredCliFragments = [
    ...
    'Do not use bare `yarn beeax dev` (watch mode)',
    'yarn beeax dev --once --verbose',
    'yarn beeax remote:list',
