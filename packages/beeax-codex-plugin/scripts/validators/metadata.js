...
const assertJsonMetadata = (fail) => {
  ...
  const packageJson = readJson('packages/beeax-codex-plugin/package.json');
  const pluginJson = readJson('packages/beeax-codex-plugin/.codex-plugin/plugin.json');
  const mcpJson = readJson('packages/beeax-codex-plugin/.mcp.json');
  ...
  const marketplaceEntry = marketplaceJson?.plugins?.find((entry) => entry.name === 'beeax');

  if (marketplaceJson && !marketplaceEntry) {
    fail('.agents/plugins/marketplace.json includes plugins but not the beeax plugin entry');
  } else if (marketplaceEntry && marketplaceEntry.source?.path !== './packages/beeax-codex-plugin') {
    fail('marketplace beeax source path must be ./packages/beeax-codex-plugin');
  }

  if (fs.existsSync(path.join(REPO_ROOT, 'plugins', 'beeax'))) {
    fail('legacy plugins/beeax path must not exist; use packages/beeax-codex-plugin directly');
  }
};
...
const assertNoBundledMcpConfig = (fail) => {
  ...
  if (fs.existsSync(gitignorePath) && readText(gitignorePath).split(/\r?\n/).includes('.mcp.json')) {
    fail('packages/beeax-codex-plugin/.gitignore must not ignore the public .mcp.json');
  }
...
const assertMarketplaceTemplate = (fail) => {
  const { readJson, readOptionalJson } = createJsonReaders(fail);
  const templatePath = 'packages/beeax-codex-plugin/templates/marketplace.example.json';
  ...
  const twentyEntry = entries.find((entry) => entry?.name === 'beeax');

  if (!twentyEntry) {
    fail(`${templatePath} must include a plugin entry named "beeax"`);
    return;
  }

  if (twentyEntry.version !== pluginJson?.version) {
    fail(`${templatePath} beeax.version must match plugin.json version`);
  }

  if (twentyEntry.source?.path !== './packages/beeax-codex-plugin') {
    fail(`${templatePath} beeax.source.path must be ./packages/beeax-codex-plugin`);
  }

  if (!twentyEntry.policy?.installation) {
    fail(`${templatePath} beeax.policy.installation is required`);
  }

  if (!twentyEntry.policy?.authentication) {
    fail(`${templatePath} beeax.policy.authentication is required`);
  }

  if (twentyEntry.category !== pluginJson?.interface?.category) {
    fail(`${templatePath} beeax.category must match plugin.json interface.category`);
  }
};
