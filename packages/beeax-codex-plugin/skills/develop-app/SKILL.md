---
name: develop-app
description: Use when the user wants to add or modify BeeAX app entities, including objects, layouts, logic functions, and front components inside an existing BeeAX app.
---

# When To Use

Pick this skill when the user wants to change what an existing BeeAX app *does* — its data model, UI, logic, or workflows. Representative triggers:

- "add a new object to my BeeAX app"
...
Do not use this skill to scaffold a brand-new app (use `create-app`), to sync/deploy/troubleshoot (use `manage-app`), to prepare marketplace assets (use `publish-app`), or to query workspace records (use `use-beeax-mcp`).
...
First, confirm the setup is sane before changing entities. The current directory should be the root of an existing BeeAX app:
...
- `src/<service>-client/<name>.ts` — wrappers around external SDKs or shared HTTP clients. One folder per service, matching BeeAX's `*-client` convention.
...
`yarn beeax dev:typecheck` checks generated app types, `yarn lint` checks local lint rules, and `yarn beeax dev --once` syncs entity definitions to the active remote. Run all three a single time once every edit is done, not repeatedly after each step. When the user explicitly asks to run tests, follow `../../references/develop-app/tests.md`.

Use the official BeeAX docs or local SDK source when exact entity fields, imports, or configuration shapes matter.
...
- Front component source, BeeAX UI imports, data hooks, runtime imports, and browser verification: `../../references/develop-app/front-components.md`
...
For front components, read `front-components.md` before implementation. Use `layout.md` for placement, `standalone-pages.md` for full-page custom UI, and `front-component-ui.md` for visual design and BeeAX UI component selection.
