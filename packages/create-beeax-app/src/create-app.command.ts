...
import { isDefined, normalizeUrl } from 'beeax-shared/utils';
import {
...
const IMAGE = 'beeaxcrm/beeax-app-dev:latest';
...
export class CreateAppCommand {
  ...
  async execute(options: CreateAppOptions = {}): Promise<void> {
    ...
    try {
      ...
      if (!skipLocalInstance) {
        this.logNextStep('Starting BeeAX server');
      ...
      if (serverReady && authSucceeded) {
        ...
        if (!syncSucceeded) {
          this.logDetail('Sync failed. Run `yarn beeax dev --once` manually.');
          return;
        }
      } else {
  ...
  private async ensureDockerServer(
    ...
    this.logDetail('Ensuring latest BeeAX server image...');
  ...
  private async syncApplication(appDirectory: string): Promise<boolean> {
    this.logDetail('Running `yarn beeax dev --once`...');
    return new Promise((resolve) => {
      const child = spawn('yarn', ['beeax', 'dev', '--once'], {
        cwd: appDirectory,
  ...
  private async authenticateWithDevKey(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          '  Authentication failed. Run `yarn beeax remote:add` manually.',
        ),
      );
  ...
  private async authenticateWithOAuth(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          `  OAuth failed: ${result.error.message}\n` +
            `  Run \`yarn beeax remote:add --url ${serverUrl}\` manually.`,
        ),
      );
      ...
      console.log(
        chalk.yellow(
          `  Authentication failed. Run \`yarn beeax remote:add --url ${serverUrl}\` manually.`,
        ),
      );
  ...
  private logSuccess(
    ...
    if (!authSucceeded) {
      console.log(chalk.white(`  ${stepNumber}. Connect to a BeeAX instance`));
      console.log(
        chalk.cyan('     yarn beeax remote:add --url <your-instance-url>\n'),
      );
      stepNumber++;
    }

    console.log(chalk.white(`  ${stepNumber}. Start developing`));
    console.log(chalk.cyan('     yarn beeax dev\n'));
    stepNumber++;

    console.log(chalk.white(`  ${stepNumber}. Open your beeax instance`));
    console.log(chalk.cyan(`     ${serverUrl}\n`));

    console.log(
      chalk.gray(
        '  Documentation: https://docs.beeax.com/developers/extend/capabilities/apps',
      ),
    );
  }
}
