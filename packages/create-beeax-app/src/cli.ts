...
const program = new Command(packageJson.name)
  .description('CLI tool to initialize a new BeeAX application')
  .version(
    packageJson.version,
    '-v, --version',
    'Output the current version of create-beeax-app.',
  )
  .argument('[directory]')
  .option('-n, --name <name>', 'Application name')
  .option('-d, --display-name <displayName>', 'Application display name')
  .option('--description <description>', 'Application description')
  .option('--url <url>', 'BeeAX server URL (default: http://localhost:2020)')
