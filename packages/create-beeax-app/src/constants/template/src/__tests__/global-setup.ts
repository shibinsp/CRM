...
import { appDevOnce, appUninstall } from 'beeax-sdk/cli';

const APP_PATH = process.cwd();
const CONFIG_DIR = path.join(os.homedir(), '.beeax');
  ...
  if (!apiUrl || !apiKey) {
    throw new Error(
      'TWENTY_API_URL and TWENTY_API_KEY must be set.\n' +
        'Start a local server: yarn beeax docker:start\n' +
...
async function checkServer(apiUrl: string) {
  ...
  try {
    ...
    throw new Error(
      `BeeAX server is not reachable at ${apiUrl}. ` +
