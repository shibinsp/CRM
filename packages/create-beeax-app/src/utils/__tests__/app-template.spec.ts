...
describe('copyBaseApplicationProject', () => {
  ...
  beforeEach(async () => {
    testAppDirectory = join(
      tmpdir(),
      `test-beeax-app-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('should update package.json with app name and SDK versions', async () => {
    ...
    expect(packageJson.devDependencies['beeax-sdk']).toBe(
      createTwentyAppPackageJson.version,
    );
    expect(packageJson.devDependencies['beeax-client-sdk']).toBe(
      createTwentyAppPackageJson.version,
    );
  });
