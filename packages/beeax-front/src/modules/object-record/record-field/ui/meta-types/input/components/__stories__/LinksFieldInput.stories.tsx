  ...
  args: {
    value: {
      ...
      secondaryLinks: [
        ...
        {
          url: 'https://github.com/beeax/beeax',
          label: 'GitHub',
        },
      ],
    },
...
export const CreatePrimaryLink: Story = {
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://www.beeax.com{enter}');

    const linkDisplay = await canvas.findByText('beeax.com');
...
export const AddSecondaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://docs.beeax.com{enter}');

    const secondaryLink = await canvas.findByText('docs.beeax.com');
...
export const DeletePrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const listItemToDelete = await canvas.findByText('BeeAX Website');
...
export const DeletePrimaryLinkAndUseSecondaryLinkAsTheNewPrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const oldPrimaryLink = canvas.queryByText('BeeAX Website');
