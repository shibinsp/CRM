...
import { ComponentDecorator } from 'beeax-ui/testing';
  ...
  args: {
    value: {
      ...
      secondaryLinks: [
        ...
        { url: 'https://www.beeax.com', label: 'Valid Link' },
      ],
    },
  },
  play: async ({ canvasElement }) => {
    ...
    expect(validLink).toHaveAttribute('href', 'https://www.beeax.com');
...
export const SingleLink: Story = {
  args: {
    value: {
      primaryLinkUrl: 'https://www.beeax.com',
      primaryLinkLabel: 'BeeAX Website',
      secondaryLinks: null,
    },
  },
  play: async ({ canvasElement }) => {
    ...
    expect(link).toHaveAttribute('href', 'https://www.beeax.com');
    expect(link).toHaveTextContent('BeeAX Website');
...
export const MultipleLinks: Story = {
  args: {
    value: {
      ...
      primaryLinkLabel: 'BeeAX Website',
      secondaryLinks: [
        { url: 'https://docs.beeax.com', label: 'Documentation' },
        { url: 'https://blog.beeax.com', label: 'Blog' },
      ],
    },
  },
  play: async ({ canvasElement }) => {
    ...
    const primaryLink = await canvas.findByText('BeeAX Website');
    expect(primaryLink).toBeVisible();
    expect(primaryLink).toHaveAttribute('href', 'https://www.beeax.com');

    const docsLink = await canvas.findByText('Documentation');
    expect(docsLink).toBeVisible();
    expect(docsLink).toHaveAttribute('href', 'https://docs.beeax.com');

    const blogLink = await canvas.findByText('Blog');
    expect(blogLink).toBeVisible();
    expect(blogLink).toHaveAttribute('href', 'https://blog.beeax.com');
  },
};
...
export const SocialMediaLinks: Story = {
  args: {
    value: {
      primaryLinkUrl: 'https://www.linkedin.com/company/beeax',
      primaryLinkLabel: 'BeeAX on LinkedIn',
      secondaryLinks: [
        { url: 'https://twitter.com/beeaxcrm', label: 'BeeAX on Twitter' },
        {
          url: 'https://www.instagram.com/twenty_hq',
          label: 'BeeAX on Instagram',
        },
      ],
    },
  },
  play: async ({ canvasElement }) => {
    ...
    const linkedinLink = await canvas.findByText('beeax');
    expect(linkedinLink).toBeVisible();
    expect(linkedinLink).toHaveAttribute(
      'href',
      'https://www.linkedin.com/company/beeax',
    );

    const twitterLink = await canvas.findByText('@beeaxcrm');
    expect(twitterLink).toBeVisible();
    expect(twitterLink).toHaveAttribute(
      'href',
      'https://twitter.com/beeaxcrm',
    );
...
export const InstagramLinks: Story = {
  args: {
    value: {
      ...
      primaryLinkLabel: 'BeeAX on Instagram',
      secondaryLinks: [
