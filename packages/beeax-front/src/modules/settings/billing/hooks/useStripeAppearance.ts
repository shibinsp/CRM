...
import { THEME_DARK, THEME_LIGHT } from 'beeax-ui/theme';
import { ThemeContext } from 'beeax-ui/theme-constants';

// Stripe's Appearance API rejects CSS color(display-p3 ...) values, which is
// how the BeeAX theme stores colors; map them to sRGB so the PaymentElement
...
const toStripeColor = (color: string): string => {
  ...
  // oxlint-disable-next-line beeax/no-hardcoded-colors
