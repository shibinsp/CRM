import { type CommandMenuContextApi } from 'beeax-shared/types';
...
const createExpressionOnlyPlaceholder = (name: string): unknown => {
  const fail = (): never => {
    throw new Error(
      `"${name}" from beeax-sdk can only be used inside a command's conditionalAvailabilityExpression, not at runtime.`,
    );
  };
