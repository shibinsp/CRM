...
const innerServerStart = async (
  ...
  if (existingUrl) {
    ...
    onProgress?.(`BeeAX server detected on ${existingUrl}`);
    ...
    if (!healthy) {
      return {
        ...
        error: {
          ...
          message:
            'BeeAX server did not become healthy in time.\n' +
            "Check: 'yarn beeax docker:logs'",
        },
      };
    }
  ...
  if (containerExists(containerName)) {
    ...
    if (existingPort !== port) {
      onProgress?.(
        `Existing container uses port ${existingPort}. Run 'yarn beeax docker:reset${isTest ? ' --test' : ''}' first to change ports.`,
      );
    }
    ...
    onProgress?.(
      `Pulling Docker image (${image}) and starting BeeAX container...`,
    );
    ...
    const runResult = spawnSync(
      ...
      [
        ...
        '-v',
        `${volumeStorage}:/app/packages/beeax-server/.local-storage`,
        image,
      ],
      { stdio: 'inherit' },
    );
    ...
    if (runResult.status !== 0) {
      return {
        ...
        error: {
          ...
          message: 'Failed to start BeeAX container.',
        },
      };
    }
  }

  onProgress?.('Waiting for BeeAX to be ready...');
