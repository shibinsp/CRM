...
export class AppGenerateClientCommand {
  async execute(options: AppGenerateClientCommandOptions): Promise<void> {
    ...
    const clientSdkPath = join(appPath, 'node_modules', 'beeax-client-sdk');

    if (!(await pathExists(clientSdkPath))) {
      console.error(
        chalk.red(
          `Cannot find beeax-client-sdk in ${appPath}.\n\n` +
            '  Install it first:\n' +
            '    yarn add beeax-client-sdk',
        ),
      );
      process.exit(1);
    }
    ...
    if (!validateAuth.serverUp) {
      console.error(
        chalk.red(
          'Cannot reach BeeAX server.\n\n' +
            '  Check your remotes:\n' +
            '    yarn beeax remote:status',
        ),
      );
      process.exit(1);
    }
    ...
    if (!validateAuth.authValid) {
      console.error(
        chalk.red(
          'Authentication failed. Run `yarn beeax remote:add` to authenticate.',
        ),
      );
      process.exit(1);
    }
