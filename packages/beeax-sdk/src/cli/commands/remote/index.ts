...
import { normalizeUrl } from 'beeax-shared/utils';
  ...
  if (serverUrl) {
    ...
    if (options.local) {
      if (!detectedUrl) {
        console.error(
          chalk.red(
            'No local BeeAX server found.\n' +
              'Start one with: yarn beeax docker:start',
          ),
        );
        process.exit(1);
      }
...
const listAction = async () => {
  ...
  if (remotes.length === 0) {
    ...
    console.log("Use 'beeax remote:add' to add one.");
  ...
  console.log(
    '\n',
    chalk.gray("Use 'beeax remote:use <name>' to change default"),
  );
};
...
export const registerRemoteCommands = (program: Command): void => {
  program
    ...
    .option('--local', 'Connect to a local BeeAX server (auto-detect)')
  ...
  const remote = program
    ...
    .description('Manage remote BeeAX servers');

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`beeax remote ${oldCmd}\` is deprecated. Use \`beeax ${newCmd}\` instead.`,
      ),
    );
