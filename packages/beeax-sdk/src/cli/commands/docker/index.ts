...
const stopAction = (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    console.log(chalk.yellow('No BeeAX server container found.'));

    return;
  }

  execSync(`docker stop ${containerName}`, { stdio: 'ignore' });
  console.log(chalk.green('BeeAX server stopped.'));
};
...
const statusAction = async (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    ...
    console.log(
      chalk.gray(
        `  Run 'yarn beeax docker:start${options.test ? ' --test' : ''}' to create one.`,
      ),
    );
...
const resetAction = (options: { test?: boolean }) => {
  ...
  const volumeData = options.test
    ? 'beeax-app-dev-test-data'
    : 'beeax-app-dev-data';
  const volumeStorage = options.test
    ? 'beeax-app-dev-test-storage'
    : 'beeax-app-dev-storage';
  ...
  console.log(chalk.green('BeeAX server data reset.'));
  console.log(
    chalk.gray(
      `Run 'yarn beeax docker:start${options.test ? ' --test' : ''}' to start a fresh instance.`,
    ),
  );
};
...
const upgradeAction = async (
  ...
  if (data.containerRecreated) {
    console.log(
      chalk.gray(
        `  Run 'yarn beeax docker:start${options.test ? ' --test' : ''}' to wait for the server to be ready.`,
      ),
    );
  }
};
...
export const registerServerCommands = (program: Command): void => {
  program
    ...
    .description(
      'Start the local BeeAX container (version defaults to the app `engines.beeax` range, then `latest`)',
    )
  ...
  program
    .command('docker:stop')
    .description('Stop the local BeeAX container')
  ...
  const server = program
    ...
    .description(
      'Manage a BeeAX server (local instance and server-side actions)',
    );

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`beeax server ${oldCmd}\` is deprecated. Use \`beeax ${newCmd}\` instead.`,
      ),
    );
