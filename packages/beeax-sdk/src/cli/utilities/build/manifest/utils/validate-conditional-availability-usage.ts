...
const EXPRESSION_VARIABLE_MODULE_SPECIFIERS = ['beeax-sdk/define', 'beeax-sdk'];
...
export const validateConditionalAvailabilityUsage = (
  ...
  return importedExpressionVariables.map(({ importedName, node }) => {
    ...
    return `${relativePath}:${line + 1}:${character + 1} - "${importedName}" is a conditional-availability expression variable from beeax-sdk but this file has no ${CONDITIONAL_AVAILABILITY_EXPRESSION_PROPERTY}. These variables can only be used inside a command's ${CONDITIONAL_AVAILABILITY_EXPRESSION_PROPERTY}, not at runtime.`;
  });
};
