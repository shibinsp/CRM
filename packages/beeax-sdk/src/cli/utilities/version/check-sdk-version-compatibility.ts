...
const getAppDeclaredSdkVersion = async (
  ...
  try {
    ...
    return (
      pkg.dependencies?.['beeax-sdk'] ??
      pkg.devDependencies?.['beeax-sdk'] ??
      undefined
    );
  } catch {
    return undefined;
...
export const checkSdkVersionCompatibility = async (
  ...
  if (cliMajor !== appMajor) {
    console.warn(
      chalk.yellow(
        `⚠ Version mismatch: your app requires beeax-sdk@${appMajor}.x ` +
          `but the CLI running is v${cliVersion}. ` +
          `Major version mismatches may cause unexpected behavior.\n` +
          `  Update with: npm install -g beeax-sdk@${appMajor}`,
      ),
    );
    console.warn('');
  }
