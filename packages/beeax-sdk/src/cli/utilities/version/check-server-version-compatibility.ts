...
export const checkServerVersionCompatibility = async (
  ...
  console.warn(
    chalk.yellow(
      `⚠ Local BeeAX server is v${info.localServerVersion} (${info.daysBehind} days behind v${info.latestServerVersion}).`,
    ),
  );
  console.warn(chalk.dim('  Update with: yarn beeax docker:upgrade'));
  console.warn('');
};
