...
### BeeAX CRM Integration (optional)

To automatically create `callRecording` records in BeeAX when a meeting starts (and mark them as ended when the meeting closes), configure:
...
The `call-recording` BeeAX app must be installed in your workspace first (`packages/beeax-apps/internal/call-recording`). Generate an API key at `<your-beeax-instance>/settings/api-webhooks`.

To launch the BeeAX Desktop application, start the server first, then the app:
