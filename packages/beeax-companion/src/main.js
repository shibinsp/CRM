...
const twentyClient = require('./beeax-client');
require('dotenv').config();

const twentyIconDataUrl = require('./assets/beeax-logo-256.png');
...
// Check BeeAX CRM integration status at startup
...
app.whenReady().then(() => {
  if (process.platform === 'darwin' && app.dock) {
    ...
    if (dockIcon && !dockIcon.isEmpty()) {
      ...
      console.log('BeeAX dock icon set successfully');
    } else {
...
// Poll Recall for the audio URL then call the BeeAX end-recording logic function.
...
async function endCallRecordingWithRetry(windowId, maxAttempts = 10, delayMs = 5000) {
  try {
    ...
    if (!meeting?.twentyRecordId || !meeting?.recallRecordingId) {
      console.log('[BeeAX] No BeeAX record or Recall recording ID found, skipping end-recording');
      return;
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        console.error(`[BeeAX] Attempt ${attempt}/${maxAttempts} — error fetching recording:`, error.message);
      }

      if (attempt < maxAttempts) {
        console.log(`[BeeAX] Audio URL not ready, retrying in ${delayMs / 1000}s (${attempt}/${maxAttempts})`);
        await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
    }

    console.error('[BeeAX] Audio URL never became available, giving up');
  } catch (error) {
    console.error('Failed to end callRecording in BeeAX:', error.message);
  }
}
...
async function createMeetingNoteAndRecord(platformName) {
  ...
  try {
    ...
    // Create a call recording record in BeeAX CRM
    if (twentyClient.isConfigured()) {
      try {
        ...
        if (twentyRecord?.id) {
          ...
          console.log('Created callRecording in BeeAX:', twentyRecord.id);
        }
      } catch (error) {
        console.error('Failed to create callRecording in BeeAX:', error.message);
      }
    }
