...
const fetchContributorStats = async (username: string) => {
  const apiUrl = `https://beeax.com/api/contributors/contributorStats/${username}`;
...
const fetchContributorImage = async (username: string) => {
  const apiUrl = `https://beeax.com/api/contributors/${username}/og.png`;
...
const runCongratulate = async () => {
  ...
  const { data: pullRequests } =
    await danger.github.api.rest.search.issuesAndPullRequests({
      q: `is:pr author:${userName} is:closed repo:beeax/beeax`,
      per_page: 2,
      page: 1,
    });
  ...
  const contributorUrl = `https://beeax.com/contributors/${userName}`;
  ...
  const message =
    ...
    `![Contributions](https://beeax.com/api/contributors/${userName}/og.png)`;
