#!/bin/bash
# =============================================================================
# BeeAX CRM — Development Environment Setup
# =============================================================================
...
#   bash packages/beeax-utils/setup-dev-env.sh          # start + configure
#   bash packages/beeax-utils/setup-dev-env.sh --down    # stop services
#   bash packages/beeax-utils/setup-dev-env.sh --reset   # wipe data + restart
#   bash packages/beeax-utils/setup-dev-env.sh --docker  # force Docker mode
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
COMPOSE_FILE="$REPO_ROOT/packages/beeax-docker/docker-compose.dev.yml"
...
if command -v npx &>/dev/null && [ -d node_modules ]; then
  npx nx reset:env beeax-front
  npx nx reset:env beeax-server
else
  for pkg in beeax-front beeax-server; do
...
if command -v npx &>/dev/null && [ -d node_modules ]; then
  if schema_exists; then
    ...
    if npx nx database:init beeax-server; then
  ...
  info "Run 'npx nx database:init beeax-server' to initialize the schema"
fi
...
echo "  npx nx start beeax-front          # frontend  -> http://localhost:3001"
echo "  npx nx start beeax-server         # backend   -> http://localhost:3000"
echo ""
