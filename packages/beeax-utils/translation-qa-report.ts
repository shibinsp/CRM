 ...
 *
 * Usage:
 *   CROWDIN_PERSONAL_TOKEN=xxx npx ts-node packages/beeax-utils/translation-qa-report.ts
 *
 * The token can be obtained from: https://beeax.crowdin.com/u/settings#api-key
 */

import * as fs from 'fs';
import * as path from 'path';

const CROWDIN_BASE_URL = 'https://beeax.api.crowdin.com/api/v2';
...
async function getToken(): Promise<string> {
  ...
  if (!token) {
    ...
    console.error(
      'Get your token from: https://beeax.crowdin.com/u/settings#api-key',
    );
    process.exit(1);
  }
...
---
**Fix in Crowdin**: https://beeax.crowdin.com/u/projects/1/all?filter=qa-issue

*To auto-fix encoding issues, run:*
\`\`\`
CROWDIN_PERSONAL_TOKEN=xxx npx ts-node packages/beeax-utils/fix-crowdin-translations.ts
\`\`\`
`;
