...
`listr`, etc.) that BeeAX never executed.
...
- **Dropped the live-endpoint introspection path** (`schema/fetchSchema.ts`),
  which was the only consumer of `undici` / `native-fetch` / `qs`. BeeAX always
...
- **Replaced `@graphql-tools/load`** with graphql's own `buildSchema` — BeeAX
...
- **`Config` was narrowed** to the schema-string inputs BeeAX actually passes
  ...
  generated client still defaults its url/fetch to `undefined` (BeeAX's wrapper
