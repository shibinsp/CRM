...
describe('RestApiClient', () => {
  ...
  beforeEach(() => {
    (globalThis as Record<string, unknown>).process = {
      env: {
        TWENTY_API_URL: 'https://api.beeax.test',
  ...
  it('should call the resolved url with a bearer token and parse the JSON response', async () => {
    ...
    expect(url).toBe('https://api.beeax.test/s/my-app/my-route');
  ...
  it('should serialize a JSON body and set the content type on post', async () => {
    ...
    await client.post('/s/my-app/my-route', { name: 'BeeAX' });
    ...
    expect(requestInit.body).toBe(JSON.stringify({ name: 'BeeAX' }));
  });
  ...
  it('should append query parameters while skipping nullish values', async () => {
    ...
    expect(url).toBe(
      'https://api.beeax.test/s/my-app/my-route?limit=10&includeArchived=false',
    );
  });
  ...
  it('should throw a RestApiClientError when the access token is missing', async () => {
    (globalThis as Record<string, unknown>).process = {
      env: { TWENTY_API_URL: 'https://api.beeax.test' },
    };
    const fetchMock = vi.fn();
