...
// straight from beeax-ui: TAG (tag color3/11), SIDEBAR (TintedIconTile
...
import { THEME_LIGHT } from 'beeax-ui/theme';

type ToneSurface = { background: string; border: string; color: string };
type TonePair = { background: string; color: string };

// Our tone names map 1:1 onto beeax-ui's tag colors, except `teal` is the
// product's `turquoise`.
...
// beeax-ui colors are `color(display-p3 r g b)` strings with 0–1 channels;
...
// The Ask-AI panel washes map onto beeax-ui's transparent ramp + primary
...
// beeax-front's graph-color registry assigns chart series from the color8
