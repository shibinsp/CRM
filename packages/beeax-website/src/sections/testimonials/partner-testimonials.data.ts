...
export const PARTNER_TESTIMONIALS: readonly PartnerTestimonialRecord[] = [
  {
    quote: msg`BeeAX gives you the kind of flexibility that actually changes what you can offer your clients. The dev experience is clean, the APIs are open, and when something needs to be customized, you can just do it. There's no fighting the platform.`,
    author: {
  ...
  {
    quote: msg`BeeAX Apps opens the door to building products, not just implementations. For example, we're developing a WhatsApp Business integration that any BeeAX's client could get. That's a recurring revenue stream we wouldn't have if we were just configuring someone else's platform.`,
    author: {
      name: msg`Mike Babiy`,
