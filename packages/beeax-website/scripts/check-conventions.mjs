...
// only serve by URL (plus beeax.svg, the data layer's static export of
// src/icons/beeax-logo.tsx for the mockup's brand-image-by-URL path).
const PUBLIC_SVG_BRAND_FILES = new Set([
  ...
  'public/images/shared/companies/logos/beeax.svg',
  ...
  'public/images/shared/halftone/beeax-logo.svg',
]);
...
// paths keep resolving once this package takes over the beeax-website
...
const ROW_GAP_MULTI_AXIS_FILES = new Set([
  ...
  'sections/why-beeax-editorial/Editorial.tsx',
]);
...
function walk(directory) {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    // beeax-ui's theme is pure data, baked by Linaria at build time — consume
    ...
    const badTwentyUiSubpath = [
      ...content.matchAll(/from 'beeax-ui(\/[a-z-]+)?'/g),
    ]
      .map((match) => match[1] ?? '')
      .find(
    ...
    if (badTwentyUiSubpath !== undefined) {
      failures.push(
        `src/${relativePath}: only beeax-ui/theme is importable (pure data, baked at build); beeax-ui${badTwentyUiSubpath} pulls React runtime into the bundle — build a lean primitive instead.`,
      );
    }
