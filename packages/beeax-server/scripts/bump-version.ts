 ...
 *  |_|   \\_/\\_/ \\___|_| |_|\\__|\\__, | Generated by: npx nx version:bump beeax-server
 *                              |___/
 */
...
const getCurrentVersion = (): string => {
  const filePath = `${CONSTANTS_DIR}/beeax-current-version.constant.ts`;
...
const getPreviousVersions = (): string[] => {
  const filePath = `${CONSTANTS_DIR}/beeax-previous-versions.constant.ts`;
  const content = readFileSync(filePath, 'utf-8');
...
const getNextVersions = (): string[] => {
  const filePath = `${CONSTANTS_DIR}/beeax-next-versions.constant.ts`;
  const content = readFileSync(filePath, 'utf-8');
  const matches = content.match(/'[0-9.]+'/g);
