...
yarn patch-commit <temp-folder> > packages/beeax-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```

Reference the patch in `packages/beeax-server/package.json`:
...
yarn patch-commit <CLEAN> > packages/beeax-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```
