...
export const successfulFilterInputByFieldMetadataType: {
  ...
  [FieldMetadataType.LINKS]: [
    {
      gqlFilterInput: {
        linksField: { primaryLinkUrl: { eq: 'beeax.com' } },
      },
      restFilterInput: 'linksField.primaryLinkUrl[eq]:"beeax.com"',
      validateFilter: (record: Record<string, any>) => {
        return record.linksField.primaryLinkUrl === 'beeax.com';
      },
    },
    {
      gqlFilterInput: {
        linksField: {
          primaryLinkLabel: { eq: 'beeax - #1 Open source CRM' },
        },
      },
      restFilterInput:
        'linksField.primaryLinkLabel[eq]:"beeax - #1 Open source CRM"',
      validateFilter: (record: Record<string, any>) => {
        return (
          record.linksField.primaryLinkLabel === 'beeax - #1 Open source CRM'
        );
      },
    },
    {
      gqlFilterInput: { linksField: { secondaryLinks: { like: '%beeax%' } } },
      restFilterInput: 'linksField.secondaryLinks[like]:"%beeax%"',
      validateFilter: (record: Record<string, any>) => {
