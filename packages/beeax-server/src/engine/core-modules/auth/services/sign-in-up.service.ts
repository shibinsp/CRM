...
import { TWENTY_ICONS_BASE_URL } from 'beeax-shared/constants';
    ...
    try {
      const { user, workspace } = await this.dataSource.transaction(
        async (entityManager) => {
          ...
          // ToS/signup, so acceptance = execution. Only relevant on BeeAX's
          // managed cloud (multi-workspace), where BeeAX is the Processor
          // hosting the data; on self-hosted deployments BeeAX is not the
