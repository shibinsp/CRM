...
import { SendApprovedAccessDomainValidation } from 'beeax-emails';
import { FileFolder, SettingsPath } from 'beeax-shared/types';
import { getSettingsPath, isDefined } from 'beeax-shared/utils';
import { Repository } from 'typeorm';

import { ApprovedAccessDomainEntity } from 'src/engine/core-modules/approved-access-domain/approved-access-domain.entity';
import { InjectWorkspaceScopedRepository } from 'src/engine/beeax-orm/workspace-scoped-repository/inject-workspace-scoped-repository.decorator';
import { WorkspaceScopedRepository } from 'src/engine/beeax-orm/workspace-scoped-repository/workspace-scoped-repository';
import {
...
export class ApprovedAccessDomainService {
  ...
  constructor(
    ...
    // eslint-disable-next-line beeax/prefer-workspace-scoped-repository
  ...
  async sendApprovedAccessDomainValidationEmail(
    ...
    await this.emailService.send({
      from: `${sender.name.firstName} ${sender.name.lastName} (via BeeAX) <${this.twentyConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to,
