...
import { type WorkspaceScopedRepository } from 'src/engine/beeax-orm/workspace-scoped-repository/workspace-scoped-repository';
  ...
  describe('sendApprovedAccessDomainValidationEmail', () => {
    ...
    it('should send a validation email if all conditions are met', async () => {
      ...
      jest
        ...
        .mockReturnValue(new URL('https://sub.beeax.com'));
      ...
      expect(emailService.send).toHaveBeenCalledWith({
        from: 'John Doe (via BeeAX) <no-reply@example.com>',
        to: email,
