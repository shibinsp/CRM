...
describe('resolveDpa', () => {
  it('defaults to EU and resolves the EU Processor entity, law and dormant SCC state', () => {
    ...
    expect(resolved.values.PROCESSOR_ENTITY).toBe('BeeAX.com SAS');
  ...
  it('resolves the US Processor entity, hosting and active SCC state', () => {
    ...
    expect(resolved.values.PROCESSOR_ENTITY).toBe('BeeAX, Inc.');
  ...
  it('substitutes the Processor entity into the contracting clause per region', () => {
    ...
    // Assert on the contracting clause (block 0), not the whole doc: "BeeAX, Inc."
    // also appears verbatim in Annex A's "for US deployments" note.
    expect(eu.blocks[0].text).toContain('between BeeAX.com SAS (');
    expect(us.blocks[0].text).toContain('between BeeAX, Inc. (');
  });
  ...
  it('renders the DPO name and contact', () => {
    ...
    expect(text).toContain('privacy@beeax.com');
  });
