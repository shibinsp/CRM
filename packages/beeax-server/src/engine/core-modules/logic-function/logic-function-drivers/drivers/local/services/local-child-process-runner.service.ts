...
export class LocalChildProcessRunnerService {
  // Symlinks everything from the deps layer except beeax-client-sdk,
  ...
  async assembleNodeModules({
    ...
    const symlinkPromises = entries
      .filter((entry) => entry.name !== 'beeax-client-sdk')
      .map((entry) =>
        fs.symlink(
    ...
    await fs.symlink(
      join(sdkNodeModules, 'beeax-client-sdk'),
      join(execNodeModules, 'beeax-client-sdk'),
      'dir',
    );
  }
