...
import { getWorkspaceScopedRepositoryToken } from 'src/engine/beeax-orm/workspace-scoped-repository/get-workspace-scoped-repository-token.util';
import {
...
const totpStrategyMocks = {
  validate: jest.fn(),
  initiate: jest.fn(() => ({
    uri: 'otpauth://totp/test@example.com?secret=RAW_OTP_SECRET&issuer=BeeAX%20-%20Test%20Workspace',
    context: {
...
describe('TwoFactorAuthenticationService', () => {
  ...
  describe('initiateStrategyConfiguration', () => {
    ...
    it('should initiate configuration for a new user', async () => {
      ...
      expect(uri).toBe(
        'otpauth://totp/test@example.com?secret=RAW_OTP_SECRET&issuer=BeeAX%20-%20Test%20Workspace',
      );
    ...
    it('should create new method when existing pending method is too old', async () => {
      ...
      expect(uri).toContain('BeeAX%20-%20Test%20Workspace');
