...
import { TwoFactorAuthenticationStrategy } from 'beeax-shared/types';
...
@Injectable()
// oxlint-disable-next-line beeax/inject-workspace-repository
export class TwoFactorAuthenticationService {
  ...
  async initiateStrategyConfiguration(
    ...
    if (
      ...
      const issuer = `BeeAX${workspaceDisplayName ? ` - ${workspaceDisplayName}` : ''}`;
    ...
    const { uri, context } = new TotpStrategy(
      ...
      userEmail,
      `BeeAX${workspaceDisplayName ? ` - ${workspaceDisplayName}` : ''}`,
    );
