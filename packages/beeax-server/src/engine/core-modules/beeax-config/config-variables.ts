...
import { type AwsRegion } from 'src/engine/core-modules/beeax-config/interfaces/aws-region.interface';
import { NodeEnvironment } from 'src/engine/core-modules/beeax-config/interfaces/node-environment.interface';
import { SupportDriver } from 'src/engine/core-modules/beeax-config/interfaces/support.interface';
...
import { CastToLogLevelArray } from 'src/engine/core-modules/beeax-config/decorators/cast-to-log-level-array.decorator';
import { CastToMeterDriverArray } from 'src/engine/core-modules/beeax-config/decorators/cast-to-meter-driver.decorator';
import { CastToPositiveNumber } from 'src/engine/core-modules/beeax-config/decorators/cast-to-positive-number.decorator';
import { CastToTypeORMLogLevelArray } from 'src/engine/core-modules/beeax-config/decorators/cast-to-typeorm-log-level-array.decorator';
import { CastToUpperSnakeCase } from 'src/engine/core-modules/beeax-config/decorators/cast-to-upper-snake-case.decorator';
import { ConfigVariablesMetadata } from 'src/engine/core-modules/beeax-config/decorators/config-variables-metadata.decorator';
import { IsAWSRegion } from 'src/engine/core-modules/beeax-config/decorators/is-aws-region.decorator';
import { IsDuration } from 'src/engine/core-modules/beeax-config/decorators/is-duration.decorator';
import { IsOptionalOrEmptyString } from 'src/engine/core-modules/beeax-config/decorators/is-optional-or-empty-string.decorator';
import { IsStrictlyLowerThan } from 'src/engine/core-modules/beeax-config/decorators/is-strictly-lower-than.decorator';
import { IsTwentySemVer } from 'src/engine/core-modules/beeax-config/decorators/is-beeax-semver.decorator';
import { ConfigVariableType } from 'src/engine/core-modules/beeax-config/enums/config-variable-type.enum';
import { ConfigVariablesGroup } from 'src/engine/core-modules/beeax-config/enums/config-variables-group.enum';
import {
  ConfigVariableException,
  ConfigVariableExceptionCode,
} from 'src/engine/core-modules/beeax-config/beeax-config.exception';
...
export class ConfigVariables {
  ...
  EMAIL_FROM_NAME = 'Felix from BeeAX';
  ...
  @ConfigVariablesMetadata({
    ...
    description:
      'Deployment region that determines the contracting DPA Processor entity, hosting region and governing law. EU (default) = BeeAX / Frankfurt / France; US = BeeAX / United States. Must match where Customer Personal Data actually lives.',
    type: ConfigVariableType.ENUM,
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description:
      'ISO date from which HTTP logic functions are no longer served on the legacy /s/ route. Functions created on or after this date are only reachable on the isolated public domain (*.withbeeax.com). Only enforced when PUBLIC_DOMAIN_URL is set; leave empty to keep serving every function on /s/ (default for self-hosting).',
    type: ConfigVariableType.STRING,
  })
  @IsDateString()
  @IsOptional()
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description: 'Base URL for the Enterprise API on beeax.com',
    isHiddenInAdminPanel: true,
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  ENTERPRISE_API_URL: string = 'https://beeax.com/api/enterprise';
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description: 'BeeAX server version',
    type: ConfigVariableType.STRING,
    isEnvOnly: true,
    isHiddenInAdminPanel: true,
  })
