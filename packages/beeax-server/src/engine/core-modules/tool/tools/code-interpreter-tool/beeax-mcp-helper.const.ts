...
export const TWENTY_MCP_HELPER = `# Auto-injected BeeAX MCP helper - provides access to BeeAX tools
import os
import json
...
class TwentyMCP:
    """Helper for calling BeeAX tools from sandboxed code.
    ...
    def call_tool(self, name: str, arguments: dict = None):
        ...
        Example:
            companies = beeax.call_tool('find_many_companies', {'limit': 5})
            # companies == {'records': [...], 'count': '5'}
        """
        if not self._available:
            raise RuntimeError('BeeAX MCP bridge not available. Missing requests library or credentials.')
...
# --------------------------------------------------------------------------
# \`beeax\` is a pre-built instance of the TwentyMCP class above. It is
# already bound in this module scope — DO NOT \`import beeax\`. There is
# no Python package by that name. Just use it directly, e.g.:
#     companies = beeax.call_tool('find_many_companies', {'limit': 10})
# --------------------------------------------------------------------------
beeax = TwentyMCP()
`;
