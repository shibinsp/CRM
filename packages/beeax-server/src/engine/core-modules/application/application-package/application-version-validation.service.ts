  ...
  async validateServerCompatibility(
    ...
    if (!isDefined(semver.validRange(requiredServerVersion))) {
      return {
        ...
        message: `App manifest declares invalid engines.beeax value "${requiredServerVersion}". Must be a valid semver range.`,
      };
    }
    ...
    if (!semver.satisfies(inferredServerVersion, requiredServerVersion)) {
      return {
        compatible: false,
        reason: 'INCOMPATIBLE',
        message: `App requires BeeAX server ${requiredServerVersion} but this server is ${inferredServerVersion}.`,
      };
    }
