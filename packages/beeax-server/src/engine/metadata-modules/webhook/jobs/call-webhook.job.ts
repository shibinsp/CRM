...
import { ensureAbsoluteUrl } from 'beeax-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: WebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-BeeAX-Webhook-Timestamp'] = Date.now().toString();
        headers['X-BeeAX-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-BeeAX-Webhook-Timestamp'],
        );
        headers['X-BeeAX-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
