...
import { validateOperationIsPermittedOrThrow } from 'src/engine/beeax-orm/repository/permissions.utils';
import {
  setWorkspaceContext,
  withWorkspaceContext,
  type ORMWorkspaceContext,
} from 'src/engine/beeax-orm/storage/orm-workspace-context.storage';
...
jest.mock('src/engine/beeax-orm/repository/permissions.utils', () => ({
  validateOperationIsPermittedOrThrow: jest.fn(),
}));

jest.mock(
  'src/engine/beeax-orm/utils/get-object-metadata-from-entity-target.util',
  () => ({
    getObjectMetadataFromEntityTarget: jest.fn(),
  }),
);

jest.mock('src/engine/beeax-orm/utils/format-data.util', () => ({
  formatData: jest.fn().mockReturnValue([]),
}));

jest.mock('src/engine/beeax-orm/utils/format-result.util', () => ({
  formatResult: jest.fn().mockReturnValue([]),
}));

jest.mock(
  'src/engine/beeax-orm/entity-manager/workspace-entity-manager',
  () => ({
    ...jest.requireActual(
      'src/engine/beeax-orm/entity-manager/workspace-entity-manager',
    ),
