import { AUTO_SELECT_SMART_MODEL_ID } from 'beeax-shared/constants';
import { type FlatAgent } from 'src/engine/metadata-modules/flat-agent/types/flat-agent.type';
import { type AllStandardAgentName } from 'src/engine/workspace-manager/beeax-standard-application/types/all-standard-agent-name.type';
import {
  type CreateStandardAgentArgs,
  createStandardAgentFlatMetadata,
} from 'src/engine/workspace-manager/beeax-standard-application/utils/agent-metadata/create-standard-agent-flat-metadata.util';
  ...
  helper: (args: Omit<CreateStandardAgentArgs, 'context'>) =>
    createStandardAgentFlatMetadata({
      ...
      context: {
        ...
        label: 'Helper',
        description:
          'AI agent specialized in helping users learn how to use BeeAX CRM',
        icon: 'IconHelp',
        prompt: `You are a Helper Agent for BeeAX. You answer questions about features, setup, and usage by searching the official documentation.
...
Always base answers on official BeeAX documentation. Be patient and helpful.`,
