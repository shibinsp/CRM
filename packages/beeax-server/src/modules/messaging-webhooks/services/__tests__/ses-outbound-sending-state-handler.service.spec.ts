...
describe('SesOutboundSendingStateHandlerService.handle', () => {
  ...
  // the event, since every beeax-managed resource shares the same prefix.
  describe.each([
    {
  ...
  // beeax-managed name prefix; the handler must noop in that case.
  it('does not update any workspace when the ARN does not carry the beeax-managed prefix', async () => {
    const { service, emailingDomainTenantStatusService } = setUp();
